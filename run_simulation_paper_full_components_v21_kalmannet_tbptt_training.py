...
- Across navigation fusion epochs, training now uses KalmanNet-style truncated
  ...
  epoch is allowed to alter the prior, innovation/residual features, and target at
  subsequent fusion epochs inside the same short trajectory.
...
- The differentiable external recurrence is implemented in the published 15-state
  ...
  measurement Jacobians already produced by the GNSS/LEO/INS model. This keeps the
  training recurrence consistent with Yan Eq. (6), while avoiding a second,
  separately rewritten INS mechanization inside PyTorch.
...
if __name__ == "__main__":
    ...
    TBPTT_CHUNK_LENGTH = _env_int("MKNET_TBPTT_CHUNK_LENGTH", 100)
    ...
    print(
        ...
        f"chunk={TBPTT_CHUNK_LENGTH}; chunk-batch={BATCH_SIZE}; "
    ...
    chunk_initial_previous_correction_15 = []
    chunk_initial_previous_previous_correction_15 = []
    ...
    for k in range(1, len(history_rows)):
        ...
        # Inside the chunk, these lagged state features are regenerated from the
        # network's own learned corrections rather than teacher-forced values.
        previous_previous_correction = (
            previous["x_post"].copy()
            if k == 1
            else history_rows[k - 2]["x_post"].copy()
        )
        transition_to_next = (
        ...
        chunk_initial_previous_correction_15.append(previous["x_post"].copy())
        chunk_initial_previous_previous_correction_15.append(
            previous_previous_correction
        )
    ...
    chunk_initial_previous_correction_15 = np.stack(
        chunk_initial_previous_correction_15
    )
    chunk_initial_previous_previous_correction_15 = np.stack(
        chunk_initial_previous_previous_correction_15
    )
    ...
    if chunk_initial_previous_correction_15.shape != (
        expected_t, INS_STATE_DIM
    ) or chunk_initial_previous_previous_correction_15.shape != (
        expected_t, INS_STATE_DIM
    ):
        raise ValueError("chunk initial correction contexts must have shape [T,15]")
    ...
    # contiguous short trajectories and shuffle those short trajectories during
    # optimization. The paper gives T=100 as its concrete example. Each chunk
    # starts from its stored Data01 context; inside the chunk, all later priors
    # and lagged Yan features are produced recursively by the learned filter.
    training_chunks = [
        (start, min(start + TBPTT_CHUNK_LENGTH, training_sample_count))
        for start in range(0, training_sample_count, TBPTT_CHUNK_LENGTH)
    ]
    if not training_chunks:
        raise RuntimeError("failed to construct truncated-BPTT chunks")

    print(f"Data01 training samples={training_sample_count}, Nmax={nmax}")
    print(
        "KalmanNet truncated-BPTT chunks:",
        len(training_chunks),
        f"(max length={TBPTT_CHUNK_LENGTH})",
    )
    ...
    initial_previous_correction_t = torch.tensor(
        chunk_initial_previous_correction_15,
        dtype=torch.float64,
        device=DEVICE,
    )
    initial_previous_previous_correction_t = torch.tensor(
        chunk_initial_previous_previous_correction_15,
        dtype=torch.float64,
        device=DEVICE,
    )
    ...
    def _rollout_training_chunk(start: int, stop: int):
        ...
        previous_correction = initial_previous_correction_t[start].clone()
        previous_previous_correction = (
            initial_previous_previous_correction_t[start].clone()
        )
        ...
        for sample_index in range(start, stop):
            ...
            # not depend on the filter posterior. Eqs. (12)-(13), however, do.
            delta_imu = fixed_raw_t[sample_index, :6]
            previous_state_innovation = previous_correction
            previous_state_residual = (
                previous_correction - previous_previous_correction
            )
            ...
            previous_previous_correction = previous_correction
    ...
    def _run_training_phase(
        ...
        for order_position, chunk_index in enumerate(chunk_order):
            ...
            batch_end = (
                len(batch_state_sums) >= BATCH_SIZE
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        training_history.append({
            "stage": "yan_kalmannet_v2_tbptt_latent_alternating",
            "epoch": epoch,
            ...
            "tbptt_chunk_length": int(TBPTT_CHUNK_LENGTH),
    ...
    torch.save(
        {
            ...
            "tbptt_chunk_length": int(TBPTT_CHUNK_LENGTH),
            "tbptt_chunk_batch_size": int(BATCH_SIZE),
            "tbptt_chunk_count": int(len(training_chunks)),
    ...
    summary = {
        ...
        "training": {
            ...
            "sequence_training": {
                ...
                "chunk_length": int(TBPTT_CHUNK_LENGTH),
                "chunk_count": int(len(training_chunks)),
                "chunk_batch_size": int(BATCH_SIZE),
