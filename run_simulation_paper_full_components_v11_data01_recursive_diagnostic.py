...
This diagnostic revision retains non-invasive per-epoch divergence logging and
restores the published eta_k -> Eq. (8) feedback after the eta-OFF ablation.
It additionally adds a post-training recursive Data01 rollout diagnostic that
uses the same online learned estimator without updating any network parameter.
These diagnostics do not change the proposed-method navigation equations.
