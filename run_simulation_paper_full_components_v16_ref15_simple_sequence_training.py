...
- Yan's published fixed offline X/M/Y construction (Eqs. 18-20) remains the sole
  training dataset. The samples are consumed in chronological order so the masked
  LSTM hidden/cell state is carried across consecutive fusion epochs as implied by
  Eqs. (24)-(25). Gradients are truncated every BATCH_SIZE epochs only for numerical
  and memory control; h/c itself is detached, not reset. The filter and
  representation blocks are optimized alternately following the general training
  principle of Ref. [15]. No additional full-trajectory recursive BPTT stage or
  network-regenerated training features are used.

This CNN/LSTM revision retains the Pooling and Flatten stages drawn explicitly in
Yan Fig. 8. Recurrent hidden/cell state is initialized once at the start of each
training/test trajectory and carried through consecutive fusion epochs. Within
each epoch the mask gates the zero-padded observation slots exactly as Eq. (25)
requires. The state is passed explicitly rather than stored on the module, so no
state can leak between Data01 and Data02. The exact pooling operator remains
unpublished; the implemented same-length masked max-pool is labelled as a
Fig.-8-guided completion.
...
class MaskedCLA(nn.Module):
    ...
    set. It is consumed chronologically so the recurrent state is continuous
    across fusion epochs, while alternating optimization follows the general
    Ref. [15] training principle. Exact pooling hyperparameters, FC-head
    tensorization, eta_k supervision/timing and behavior when N_test>Nmax_train
    remain explicitly documented project completions.
    """
...
if __name__ == "__main__":
    ...
    # Yan et al. do not publish a mini-batch size. Here BATCH_SIZE is used only
    # as the chronological truncated-BPTT chunk length. Samples are never
    # shuffled, and LSTM h/c is detached rather than reset between chunks.
    ...
    print(
        ...
        f"chronological fixed Data01; truncated BPTT chunk={BATCH_SIZE}; "
        "h/c carried across chunks",
    )
    ...
    # Keep Yan's fixed X/M/Y samples in their physical chronological order.
    # The LSTM state is carried from one fusion epoch to the next; BATCH_SIZE only
    # controls how often the graph is detached/backpropagated (truncated BPTT).
    ...
    print(
        ...
        "ON for chronological fixed-Data01 training/diagnostic/test; reset only at trajectory boundary",
    )
    print(
        "Kalman gain output:",
    ...
    def _run_training_phase(
        ...
        """One Ref.[15]-style alternating pass over chronological Yan X/M/Y.

        The external navigation features remain the fixed offline quantities
        published by Yan et al. Only the neural LSTM state is recurrent here.
        Gradients are truncated every BATCH_SIZE fusion epochs to avoid the
        unstable unpublished full-trajectory BPTT stage used in v15.
        """
        ...
        for sample_index in range(len(dataset)):
            ...
            if chunk_end:
                ...
                # Truncated BPTT: preserve recurrent memory while cutting the
                # autograd graph. Do not reset h/c here.
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        # Eqs. (18)-(20) Data01 dataset is kept unchanged and traversed in time
        # order so the masked-LSTM recurrence used at test time is also present
        # during training. No network-generated feature recursion is introduced.
        ...
        training_history.append({
            "stage": "yan_fixed_offline_Data01_ref15_chronological_alternating",
            "epoch": epoch,
    ...
    # of this chronological alternating training pass.
    model.eval()
    ...
    # while the chronological h/c carry above supplies the temporal LSTM contract.
    recursive_training_metrics = None
    selected_training_stage = (
        "yan_fixed_offline_Data01_ref15_chronological_truncated_BPTT"
    )
    model.eval()
    ...
    torch.save(
        {
            ...
            "sequence_training": (
                "chronological_fixed_XMY_truncated_BPTT; "
                "hc_detached_not_reset_every_chunk"
            ),
    ...
    # =========================================================================
    # DIAGNOSTIC ONLY. The chronological Yan fixed-X/M/Y training has finished
    ...
    summary = {
        ...
        "paper_components": {
            ...
            "training_Eq30_to_Eq32": "Yan_fixed_Data01_chronological_truncated_BPTT_ref15_alternating",
            "Fig8_eta": {
                "enabled": True,
        ...
        "unavoidable_unpublished_completions": [
            ...
            "chronological_truncated_BPTT_chunk_length_Yan_batching_unpublished",
            "per_slot_LSTM_hc_tensorization_across_fusion_epochs_from_Eq25_and_Ref15_sequence_contract",
        ...
        "training": {
            ...
            "sequence_training": {
                "scope": "Data01_fixed_Eqs18_to_20_chronological",
                "gradient": "truncated_BPTT_over_masked_LSTM_state_only",
                "chunk_length": int(BATCH_SIZE),
                "neural_recurrent_state": (
                    "h_c_reset_once_at_Data01_start_carried_across_epochs_"
                    "and_detached_between_chunks"
                ),
