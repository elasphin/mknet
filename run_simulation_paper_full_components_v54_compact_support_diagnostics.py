...
class LEODownlinkSimulator:
    ...
    def simulate_one(
        ...
        # Preserve prior LEO variance: Ref.[35] atmosphere + elevation MP/NLOS; no URA/receiver term.
        iono_sigma_m = (
    ...
    def simulate_epoch(self, receive_time_gpst_s: float, receiver_position_ecef_m: Array):
        ...
        for sat_id in self.provider.satellite_ids:
            try:
                ...
                # TLE/SGP4 replaces STK/HPOP; skip only a satellite whose light-time iteration fails.
...
class MaskedStackedLSTM(nn.Module):
    ...
    def forward(
        ...
        # Eq. (25): masked positions retain the latest valid hidden state.
...
class MaskedCLA(nn.Module):
    ...
    def __init__(
        ...
        # Fig. 8 Masked FC -> KG_k; exact reshape is unpublished, so one FC emits [state,Nmax].
    ...
    def forward(
        ...
        # Yan Eq. (15)/(16): [fixed_24, residual_Nk, innovation_Nk, zero suffix] for this 9-state branch.
