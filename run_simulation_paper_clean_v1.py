...
if __name__ == "__main__":
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        # Fig. 15 reports the training curve as position RMSE in metres, and the
        # paper describes postprocessed position as the training label. Use the
        # current-epoch validation position RMSE for checkpoint selection instead
        # of allowing velocity/attitude scale choices or the unpublished eta
        # completion to select a checkpoint with worse positioning. The full
        # 9-state Eq. (30)-type objective still supplies the training gradients.
        val_position_rmse_m = validation["current_position_rmse_m"]
        if val_position_rmse_m < best_val_position_rmse_m - EARLY_STOP_MIN_DELTA:
            best_val_position_rmse_m = val_position_rmse_m
            best_epoch = epoch
            best_state = {
        ...
        if epochs_without_improvement >= EARLY_STOP_PATIENCE:
            print(
                "early stopping: validation position RMSE "
                f"did not improve for {EARLY_STOP_PATIENCE} complete alternating "
                f"cycles; best epoch={best_epoch}, "
                f"best val position RMSE={best_val_position_rmse_m:.6g} m"
            )
            break
