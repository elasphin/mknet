...
class MaskedStackedLSTM(nn.Module):
    ...
    Each call scans the CNN features of one padded Eq. (15) vector. At position t,
    a valid mask permits the stacked LSTM update; a zero mask retains the preceding
    hidden and cell states exactly as prescribed by Eq. (25). The final valid h/c
    is returned for fusion epoch k+1.
    """
...
if __name__ == "__main__":
    ...
    print(
        ...
        "64 units x 5 layers, dropout=0.2; Eq.(23) feature mask gates every "
        "Eq.(15) position update; attention is over valid current-epoch positions "
        "per Eqs.(26)-(29)",
    )
    print(
    ...
    def _set_alternating_phase(phase: str) -> None:
        """Latent-KalmanNet Algorithm-2 coordinate update with a fixed counterpart.

        ``requires_grad=False`` freezes the opposite parameter block.  We also put
        that frozen block in evaluation mode.  This matters for theta because Yan's
        five-layer LSTM contains dropout=0.2: leaving dropout active while theta is
        supposedly fixed would change the filter mapping stochastically during the
        psi update, which is not a strict alternating minimization step.  Evaluation
        mode does not block gradients with respect to the frozen block's *inputs*,
        so the psi phase still backpropagates through the fixed theta mapping.
        """
        ...
        # Set the top-level module to training mode first, then explicitly override
        # the frozen child block so stochastic training behavior cannot leak into it.
        model.train()
    ...
    torch.save(
        {
            ...
            "checkpoint_schema": "v32_9_state_mask_fixed_latent_alternating_partition",
