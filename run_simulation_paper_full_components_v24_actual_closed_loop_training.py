...
class MaskedCLA(nn.Module):
    ...
    training, the navigation forward state is replayed chronologically with the same
    Eq. (8)+INS mechanization used online, so estimator-dependent features are
    generated from the learned trajectory.  The representation/filter parameter
    blocks are optimized alternately following Latent-KalmanNet. No synthetic
    noisy-prior augmentation is added. Exact pooling hyperparameters,
...
if __name__ == "__main__":
    ...
    # Yan et al. do not publish the mini-batch size.  In this revision the forward
    # navigation trajectory remains chronological and recursive; BATCH_SIZE only
    # controls how many consecutive fusion-epoch Eq. (30) losses are accumulated
    # before one Adam step.  It never resets or shuffles the navigation state.
    ...
    print(
        ...
        "actual closed-loop Data01 forward rollout: learned posterior -> Eq.(8) IMU "
        "compensation -> ECEF INS mechanization -> next GNSS/LEO innovation/features; "
        f"consecutive-loss batch={BATCH_SIZE}; no Phi/H cross-fusion surrogate; "
    ...
    # The navigation trajectory is not divided/reset for optimization.  The full
    # available Data01 sequence is replayed chronologically in each alternating
    # phase so estimator-dependent inputs are generated by the learned filter itself.
    # BATCH_SIZE only controls gradient accumulation over consecutive fusion losses.
    if training_sample_count < 1:
        raise ValueError("closed-loop training requires at least one learned fusion sample")

    print(f"Data01 training samples={training_sample_count}, Nmax={nmax}")
    print(
        "Closed-loop training trajectory:",
        f"1 chronological Data01 rollout; Adam step every up to {BATCH_SIZE} "
        "consecutive learned fusion epochs; no chunk reset/shuffle",
    )
    ...
    #       current posterior produces the next prior/features; do not import V2
    #       chunk mechanics because Yan does not publish that training detail.
    ...
    def _initial_training_context():
        """Ordinary-TC first row is causal context only, as in Data02 warm start."""
        warm = history_rows[0]
    ...
    def _run_actual_closed_loop_data01(
        phase: str | None,
        optimizer: torch.optim.Optimizer | None,
    ):
        """Replay Data01 with the real Eq. (8)+INS forward navigation loop.

        The learned posterior is detached only for the physical NumPy navigation
        propagation.  Consequently, the current Eq. (30) loss trains the current
        KG end-to-end, while the next network input is generated from the actual
        learned navigation trajectory rather than from v23's Phi/H approximation.
        """
        training = optimizer is not None
        if training:
            if phase is None:
                raise ValueError("training pass requires an alternating phase")
        ...
        nav_train = history_rows[0]["posterior_nav"].copy()
        previous_context = _initial_training_context()
        ...
        with grad_context:
            for row_index in range(1, len(history_rows)):
                ...
                # IMU samples until the next usable fusion epoch.  Eq. (8) eta is
                # held over this interval, matching the existing online completion.
                for imu_index, dt in row["preceding_interval_segments"]:
                    ...
                    # Eq. (14) feature path uses bias-compensated IMU without eta,
                    # exactly like the Data02 online feature construction.
                ...
                # The simulated LEO pseudorange realization is raw training data and
                # is fixed once.  Only its prediction from the current nav changes.
                measurements = retain_clock_observable_measurements(
                ...
                # not the classical trajectory used only to initialize Data01.
                ...
                if not torch.isfinite(state_loss):
                    raise FloatingPointError(
                        "non-finite Yan Eq.(30) loss during actual closed-loop "
                        f"Data01 training at history row {row_index}, "
                ...
                # Fig. 7/8 forward state update.  The detached correction is injected
                # into the real NavigationState, then the next interval is propagated
                # by mechanize_ecef.  No Phi/H approximation is used here.
                ...
                if (
                    ...
                    raise FloatingPointError(
                        "non-finite learned correction/eta during actual Data01 rollout"
                    )
                ...
                if training:
                    batch_losses.append(state_loss)
                    batch_count += 1
                    if batch_count >= BATCH_SIZE:
                        batch_state_loss = torch.stack(batch_losses).mean()
                        objective = batch_state_loss + _eq32_regularization()
                        if not torch.isfinite(objective):
                            raise FloatingPointError(
                                f"non-finite {phase} Eq.(32) objective"
                            )
                        objective.backward()
                        optimizer.step()
                        optimizer.zero_grad(set_to_none=True)
            ...
            if training and batch_losses:
                ...
                if not torch.isfinite(objective):
                    raise FloatingPointError(
                        f"non-finite final {phase} Eq.(32) objective"
                    )
                objective.backward()
                optimizer.step()
                optimizer.zero_grad(set_to_none=True)
        ...
        if total_count <= 0:
            raise RuntimeError(
                "actual closed-loop Data01 training produced no learned fusion updates"
            )
        eq30 = total_state_sum / total_count
        objective_value = (
            total_objective_weighted / total_count
            if training
            else eq30 + float(_eq32_regularization().detach().cpu())
        )
        return {
            ...
            "max_prior_position_error_m": float(max_position_error_m),
    ...
    def _run_training_phase(
        ...
        """One chronological actual-navigation pass for one alternating block."""
        metrics = _run_actual_closed_loop_data01(phase, optimizer)
        metrics["epoch"] = int(epoch)
        return metrics

    def _evaluate_actual_closed_loop_training_objective():
        return _run_actual_closed_loop_data01(None, None)
    ...
    print(
        "\n=== YAN EQ.30/EQ.32 + ACTUAL CLOSED-LOOP GNSS/LEO/INS FORWARD "
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        # Latent-KalmanNet alternating principle: optimize theta, then psi.  Each
        # phase independently replays the same chronological Data01 forward loop
        # from the ordinary-TC causal context; Data02 is never consulted.
        ...
        training_history.append({
            "stage": "yan_actual_closed_loop_latent_alternating",
            "epoch": epoch,
        ...
        print(
            ...
            f"updates(theta/psi)={filter_phase['learned_updates']}/"
            f"{representation_phase['learned_updates']}, "
    ...
    selected_training_stage = (
        "yan_eq30_actual_closed_loop_ins_latent_alternating_final_epoch"
    )
    ...
    torch.save(
        {
            ...
            "checkpoint_schema": "full_15_state_gain_actual_closed_loop_v24",
            "nmax": nmax,
            ...
            "sequence_training": (
                "Yan_Fig7_Fig8_actual_closed_loop_forward;learned_posterior_to_"
                "Eq8_IMU_compensation_to_ECEF_INS_to_next_features;no_Phi_H_surrogate;"
                "current_epoch_Eq30_gradient_only"
            ),
            "consecutive_loss_batch_size": int(BATCH_SIZE),
            ...
            "external_filter_state": (
                "recursive_across_full_Data01_forward_trajectory_via_actual_INS"
            ),
    ...
    summary = {
        ...
        "training": {
            ...
            "sequence_training": {
                ...
                "cross_fusion_navigation_gradient": "detached_unpublished_by_Yan",
                "consecutive_loss_batch_size": int(BATCH_SIZE),
                "trajectory_reset_or_shuffle": False,
                "external_filter_recurrence": (
                    "learned_posterior_physically_propagated_to_next_prior_and_features"
                ),
