...
This diagnostic revision adds non-invasive per-epoch divergence logging, an
optional eta-feedback ablation, and an independent classical Data02 reference
baseline.  With ETA_FEEDBACK_ON=True these diagnostics do not change the
proposed-method navigation equations or learned update.

The paper does not publish enough information to uniquely reconstruct the exact
pooling operator in Fig. 8, the eta_k training target/timing, the random LEO
masking-angle distribution, the exact FC tensorization, the LEO clock-error
generator, or the FDE false-alarm probability. Those details are not silently
invented. Compact, clearly labelled completions are used only where execution
...
# =============================================================================
# DIVERGENCE-DIAGNOSTIC ABLATIONS
# =============================================================================
# These switches do not alter the paper implementation itself.  This file is
# configured specifically for the controlled eta-feedback ablation requested
# after the divergence diagnostic run:
#   - FDE/DIA remains ON exactly as in the paper-reproduction path;
#   - the network still predicts eta_k;
#   - only eta_k -> Eq. (8) feedback into the following IMU interval is bypassed.
# This isolates the effect of the learned eta feedback without deleting,
# simplifying, or replacing any published component.
ETA_FEEDBACK_ON = False

# The independent classical Data02 baseline was already established as stable
# in the preceding diagnostic run.  It is disabled here only to avoid repeating
# the same reference computation; this flag does not modify the Masked KalmanNet
# estimator, FDE/DIA logic, INS mechanization, or any paper-derived equation.
...
#     estimation. Its target/timing are unpublished; a documented physics-based
#     Eq. (8) target is reconstructed from adjacent postprocessed P/V/A intervals.
...
class MaskedCLA(nn.Module):
    ...
    bias-gain rows are padded with exact zeros because the supplied truth has no
    bias labels. Fig. 8 also shows eta_k; Eq. (8) identifies it physically with
    residual gyro/accelerometer measurement errors. Yan does not publish eta labels,
    so v7 reconstructs an explicit training target from postprocessed attitude/
    velocity kinematics over the following fusion interval. This is a documented
    physics-based completion, not an author-claimed training detail.
    ...
    def __init__(
        ...
        # eta physically as residual gyro/accelerometer measurement errors. The
        # direct target used below is a declared completion because Yan et al. do
        # not publish its target/timing.
        self.imu_error_head = nn.Linear(
            64,
            IMU_ERROR_DIM,
        )
        # Zero-output initialization prevents arbitrary IMU feedback before
        # training; unlike v6, the branch remains trainable and is supervised by
        # an Eq. (8)-consistent target reconstructed from postprocessed kinematics.
...
if __name__ == "__main__":
    ...
    def _eta_target_from_postprocessed_interval(current_row, next_row):
        """Eq. (8)-consistent residual IMU-error target over one fusion interval.

        The target is reconstructed only from Data01 postprocessed navigation P/V/A
        and measured IMU. Yan Fig. 8 requires eta_k but does not publish its target;
        the reconstruction follows the IMU-recomputed principle of Ref. [24], which
        obtains ideal gyro/specific-force outputs from fused P/V/A.
        """
        t0, t1 = float(current_row["time"]), float(next_row["time"])
        dt = t1 - t0
        if dt <= 0.0:
            raise ValueError("eta target interval must be positive")
        i0 = int(np.searchsorted(imr_time, t0, side="right"))
        i1 = int(np.searchsorted(imr_time, t1, side="right"))
        if i1 <= i0:
            i0 = max(0, min(len(imr_time) - 1, i0))
            i1 = min(len(imr_time), i0 + 1)

        measured_gyro = np.mean(imr.angular_rate_body_radps[i0:i1], axis=0)
        measured_accel = np.mean(imr.acceleration_body_mps2[i0:i1], axis=0)

        j0 = int(current_row["fusion_index"])
        j1 = int(next_row["fusion_index"])
        C0 = fusion_truth_body_to_ecef[j0]
        C1 = fusion_truth_body_to_ecef[j1]
        v0 = fusion_imu_truth_velocity[j0]
        v1 = fusion_imu_truth_velocity[j1]
        p0 = fusion_imu_truth_position[j0]

        # Invert this file's ECEF attitude mechanization:
        # C1 = Exp(-Omega_ie dt) C0 Exp(omega_ib^b dt).
        body_increment = (
            C0.T @ _so3_exponential(OMEGA_IE_E * dt) @ C1
        )
        true_gyro = SpatialRotation.from_matrix(_rotation(body_increment)).as_rotvec() / dt

        # Ref. [24]'s IMU-recomputed method derives ideal gyro/specific-force
        # outputs from accurate fused P/V/A. Here the inversion is written in the
        # same ECEF convention as mechanize_ecef().
        Cmid = 0.5 * (C0 + C1)
        acceleration_e = (v1 - v0) / dt
        force_rhs_e = (
            acceleration_e
            - effective_gravity_ecef(p0)
            + 2.0 * np.cross(OMEGA_IE_E, v0)
        )
        true_specific_force_b = np.linalg.solve(Cmid, force_rhs_e)

        eta_g = measured_gyro - true_gyro
        eta_a = measured_accel - true_specific_force_b
        return np.concatenate([eta_g, eta_a])

    eta_targets = np.zeros((len(feature_time), IMU_ERROR_DIM), dtype=float)
    eta_target_valid = np.zeros(len(feature_time), dtype=bool)
    # feature sample s corresponds to history row k=s+1.
    for s in range(len(feature_time) - 1):
        k = s + 1
        eta_targets[s] = _eta_target_from_postprocessed_interval(
            history_rows[k], history_rows[k + 1]
        )
        eta_target_valid[s] = True
    ...
    dataset = TensorDataset(
        ...
        torch.tensor(eta_targets, dtype=torch.float32),
        torch.tensor(eta_target_valid, dtype=torch.bool),
    )
    ...
    valid_eta = eta_targets[eta_target_valid]
    eta_target_rms = (
        np.sqrt(np.mean(valid_eta**2, axis=0)) if len(valid_eta) else np.zeros(IMU_ERROR_DIM)
    )
    print(f"Data01 training samples={len(dataset)}, Nmax={nmax}")
    print("Data01 eta target RMS [gyro(3), accel(3)]:", eta_target_rms)
    ...
    #
    # Fig. 8 also depicts eta_k / IMU-error output and Fig. 2 feeds it to
    # Error compensation. Yan et al. do not publish its target/timing, so this
    # reproduction uses the smallest physics-based completion: eta is the
    # residual gyro/accelerometer measurement error reconstructed from the
    # following postprocessed P/V/A interval through Eq. (8) and the same INS
    # mechanization. This completion is reported explicitly and is not attributed
    # to Yan et al.
    #
    ...
    def _regularized_eq32_objective(
        ...
        """Yan Eq. (30)/(32) state loss plus the required Fig. 8 Eq. (8) branch.

        The eta term is a physics-based completion because Yan publishes the
        eta_k output and feedback path but not its direct supervision equation.
        """
        eq30_loss, position_rmse = _state_prediction_loss(
            kg_state_update, current_target_9
        )
        if torch.any(eta_valid):
            eta_residual = (
                network_output.imu_error[eta_valid]
                - eta_target[eta_valid]
            )
            eta_loss = torch.mean(eta_residual.double() ** 2)
        else:
            eta_loss = eq30_loss.new_zeros(())
        ...
        if GAMMA_L2:
            ...
            eq32_state_objective = eq30_loss + GAMMA_L2 * l2_theta
        else:
            eq32_state_objective = eq30_loss

        # Eq. (32) remains the published state objective. The eta MSE is a
        # separately identified Ref. [24]-guided completion required to train
        # the second Fig. 8 output rather than silently freezing it.
        objective = eq32_state_objective + eta_loss
        return objective, eq30_loss, eta_loss, position_rmse
    ...
    def _run_training_phase(
        ...
        for batch in epoch_batches:
            ...
            objective, eq30_loss, eta_loss, position_rmse_b = _regularized_eq32_objective(
                output,
                kg_state_update,
                target_b,
                eta_target_b,
            ...
            eta_sum += float(eta_loss.detach().cpu()) * batch_size_now
    ...
    def _evaluate_state_objective(loader):
        ...
        with torch.inference_mode():
            for batch in loader:
                ...
                output = model(fixed_b, obs_b, mask_b, channel_b)
                kg_state_update = fig8_state_update(output, innovation_b)
                objective, eq30_loss, eta_loss, position_rmse_b = (
                ...
                if (
                    ...
                    or not torch.isfinite(eq30_loss)
                    or not torch.isfinite(eta_loss)
                ):
                    raise FloatingPointError(
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        training_history.append({
            ...
            "train_eta_loss": final_training_metrics["eta_loss"],
        ...
        print(
            ...
            f"train_etaMSE={final_training_metrics['eta_loss']:.6g}, "
    ...
    summary = {
        ...
        "paper_components": {
            ...
            "MaskedCLA_Eq10_to_Eq29": "implemented",
            "training_Eq30_to_Eq32": "fixed_Data01_supervised_Adam_LR_0p01_ref15_alternating",
            "Fig8_eta": {
                "enabled": True,
                "feedback_enabled_in_this_run": bool(ETA_FEEDBACK_ON),
                "dimension": 6,
                "interpretation": "[epsilon_g(3),epsilon_a(3)]",
                "training": "Ref24_guided_IMU_recomputed_target_from_adjacent_postprocessed_PVA_completion",
            },
            "FDE_Eq33_to_Eq34": {
        ...
        "unavoidable_unpublished_completions": [
            ...
            "eta_supervision_target_and_timing_Yan_unpublished_Ref24_guided_completion",
        ...
        "training": {
            ...
            "final_eta_loss": float(final_training_metrics["eta_loss"]),
