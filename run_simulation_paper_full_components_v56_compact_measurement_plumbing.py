...
def _torch_mechanize_ecef(
    ...
    # Avoid SVD re-orthogonalization here because its gradient is ill-conditioned near equal singular values.
    Cmid = 0.5 * (C0 + C1)
...
if __name__ == "__main__":
    ...
    def _set_alternating_phase(phase: str) -> None:
        ...
        # Keep train() for cuDNN LSTM backward even when theta parameters are frozen.
        model.train()
    ...
    def _rollout_actual_closed_loop_trajectory(
        ...
        *,
        ...
        initialization_mode: str = "stored_causal",
    ):
        ...
        if initialization_mode == "kalmannet_v2_truth":
            # V2 initializes each T+1 block from labeled truth navigation, not a classical-KF posterior.
            boundary_row = history_rows[start_sample]
            boundary_fusion_index = int(boundary_row["fusion_index"])
            nav_train = TorchNavigationState(
                fusion_truth_position_t[boundary_fusion_index].clone(),
                fusion_truth_velocity_t[boundary_fusion_index].clone(),
                fusion_truth_dcm_t[boundary_fusion_index].clone(),
            )

            # At a V2 boundary, form residuals about truth and zero filter-dependent lagged state deltas.
            stored_boundary_context = _torch_training_context_from_stored(
                start_sample
            )
            boundary_epoch = gnss_epochs[boundary_fusion_index]
            boundary_gnss = gnss_preprocessor.prepare_epoch(
                boundary_epoch,
                nav_train.detached_numpy(),
                lever_arm_b_m,
            )
            boundary_measurements = retain_clock_observable_measurements(
                tuple(boundary_gnss) + tuple(boundary_row["leo_measurements"])
            )
            boundary_residual, boundary_sat_ids = _torch_measurement_innovation(
                nav_train, boundary_measurements
            )
            ...
            previous_context = {
                ...
                "x_pred": zero_state.clone(),
                "x_post": zero_state.clone(),
                "state_innovation": zero_state.clone(),
                "state_residual": zero_state.clone(),
                "accel": stored_boundary_context["accel"],
                "gyro": stored_boundary_context["gyro"],
            }
        else:
        ...
        # Independent short trajectories reset the Masked-LSTM recurrent state,
        # analogous to KalmanNet init_hidden_KNet() at the start of a batch.
        ...
        with grad_context:
            for sample_index in range(start_sample, stop_sample):
                ...
                # Previous learned posterior drives the next prior; checkpointing saves memory without truncating gradients.
                ...
                # Availability/transmit-time/atmosphere are detached; range prediction and innovation remain differentiable.
                epoch = gnss_epochs[fusion_index]
                gnss_measurements = gnss_preprocessor.prepare_epoch(
                ...
                # Eq. (30) remains error-state MSE while future loss can backpropagate through earlier corrections.
        ...
        if training:
            ...
            # Accumulate per-trajectory gradients to match a mini-batch while releasing each INS graph.
    ...
    def _run_training_phase(
        ...
        for batch_start in range(0, trajectory_count, TRAJECTORY_BATCH_SIZE):
            ...
            # Apply Eq. (32) regularization once per trajectory batch; frozen parameters receive no gradient.
        ...
        if total_count <= 0 or not trajectory_eq30_values:
            raise RuntimeError(
                "KalmanNet-V2 Data01 actual-forward training produced no updates"
            )
    ...
    def _run_v1_training_phase(
        ...
        # V1 uses one full recursive BPTT graph, continuing V2 weights/Adam and initializing from known truth.
        metrics = _rollout_actual_closed_loop_trajectory(
            0,
        ...
        if not torch.isfinite(regularization):
            raise FloatingPointError(
                f"non-finite {phase} V1 Eq.(32) regularization"
            )
        if GAMMA_L2:
            regularization.backward()
        if not _active_gradient_finite(phase):
            raise FloatingPointError(
                f"non-finite {phase} V1 gradient before optimizer step"
            )

        objective = metrics["trajectory_eq30"] + float(
            regularization.detach().cpu()
        )
        if not math.isfinite(objective):
            raise FloatingPointError(
                f"non-finite {phase} V1 full-trajectory objective before step"
            )

        # One optimizer step after the complete full-trajectory forward/backward
        # pass, consistent with direct full-trajectory BPTT (KalmanNet V1).
        optimizer.step()

        return {
            "objective": float(objective),
            "eq30": float(metrics["trajectory_eq30"]),
            "trajectory_mean_eq30": float(metrics["trajectory_eq30"]),
            "position_rmse_m": math.sqrt(
                metrics["position_sum"] / metrics["learned_updates"]
            ),
            "learned_updates": int(metrics["learned_updates"]),
            "skipped_no_measurements": int(metrics["skipped_no_measurements"]),
            "optimizer_steps": 1,
            "trajectory_count": 1,
            "sequence_batch_size": 1,
            "max_prior_position_error_m": float(
                metrics["max_prior_position_error_m"]
            ),
            "max_innovation_abs_m": float(
                metrics["max_innovation_abs_m"]
            ),
            "max_gain_fro_norm": float(metrics["max_gain_fro_norm"]),
            "max_position_correction_norm_m": float(
                metrics["max_position_correction_norm_m"]
            ),
            "max_velocity_correction_norm_mps": float(
                metrics["max_velocity_correction_norm_mps"]
            ),
            "max_attitude_correction_norm_rad": float(
                metrics["max_attitude_correction_norm_rad"]
            ),
            "epoch": int(epoch),
        }
