...
if __name__ == '__main__':
        ...
        def core(position, velocity, dcm):
            ...
            for imu_index, dt in segment_tuple:
                feature_gyro, feature_accel = _torch_compensate_imu(
                    training_imr_gyro_t[imu_index], training_imr_accel_t[imu_index]
                )
    ...
    def _rollout(start_sample: int, stop_sample: int, *, phase: str | None=None, initial_state=None, loss_start_sample: int | None=None):
        ...
        if initial_state is None:
            ...
            nav_state = TorchNavigationState.from_numpy(
                history_rows[start_sample]['posterior_nav'], device=DEVICE, dtype=torch.float64
            )
            ...
            for sample_index in range(start_sample, stop_sample):
                ...
                if sample_index >= metric_start:
                    target_state = torch.cat((
                        ...
                        _torch_attitude_error_state_target(nav_state.body_to_ecef_dcm, fusion_truth_dcm_t[fusion_index]),
                    ))
                ...
                nav_state = _torch_inject_error_state(nav_state, correction)
    ...
    def _parameter_l2(parameters) -> float:
        with torch.no_grad():
    ...
    def _train_phase(phase: str, optimizer: torch.optim.Optimizer, epoch: int):
        ...
        gain_l2_before = _parameter_l2(model.gain_head.parameters())
        ...
        out = {
            ...
            'gain_head_parameter_l2_after': float(_parameter_l2(model.gain_head.parameters())),
