...
if __name__ == "__main__":
    ...
    KNET_V2_WARMUP_EPOCHS = _env_int(
        "MKNET_V2_WARMUP_EPOCHS", TRAINING_EPOCHS
    )
    KNET_V1_FINE_TUNE_EPOCHS = _env_int(
        "MKNET_V1_FINE_TUNE_EPOCHS", TRAINING_EPOCHS
    )
    ...
    # does not publish periodic resets to a conventional KF.
    LEARNING_RATE = 0.01
    # Both alternating blocks use Yan's single published initial LR=0.01.
    ...
    # Yan FDE runs online before learned KG; Ref.[33] alpha=1e-3 fills Yan's unpublished false-alarm setting.
    ...
    # Stop once the same first MAX_FUSION_EPOCHS usable epochs have been collected.
    gnss_epochs = tuple(
        rinex.iter_epochs(
    ...
    # ROVE truth is GNSS-antenna; ISA-100C truth is IMU. Neither provides IMU-bias truth.
    ...
    # Check ROVE truth = IMU truth + documented lever arm, confirming reference-point assignment.
    ...
    # Replay exact IMU segments in training using the same 9-state ECEF mechanization; no stored Phi/H surrogate.
    ...
    # Data01 classical TC/INS pass used to build training history.
    ...
    # --- 6. YAN Eqs. (10)-(21): CAUSAL FEATURES + PADDING/MASKS ---
    # Use completed previous-state context to avoid same-epoch circularity; Eq. (16) supplies fixed-Nmax padding.
    nmax = max(len(row["sat_ids"]) for row in history_rows[1:])

    # Data01 raw capacity = valid GNSS + visible LEO before estimator-side removals.
    ...
    # Fixed Eq. (16) network capacity, restored to the pre-v48 behavior:
    # large enough for the maximum selected sequence capacity in Data01/Data02.
    network_nmax = max(
        int(nmax),
    ...
    # First classical row provides lagged context; training regenerates estimator-dependent features recursively.
    # Build fixed Data01 Yan Eq.(15) arrays from the classical history.
    ...
    # Padded Data01 arrays support normalization/one-step checks; recursive training rebuilds Eqs. (10)-(14).
    ...
    # Raw Data01 feature envelope is diagnostic only; it never rescales or feeds Data02.
    ...
    # --- 7. YAN Eqs. (18)-(20) + DATA01-ONLY CONDITIONING ---
    # Yan does not publish feature scaling; Data01-only affine scaling is a numerical completion.
    random.seed(SEED)
    np.random.seed(SEED)
    ...
    # Data01-only numerical conditioning.
    ...
    # KalmanNet V2 Short_Traj_Split: one known initial state + T recursive samples; no KF reset.
    ...
    while v2_start + KNET_V2_TRAJECTORY_LENGTH <= training_sample_count:
        training_trajectories.append(
            (v2_start, v2_start + KNET_V2_TRAJECTORY_LENGTH)
        )
        # Exact Short_Traj_Split semantics: T trained samples plus one
        # independent initial-state sample for the next block.
        v2_start += KNET_V2_TRAJECTORY_LENGTH + 1

    trajectory_count = len(training_trajectories)
    if trajectory_count < 2:
        raise ValueError(
            "KalmanNet V2 requires at least two complete short trajectories so "
            "the trajectory mini-batch satisfies M < N. "
            f"Data01 samples={training_sample_count}, "
            f"MKNET_V2_TRAJECTORY_LENGTH={KNET_V2_TRAJECTORY_LENGTH}. "
            "For the current capped diagnostic run, reduce "
            "MKNET_V2_TRAJECTORY_LENGTH (for example to 10); for the scientific "
            "run, remove the fusion cap and retain the paper-example value 100."
        )

    TRAJECTORY_BATCH_SIZE = min(KNET_V2_BATCH_SIZE, trajectory_count - 1)
    v2_trained_sample_count = int(
        sum(stop - start for start, stop in training_trajectories)
    )
    v2_initialization_sample_count = int(trajectory_count)
    # Match Short_Traj_Split: ignore inter-block transitions and any incomplete tail.
    v2_unused_sample_count = int(
        training_sample_count - v2_trained_sample_count
    )
    ...
    print(
        ...
        "mode=KalmanNet_V2_short_trajectories, "
        f"Tshort={KNET_V2_TRAJECTORY_LENGTH}, "
        f"trajectories={trajectory_count}, "
        f"trajectory_batch={TRAJECTORY_BATCH_SIZE}, "
        "init=ground_truth_navigation_state, "
        f"V2_warmup_epochs={KNET_V2_WARMUP_EPOCHS}, "
        f"V1_finetune_epochs={KNET_V1_FINE_TUNE_EPOCHS}, "
        f"lr_theta={FILTER_BLOCK_LEARNING_RATE:g}, "
        f"lr_psi={REPRESENTATION_BLOCK_LEARNING_RATE:g}, "
        f"FDE={'ON' if FDE_DIA_ON else 'OFF'}"
    )

    # --- 8. MASKED CLA + YAN Eq. (30)/(32) CLOSED-LOOP TRAINING ---
    # Yan leaves BPTT/batching/parameter partition unpublished; recursion follows KalmanNet and the split is project-completed.
    model = MaskedCLA(
        nmax=network_nmax,
        dropout=0.2,
    ).to(DEVICE)
    # FC initialization is unpublished; zero gain is a neutral project completion.
    with torch.no_grad():
    ...
    # Stored X/M/Y tensors are one-step diagnostics only; recursive training rebuilds features from learned navigation.
    ...
    # Build lagged Yan Eqs.(11)-(14) context from a completed epoch.
    ...
    # Build Yan Eq.(15) training features from the current recursive navigation state.
    ...
    # Pad one training epoch to fixed Nmax with the existing masks.
    ...
    # KalmanNet-style Torch recurrence keeps learned posterior -> next prior differentiable across Data01.
    ...
    # Torch causal context for differentiable recursive training.
    ...
    # Differentiable pseudorange innovation; measurement availability/preprocessing stays detached.
    ...
    # Torch Yan Eq.(15) feature tensors for recursive BPTT.
    ...
    # Torch fixed-Nmax padding/masks for one training epoch.
    ...
    # Differentiable 9-state INS propagation between fusion epochs.
    ...
    # Closed-loop Data01 recursive rollout used by both V2 and V1 training.
    ...
    # KalmanNet V2 alternating phase over short trajectories.
    ...
    # KalmanNet V1 alternating phase over the full chronological trajectory.
    ...
    # Read-only recursive Data01 training-objective evaluation.
    ...
    # Read-only teacher-forced/one-step Eq.(30) diagnostic.
    ...
    # Pre-training zero-K baseline separates pure INS drift from learned-correction instability.
    pretraining_zero_gain_metrics = _evaluate_actual_closed_loop_training_objective()
    print(
        "pre-training continuous zero-K Data01 baseline: "
        ...
        f"maxPosCorr={pretraining_zero_gain_metrics['max_position_correction_norm_m']:.3f} m"
    )

    print("\n=== DATA01 TRAINING ===")

    training_history.append({
        "stage": "pretraining_continuous_zero_gain_data01_baseline",
        ...
        "max_position_correction_norm_m": float(
            pretraining_zero_gain_metrics["max_position_correction_norm_m"]
        ),
    ...
    # Stage 1: KalmanNet V2 warm-up on shuffled short trajectories.
    print("\n--- KalmanNet V2 warm-up ---")
    # KalmanNet V2 short-trajectory warm-up.
    ...
    # Stage 2: V1 full chronological tuning, continuing the same weights and Adam states.
    print("\n--- KalmanNet V1 full-sequence fine-tuning ---")
    # KalmanNet V1 full-trajectory fine-tuning.
    ...
    training_performance_summary = {
        "zero_gain_closed_loop": {
            "eq30": float(pretraining_zero_gain_metrics["eq30"]),
            "position_rmse_m": zero_gain_rmse,
            "max_innovation_abs_m": float(pretraining_zero_gain_metrics["max_innovation_abs_m"]),
        },
        "trained_closed_loop": {
    ...
    selected_training_stage = (
        "kalmannet_V2_warmup_then_V1_full_sequence_tuning_"
        "yan_eq30_latent_alternating_final_epoch"
    )

    # Causal online feature construction for paper Eqs. (11)-(14).
    # Build lagged online context from the completed previous fusion epoch.
    ...
    # Build online Yan Eq.(15) features using the same training conventions.
    ...
    # Pad one online epoch to the trained fixed Nmax.
    ...
    # Diagnostic-only feature-range shift metrics.
    ...
    # --- 8A. TRAINING CHECKPOINT ---
    # Training/model selection are Data01-only; Data02 stays independent.
    ...
    # --- 8B. RECURSIVE DATA01 DIAGNOSTIC ---
    # Read-only eval replay tests closed-loop stability and compares 3-D RMSE with classical TC/KF.
    # Read-only recursive Data01 closed-loop diagnostic.
    ...
    # Independent Data02 online Masked-KalmanNet evaluation.
    ...
    # Enforce one online diagnostic schema; FDE-empty INS-only rows use NaN learned-gain fields.
    ...
    # Export Data02 trajectory diagnostics.
    ...
    # Export Data02 FDE event diagnostics.
    ...
    # Diagnostic Data02 classical TC baseline uses the same inputs/FDE but replaces Masked KalmanNet with KF update.
    classical_baseline_rmse = None
    # Optional independent classical Data02 TC/KF baseline.
    ...
    # Complete run summary/provenance dictionary.
    ...
    # Compact Data02 performance summary.
