...
if __name__ == '__main__':
    ...
    print(
        f'training setup: Data01_samples={training_sample_count}, '
        f'Nmax={network_nmax}, epochs={TRAINING_EPOCHS}, '
        f'optimizer=Adam, lr={LEARNING_RATE:g}, weight_decay={WEIGHT_DECAY:g}, '
        f'loss=MSE, alternation=theta_then_psi, batches=Q1_single_chronological_sequence'
    )
    ...
    filter_optimizer = torch.optim.Adam(filter_parameters, lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
    representation_optimizer = torch.optim.Adam(representation_parameters, lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
    loss_fn = nn.MSELoss(reduction='mean')

    fixed_train_t = torch.as_tensor(fixed_network, dtype=torch.float32, device=DEVICE)
    observations_train_t = torch.as_tensor(observations_network, dtype=torch.float32, device=DEVICE)
    mask_train_t = torch.as_tensor(satellite_masks, dtype=torch.bool, device=DEVICE)
    channel_train_t = torch.as_tensor(channel_masks, dtype=torch.bool, device=DEVICE)
    innovation_train_t = torch.as_tensor(innovation_padded, dtype=torch.float32, device=DEVICE)
    target_train_t = torch.as_tensor(target_states_9, dtype=torch.float32, device=DEVICE)

    def _sequence_training_loss():
        recurrent_state = None
        losses = []
        position_squared_error = fixed_train_t.new_zeros(())
        for sample_index in range(training_sample_count):
            output = model(
                fixed_train_t[sample_index:sample_index + 1],
                observations_train_t[sample_index:sample_index + 1],
                mask_train_t[sample_index:sample_index + 1],
                channel_train_t[sample_index:sample_index + 1],
                recurrent_state=recurrent_state,
            )
            recurrent_state = output.recurrent_state
            correction = fig8_state_update(
                output,
                innovation_train_t[sample_index:sample_index + 1],
            )[0, :SUPERVISED_STATE_DIM]
            target = target_train_t[sample_index, :SUPERVISED_STATE_DIM]
            losses.append(loss_fn(correction, target))
            position_squared_error = position_squared_error + torch.sum((correction[:3] - target[:3]) ** 2)
        mean_mse = torch.stack(losses).mean()
        position_rmse = torch.sqrt(position_squared_error / (training_sample_count * 3))
        return mean_mse, position_rmse

    training_history = []

    print('\n=== DATA01 TRAINING: Yan Eq.(30)-(32) + alternating optimization ===')
    for epoch in range(1, TRAINING_EPOCHS + 1):
        # theta phase: masked LSTM + attention + gain head
        for parameter in model.parameters():
            parameter.requires_grad_(False)
        for parameter in filter_parameters:
            parameter.requires_grad_(True)
        model.train()
        filter_optimizer.zero_grad(set_to_none=True)
        filter_mse, filter_position_rmse = _sequence_training_loss()
        filter_mse.backward()
        filter_optimizer.step()

        # psi phase: masked convolutional representation block
        for parameter in model.parameters():
            parameter.requires_grad_(False)
        ...
        representation_mse, representation_position_rmse = _sequence_training_loss()
        representation_mse.backward()
        representation_optimizer.step()

        training_history.append({
            'epoch': int(epoch),
            'filter_theta_mse': float(filter_mse.detach().cpu()),
            'filter_theta_position_rmse_m': float(filter_position_rmse.detach().cpu()),
            'representation_psi_mse': float(representation_mse.detach().cpu()),
            'representation_psi_position_rmse_m': float(representation_position_rmse.detach().cpu()),
            'learning_rate': float(LEARNING_RATE),
            'weight_decay': float(WEIGHT_DECAY),
            'alternating_order': 'theta_then_psi',
        })

        if epoch == 1 or epoch % 10 == 0 or epoch == TRAINING_EPOCHS:
            print(
                f'epoch {epoch:03d}/{TRAINING_EPOCHS}: '
                f'theta_MSE={float(filter_mse.detach().cpu()):.6g}, '
                f'psi_MSE={float(representation_mse.detach().cpu()):.6g}, '
                f'psi_posRMSE={float(representation_position_rmse.detach().cpu()):.3f} m'
            )
    ...
    with torch.inference_mode():
        final_training_mse, final_training_position_rmse = _sequence_training_loss()

    final_training_metrics = {
        'eq30_mse': float(final_training_mse.cpu()),
        'position_rmse_m': float(final_training_position_rmse.cpu()),
    }
    selected_training_stage = 'Yan_Eq30_Eq32_alternating_theta_then_psi'
    best_epoch = TRAINING_EPOCHS

    print(
        f'final Data01 training: MSE={final_training_metrics["eq30_mse"]:.6g}, '
        f'position_RMSE={final_training_metrics["position_rmse_m"]:.3f} m'
    )
    ...
    torch.save({
        ...
        'checkpoint_schema': 'masked_cla_reference_aligned_training',
        ...
        'training_batching': 'Q=1_single_chronological_Data01_sequence',
        'validation_split': False,
        'gradient_clipping': False,
        'tbptt_detach': False,
        'short_trajectory_truth_restarts': False,
        'extra_v1_finetune_stage': False,
        'custom_gain_head_scaling': False,
        'latent_encoder_warm_start': False,
        'latent_encoder_warm_start_reason': 'Yan_does_not_define_an_independent_supervised_target_or_loss_for_the_masked_CNN_representation',
        'final_training_mse': float(final_training_metrics['eq30_mse']),
        'final_training_position_rmse_m': float(final_training_metrics['position_rmse_m']),
    ...
    summary = {
        ...
        'intentional_project_differences': {
            ...
            'state_dimension': int(INS_STATE_DIM),
        },
        'dataset_protocol': {
        ...
        'training': {
            'basis': [
                'Yan_Eq30_supervised_state_error',
                'Yan_Eq32_sequence_average_plus_L2_regularization',
                'Yan_TableIII_MSE_Adam_LR_0.01',
                'Yan_alternating_optimization',
                'Latent_KalmanNet_freeze_one_block_train_the_other',
                'KalmanNet_TSP_MSELoss_mean_Adam_weight_decay',
            ],
            ...
            'theta_filter': 'masked_lstm+masked_attention+gain_head',
            'psi_representation': 'masked_conv',
            'batching': 'Q=1_single_chronological_Data01_sequence',
            'validation_split': False,
            'gradient_clipping': False,
            'tbptt_detach': False,
            'short_trajectory_training': False,
            'truth_restart_between_training_chunks': False,
            'extra_v1_finetune': False,
            'custom_gain_head_initial_scale': False,
            'encoder_warm_start': False,
            'final_training_mse': float(final_training_metrics['eq30_mse']),
            'final_training_position_rmse_m': float(final_training_metrics['position_rmse_m']),
        },
        'test': {
