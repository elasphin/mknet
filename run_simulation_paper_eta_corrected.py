...
- Fig. 8 explicitly requires a second network output eta_k for inertial-navigation
  ...
  the eta branch is trained without invented sensor-error labels by a teacher-forced
  one-step forward INS state loss over that same future interval. The zero-order hold
  and the auxiliary forward-state training loss are explicit paper-guided completions,
  because Yan et al. do not publish eta_k timing or a separate eta supervision rule.
...
if __name__ == "__main__":
    ...
    #   eta_k is trained on the *following* IMU interval using postprocessed truth
    #   only at the interval endpoints (teacher forcing). The local Eq. (8)+INS
    #   sensitivity maps eta_k to a next-epoch 9-state correction, and the loss is
    #   another navigation-state squared error. At runtime the Jacobian disappears:
    #   eta_k is held and applied directly in compensate_imu() until the next
    #   usable network output.
    #
    # The current-state Eq. (30) loss and forward eta navigation-state loss are
    # averaged with equal weight. That averaging is an explicit reproducibility
    # completion; the paper does not publish an eta-specific weighting schedule.
    ...
    def _regularized_joint_loss(
        kg_state_update,
        current_target_9,
        eta_forward_state_update_9,
        eta_forward_target_9,
    ):
        kg_eq30_loss, current_position_rmse = _state_prediction_loss(
            kg_state_update,
            current_target_9,
        )
        eta_forward_state_loss, eta_forward_position_rmse = _state_prediction_loss(
            eta_forward_state_update_9,
            eta_forward_target_9,
        )
        joint_state_loss = 0.5 * (
            kg_eq30_loss + eta_forward_state_loss
        )
        ...
        if GAMMA_L2:
            ...
            objective = joint_state_loss + GAMMA_L2 * l2
        else:
            objective = joint_state_loss
        return (
            objective,
            joint_state_loss,
            kg_eq30_loss,
            eta_forward_state_loss,
            current_position_rmse,
            eta_forward_position_rmse,
        )

    print(
        "\n=== FIG. 8 KG + FIG. 2 FORWARD-ETA TRAINING WITH "
        "CHRONOLOGICAL VALIDATION (DATA01) ==="
    )
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        train_joint_sum = 0.0
        train_kg_eq30_sum = 0.0
        train_eta_forward_sum = 0.0
        train_position_rmse_squared_sum = 0.0
        train_eta_forward_position_rmse_squared_sum = 0.0
        train_eta_gyro_sq_sum = 0.0
        train_eta_accel_sq_sum = 0.0
        train_count = 0
        ...
        for batch in train_loader:
            ...
            (
                ...
                eta_forward_state_loss,
                position_rmse_b,
                eta_forward_position_rmse_b,
            ) = _regularized_joint_loss(
                kg_state_update,
                target_b,
                eta_forward_state_update_9,
                eta_forward_target_b,
            ...
            train_objective_sum += float(loss.detach().cpu()) * batch_size_now
            train_joint_sum += float(joint_state_loss.detach().cpu()) * batch_size_now
            train_kg_eq30_sum += float(kg_eq30_loss.detach().cpu()) * batch_size_now
            train_eta_forward_sum += float(
                eta_forward_state_loss.detach().cpu()
            ) * batch_size_now
            train_position_rmse_squared_sum += (
                float(position_rmse_b.detach().cpu()) ** 2 * batch_size_now
            )
            train_eta_forward_position_rmse_squared_sum += (
                float(eta_forward_position_rmse_b.detach().cpu()) ** 2
                * batch_size_now
            )
            train_eta_gyro_sq_sum += float(
                torch.mean(
                    output.imu_error[:, ETA_GYRO_SLICE] ** 2
                ).detach().cpu()
            ) * batch_size_now
            train_eta_accel_sq_sum += float(
                torch.mean(
        ...
        train_objective = train_objective_sum / train_count
        train_joint = train_joint_sum / train_count
        train_kg_eq30 = train_kg_eq30_sum / train_count
        train_eta_forward = train_eta_forward_sum / train_count
        train_position_rmse = math.sqrt(
            train_position_rmse_squared_sum / train_count
        )
        train_eta_forward_position_rmse = math.sqrt(
            train_eta_forward_position_rmse_squared_sum / train_count
        )
        train_eta_gyro_rms = math.sqrt(train_eta_gyro_sq_sum / train_count)
        train_eta_accel_rms = math.sqrt(train_eta_accel_sq_sum / train_count)

        model.eval()
        val_objective_sum = 0.0
        val_joint_sum = 0.0
        val_kg_eq30_sum = 0.0
        val_eta_forward_sum = 0.0
        val_position_rmse_squared_sum = 0.0
        val_eta_forward_position_rmse_squared_sum = 0.0
        val_eta_gyro_sq_sum = 0.0
        val_eta_accel_sq_sum = 0.0
        val_count = 0
        ...
        with torch.inference_mode():
            for batch in val_loader:
                ...
                (
                    ...
                    val_eta_forward_b,
                    val_position_rmse_b,
                    val_eta_forward_position_rmse_b,
                ) = _regularized_joint_loss(
                    kg_state_update,
                    target_b,
                    eta_forward_state_update_9,
                ...
                if (
                    not torch.isfinite(val_objective_b)
                    or not torch.isfinite(val_joint_b)
                ):
                    raise FloatingPointError(
                ...
                batch_size_now = len(fixed_b)
                val_objective_sum += (
                    float(val_objective_b.detach().cpu()) * batch_size_now
                )
                val_joint_sum += float(val_joint_b.detach().cpu()) * batch_size_now
                val_kg_eq30_sum += (
                    float(val_kg_eq30_b.detach().cpu()) * batch_size_now
                )
                val_eta_forward_sum += (
                    float(val_eta_forward_b.detach().cpu()) * batch_size_now
                )
                val_position_rmse_squared_sum += (
                    float(val_position_rmse_b.detach().cpu()) ** 2
                    * batch_size_now
                )
                val_eta_forward_position_rmse_squared_sum += (
                    float(val_eta_forward_position_rmse_b.detach().cpu()) ** 2
                    * batch_size_now
                )
                val_eta_gyro_sq_sum += float(
                    torch.mean(
                        output.imu_error[:, ETA_GYRO_SLICE] ** 2
                    ).detach().cpu()
                ) * batch_size_now
                val_eta_accel_sq_sum += float(
                    torch.mean(
                        output.imu_error[:, ETA_ACCEL_SLICE] ** 2
                    ).detach().cpu()
                ) * batch_size_now
        ...
        val_objective = val_objective_sum / val_count
        val_joint = val_joint_sum / val_count
        val_kg_eq30 = val_kg_eq30_sum / val_count
        val_eta_forward = val_eta_forward_sum / val_count
        val_position_rmse = math.sqrt(
            val_position_rmse_squared_sum / val_count
        )
        val_eta_forward_position_rmse = math.sqrt(
            val_eta_forward_position_rmse_squared_sum / val_count
        )
        val_eta_gyro_rms = math.sqrt(val_eta_gyro_sq_sum / val_count)
        val_eta_accel_rms = math.sqrt(val_eta_accel_sq_sum / val_count)
        current_lr = float(optimizer.param_groups[0]["lr"])

        training_history.append({
            "stage": "supervised_train_validation",
            "epoch": epoch,
            "train_objective_eq32_completion": train_objective,
            "train_joint_state_loss": train_joint,
            "train_kg_eq30_current_state_loss": train_kg_eq30,
            "train_eta_forward_state_loss_completion": train_eta_forward,
            "train_current_position_rmse_m": train_position_rmse,
            "train_eta_forward_position_rmse_m": train_eta_forward_position_rmse,
            "val_objective_eq32_completion": val_objective,
            "val_joint_state_loss": val_joint,
            "val_kg_eq30_current_state_loss": val_kg_eq30,
            "val_eta_forward_state_loss_completion": val_eta_forward,
            "val_current_position_rmse_m": val_position_rmse,
            "val_eta_forward_position_rmse_m": val_eta_forward_position_rmse,
            "train_eta_gyro_rms_radps": train_eta_gyro_rms,
            "train_eta_accel_rms_mps2": train_eta_accel_rms,
            "val_eta_gyro_rms_radps": val_eta_gyro_rms,
            "val_eta_accel_rms_mps2": val_eta_accel_rms,
            "learning_rate": current_lr,
        })

        print(
            f"epoch {epoch:03d}/{TRAINING_EPOCHS}: "
            f"train_KG_Eq30={train_kg_eq30:.6g}, "
            f"val_KG_Eq30={val_kg_eq30:.6g}, "
            f"train_etaForward={train_eta_forward:.6g}, "
            f"val_etaForward={val_eta_forward:.6g}, "
            f"val_joint={val_joint:.6g}, "
            f"val_posRMSE={val_position_rmse:.3f} m, "
            f"val_etaFuturePosRMSE={val_eta_forward_position_rmse:.3f} m, "
            f"eta_g_RMS={val_eta_gyro_rms:.3g} rad/s, "
            f"eta_a_RMS={val_eta_accel_rms:.3g} m/s^2, "
            f"val_objective={val_objective:.6g}, "
            f"lr={current_lr:.3g}"
        )

        # Model selection must include both outputs. Using only the same-epoch KG
        # Eq. (30) would leave the forward eta branch outside checkpoint selection.
        if val_joint < best_val_joint_state_loss - EARLY_STOP_MIN_DELTA:
            best_val_joint_state_loss = val_joint
            best_val_eq30 = val_kg_eq30
            best_val_eta_forward = val_eta_forward
        ...
        if epochs_without_improvement >= EARLY_STOP_PATIENCE:
            print(
                "early stopping: validation joint state loss did not improve for "
                f"{EARLY_STOP_PATIENCE} consecutive epochs; "
                f"best epoch={best_epoch}, "
                f"best val joint={best_val_joint_state_loss:.6g}"
            )
            break
    ...
    torch.save(
        {
            ...
            "stage": "fig8_KG_plus_fig2_forward_eta_train_validation_best_checkpoint",
            "nmax": nmax,
            "fixed_mean": fixed_mean,
            "fixed_std": fixed_std,
            "obs_mean": obs_mean,
            ...
            "loss": "current_KG_Eq30_plus_teacher_forced_forward_eta_state_loss_then_Eq32_L2_completion",
    ...
    def _online_feature_arrays(
        ...
        delta_accel = current_accel - previous_online["accel"]
        delta_gyro = current_gyro - previous_online["gyro"]
        previous_state_innovation = (
        ...
        if (
            not np.all(np.isfinite(fixed_k))
            or not np.all(np.isfinite(obs_k))
            or not np.all(np.isfinite(innovation_k))
        ):
            raise FloatingPointError(
                "non-finite feature generated during closed-loop Data01 rollout"
            )
        return (
            fixed_k,
            obs_k,
    ...
    def _closed_loop_finetune_pass(pass_index: int):
        ...
        for event in build_exact_fusion_timeline(
            ...
            if previous_online is None:
                ...
                previous_online = {
                    "sat_ids": measurement_model.sat_ids,
                    "residual": warm_residual.copy(),
                    "x_pred": warm_pred.copy(),
                    "x_post": warm_post.copy(),
                    "accel": feature_accel.copy(),
                    "gyro": feature_gyro.copy(),
                }
                current_eta_roll = np.zeros(IMU_ERROR_DIM)
                continue
    ...
    selected_training_stage = (
        ...
        if ENABLE_STABILITY_COMPLETION
        else "fig8_KG_plus_fig2_forward_eta_train_validation_best_checkpoint"
    )
    ...
    if REQUIRE_RECURSIVE_DATA01_DIAGNOSTIC:
        ...
        try:
            ...
            raise RuntimeError(
                "The trained model is already unstable during the recursive "
                "Data01 diagnostic. Independent Data02 testing is blocked to "
                "avoid another long run ending in numerical divergence. "
    ...
    torch.save(
        {
            ...
            "imu_error_head": "trainable_6D_[epsilon_g_radps,epsilon_a_mps2]_teacher_forced_forward_Eq8_state_loss",
            "eta_k_timing_completion": "fusion_k_output_zero_order_hold_over_following_IMU_interval",
            ...
            "paper_baseline_loss": "current_KG_Eq30_plus_teacher_forced_forward_eta_state_loss_then_Eq32_L2_completion",
    ...
    for event in test_timeline:
        ...
        if previous_online is None:
            ...
            previous_online = {
                ...
                "accel": last_feature_accel.copy(),
                "gyro": last_feature_gyro.copy(),
            }
            current_eta_test = np.zeros(IMU_ERROR_DIM)
        ...
        delta_accel = last_feature_accel - previous_online["accel"]
        delta_gyro = last_feature_gyro - previous_online["gyro"]
        previous_state_innovation = (
            previous_online["x_post"] - previous_online["x_pred"]
        )
        previous_state_residual = (
    ...
    summary = {
        ...
        "masked_cla_protocol": {
            ...
            "eta_training_completion": "teacher_forced_one_step_forward_navigation_state_loss",
            "eta_direct_sensor_error_labels": False,
            "training_status": "Fig8_KG_current_state_loss_plus_Fig2_forward_eta_teacher_forced_state_loss",
            "training_features": "offline_fixed_labeled_dataset_Yan_Eqs18_to_20",
            "recursive_training_claimed_by_paper": False,
            "bptt_across_navigation_epochs": False,
            "loss": "current_KG_Eq30_plus_forward_eta_state_loss_completion_plus_Eq32_L2",
            "alternating_optimization": "not_reproduced_exactly_schedule_unpublished",
        ...
        "online_update_protocol": {
            "state_correction": "dx_equals_Knet_times_raw_innovation",
        ...
        "limitations": [
            ...
            "The eta branch is trained with a teacher-forced next-interval navigation-state loss and equal averaging with the current KG state loss. This avoids direct eta pseudo-labels and preserves the Fig. 2 causal direction, but the paper does not publish this eta-specific training rule. One chronological boundary sample is dropped so its future eta target cannot leak validation truth into training.",
            "The paper states that parameters are optimized alternately but does not publish a reproducible alternating-optimization schedule; this run uses Adam on the trainable Masked-CLA parameters without inventing that schedule.",
