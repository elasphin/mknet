...
if __name__ == '__main__':
    ...
    def _clip_active_gradient(phase: str) -> tuple[float, float]:
        """Clip the active block's global L2 gradient norm.

        Returns:
            (unclipped_norm, clipped_norm)

        This follows the stability pattern used in
        SongJgit/KalmanNet4SensorFusion. PyTorch's clip_grad_norm_ modifies
        gradients in-place and is applied immediately before optimizer.step().
        """
        parameters = filter_parameters if phase == 'filter' else representation_parameters
        active_parameters = [
        ...
        return float(total_norm_before.detach().cpu()), float(total_norm_after)
