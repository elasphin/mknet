...
# Project deviations: pseudorange-only, TLE/SGP4 LEO, 9-state no-bias/no-eta.
# Data01 trains; Data02 is an independent evaluation.
...
# Integrity monitoring remains separate from the removed FDE/DIA path.
# If its optional external implementation is absent, navigation/training still run and PL outputs are NaN.
try:
...
# Project choice: causal groupwise L2 feature normalization.
# Yan does not publish an input-normalization rule.
...
GOGPS_S1_DBHZ = 50.0
GOGPS_A_DB = 20.0
# Calendar epoch to continuous GPST seconds.
...
# Attach GPS week to IMR time-of-week.
def anchor_imr_gpst(tow_s: np.ndarray, anchor_time_gpst_s: float) -> np.ndarray:
    ...
    return time_gpst_s
# Check a finite, strictly increasing GPST axis.
...
# ECEF metres to WGS-84 latitude, longitude, height.
...
# SmartPNT passive vehicle-to-body Z-X-Y rotation.
@lru_cache(maxsize=8)
...
# GPS ICD Klobuchar ionospheric delay.
...
# Earth-rotation-corrected range and LOS.
...
# Elevation and azimuth from receiver NED.
        ...
        with self.path.open('r', encoding='ascii', errors='replace') as stream:
            ...
            for line in stream:
                ...
                if max_epochs is not None and yielded >= int(max_epochs):
                    break
# --- GNSS precise products are parsed once inside GNSSProcessor below. ---
# --- SmartPNT IMR ---
IMR_HEADER_FORMAT_BODY = "8scdiidddiid32s?BBB32s6h?iii354s"
IMR_RECORD_FORMAT_BODY = "d6i"
# Read the IMR layout and scales used here.
@lru_cache(maxsize=8)
...
# Interpolate IE truth at requested GPST epochs.
...
# Build body-to-ECEF DCM with IE/SmartPNT convention.
...
# SO(3) exponential by Rodrigues formula.
...
# Project a drifted DCM back to SO(3).
...
# Cached Earth-rotation transition.
...
# Prepare corrected GNSS pseudoranges from precise products.
class GNSSProcessor:
...
# 9-state ECEF error dynamics; Yan Eq. (7) is 15-state.
...
# Discretize process dynamics and noise with Van Loan.
...
# Keep observations that preserve clock elimination.
...
# Limit observations to fixed Data01 Nmax without test truth.
...
def _clock_projection(measurements, variances: Array) -> Array:
    ...
    return projector
# Build pseudorange innovation, H, and projected R.
def measurement_model(nav: NavState, measurements, lever_arm_b_m: Array) -> TCModel:
    measurements = clock_observable(measurements)
    n = len(measurements)
    if n == 0:
        return TCModel(np.empty(0), np.zeros((0, INS_STATE_DIM)), np.zeros((0, 0)), (), np.zeros((0, 0)))
    ...
    R = 0.5 * (R + R.T)
    return TCModel(innovation, H, R, tuple(sat_ids), projector)
# Recompute posterior innovation only.
...
# --- Integrity monitoring (FDE/DIA intentionally removed) ---
INTEGRITY = IntegrityMonitor(ecef_llh, ecef_to_ned) if IntegrityMonitor is not None else None
# Conventional 9-state tightly-coupled KF update.
def kf_update(P: Array, innovation: Array, H: Array, R: Array):
    ...
    return (dx, P_reset, K)
# Joseph covariance update for the learned gain.
...
# Reset covariance after error-state feedback.
...
# Differentiable SO(3) exponential.
...
def _torch_so3_log(rotation_matrix: torch.Tensor) -> torch.Tensor:
    ...
    return factor * vee
# Differentiable ECEF mechanization for Data01 BPTT.
...
# Propagate TLE/SGP4 WGS-72 states and transform TEME to ECEF.
class TLEProvider:
...
# Ref. [35] ionospheric residual sigma.
...
# Simulate LEO pseudoranges with TLE/SGP4.
class LEOSimulator:
...
# Masked-CLA output and recurrent state.
...
# Yan Eqs. (22)-(23) masked convolution.
class MaskedConv(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        if x.ndim != 2 or mask.ndim != 2:
            raise ValueError('MaskedConv expects x/mask=[B,D]')
        if x.shape != mask.shape:
            raise ValueError('MaskedConv x/mask shapes do not match')
...
# Yan Eqs. (24)-(25) temporal masked LSTM; k is time.
class MaskedLSTM(nn.Module):
    ...
    def forward(
        ...
        if x.ndim != 3 or mask.ndim != 2:
            raise ValueError('MaskedLSTM expects x=[B,D,F] and mask=[B,D]')
        if x.shape[:2] != mask.shape:
            raise ValueError('MaskedLSTM x/mask position shapes do not match')
        if x.shape[2] != self.input_size:
            raise ValueError(
                f'MaskedLSTM feature size must be {self.input_size}, got {x.shape[2]}'
            )
...
class MaskedCLA(nn.Module):
    def __init__(self, nmax: int, dropout: float=0.2) -> None:
        ...
        # Yan does not publish KG-head initialization; zero gain gives a safe baseline.
    ...
    def forward(self, fixed: torch.Tensor, observations: torch.Tensor, mask: torch.Tensor, channel_mask: torch.Tensor, recurrent_state: tuple[torch.Tensor, torch.Tensor] | None=None) -> CLAOutput:
        ...
        for batch_index, count in enumerate(satellite_count.detach().cpu().tolist()):
            ...
            # Yan Eqs. (15)-(16): fixed features, residual, innovation, then zero padding.
            packed_values.append(torch.cat((
                fixed[batch_index],                         # Delta alpha, Delta omega, Delta xhat, Delta xtilde
                observations[batch_index, :count, 0],       # Delta y_{k-1}
                observations[batch_index, :count, 1],       # Delta ytilde_k
                observations.new_zeros(suffix_length),      # Eq. (16) zero-padding
            ), dim=0))
        ...
        # Yan: CNN over padded positions, LSTM across epochs k, attention over valid positions.
...
# Apply learned gain: dx = K_net @ innovation.
...
if __name__ == '__main__':
    ...
    OUTPUT_DIR = Path(os.environ.get('MKNET_OUTPUT_DIR') or ('/kaggle/working/direct_run' if Path('/kaggle/input').is_dir() and Path('/kaggle/working').is_dir() else 'direct_run')).expanduser()
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    # Default cap gives about 1600 lagged Data01 samples; environment variables override it.
    MAX_FUSION_EPOCHS = None if os.environ.get('MKNET_MAX_FUSION_EPOCHS', '1601').strip().lower() in {'', 'none', 'all'} else int(os.environ.get('MKNET_MAX_FUSION_EPOCHS', '1601'))
    MAX_TEST_FUSION_EPOCHS = None if os.environ.get('MKNET_MAX_TEST_FUSION_EPOCHS', '300').strip().lower() in {'', 'none', 'all'} else int(os.environ.get('MKNET_MAX_TEST_FUSION_EPOCHS', '300'))

    # Recursive TBPTT with validation selection; LR=1e-4 is the stable project default.
    TRAINING_EPOCHS = int(os.environ.get('MKNET_TRAINING_EPOCHS', 20))
    LEARNING_RATE = float(os.environ.get('MKNET_LEARNING_RATE', 1e-4))
    FILTER_BLOCK_LEARNING_RATE = LEARNING_RATE
    REPRESENTATION_BLOCK_LEARNING_RATE = LEARNING_RATE
    OPTIMIZER_WINDOW_SIZE = int(os.environ.get('MKNET_OPTIMIZER_WINDOW_SIZE', 4))
    GRADIENT_CLIP_NORM = float(os.environ.get('MKNET_GRADIENT_CLIP_NORM', 1.0))
    EARLY_STOPPING_PATIENCE = int(os.environ.get('MKNET_EARLY_STOPPING_PATIENCE', 6))
    ...
    missing = [str(path) for path in (README_XML_PATH, IMU_ERROR_MODEL_PATH, ROVE_GROUND_TRUTH_PATH, IMU_GROUND_TRUTH_PATH, RINEX_OBS_PATH, IMR_PATH, SP3_PATH, CLK_PATH, NAV_PATH, LEO_TLE_DIR) if not Path(path).exists()]
    if missing:
        raise FileNotFoundError('Missing input files:\n' + '\n'.join(missing))
    rover_xml = next((item for item in ET.fromstring(README_XML_PATH.read_text(encoding='utf-8', errors='replace')).findall('ROVE') if (item.findtext('ID') or '').strip() == '01'), None)
    ...
    truth_reference_error_m = np.linalg.norm(antenna_position[0] - (initial_imu_truth_position + C_b_e @ lever_arm_b_m))
    ...
    d2r = np.pi / 180.0
    P0 = np.diag(np.concatenate((imu_model_values['ISDV_Pos'], imu_model_values['ISDV_Vel'], imu_model_values['ISDV_Att'] * d2r)) ** 2)
    Qc = np.diag(np.concatenate((imu_model_values['PNSD_Pos'], imu_model_values['PNSD_Vel'], imu_model_values['PNSD_Att'] * d2r)) ** 2)
    ...
    gnss_preprocessor = GNSSProcessor(SP3_PATH, CLK_PATH, min_elevation_deg=MIN_GNSS_ELEVATION_DEG, use_ionosphere=USE_IONOSPHERE, use_troposphere=USE_TROPOSPHERE, broadcast_ionosphere_coefficients=ionosphere_coefficients)
    ...
    for event_kind, event_start, event_end, event_index in fusion_timeline(imr_time, fusion_time, through_last_fusion=True):
        ...
        if not measurements:
            continue
        prior_nav = NavState(nav.position_ecef_m.copy(), nav.velocity_ecef_mps.copy(), nav.body_to_ecef_dcm.copy())
        measurement_model = measurement_model(nav, measurements, lever_arm_b_m)
        correction, P, _ = kf_update(P, measurement_model.innovation, measurement_model.H, measurement_model.R)
        nav = inject_error(nav, correction)
        posterior_residual = innovation_only(nav, measurements, lever_arm_b_m)
        # Freeze y_k; only h(xhat_{k|k-1}) and innovation are recomputed recursively.
        fixed_measurements = tuple(measurements)
        if tuple(m.sat_id for m in fixed_measurements) != tuple(measurement_model.sat_ids):
        ...
        history_rows.append({
            ...
            'gyro': last_gyro.copy(),
            'target_state_9': np.concatenate([fusion_imu_truth_position[fusion_index] - prior_nav.position_ecef_m, fusion_imu_truth_velocity[fusion_index] - prior_nav.velocity_ecef_mps, SpatialRotation.from_matrix(fusion_truth_body_to_ecef[fusion_index] @ prior_nav.body_to_ecef_dcm.T).as_rotvec() / ATTITUDE_FEEDBACK_SIGN]),
    ...
    # Nmax is fixed from Data01 only.
    ...
    if TEST_DATASET_DIR is not None:
        ...
        if len(test_dataset_candidates) != 1:
            raise RuntimeError(f"TEST_DATASET_DIR is not set and automatic selection is ambiguous. Found {len(test_dataset_candidates)} non-training dataset directories: {', '.join(path.name for path in test_dataset_candidates) or 'none'}. Set TEST_DATASET_DIR to the exact second SmartPNT-Pos dataset.")
    ...
    for k in range(1, len(history_rows)):
        ...
        # Yan Eq. (15) fixed order; state terms use the causal previous epoch.
        ...
        innovation_k = np.zeros(network_nmax)
        innovation_k[:n] = current['innovation']
        current_mask = np.zeros(network_nmax, dtype=bool)
        current_mask[:n] = True
        previous_residual_by_sat = dict(zip(previous['sat_ids'], previous['residual']))
        residual_k = np.zeros(network_nmax)
        residual_mask = np.zeros(network_nmax, dtype=bool)
    ...
    if innovation_padded.shape != (expected_t, network_nmax):
        raise ValueError('innovation_padded must have shape [T,Nmax]')
    if channel_masks.shape != (expected_t, network_nmax, OBSERVATION_FEATURE_DIM):
    ...
    # Normalize each fixed group and each valid observation channel independently.
    ...
    def _normalize_fixed(fixed_raw: Array) -> Array:
        ...
        return np.concatenate([
            _l2_norm(fixed_raw[0:3]),    # Delta alpha
            _l2_norm(fixed_raw[3:6]),    # Delta omega
            _l2_norm(fixed_raw[6:15]),   # Delta xhat
            _l2_norm(fixed_raw[15:24]),  # Delta xtilde
        ])
    ...
    fixed_normalized = np.stack([
        _normalize_fixed(row) for row in fixed
    ])
    observations_normalized = np.stack([
        _normalize_obs(obs_row, mask_row)
    ...
    # --- MASKED-CLA + ALTERNATING TRAINING ---
    # Project split: CNN=representation; LSTM+attention+KG=filter. Both use Yan's state loss.
    ...
    # Build and normalize Yan Eqs. (10)-(17) input.
    ...
    # Recursive Data01 rollout with TBPTT carry.
        ...
        with (torch.enable_grad() if training else torch.inference_mode()):
            for sample_index in range(start_sample, stop_sample):
                ...
                if row['preceding_interval_segments']:
                    nav_state, feature_gyro, feature_accel = _propagate(
                ...
                max_prior_position_error_m = max(max_prior_position_error_m, float(torch.linalg.vector_norm(
                ...
                measurements = tuple(clock_observable(measurements))
                variances = np.asarray([float(m.variance_m2) for m in measurements], dtype=float)
                prepared = (
                ...
                fixed_nn, obs_nn, mask_nn, channel_nn, innovation_nn = _network_input(
                ...
                max_gain_fro_norm = max(max_gain_fro_norm, float(torch.linalg.matrix_norm(output.kalman_gain[0, :, :len(current_sat_ids)]).detach().cpu()))
                ...
                if sample_index >= metric_start:
                    state_error = torch.cat((
                        ...
                        _torch_so3_log(fusion_truth_dcm_t[fusion_index].reshape(3, 3) @ nav_state.body_to_ecef_dcm.reshape(3, 3).T) / ATTITUDE_FEEDBACK_SIGN,
    ...
    # Train one alternating block over the chronological trajectory.
    def _train_block(phase: str, optimizer: torch.optim.Optimizer, epoch: int):
    ...
    # Causal recursive evaluation through the requested prefix.
    ...
    def _monitor(scope: str, metric_start: int, metric_stop: int):
        try:
            metrics = _evaluate(metric_start, metric_stop)
    ...
    # Nonrecursive Yan X/M/Y diagnostic.
    def _teacher_forced():
        model.eval()
        model.lstm.lstm.dropout = LSTM_CONFIGURED_DROPOUT
        state_sum = 0.0
    ...
    pretraining_zero_gain_metrics = _monitor(
        'pre-training zero-gain Data01 training rollout', 0, training_sample_count
    )
    pretraining_validation_metrics = _monitor(
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        try:
            filter_phase = _train_block('filter', filter_optimizer, epoch)
            representation_phase = _train_block('representation', representation_optimizer, epoch)
        except FloatingPointError as error:
    ...
    final_training_metrics = _evaluate(0, training_sample_count)
    final_validation_metrics = _evaluate(validation_start_sample, data01_total_sample_count)
    final_teacher_forced_metrics = _teacher_forced()
    ...
    # Store the completed causal fusion context.
    def _online_context(sat_ids, posterior_residual, x_pred, x_post, accel, gyro, previous_context=None):
        ...
        context = {'sat_ids': tuple(sat_ids), 'residual': np.asarray(posterior_residual, dtype=float).copy(), 'x_pred': x_pred.copy(), 'x_post': x_post.copy(), 'state_innovation': (x_post - x_pred).copy(), 'state_residual': (np.zeros(INS_STATE_DIM) if previous_context is None else x_post - np.asarray(previous_context['x_post'], dtype=float).reshape(INS_STATE_DIM)).copy(), 'accel': np.asarray(accel, dtype=float).reshape(3).copy(), 'gyro': np.asarray(gyro, dtype=float).reshape(3).copy()}
        ...
        return context
    # Build one causal online feature set.
    def _online_features(previous_online, current_sat_ids, innovation_now, current_accel, current_gyro):
        n = len(current_sat_ids)
        ...
        fixed_k = _normalize_fixed(fixed_k_raw)
        obs_k = _normalize_obs(obs_k_raw, channel_k)
    ...
    # Pad one online epoch to fixed Nmax.
    def _pad_epoch(fixed_k, obs_k, current_mask, channel_k, innovation_k, slot_capacity: int):
    ...
    # Compare Data02 raw features with the Data01 envelope.
    def _feature_shift(fixed_raw, obs_raw, channel_mask):
        fixed_raw = np.asarray(fixed_raw, dtype=float).reshape(FIXED_FEATURE_DIM)
        obs_raw = np.asarray(obs_raw, dtype=float).reshape(-1, OBSERVATION_FEATURE_DIM)
        channel_mask = np.asarray(channel_mask, dtype=bool).reshape(-1, OBSERVATION_FEATURE_DIM)
        obs_outside_count = 0
        ...
        return {'fixed_ood_fraction': float(np.mean((fixed_raw < train_fixed_min) | (fixed_raw > train_fixed_max))), 'fixed_max_abs_train_ratio': float(np.max(np.abs(fixed_raw) / train_fixed_abs_max)), 'obs_ood_fraction': float(obs_outside_count / obs_valid_count) if obs_valid_count else 0.0, 'obs_max_abs_train_ratio': float(obs_ratio)}
    ...
    # Recursive Eq. (30) metric used by checkpoint selection.
    epoch_loss_rows = [
    ...
    if epoch_loss_rows:
        ...
        if finite_positive.size and float(np.max(finite_positive) / max(np.min(finite_positive), 1e-300)) >= 100.0:
            ax.set_yscale('log')
    ...
    if RUN_RECURSIVE_DATA01_DIAGNOSTIC:
        ...
        data01_diag_leo = LEOSimulator(tle_provider, leo_klobuchar, seed=LEO_SEED, tx_epsilon_position_m=LEO_TX_EPSILON_POSITION_M, tx_max_iterations=LEO_TX_MAX_ITERATIONS, minimum_elevation_deg=LEO_MIN_ELEVATION_DEG, prefilter_guard_deg=LEO_PREFILTER_GUARD_DEG, use_ionosphere=USE_IONOSPHERE, use_troposphere=USE_TROPOSPHERE)
        diag_nav = NavState(initial_nav.position_ecef_m.copy(), initial_nav.velocity_ecef_mps.copy(), initial_nav.body_to_ecef_dcm.copy())
        diag_P = P0.copy()
        diag_rows = []
        diag_previous = None
        diag_warm_start = None
        diag_recurrent_state = None
        classical_position_by_fusion_index = {int(row['fusion_index']): row['posterior_nav'].position_ecef_m + row['posterior_nav'].body_to_ecef_dcm @ np.asarray(lever_arm_b_m).reshape(3) for row in history_rows}
        ...
        try:
            for event_kind, event_start, event_end, event_index in fusion_timeline(imr_time, fusion_time, through_last_fusion=True):
                if event_kind == 'imu':
                    ...
                    diag_feature_gyro, diag_feature_accel = (diag_last_gyro, diag_last_accel)
                    diag_nav = mechanize(diag_nav, diag_last_gyro, diag_last_accel, dt)
                    diag_Phi, diag_Qd = van_loan(error_dynamics(diag_nav, diag_last_accel), Qc, dt)
                ...
                diag_measurements = clock_observable(tuple(gnss_preprocessor.prepare_epoch(epoch, diag_nav, lever_arm_b_m)) + tuple(data01_diag_leo.simulate_epoch(t, fusion_antenna_truth_position[fusion_index])))
                if not diag_measurements:
                    continue
                truth_position_now = fusion_antenna_truth_position[fusion_index]
                prior_error_3d_m = float(np.linalg.norm(diag_nav.position_ecef_m + diag_nav.body_to_ecef_dcm @ np.asarray(lever_arm_b_m).reshape(3) - truth_position_now))
                ...
                if diag_previous is None:
                    warm_model = measurement_model(diag_nav, diag_measurements, lever_arm_b_m)
                    warm_correction, diag_P, _ = kf_update(diag_P, warm_model.innovation, warm_model.H, warm_model.R)
                    diag_nav = inject_error(diag_nav, warm_correction)
                    diag_previous = _online_context(warm_model.sat_ids, innovation_only(diag_nav, diag_measurements, lever_arm_b_m), np.zeros(INS_STATE_DIM), warm_correction, diag_feature_accel, diag_feature_gyro, previous_context=None)
                    diag_warm_start = {'method': 'ordinary_TC_KF_context_only', 'time_gpst_s': float(t), 'excluded_from_learned_metrics': True}
                    continue
                diag_model = measurement_model(diag_nav, diag_measurements, lever_arm_b_m)
                current_sat_ids = diag_model.sat_ids
                fixed_k, obs_k, current_mask, channel_k, innovation_k, fixed_k_raw, obs_k_raw = _online_features(diag_previous, current_sat_ids, diag_model.innovation, diag_feature_accel, diag_feature_gyro)
                fixed_nn, obs_nn, mask_nn, channel_nn, innovation_nn = _pad_epoch(fixed_k, obs_k, current_mask, channel_k, innovation_k, network_nmax)
                with torch.inference_mode():
                    ...
                    diag_correction = state_update(diag_output, torch.tensor(innovation_nn[None], dtype=torch.float32, device=DEVICE))[0]
                diag_correction = diag_correction.cpu().numpy().astype(float)
                active_gain = diag_output.kalman_gain[0].cpu().numpy().astype(float)[:, :len(diag_measurements)]
                ...
                diag_P = learned_covariance(diag_P, active_gain, diag_model.H, diag_model.R, diag_correction)
                diag_nav = inject_error(diag_nav, diag_correction)
                posterior_position = diag_nav.position_ecef_m + diag_nav.body_to_ecef_dcm @ np.asarray(lever_arm_b_m).reshape(3)
                posterior_error_3d_m = float(np.linalg.norm(posterior_position - truth_position_now))
                posterior_residual = innovation_only(diag_nav, diag_measurements, lever_arm_b_m)
                diag_previous = _online_context(current_sat_ids, posterior_residual, np.zeros(INS_STATE_DIM), diag_correction, diag_feature_accel, diag_feature_gyro, previous_context=diag_previous)
                diag_rows.append({'time': t, 'position': posterior_position.copy(), 'truth': truth_position_now.copy(), 'prior_error_3d_m': prior_error_3d_m, 'posterior_error_3d_m': posterior_error_3d_m, 'classical_posterior_error_3d_m': classical_error_3d_m, 'gain_fro_norm': float(np.linalg.norm(active_gain)), 'correction_position_norm_m': float(np.linalg.norm(diag_correction[:3])), 'measurement_innovation_max_abs_m': float(np.max(np.abs(diag_model.innovation))) if len(diag_model.innovation) else 0.0, 'obs_max_abs_train_ratio': float(_feature_shift(fixed_k_raw, obs_k_raw, channel_k)['obs_max_abs_train_ratio'])})
        finally:
        ...
        diag_fields = ('time', 'prior_error_3d_m', 'posterior_error_3d_m', 'classical_posterior_error_3d_m', 'gain_fro_norm', 'correction_position_norm_m', 'measurement_innovation_max_abs_m', 'obs_max_abs_train_ratio')
        ...
        for i, (est, truth_i) in enumerate(zip(diag_estimate, diag_truth)):
            ...
            diag_ned_error[i] = ecef_to_ned(lat, lon) @ (est - truth_i)
        diag_error_3d = np.linalg.norm(diag_ned_error, axis=1)
        diag_rmse = np.append(np.sqrt(np.mean(diag_ned_error ** 2, axis=0)), float(np.sqrt(np.mean(diag_error_3d ** 2))))
        learned_gate = np.asarray(diag_error_3d, dtype=float).reshape(-1)
        classical_gate = np.asarray([row['classical_posterior_error_3d_m'] for row in diag_rows], dtype=float).reshape(-1)
        ...
        data01_recursive_summary = {'epochs': int(len(diag_rows)), 'causal_warm_start': diag_warm_start, 'rmse_n_e_d_3d_m': [float(x) for x in diag_rmse], 'recursive_same_epoch_acceptance_gate': data01_acceptance_gate, 'first_error_gt_100m': _diag_first(diag_error_3d > 100.0), 'first_error_gt_1km': _diag_first(diag_error_3d > 1000.0), 'first_error_gt_100km': _diag_first(diag_error_3d > 100000.0), 'first_obs_feature_gt_10x_training_absmax': _diag_first(np.asarray([row['obs_max_abs_train_ratio'] for row in diag_rows], dtype=float) > 10.0), 'max_innovation_abs_m': float(np.nanmax([row['measurement_innovation_max_abs_m'] for row in diag_rows])), 'max_gain_fro_norm': float(np.nanmax([row['gain_fro_norm'] for row in diag_rows])), 'max_position_correction_norm_m': float(np.nanmax([row['correction_position_norm_m'] for row in diag_rows])), 'scope': 'frozen_weights_same_Data01_recursive_diagnostic_before_Data02'}
        with (OUTPUT_DIR / 'recursive_data01_diagnostics.csv').open('w', newline='', encoding='utf-8') as stream:
    ...
    test_rover_xml = next((item for item in ET.fromstring(test_readme_path.read_text(encoding='utf-8', errors='replace')).findall('ROVE') if (item.findtext('ID') or '').strip() == '01'), None)
    ...
    test_missing = [str(path) for path in (IMU_ERROR_MODEL_PATH, LEO_TLE_DIR, test_imu_ground_truth_path, test_imr_path) if not Path(path).exists()]
    if test_missing:
    ...
    test_imu_truth = load_truth(test_imu_ground_truth_path)
    ...
    test_imr_path, test_imr_data_rate_hz, test_imr_gyro_scale, test_imr_accel_scale, test_imr_time_tag_bias_ms, test_imr_record_dtype, test_imr_record_count, test_imr_record_bytes = read_imr_layout(test_imr_path)
    ...
    test_requested_stop = min(int(np.searchsorted(test_imr_time_all, test_fusion_time[-1], side='left')) + 1, min(int(np.searchsorted(test_imr_time_all, test_common_end, side='left')) + 1, len(test_imr_time_all)))
    ...
    test_truth_reference_error_m = np.linalg.norm(test_antenna_position[0] - (test_imu_position[0] + test_C_b_e @ test_lever_arm_b_m))
    ...
    test_initial_nav = NavState(test_imu_position[0].copy(), test_imu_velocity[0].copy(), test_C_b_e)
    test_P0 = np.diag(np.concatenate((test_imu_model_values['ISDV_Pos'], test_imu_model_values['ISDV_Vel'], test_imu_model_values['ISDV_Att'] * d2r)) ** 2)
    test_Qc = np.diag(np.concatenate((test_imu_model_values['PNSD_Pos'], test_imu_model_values['PNSD_Vel'], test_imu_model_values['PNSD_Att'] * d2r)) ** 2)
    ...
    test_gnss_preprocessor = GNSSProcessor(unique_file(test_root, ('*.SP3', '*.sp3'), 'SP3 precise-orbit'), unique_file(test_root, ('*.CLK', '*.clk'), 'RINEX clock'), min_elevation_deg=MIN_GNSS_ELEVATION_DEG, use_ionosphere=USE_IONOSPHERE, use_troposphere=USE_TROPOSPHERE, broadcast_ionosphere_coefficients=test_ionosphere_coefficients)
    test_leo_klobuchar = None if test_ionosphere_coefficients is None else test_ionosphere_coefficients['G']
    test_leo_simulator = LEOSimulator(tle_provider, test_leo_klobuchar, seed=TEST_LEO_SEED, tx_epsilon_position_m=LEO_TX_EPSILON_POSITION_M, tx_max_iterations=LEO_TX_MAX_ITERATIONS, minimum_elevation_deg=LEO_MIN_ELEVATION_DEG, prefilter_guard_deg=LEO_PREFILTER_GUARD_DEG, use_ionosphere=USE_IONOSPHERE, use_troposphere=USE_TROPOSPHERE)
    print('\n=== DATA02 INDEPENDENT TEST ===')
    model = model.to(DEVICE)
    model.eval()
    nav = NavState(test_initial_nav.position_ecef_m.copy(), test_initial_nav.velocity_ecef_mps.copy(), test_initial_nav.body_to_ecef_dcm.copy())
    P = test_P0.copy()
    online_rows = []
    ...
    for event_kind, event_start, event_end, event_index in fusion_timeline(test_imr_time, test_fusion_time, through_last_fusion=True):
        if event_kind == 'imu':
            ...
            nav = mechanize(nav, last_gyro, last_accel, dt)
            F_error = error_dynamics(nav, last_accel)
            Phi, Qd = van_loan(F_error, test_Qc, dt)
            P = Phi @ P @ Phi.T + Qd
            P = 0.5 * (P + P.T)
            continue
        fusion_index = event_index
        ...
        prior_error_3d_m = float(np.linalg.norm(nav.position_ecef_m + nav.body_to_ecef_dcm @ np.asarray(test_lever_arm_b_m).reshape(3) - truth_position_now))
        if previous_online is None:
            warm_measurement_model = measurement_model(nav, measurements, test_lever_arm_b_m)
            warm_correction, P, _ = kf_update(P, warm_measurement_model.innovation, warm_measurement_model.H, warm_measurement_model.R)
            nav = inject_error(nav, warm_correction)
            previous_online = _online_context(warm_measurement_model.sat_ids, innovation_only(nav, measurements, test_lever_arm_b_m), zero_error_state_np, warm_correction, last_feature_accel, last_feature_gyro, previous_context=None)
            ...
            continue
        full_measurement_model = measurement_model(nav, measurements, test_lever_arm_b_m)

        # The neural architecture is frozen at the Data01-derived Nmax.
        # If Data02 has more observations, only the neural input is capped.
        data02_nmax_before_capacity = max(
        ...
        measurement_model = full_measurement_model if pre_capacity_count == len(measurements) else measurement_model(nav, measurements, test_lever_arm_b_m)
        n = len(measurements)
        current_sat_ids = measurement_model.sat_ids
        fixed_k, obs_k, current_mask, channel_k, innovation_k, fixed_k_raw, obs_k_raw = _online_features(previous_online, current_sat_ids, measurement_model.innovation, last_feature_accel, last_feature_gyro)
        full_innovation = np.asarray(full_measurement_model.innovation, dtype=float)
        fixed_nn, obs_nn, mask_nn, channel_nn, innovation_nn = _pad_epoch(fixed_k, obs_k, current_mask, channel_k, innovation_k, test_recurrent_capacity)
        with torch.inference_mode():
            ...
            correction = state_update(output, torch.tensor(innovation_nn[None], dtype=torch.float32, device=DEVICE))[0]
        correction = correction.cpu().numpy().astype(float)
        active_gain = output.kalman_gain[0].cpu().numpy().astype(float)[:, :n]
        ...
        P = learned_covariance(P, active_gain, measurement_model.H, measurement_model.R, correction)
        nav = inject_error(nav, correction)
        ...
        previous_online = _online_context(current_sat_ids, innovation_only(nav, measurements, test_lever_arm_b_m), zero_error_state_np, correction.copy(), last_feature_accel, last_feature_gyro, previous_context=previous_online)
        counts = Counter((m.constellation for m in measurements))
        online_rows.append({'time': t, 'position': posterior_position.copy(), 'truth': test_fusion_antenna_truth_position[fusion_index].copy(), 'hpl_m': hpl_m, 'vpl_m': vpl_m, 'n_input': pre_capacity_count, 'n_total': n, 'n_gnss': counts['G'] + counts['C'], 'n_leo': counts['L'], 'sat_ids': current_sat_ids, 'prior_error_3d_m': prior_error_3d_m, 'posterior_error_3d_m': posterior_error_3d_m, 'measurement_innovation_rms_m': float(np.sqrt(np.mean(full_innovation ** 2))) if full_innovation.size else 0.0, 'measurement_innovation_max_abs_m': float(np.max(np.abs(full_innovation))) if full_innovation.size else 0.0, 'knet_innovation_rms_m': float(np.sqrt(np.mean(innovation_k ** 2))) if innovation_k.size else 0.0, 'knet_innovation_max_abs_m': float(np.max(np.abs(innovation_k))) if innovation_k.size else 0.0, 'gain_fro_norm': float(np.linalg.norm(active_gain)), 'gain_max_abs': float(np.max(np.abs(active_gain))) if active_gain.size else 0.0, 'gain_navigation_rows_fro_norm': float(np.linalg.norm(active_gain[:SUPERVISED_STATE_DIM, :])), 'correction_position_norm_m': float(np.linalg.norm(correction[0:3])), 'correction_velocity_norm_mps': float(np.linalg.norm(correction[3:6])), 'correction_attitude_norm_rad': float(np.linalg.norm(correction[6:9])), **_feature_shift(fixed_k_raw, obs_k_raw, channel_k), 'fusion_mode': 'MaskedCLA'})
    if not online_rows:
    ...
    rmse = np.append(np.sqrt(np.mean(ned_error ** 2, axis=0)), float(np.sqrt(np.mean(error_3d ** 2))))
    diagnostic_fields = ('prior_error_3d_m', 'posterior_error_3d_m', 'measurement_innovation_rms_m', 'measurement_innovation_max_abs_m', 'knet_innovation_rms_m', 'knet_innovation_max_abs_m', 'gain_fro_norm', 'gain_max_abs', 'gain_navigation_rows_fro_norm', 'correction_position_norm_m', 'correction_velocity_norm_mps', 'correction_attitude_norm_rad', 'fixed_ood_fraction', 'fixed_max_abs_train_ratio', 'obs_ood_fraction', 'obs_max_abs_train_ratio')
    diagnostic_arrays = {name: np.asarray([row[name] for row in online_rows], dtype=float) for name in diagnostic_fields}
    def _first_event(mask):
    ...
    divergence_diagnostics = {'first_error_gt_100m': _first_event(error_3d > 100.0), 'first_error_gt_1km': _first_event(error_3d > 1000.0), 'first_error_gt_100km': _first_event(error_3d > 100000.0), 'first_error_growth_gt_10x': _first_event(growth_ratio > 10.0), 'first_obs_feature_gt_10x_training_absmax': _first_event(diagnostic_arrays['obs_max_abs_train_ratio'] > 10.0), 'max_measurement_innovation_abs_m': float(np.nanmax(diagnostic_arrays['measurement_innovation_max_abs_m'])), 'max_knet_innovation_abs_m': float(np.nanmax(diagnostic_arrays['knet_innovation_max_abs_m'])), 'max_gain_fro_norm': float(np.nanmax(diagnostic_arrays['gain_fro_norm'])), 'max_position_correction_norm_m': float(np.nanmax(diagnostic_arrays['correction_position_norm_m'])), 'max_fixed_feature_abs_training_ratio': float(np.nanmax(diagnostic_arrays['fixed_max_abs_train_ratio'])), 'max_observation_feature_abs_training_ratio': float(np.nanmax(diagnostic_arrays['obs_max_abs_train_ratio']))}
    np.savez(OUTPUT_DIR / 'test_evaluation.npz', dataset_dir=np.asarray(str(test_dataset_dir)), time_gpst_s=online_time, estimate_ecef_m=estimate, truth_ecef_m=truth_aligned, ned_error_m=ned_error, error_3d_m=error_3d, horizontal_error_m=horizontal_error, vertical_error_m=vertical_error, hpl_m=hpl, vpl_m=vpl, **diagnostic_arrays, error_growth_ratio=growth_ratio, rmse_north_east_down_3d_m=rmse, cdf_probability=np.arange(1, len(error_3d) + 1) / len(error_3d), north_cdf_absolute_error_m=np.sort(np.abs(ned_error[:, 0])), east_cdf_absolute_error_m=np.sort(np.abs(ned_error[:, 1])), down_cdf_absolute_error_m=np.sort(np.abs(ned_error[:, 2])), three_d_cdf_error_m=np.sort(error_3d))
    with (OUTPUT_DIR / 'test_trajectory.csv').open('w', newline='', encoding='utf-8') as stream:
        writer = csv.writer(stream)
        writer.writerow(['time_gpst_s', 'n_input_before_capacity', 'n_used', 'n_gnss', 'n_leo', 'sat_ids_used', 'fusion_mode', 'hpl_m', 'vpl_m', 'x_ecef_m', 'y_ecef_m', 'z_ecef_m'])
        for row in online_rows:
            writer.writerow([row['time'], row['n_input'], row['n_total'], row['n_gnss'], row['n_leo'], ';'.join(row['sat_ids']), row['fusion_mode'], row['hpl_m'], row['vpl_m'], *row['position'].tolist()])
    ...
    if RUN_CLASSICAL_TEST_BASELINE:
        baseline_leo_simulator = LEOSimulator(tle_provider, test_leo_klobuchar, seed=TEST_LEO_SEED, tx_epsilon_position_m=LEO_TX_EPSILON_POSITION_M, tx_max_iterations=LEO_TX_MAX_ITERATIONS, minimum_elevation_deg=LEO_MIN_ELEVATION_DEG, prefilter_guard_deg=LEO_PREFILTER_GUARD_DEG, use_ionosphere=USE_IONOSPHERE, use_troposphere=USE_TROPOSPHERE)
        baseline_nav = NavState(test_initial_nav.position_ecef_m.copy(), test_initial_nav.velocity_ecef_mps.copy(), test_initial_nav.body_to_ecef_dcm.copy())
        ...
        for event_kind, event_start, event_end, event_index in fusion_timeline(test_imr_time, test_fusion_time, through_last_fusion=True):
            if event_kind == 'imu':
                ...
                baseline_nav = mechanize(baseline_nav, baseline_last_gyro, baseline_last_accel, dt)
                F_baseline = error_dynamics(baseline_nav, baseline_last_accel)
                Phi_baseline, Qd_baseline = van_loan(F_baseline, test_Qc, dt)
            ...
            baseline_measurements = clock_observable(tuple(baseline_gnss) + tuple(baseline_leo))
            if baseline_measurements:
                baseline_model = measurement_model(baseline_nav, baseline_measurements, test_lever_arm_b_m)
                baseline_dx, baseline_P, _ = kf_update(baseline_P, baseline_model.innovation, baseline_model.H, baseline_model.R)
                baseline_nav = inject_error(baseline_nav, baseline_dx)
            baseline_position = baseline_nav.position_ecef_m + baseline_nav.body_to_ecef_dcm @ np.asarray(test_lever_arm_b_m).reshape(3)
        ...
        if baseline_rows:
            ...
            for i, (est, truth_i) in enumerate(zip(baseline_estimate, baseline_truth)):
                lat, lon, _ = ecef_llh(truth_i)
                baseline_ned_error[i] = ecef_to_ned(lat, lon) @ (est - truth_i)
    ...
    summary = {
        ...
        'training': {
            ...
            'optimizer': 'Adam',
            'learning_rate': float(LEARNING_RATE),
            'Yan_reported_initial_learning_rate': 0.01,
        ...
        'paper_components': {
            'FDE_DIA': 'removed_by_project_request',
            'integrity_enabled': bool(INTEGRITY is not None),
