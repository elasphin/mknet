...
- FDE is enabled in the online closed-loop path. Detection/identification follow
  Yan et al. Eq. (33) together with the DIA formulation of Ref. [33] Eqs. (4)-(8).
  The INS-predicted innovation is tested before the learned KG is applied. Each
  pseudorange is represented by a 1-D measurement-fault hypothesis. When a fault
  hypothesis is selected, the code now applies Yan et al. Eq. (34) directly:
      x_i = x_0^+ - L_i nu
      P_i = P_0^+ + L_i Q_nu_nu L_i^T
  where L_i is formed from Ref. [33] Appendix Eq. (39). In the KalmanNet hybrid,
  x_0^+ is the nominal learned-KG posterior and P_0^+ is the corresponding
  Joseph covariance bookkeeping already used by this reproduction. This is the
  closest literal integration of Yan Eq. (34) with the paper's learned KG.
  The earlier implementation that physically deleted the identified pseudorange
  and reran Masked-CLA has been removed because that is not the DIA adaptation
  written in Eq. (34).
...
class Ref33FDEResult:
    """DIA decision computed from the pre-update INS-predicted innovation."""
    ...
    dof: int
    unresolved: bool
    local_identification_probability: float
...
def _ref33_psd_pinv_and_rank(matrix: Array) -> tuple[Array, int]:
    ...
    # Ref. [33] does not introduce an absolute covariance-eigenvalue cutoff.
    # Use only a machine-precision-relative tolerance. The previous 1e-10*scale
    # floor could incorrectly remove valid residual-space modes when Q_nu_nu
    # became strongly scaled during a recursive rollout.
    tolerance = (
        100.0
        * np.finfo(float).eps
        * max(matrix.shape)
...
def _ref33_identify_single_pseudorange_fault(
    ...
    pseudorange-only reproduction the minimal transparent completion is one
    one-dimensional measurement-fault hypothesis per pseudorange.

    Before receiver-clock nuisance elimination, a fault in raw observation j has
    direction e_j. In the actual residual space used by this project,

        C_j = clock_projector @ e_j.

    For q_j=1 the local statistic is
        (C_j^T Q^-1 nu)^2 / (C_j^T Q^-1 C_j),
    and Ref. [33] Eq. (7) chooses the hypothesis with the largest chi-square CDF.

    L_i is then computed exactly in the form of Ref. [33] Appendix Eq. (39):
        L_i = P^- H^T Q^-1 C_i C_i^+.
    """
    nu = np.asarray(measurement_model.innovation, dtype=float)
    ...
    for j in range(len(nu)):
        ...
        # C_i^T Q^+ C_i has units of 1/m^2. A fixed cutoff such as 1e-14
        # is therefore not scale invariant and is not part of Ref. [33].
        # A valid fault direction is rejected only when its weighted norm is
        # numerically indistinguishable from zero relative to Q^+ and ||C_i||.
        denominator_scale = (
            qpinv_norm_2 * float(C_i @ C_i)
        )
        ...
        probability = float(chi2.cdf(local_statistic, df=1))
        estimated_fault_m = float(numerator / denominator)

        C_i_plus = (C_i @ Q_pinv) / denominator
        Q_Ci = np.outer(C_i, C_i_plus)
        L_i = K0 @ Q_Ci
        dia_state_correction = -(L_i @ nu)
        ...
        candidate = {
            ...
            "L_i": L_i,
            "dia_state_correction": dia_state_correction,
        }
...
def ref33_fde_dia_decision(
    ...
    """Compute Yan/Ref. [33] D-I decision without changing the measurements.

    Ref. [33] Eq. (8) selects exactly one hypothesis after H0 is rejected.
    Therefore this function performs one global test and one Identification
    decision. It does NOT repeatedly delete observations and re-test them.

    The selected L_i is applied later, after the nominal KalmanNet posterior has
    been formed, through Yan et al. Eq. (34). This preserves the required timing:
    Detection/Identification use the raw INS-predicted innovation, before the
    learned KG is applied to the state.
    """
    current = tuple(retain_clock_observable_measurements(measurements))
    model = build_measurement_model(nav, current, lever_arm_b_m)

    if not current:
        return Ref33FDEResult(
            (), model, False, (), (),
            0.0, float("inf"), 0, False,
            0.0, 0.0, None, np.zeros((0, 0)), np.zeros(INS_STATE_DIM),
        )

    statistic, threshold, dof, Q_nu_nu, Q_pinv = _ref33_detection_terms(
        prior_covariance, model, alpha
    )
    ...
    if not enabled or dof == 0 or statistic <= threshold:
        return Ref33FDEResult(
            ...
            0.0, 0.0, None, Q_nu_nu, np.zeros(INS_STATE_DIM),
        )

    identification = _ref33_identify_single_pseudorange_fault(
        prior_covariance, model, Q_pinv
    )
    if identification is None:
        # With one projected 1-D hypothesis per pseudorange, the C_i columns
        # span the same clock-projected residual space. Therefore a nonzero
        # global detection subspace with no identifiable C_i is a numerical
        # inconsistency, not a legitimate Ref. [33] decision outcome.
        raise FloatingPointError(
            "Ref. [33] global test rejected H0 but no projected "
            "pseudorange fault hypothesis had a numerically nonzero "
            "weighted norm; "
            f"statistic={statistic:.6g}, threshold={threshold:.6g}, "
            f"dof={dof}, ||Q_pinv||2={np.linalg.norm(Q_pinv, ord=2):.6g}, "
            f"||innovation||2={np.linalg.norm(model.innovation):.6g}"
        )
    ...
    return Ref33FDEResult(
        ...
        dof,
        False,
        float(identification["probability"]),
        float(identification["estimated_fault_m"]),
        np.asarray(identification["L_i"], dtype=float),
        Q_nu_nu,
        np.asarray(identification["dia_state_correction"], dtype=float),
    )


def apply_yan_eq34_dia_adaptation(
    nominal_correction: Array,
    nominal_posterior_covariance: Array,
    fde_result: Ref33FDEResult,
) -> tuple[Array, Array, Array]:
    """Apply Yan et al. Eq. (34) to the nominal posterior.

    In this KalmanNet reproduction:
      x_0^+  -> nominal learned error-state correction K_net * nu
      P_0^+  -> Joseph covariance bookkeeping for that learned gain

    If a fault is identified,
      x_i = x_0^+ - L_i nu
      P_i = P_0^+ + L_i Q_nu_nu L_i^T.

    The interpretation of x_0^+/P_0^+ as the KalmanNet nominal posterior is a
    necessary hybrid completion: Yan et al. publish Eq. (34) and a learned KG,
    but do not separately state a covariance update equation for the learned KG.
    """
    correction = np.asarray(nominal_correction, dtype=float).reshape(INS_STATE_DIM)
    P0_plus = np.asarray(nominal_posterior_covariance, dtype=float)
    if P0_plus.shape != (INS_STATE_DIM, INS_STATE_DIM):
        raise ValueError("nominal posterior covariance has wrong shape")

    if not fde_result.detected:
        return correction, 0.5 * (P0_plus + P0_plus.T), np.zeros(INS_STATE_DIM)

    if fde_result.unresolved or fde_result.L_i is None:
        raise RuntimeError(
            "FDE detected a fault but no Ref. [33] hypothesis could be identified"
        )

    nu = np.asarray(fde_result.measurement_model.innovation, dtype=float)
    L_i = np.asarray(fde_result.L_i, dtype=float)
    Q_nu_nu = np.asarray(fde_result.Q_nu_nu, dtype=float)

    if L_i.shape != (INS_STATE_DIM, len(nu)):
        raise ValueError("L_i has inconsistent dimensions")
    if Q_nu_nu.shape != (len(nu), len(nu)):
        raise ValueError("Q_nu_nu has inconsistent dimensions")

    dia_term = L_i @ nu
    corrected = correction - dia_term
    ...
    if (
        ...
        or not np.all(np.isfinite(P_i))
        or not np.all(np.isfinite(dia_term))
    ):
        raise FloatingPointError("non-finite Yan Eq. (34) DIA adaptation")

    return corrected, P_i, dia_term
...
def _new_fde_stats() -> dict:
    return {
        ...
        "max_dia_state_correction_norm": 0.0,
...
def _accumulate_fde_stats(stats: dict, result: Ref33FDEResult) -> None:
    ...
    stats["adapted_epochs"] += int(result.detected and not result.unresolved)
    ...
    stats["max_dia_state_correction_norm"] = max(
        stats["max_dia_state_correction_norm"],
        float(np.linalg.norm(result.dia_state_correction)),
    )
...
if __name__ == "__main__":
    ...
    def _recursive_data01_diagnostic_rollout():
        ...
        for event in build_exact_fusion_timeline(
            ...
            if previous_online is None:
                ...
                correction_nominal, P_nominal, _ = kalman_measurement_update(
                    P_diag,
                    measurement_model.innovation,
                    measurement_model.H,
                    measurement_model.R,
                )
                correction, P_diag, _ = apply_yan_eq34_dia_adaptation(
                    correction_nominal,
                    P_nominal,
                    fde_result,
                )
            ...
            P_nominal = learned_gain_covariance_update(
                P_diag,
                active_gain,
                measurement_model.H,
                measurement_model.R,
    ...
    if REQUIRE_RECURSIVE_DATA01_DIAGNOSTIC:
        ...
        print(
            ...
            f"adapted={recursive_data01_diagnostic['fde']['adapted_epochs']}, "
    ...
    for event in test_timeline:
        ...
        if previous_online is None:
            ...
            warm_correction_nominal, P_nominal, _ = kalman_measurement_update(
                P,
                measurement_model.innovation,
                measurement_model.H,
                measurement_model.R,
            )
            warm_correction, P, _ = apply_yan_eq34_dia_adaptation(
                warm_correction_nominal,
                P_nominal,
                fde_result,
            )
        ...
        # Nominal H0 posterior produced by the learned KG. Detection and
        # Identification above used the untouched INS-predicted innovation.
        P_nominal = learned_gain_covariance_update(
            P,
            active_gain,
            measurement_model.H,
        ...
        correction, P, dia_term = apply_yan_eq34_dia_adaptation(
            correction,
            P_nominal,
            fde_result,
        )
        ...
        online_rows.append({
            ...
            "fde_dia_state_correction_norm": float(np.linalg.norm(dia_term)),
    ...
    with (
        ...
        for row in online_rows:
            writer.writerow([
                ...
                row["fde_dia_state_correction_norm"],
    ...
    summary = {
        ...
        "fde_dia_enabled": bool(ENABLE_FDE),
        "fde_protocol": {
            ...
            "adaptation": "Yan_Eq34_xi_equals_x0plus_minus_Li_nu_and_Pi_equals_P0plus_plus_Li_Q_LiT",
            "nominal_x0plus": "KalmanNet_Knet_times_INS_predicted_innovation",
            "nominal_P0plus": "learned_gain_Joseph_covariance_bookkeeping_completion",
            ...
            "multiple_fault_policy": "single_selected_hypothesis_per_Ref33_Eq6_Eq8_no_iterative_deletion",
            "dia_Li": "Ref33_Appendix_Eq39_applied_in_Yan_Eq34",
        ...
        "limitations": [
            ...
            "Yan Eq. (34) is now applied directly after the nominal learned-KG posterior: dx_i=dx_0-L_i*nu and P_i=P_0+L_i*Q_nu_nu*L_i^T. Interpreting x_0+/P_0+ as the KalmanNet nominal posterior is a necessary hybrid completion because Yan et al. do not publish a separate covariance-update equation for the learned KG.",
    ...
    print(
        "FDE test checked/detected/identified/adapted/unresolved:",
        ...
        "/",
        test_fde_stats["adapted_epochs"],
        "/",
        test_fde_stats["unresolved_epochs"],
    )
    print(
        "FDE test max statistic/threshold ratio / max estimated fault / max DIA correction:",
        f"{test_fde_stats['max_statistic_to_threshold_ratio']:.6g}",
        "/",
        f"{test_fde_stats['max_abs_estimated_fault_m']:.3f} m",
        "/",
        f"{test_fde_stats['max_dia_state_correction_norm']:.6g}",
    )
