...
# Yan does not publish an input-scaling rule.
# The official KalmanNet code L2-normalizes each state/observation feature vector
# independently at every time step before KG estimation.  Apply that rule only to
# the four Yan feature groups that correspond to KalmanNet state/observation
# difference features.  The Yan-specific IMU-difference block has no direct
# KalmanNet counterpart, so its existing Data01-only affine conditioning is kept.
FEATURE_STANDARDIZATION_ON = True  # applies only to [delta-alpha, delta-omega]
...
# Official KalmanNet code uses F.normalize(..., p=2, dim=feature_dim, eps=1e-12)
# for obs_diff, obs_innov_diff, fw_evol_diff and fw_update_diff.
KNET_FEATURE_NORM_EPS = 1e-12
IMU_DELTA_DIM = 6
STATE_RESIDUAL_SLICE = slice(IMU_DELTA_DIM, IMU_DELTA_DIM + INS_STATE_DIM)
STATE_INNOVATION_SLICE = slice(
    IMU_DELTA_DIM + INS_STATE_DIM,
    IMU_DELTA_DIM + 2 * INS_STATE_DIM,
)


def _condition_fixed_features_numpy(
    fixed_raw: np.ndarray,
    fixed_mean: np.ndarray,
    fixed_std: np.ndarray,
) -> np.ndarray:
    """KalmanNet-guided conditioning for Yan's fixed Eq. (15) feature block."""
    values = np.asarray(fixed_raw, dtype=float)
    if values.shape[-1] != FIXED_FEATURE_DIM:
        raise ValueError("fixed feature dimension mismatch during conditioning")

    conditioned = values.copy()

    # Yan-specific IMU residuals have no direct KalmanNet feature analogue.
    # Preserve the existing Data01-only affine conditioning only for these 6 terms.
    if FEATURE_STANDARDIZATION_ON:
        conditioned[..., :IMU_DELTA_DIM] = (
            values[..., :IMU_DELTA_DIM]
            - fixed_mean[:IMU_DELTA_DIM]
        ) / fixed_std[:IMU_DELTA_DIM]
    else:
        conditioned[..., :IMU_DELTA_DIM] = values[..., :IMU_DELTA_DIM]

    if KALMANNET_L2_FEATURE_NORMALIZATION_ON:
        for block_slice in (STATE_RESIDUAL_SLICE, STATE_INNOVATION_SLICE):
            block = values[..., block_slice]
            norm = np.linalg.norm(block, ord=2, axis=-1, keepdims=True)
            conditioned[..., block_slice] = block / np.maximum(
                norm, KNET_FEATURE_NORM_EPS
            )
    else:
        conditioned[..., STATE_RESIDUAL_SLICE] = values[..., STATE_RESIDUAL_SLICE]
        conditioned[..., STATE_INNOVATION_SLICE] = values[..., STATE_INNOVATION_SLICE]

    return conditioned


def _condition_observation_features_numpy(
    obs_raw: np.ndarray,
    channel_mask: np.ndarray,
) -> np.ndarray:
    """Masked KalmanNet-style L2 normalization over the valid satellite dimension."""
    values = np.asarray(obs_raw, dtype=float)
    valid = np.asarray(channel_mask, dtype=bool)
    if values.shape != valid.shape or values.shape[-1] != OBSERVATION_FEATURE_DIM:
        raise ValueError("observation feature/mask shape mismatch during conditioning")

    masked = np.where(valid, values, 0.0)
    if not KALMANNET_L2_FEATURE_NORMALIZATION_ON:
        return masked

    # The satellite axis is the penultimate axis for both [N,2] and [T,N,2].
    norm = np.linalg.norm(masked, ord=2, axis=-2, keepdims=True)
    normalized = masked / np.maximum(norm, KNET_FEATURE_NORM_EPS)
    return np.where(valid, normalized, 0.0)


def _condition_fixed_features_torch(
    fixed_raw: torch.Tensor,
    fixed_mean: torch.Tensor,
    fixed_std: torch.Tensor,
) -> torch.Tensor:
    """Differentiable counterpart of _condition_fixed_features_numpy."""
    if fixed_raw.shape[-1] != FIXED_FEATURE_DIM:
        raise ValueError("torch fixed feature dimension mismatch during conditioning")

    imu = fixed_raw[..., :IMU_DELTA_DIM]
    if FEATURE_STANDARDIZATION_ON:
        imu = (
            imu - fixed_mean[:IMU_DELTA_DIM]
        ) / fixed_std[:IMU_DELTA_DIM]

    state_residual = fixed_raw[..., STATE_RESIDUAL_SLICE]
    state_innovation = fixed_raw[..., STATE_INNOVATION_SLICE]
    if KALMANNET_L2_FEATURE_NORMALIZATION_ON:
        state_residual = F.normalize(
            state_residual, p=2, dim=-1, eps=KNET_FEATURE_NORM_EPS
        )
        state_innovation = F.normalize(
            state_innovation, p=2, dim=-1, eps=KNET_FEATURE_NORM_EPS
        )

    return torch.cat((imu, state_residual, state_innovation), dim=-1)


def _condition_observation_features_torch(
    obs_raw: torch.Tensor,
    channel_mask: torch.Tensor,
) -> torch.Tensor:
    """Differentiable masked KalmanNet-style observation normalization."""
    if obs_raw.shape != channel_mask.shape:
        raise ValueError("torch observation feature/mask shape mismatch")
    if obs_raw.shape[-1] != OBSERVATION_FEATURE_DIM:
        raise ValueError("torch observation feature dimension mismatch")

    valid = channel_mask.bool()
    masked = torch.where(valid, obs_raw, torch.zeros_like(obs_raw))
    if not KALMANNET_L2_FEATURE_NORMALIZATION_ON:
        return masked
...
if __name__ == "__main__":
    ...
    # Neither the paper nor the provided KalmanNet code publishes a universal
    # warm-up/fine-tune epoch split.  Keep the two horizons explicit instead of
    # inventing an undocumented ratio.  For backward compatibility both default
    # to the existing requested training horizon and can be overridden separately.
    ...
    # =========================================================================
    # =========================================================================
    # 7. YAN Eqs. (18)-(20) + KALMANNET-GUIDED FEATURE CONDITIONING
    # =========================================================================
    # Yan defines the physical Eq. (15) features/padding/masks but does not publish
    # an input-scaling rule.  The official KalmanNet implementation L2-normalizes
    # its observation/state difference features independently at each time step.
    # Apply that rule to Yan's corresponding state and observation feature groups.
    # The Yan-specific IMU-difference block retains the prior Data01-only affine
    # conditioning because KalmanNet provides no corresponding normalization rule.
    random.seed(SEED)
    np.random.seed(SEED)
    ...
    # Store full-length mean/std arrays for checkpoint compatibility, but only
    # the six IMU-difference entries use Data01 z-score conditioning.
    fixed_mean = np.zeros(FIXED_FEATURE_DIM, dtype=float)
    fixed_std = np.ones(FIXED_FEATURE_DIM, dtype=float)
    if FEATURE_STANDARDIZATION_ON:
        fixed_mean[:IMU_DELTA_DIM] = fixed[:, :IMU_DELTA_DIM].mean(axis=0)
        imu_std = fixed[:, :IMU_DELTA_DIM].std(axis=0)
        imu_std[imu_std < 1e-8] = 1.0
        fixed_std[:IMU_DELTA_DIM] = imu_std

    # KalmanNet normalization is per time step and does not use fitted
    # observation mean/std statistics.
    obs_mean = np.zeros(OBSERVATION_FEATURE_DIM, dtype=float)
    obs_std = np.ones(OBSERVATION_FEATURE_DIM, dtype=float)

    fixed_normalized = _condition_fixed_features_numpy(
        fixed, fixed_mean, fixed_std
    )
    observations_normalized = _condition_observation_features_numpy(
        observations, channel_masks
    )
    normalizer_source = (
        "KalmanNet_per_timestep_L2_for_state_and_observation_features;"
        "Data01_zscore_only_for_Yan_IMU_difference_block"
    )
    ...
    np.savez(
        ...
        kalmannet_l2_feature_normalization_on=np.asarray(
            KALMANNET_L2_FEATURE_NORMALIZATION_ON
        ),
        kalmannet_l2_eps=np.asarray(KNET_FEATURE_NORM_EPS),
    )
    ...
    # KalmanNet V2 training:
    # each independent short trajectory uses T+1 consecutive labeled states;
    # the first state is the known trajectory initial condition and the remaining
    # T transitions are trained recursively.  This follows the official
    # Short_Traj_Split implementation and does NOT reset to a classical KF state.
    ...
    # All transitions not belonging to a complete V2 short trajectory are
    # intentionally unused for V2 optimization: this includes the one transition
    # between adjacent T+1 blocks and any incomplete tail, matching the official
    # Short_Traj_Split behavior.
    ...
    def _training_feature_arrays(
        ...
        fixed_nn = _condition_fixed_features_numpy(
            fixed_raw, fixed_mean, fixed_std
        )
        obs_nn = _condition_observation_features_numpy(
            obs_raw, channel_mask
        )
    ...
    def _torch_training_feature_tensors(
        ...
        fixed_nn = _condition_fixed_features_torch(
            fixed_raw, fixed_mean_t, fixed_std_t
        )
        obs_nn = _condition_observation_features_torch(
            obs_raw, channel_mask
        )
    ...
    def _rollout_actual_closed_loop_trajectory(
        ...
        *,
        ...
        if initialization_mode == "kalmannet_v2_truth":
            # Official KalmanNet V2 uses the first labeled state in each T+1
            # block as the short trajectory initial state.  For Yan's navigation
            # state this maps to the postprocessed truth position/velocity/attitude
            # at the chunk boundary.  No classical-KF posterior reset is used.
            ...
            # Yan's lagged Eq. (11)-(14) features require a previous-epoch context,
            # while KalmanNet V2 itself only specifies the labeled initial state.
            # Build the observation residual about that truth state and set the
            # filter-dependent state deltas to zero at the independent trajectory
            # boundary.  IMU samples are exogenous and are retained from Data01.
    ...
    def _run_v1_training_phase(
        ...
        # KalmanNet V1 uses the full trajectory in one recursive BPTT graph.
        # Continue directly from the V2-warmed network/Adam states.  The trajectory
        # is initialized from its known ground-truth initial navigation state,
        # matching KalmanNet InitSequence semantics; no classical-KF posterior is
        # used as a reset or teacher state inside the trajectory.
        metrics = _rollout_actual_closed_loop_trajectory(
            0,
    ...
    # ---------------------------------------------------------------------
    # Stage 2: KalmanNet V1 tuning on the complete chronological trajectory.
    # The same model parameters AND Adam optimizer states are continued from V2.
    # ---------------------------------------------------------------------
    ...
    def _online_feature_arrays(
        ...
        fixed_k = _condition_fixed_features_numpy(
            fixed_k_raw, fixed_mean, fixed_std
        )
        obs_k = _condition_observation_features_numpy(
            obs_k_raw, channel_k
        )
    ...
    torch.save(
        {
            ...
            "kalmannet_l2_feature_normalization_on": bool(
                KALMANNET_L2_FEATURE_NORMALIZATION_ON
            ),
            "kalmannet_l2_feature_normalization_eps": float(KNET_FEATURE_NORM_EPS),
            ...
            "sequence_training": (
                ...
                "KalmanNet_V2_warmup_then_V1_full_sequence_tuning;"
    ...
    summary = {
        ...
        "unavoidable_unpublished_completions": [
            ...
            "KalmanNet_official_code_per_timestep_L2_normalization_for_state_and_observation_feature_groups",
            "Yan_variable_Nk_masked_observation_L2_normalization_is_project_adaptation_of_KalmanNet_fixed_n_rule",
            "Data01_only_zscore_retained_only_for_Yan_specific_IMU_difference_block",
        ...
        "training": {
            ...
            "feature_standardization": {
                "imu_difference_zscore_enabled": bool(FEATURE_STANDARDIZATION_ON),
                "imu_difference_fit_scope": "Data01_only",
                "kalmannet_l2_state_observation_features": bool(
                    KALMANNET_L2_FEATURE_NORMALIZATION_ON
                ),
                "kalmannet_l2_eps": float(KNET_FEATURE_NORM_EPS),
                "state_blocks_normalized_separately": [
                    "lagged_state_residual",
                    "lagged_state_innovation",
                ],
                "observation_blocks_normalized_separately": [
                    "lagged_observation_residual_valid_slots",
                    "current_observation_innovation_valid_slots",
                ],
                "targets_normalized": False,
                "innovation_for_state_update_normalized": False,
                "paper_status": (
                    "Yan_input_scaling_unpublished;"
                    "state_observation_L2_conditioning_from_official_KalmanNet_code;"
                    "mask_adaptation_required_for_variable_Nk;"
                    "IMU_zscore_is_existing_project_completion"
                ),
            },
            "alternating_partition": {
