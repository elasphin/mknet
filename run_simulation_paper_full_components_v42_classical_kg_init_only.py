...
if __name__ == "__main__":
    ...
    for event in training_timeline:
        ...
        history_rows.append({
            ...
            "classical_gain": classical_gain.copy(),
    ...
    # Yan does not publish KG-head initialization. Before the first optimizer
    # step, this head is deterministically initialized from the Data01 classical
    # TC/KF gains without changing the Masked-CLA inference architecture.
    ...
    # Classical-KF-informed KG initialization. The classical gain is available
    # from the existing Data01 TC pass:
    #     K_k = P^-_k H_k^T (H_k P^-_k H_k^T + R_k)^-1.
    # Yan does not publish an FC initializer, so this is an explicit
    # implementation completion. It is restricted to Data01 and only initializes
    # the already-published KG output head; the subsequent Yan Eq. (30)/(32)
    # training remains unchanged.
    classical_gain_targets = np.zeros(
        (training_sample_count, INS_STATE_DIM, network_nmax),
        dtype=np.float64,
    )
    for sample_index, row in enumerate(history_rows[1:]):
        classical_gain_k = np.asarray(row["classical_gain"], dtype=np.float64)
        measurement_count = len(row["sat_ids"])
        if classical_gain_k.shape != (INS_STATE_DIM, measurement_count):
            raise ValueError(
                "stored Data01 classical gain has inconsistent dimensions"
            )
        if measurement_count > network_nmax:
            raise ValueError(
                "stored Data01 classical gain exceeds Masked-CLA Nmax"
            )
        classical_gain_targets[
            sample_index, :, :measurement_count
        ] = classical_gain_k

    captured_gain_contexts = []

    def _capture_gain_head_context(module, inputs):
        del module
        captured_gain_contexts.append(
            inputs[0].detach().to(dtype=torch.float64).cpu()
        )

    gain_context_hook = model.gain_head.register_forward_pre_hook(
        _capture_gain_head_context
    )
    model.eval()
    recurrent_state = None
    try:
        with torch.inference_mode():
            for sample_index in range(training_sample_count):
                ...
                output_init = model(
                    torch.tensor(
                        fixed_init[None], dtype=torch.float32, device=DEVICE
                    ),
                    torch.tensor(
                        obs_init[None], dtype=torch.float32, device=DEVICE
                    ),
                    torch.tensor(
                        mask_init[None], dtype=torch.bool, device=DEVICE
                    ),
                    torch.tensor(
                        channel_init[None], dtype=torch.bool, device=DEVICE
                    ),
                    recurrent_state=recurrent_state,
                )
                recurrent_state = output_init.recurrent_state
    finally:
        gain_context_hook.remove()

    if len(captured_gain_contexts) != training_sample_count:
        raise RuntimeError(
            "failed to collect one KG-head context for each Data01 training sample"
        )

    gain_context_matrix = torch.cat(captured_gain_contexts, dim=0).numpy()
    if gain_context_matrix.shape != (training_sample_count, 64):
        raise RuntimeError(
            "unexpected Masked-CLA attention-context dimensions during KG initialization"
        )
    if not np.all(np.isfinite(gain_context_matrix)):
        raise FloatingPointError(
            "non-finite Data01 context encountered during classical-KG initialization"
        )

    design_matrix = np.concatenate(
        (
            gain_context_matrix,
            np.ones((training_sample_count, 1), dtype=np.float64),
        ),
        axis=1,
    )
    gain_target_matrix = classical_gain_targets.reshape(
        training_sample_count,
        INS_STATE_DIM * network_nmax,
    )
    # np.linalg.lstsq returns the minimum-norm solution for this generally
    # underdetermined linear-head initialization; no ridge coefficient or
    # hand-selected KG magnitude is introduced.
    head_solution, _, head_rank, head_singular_values = np.linalg.lstsq(
        design_matrix,
        gain_target_matrix,
        rcond=None,
    )
    if not np.all(np.isfinite(head_solution)):
        raise FloatingPointError(
            "classical-KG least-squares initialization produced non-finite parameters"
        )

    with torch.no_grad():
        model.gain_head.weight.copy_(
            torch.tensor(
                head_solution[:-1].T,
                dtype=model.gain_head.weight.dtype,
                device=model.gain_head.weight.device,
            )
        )
        model.gain_head.bias.copy_(
            torch.tensor(
                head_solution[-1],
                dtype=model.gain_head.bias.dtype,
                device=model.gain_head.bias.device,
            )
        )

    fitted_gain_matrix = design_matrix @ head_solution
    kg_init_rmse = float(
        np.sqrt(np.mean((fitted_gain_matrix - gain_target_matrix) ** 2))
    )
    kg_init_max_abs = float(np.max(np.abs(fitted_gain_matrix)))
    kg_target_max_abs = float(np.max(np.abs(gain_target_matrix)))
    print(
        "classical-KF KG initialization: "
        f"fitRMSE={kg_init_rmse:.6g}, "
        f"targetMaxAbs={kg_target_max_abs:.6g}, "
        f"fittedMaxAbs={kg_init_max_abs:.6g}, "
        f"rank={int(head_rank)}/{design_matrix.shape[1]}"
    )
    training_history.append({
        "stage": "classical_kf_kg_head_initialization",
        "source": "Data01_classical_TC_KF_only",
        "method": "minimum_norm_linear_least_squares_on_gain_head_context",
        "fit_rmse": kg_init_rmse,
        "target_max_abs": kg_target_max_abs,
        "fitted_max_abs": kg_init_max_abs,
        "design_rank": int(head_rank),
        "design_columns": int(design_matrix.shape[1]),
        "smallest_singular_value": (
            float(head_singular_values[-1])
            if len(head_singular_values)
            else None
        ),
        "Data02_used": False,
    })
    ...
    summary = {
        ...
        "unavoidable_unpublished_completions": [
            ...
            "Data01_classical_TC_KF_KG_minimum_norm_head_initialization_Yan_does_not_publish_FC_initialization",
