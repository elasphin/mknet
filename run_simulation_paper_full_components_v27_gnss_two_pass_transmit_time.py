...
- Yan et al. do not publish a direct eta_k target or full 15-state postprocessed
  bias truth.  Under the detached exact-forward training used here, the zero-
  initialized eta head and the six unlabeled bias-gain rows receive no fabricated
  auxiliary target; they therefore remain neutral unless a published supervision
  path is available.  Their Fig. 8/Eq. (8) interfaces remain present online.
...
#     is introduced.  In the actual-forward detached training below, these rows
#     remain neutral because Yan does not publish a future-state bias supervision.
#   - Fig. 8 also outputs eta_k for inertial-navigation measurement-error
#     estimation. Its target/timing are unpublished.  The Eq. (8) interface is
#     retained, but no direct eta label or v23 finite-difference/Phi surrogate is
#     invented; the zero-initialized eta head therefore remains neutral here.
...
class MaskedCLA(nn.Module):
    ...
    def __init__(
        ...
        # does not publish its target/timing. Because the exact NumPy navigation
        # state is detached between fusion epochs in this reproduction, no future-
        # state gradient is fabricated for eta_k; its interface is retained but the
        # zero-initialized branch remains neutral during the present training stage.
        self.imu_error_head = nn.Linear(
            64,
            IMU_ERROR_DIM,
        )
        # Zero-output initialization prevents arbitrary IMU feedback before
        # training. With detached cross-fusion navigation, this branch receives no
        # invented auxiliary loss and therefore remains neutral. Initialization is
        # an explicit reproducibility completion.
...
if __name__ == "__main__":
    ...
    def _rollout_actual_closed_loop_trajectory(
        ...
        *,
        ...
        """Replay one Data01 short trajectory with the actual Fig.7 forward loop.

        Sample index ``s`` corresponds to ``history_rows[s+1]``; therefore the
        stored posterior/context at ``history_rows[start_sample]`` is a strictly
        causal initial condition for the first learned update in this trajectory.

        The exact NumPy INS state is detached between fusion epochs, as in v24.
        Network parameters are held fixed for the entire trajectory.  When
        ``backward_scale`` is provided, the mean Eq.(30) loss of this trajectory is
        backpropagated with that scale, but no optimizer step is performed here.
        """
        ...
        nav_train = history_rows[start_sample]["posterior_nav"].copy()
        previous_context = _stored_training_context(start_sample)
        current_eta = np.zeros(IMU_ERROR_DIM, dtype=float)
        ...
        with grad_context:
            for sample_index in range(start_sample, stop_sample):
                ...
                # Fig. 7: propagate the preceding learned posterior through all raw
                # IMU samples to the next usable fusion epoch.  Eq. (8) eta is held
                # over the interval, matching the online completion.
                for imu_index, dt in row["preceding_interval_segments"]:
                    propagation_gyro, propagation_accel = compensate_imu(
                        imr.angular_rate_body_radps[int(imu_index)],
                        imr.acceleration_body_mps2[int(imu_index)],
                        nav_train.gyroscope_bias_body_radps,
                        nav_train.accelerometer_bias_body_mps2,
                        gyroscope_measurement_error_body_radps=(
                            current_eta[ETA_GYRO_SLICE] if ETA_FEEDBACK_ON else None
                        ),
                        accelerometer_measurement_error_body_mps2=(
                            current_eta[ETA_ACCEL_SLICE] if ETA_FEEDBACK_ON else None
                        ),
                    )
                    # Eq. (14) uses the bias-compensated IMU values without eta,
                    # exactly as in the online Data02 feature construction.
                ...
                if not _navigation_state_finite(nav_train):
                    raise FloatingPointError(
                        "non-finite INS state inside actual-forward training "
                ...
                # Record the physical prior error before satellite preprocessing.
                # This keeps navigation divergence directly observable even if a
                # particular satellite is unavailable after measurement preparation.
                prior_position_error_m = float(
                    np.linalg.norm(
                        fusion_imu_truth_position[fusion_index]
                        - nav_train.position_ecef_m
                    )
                )
                max_prior_position_error_m = max(
                    max_prior_position_error_m, prior_position_error_m
                )
                epoch = gnss_epochs[fusion_index]
                gnss_measurements = gnss_preprocessor.prepare_epoch(
                    epoch, nav_train, lever_arm_b_m
                )
                # LEO pseudorange realizations are fixed raw Data01 measurements;
                # their predicted ranges are rebuilt from the current learned nav.
                measurements = retain_clock_observable_measurements(
                ...
                valid_gain = output.kalman_gain[
                    0, :, : len(current_sat_ids)
                ]
                ...
                # Eq. (30) truth is recomputed against the CURRENT learned prior.
                target_state_9_now = np.concatenate([
                    fusion_imu_truth_position[fusion_index]
                    - nav_train.position_ecef_m,
                    fusion_imu_truth_velocity[fusion_index]
                ...
                if not np.all(np.isfinite(target_state_9_now)):
                    raise FloatingPointError(
                        "non-finite Eq.(30) target inside actual-forward training "
                        f"trajectory={trajectory_id}, bounds=[{start_sample},{stop_sample}), "
                        f"history_row={row_index}, optimizer_step={optimizer_step_index}"
                    )
                ...
                state_error_9 = (
                    target_tensor
                    - correction[:SUPERVISED_STATE_DIM].to(dtype=torch.float64)
                )
                state_loss = torch.sum(state_error_9**2)
                position_loss = torch.sum(state_error_9[:3] ** 2)
                if not torch.isfinite(state_loss):
                    raise FloatingPointError(
                        "non-finite Yan Eq.(30) loss inside actual-forward training "
                        f"trajectory={trajectory_id}, bounds=[{start_sample},{stop_sample}), "
                ...
                max_prior_position_error_m = max(
                    ...
                    float(np.linalg.norm(target_state_9_now[:3])),
                )

                # Fig. 7/8 forward state update.  The correction is detached only
                # when entering the NumPy navigation state.  Network weights remain
                # unchanged until this whole short trajectory has completed.
                correction_np = correction.detach().cpu().numpy().astype(float)
                eta_np = output.imu_error[0].detach().cpu().numpy().astype(float)
                if (
                    not np.all(np.isfinite(correction_np))
                    or not np.all(np.isfinite(eta_np))
                ):
                    raise FloatingPointError(
                        "non-finite learned correction/eta inside actual-forward "
                        f"trajectory={trajectory_id}, bounds=[{start_sample},{stop_sample}), "
                        f"history_row={row_index}, optimizer_step={optimizer_step_index}"
                    )
                max_position_correction_norm_m = max(
                    max_position_correction_norm_m,
                    float(np.linalg.norm(correction_np[0:3])),
                )
                max_velocity_correction_norm_mps = max(
                    max_velocity_correction_norm_mps,
                    float(np.linalg.norm(correction_np[3:6])),
                )
                max_attitude_correction_norm_rad = max(
                    max_attitude_correction_norm_rad,
                    float(np.linalg.norm(correction_np[6:9])),
                )
                max_eta_norm = max(max_eta_norm, float(np.linalg.norm(eta_np)))

                nav_train = inject_error_state(nav_train, correction_np)
                current_eta = (
                    eta_np.copy() if ETA_FEEDBACK_ON else np.zeros(IMU_ERROR_DIM)
                )
                posterior_residual = build_innovation_only(
                    nav_train, measurements, lever_arm_b_m
                )
                previous_context = _training_context_from_completed_epoch(
                    current_sat_ids,
                    posterior_residual,
                    np.zeros(INS_STATE_DIM),
                    correction_np,
        ...
        if total_count <= 0:
            raise RuntimeError(
                "actual-forward training trajectory produced no learned fusion updates: "
                f"trajectory={trajectory_id}, bounds=[{start_sample},{stop_sample})"
            )
    ...
    selected_training_stage = (
        "yan_eq30_actual_forward_short_trajectory_latent_alternating_final_epoch"
    )
    ...
    torch.save(
        {
            ...
            "bias_gain_training": (
                "no_direct_bias_truth_labels; detached exact-forward training "
                "does not fabricate future-state bias supervision; zero-initialized "
                "bias gain rows remain neutral"
            ),
            "fixed_mean": fixed_mean,
            "fixed_std": fixed_std,
            "obs_mean": obs_mean,
            ...
            "eta_training": (
                "no_direct_eta_label_published; exact forward Eq8 interface retained; "
                "detached cross_fusion state means zero_initialized eta remains neutral"
            ),
    ...
    for event in test_timeline:
        ...
        # b_g.  The public dataset has no direct bias truth and this revision does
        # not fabricate future-state bias supervision; the zero-initialized bias
        # gain rows therefore remain neutral unless a published target is supplied.
        if (
            not np.all(np.isfinite(active_gain))
    ...
    summary = {
        ...
        "paper_components": {
            ...
            "Fig8_eta": {
                ...
                "training": "no_direct_eta_label_published; exact_Eq8_forward_interface_retained; detached_cross_fusion_gradient_keeps_zero_initialized_eta_neutral",
            },
            "FDE_Eq33_to_Eq34": {
        ...
        "training": {
            ...
            "bias_gain_training": (
                "no_direct_bias_labels; no_fabricated_cross_fusion_bias_loss; "
                "zero_initialized_bias_rows_remain_neutral"
            ),
            "eta_training": (
                "no_direct_eta_loss_published; Eq8_forward_interface_retained; "
                "zero_initialized_eta_remains_neutral_under_detached_navigation_state"
            ),
