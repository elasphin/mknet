...
- Eqs. (10)-(15) are implemented causally: current innovation/IMU increments are
  ...
  The first usable fusion epoch uses an all-zero previous context, after which all
  residual/state-history inputs are generated by the Masked KalmanNet itself.
...
- Offline training is now a recursive self-rollout completion: Data01 is propagated
  with the same learned KG/eta feedback topology used online; postprocessed truth is
  used only in the supervised Eq. (30) loss and never resets the navigation state.
  Yan et al. explicitly require offline supervised training and online recursive
  navigation but do not publish how Eqs. (10)-(15) training histories are generated.
  This on-policy/self-rollout construction is therefore marked paper-consistent,
  not paper-exact. No classical KF teacher-forcing history is used.
...
- Yan et al. state that the network parameters are optimized alternately and cite
  ...
  Before each outer alternating epoch, a frozen copy of the current network performs a
  complete causal Data01 self-rollout to regenerate X,M,Y from its own prior/posterior
  history. The optimization step then treats that regenerated rollout as the fixed
  offline dataset of Eqs. (18)-(20) for that outer epoch. Gradients are not propagated
  through multiple navigation epochs because the paper publishes no BPTT schedule.
...
# in the learned recursive training and online passes.
...
def eta_interval_state_sensitivity_9(
    ...
    This Jacobian is used only in the offline self-rollout-conditioned
    supervision of the forward eta feedback branch. It maps a constant eta_k applied over the *future* IMU
...
# Numerical differentiation steps used only for the offline self-rollout-conditioned
...
class MaskedCLA(nn.Module):
    ...
    The offline training set is regenerated causally from the network's own
    recursive Data01 navigation trajectory before each outer alternating epoch.
    This preserves Eqs. (10)-(21) and the offline/online split in Fig. 2 while
    removing the classical-KF train/test history mismatch. Yan et al. do not
    publish this history-generation detail or a multi-epoch BPTT schedule, so
    self-rollout regeneration is an explicit paper-consistent completion.
    ...
    self-rollout-conditioned one-step forward navigation-state loss at the next usable fusion
    ...
    def __init__(
        ...
        # Recursive self-rollout bootstrap completion. The paper does not publish
        # FC initialization. Exact-zero gain/eta outputs make the first offline
        # rollout an INS-only trajectory rather than an arbitrary random-feedback
        # trajectory. Eq. (30) gradients update the gain head immediately in the
        # first theta phase; no classical KF target/gain is injected.
        ...
        # is trained through a self-rollout-conditioned future-interval mechanization
...
def fig2_eta_forward_state_update_9(
    ...
    """Linearized forward state effect of the Fig. 2 eta_k feedback path.
...
if __name__ == "__main__":
    ...
    # PAPER-CONSISTENT COMPLETION FOR UNPUBLISHED TRAINING RECURSION:
    # every outer epoch regenerates Data01 features from a frozen recursive
    # Masked-KalmanNet rollout. Truth is used only for Eq. (30) supervision.
    # No classical-KF teacher forcing and no truth state reset are used.
    REGENERATE_SELF_ROLLOUT_EVERY_EPOCH = True
    ...
    # =========================================================================
    # 5. PAPER-CONSISTENT OFFLINE RECURSIVE SELF-ROLLOUT TRAINING SETUP
    # =========================================================================
    # Yan et al. Fig. 2 separates offline training from online testing, while
    # Fig. 7 shows recursive IMU propagation and fusion-state feedback. Eqs.
    # (10)-(15) define features containing residual/state history, but the paper
    # does not publish whether that history is generated by a classical KF or by
    # KalmanNet itself. To avoid the observed train/test distribution mismatch,
    # this implementation uses the same learned closed-loop history in training
    # and testing. This is a declared paper-consistent causal completion.
    #
    # Important constraints preserved here:
    #   * Data01 only is used for training/validation/normalization.
    #   * truth is used only for Eq. (30) supervision and LEO simulation.
    #   * the navigation state is NEVER reset to truth during a rollout.
    #   * no classical KF update is used to generate training history.
    #   * FDE remains online-only, matching Yan et al. Fig. 2 / Sec. II-D.
    #   * no BPTT through multiple navigation epochs is claimed: before each
    #     outer epoch a frozen model regenerates a fixed Eqs. (18)-(20) dataset,
    #     then Ref. [15]-style theta->psi optimization is applied to that dataset.
    ...
    def _truth_target_9(fusion_index: int, prior_nav: NavigationState) -> np.ndarray:
        return np.concatenate([
    ...
    def _make_online_context(
        ...
        """Store the completed causal context for Yan Eqs. (11)-(14)."""
        ...
        if (
            ...
            raise FloatingPointError("invalid completed fusion context")
        return context

    def _raw_causal_feature_arrays(
        previous_online,
        current_sat_ids,
        innovation_now,
        current_accel,
        ...
        """Yan Eqs. (10)-(15), using only already-completed previous context."""
        ...
        if (
            ...
            or channel_mask.shape != (n, OBSERVATION_FEATURE_DIM)
            or not np.all(np.isfinite(fixed_raw))
            or not np.all(np.isfinite(obs_raw))
            or not np.all(np.isfinite(innovation_k))
        ):
            raise FloatingPointError("invalid raw causal feature")
        return fixed_raw, obs_raw, current_mask, channel_mask, innovation_k

    def _normalize_causal_feature_arrays(
        fixed_raw,
        obs_raw,
        channel_mask,
        normalizer,
        ...
        if normalizer is None:
            # Used only for the exact-zero-output bootstrap rollout. Because the
            # gain and eta heads are both initialized to zero, these values cannot
            # feed back into navigation before the train-only normalizer is fitted.
            fixed_k = np.asarray(fixed_raw, dtype=float).copy()
            obs_k = np.where(channel_mask, obs_raw, 0.0).astype(float, copy=False)
        else:
            fixed_mean_, fixed_std_, obs_mean_, obs_std_ = normalizer
            fixed_k = (fixed_raw - fixed_mean_) / fixed_std_
            obs_k = (
                obs_raw - obs_mean_.reshape(1, 2)
            ) / obs_std_.reshape(1, 2)
            obs_k = np.where(channel_mask, obs_k, 0.0)

        if not np.all(np.isfinite(fixed_k)) or not np.all(np.isfinite(obs_k)):
            raise FloatingPointError("non-finite normalized causal feature")
        return fixed_k, obs_k

    # Fixed chronological boundary independent of the network trajectory. This
    # prevents the changing number of usable self-rollout samples from moving the
    # train/validation split across outer epochs.
    validation_start_fusion_index = int(
        math.floor((1.0 - VALIDATION_FRACTION) * len(fusion_time))
    )
    validation_start_fusion_index = min(
        max(validation_start_fusion_index, 2),
        len(fusion_time) - 1,
    )
    validation_start_time = float(fusion_time[validation_start_fusion_index])

    # Paper Nmax is a property of the training dataset. Determine a reference
    # capacity from Data01 geometry using postprocessed truth only for visibility/
    # simulation bookkeeping; these values are never supplied to the network.
    reference_leo_for_nmax = LEODownlinkSimulator(
        tle_provider,
        leo_klobuchar,
        seed=LEO_SEED,
        tx_epsilon_position_m=LEO_TX_EPSILON_POSITION_M,
    ...
    reference_nmax = 0
    for fusion_index, epoch in enumerate(gnss_epochs):
        truth_nav = _truth_nav_at_fusion(fusion_index)
        gnss_ref = gnss_preprocessor.prepare_epoch(
            epoch, truth_nav, lever_arm_b_m
        )
        leo_ref = reference_leo_for_nmax.simulate_epoch(
            epoch.time_gpst_s,
            fusion_antenna_truth_position[fusion_index],
        )
        measurements_ref = retain_clock_observable_measurements(
            tuple(gnss_ref) + tuple(leo_ref)
        )
        reference_nmax = max(reference_nmax, len(measurements_ref))
    if reference_nmax <= 0:
        raise RuntimeError("Data01 reference scan produced no usable measurements")
    nmax = int(reference_nmax)

    # =========================================================================
    # 6. MASKED CLA MODEL + RECURSIVE DATA01 ROLLOUT GENERATOR
    # =========================================================================
    model = MaskedCLA(nmax=nmax, dropout=0.2).to(DEVICE)

    representation_modules = (model.conv, model.lstm, model.attention)
    filter_modules = (model.gain_head, model.imu_error_head)
    ...
    def _generate_recursive_data01_samples(normalizer):
        """Frozen-model causal Data01 rollout; truth never resets navigation."""
        model.eval()
        ...
        samples = []
        pending_sample = None
        interval_segments_since_previous_usable = []
        ...
        # Zero-history initialization lets KalmanNet, not a classical KF, own the
        # first usable fusion update. Eqs. (11)-(13) simply have no prior sample.
        ...
        with torch.inference_mode():
            for event in build_exact_fusion_timeline(
                ...
                if isinstance(event, PropagationSegment):
                    ...
                    interval_segments_since_previous_usable.append((imu_index, dt))
                    continue

                fusion_index = event.fusion_index
                t = event.time_gpst_s
                epoch_data = gnss_epochs[fusion_index]
                if abs(epoch_data.time_gpst_s - t) > 1e-9:
                    raise RuntimeError(
                        "recursive training fusion timeline/epoch timestamp mismatch"
                    )
                ...
                # The previous usable output eta_{k-1} acts over the propagation
                # interval that ends here. Finalize its auxiliary Eq. (30)-type
                # future-state supervision using the previous KNet posterior as
                # the interval start, never postprocessed truth.
                if pending_sample is not None:
                    future_base_end_nav = _replay_imu_interval_with_eta(
                        pending_sample["posterior_nav"],
                        tuple(interval_segments_since_previous_usable),
                        imr,
                        np.zeros(IMU_ERROR_DIM),
                    )
                    future_truth_nav = _truth_nav_at_fusion(fusion_index)
                    pending_sample["eta_forward_target_9"] = _navigation_delta_9(
                        future_base_end_nav,
                        future_truth_nav,
                    )
                    pending_sample["eta_forward_sensitivity_9"] = (
                    ...
                    pending_sample["eta_future_end_time"] = float(t)
                    samples.append(pending_sample)
                    pending_sample = None

                prior_nav = nav_roll.copy()
                measurement_model = build_measurement_model(
                    prior_nav, measurements, lever_arm_b_m
                )
                fixed_raw, obs_raw, current_mask, channel_mask, innovation_k = (
                    _raw_causal_feature_arrays(
                        previous_online,
                ...
                fixed_k, obs_k = _normalize_causal_feature_arrays(
                    fixed_raw,
                    obs_raw,
                    channel_mask,
                    normalizer,
                ...
                output = model(
                    ...
                    torch.tensor(
                        channel_mask[None], dtype=torch.bool, device=DEVICE
                    ),
                )
                ...
                if (
                    ...
                    raise FloatingPointError(
                        "non-finite recursive training output at "
                ...
                if not np.allclose(
                    ...
                    raise RuntimeError(
                        "recursive training produced nonzero unsupervised bias-gain rows"
                    )

                target_state_9 = _truth_target_9(fusion_index, prior_nav)
                ...
                posterior_error_9 = _truth_target_9(fusion_index, nav_roll)
                ...
                pending_sample = {
                    ...
                    "fixed_raw": fixed_raw.copy(),
                    "obs_raw": obs_raw.copy(),
                    "current_mask": current_mask.copy(),
                    "channel_mask": channel_mask.copy(),
                    "innovation": innovation_k.copy(),
                    "target_state_9": target_state_9.copy(),
                    "correction_15": correction.copy(),
                    "eta_6": eta.copy(),
                    "posterior_error_9": posterior_error_9.copy(),
                    "posterior_nav": nav_roll.copy(),
                    "truth_position": fusion_imu_truth_position[fusion_index].copy(),
                    "posterior_position": nav_roll.position_ecef_m.copy(),
                }
                interval_segments_since_previous_usable = []

        # The final pending sample has no future usable fusion interval and cannot
        # supervise eta_k; it is intentionally omitted, matching the finite-record
        # boundary treatment of the earlier implementation.
        if len(samples) < 3:
            raise RuntimeError(
                "recursive Data01 rollout produced fewer than three supervised samples"
            )
        return samples

    def _split_rollout_samples(samples):
        train_samples = [
            sample
            for sample in samples
            if sample["time"] < validation_start_time
            and sample["eta_future_end_time"] < validation_start_time
        ]
        val_samples = [
            sample for sample in samples
            if sample["time"] >= validation_start_time
        ]
        if len(train_samples) < 1 or len(val_samples) < 1:
            raise RuntimeError(
                "recursive rollout does not provide nonempty train/validation subsets"
            )
        return train_samples, val_samples

    def _fit_train_only_normalizer(samples):
        train_samples, _ = _split_rollout_samples(samples)
        fixed_train = np.stack([sample["fixed_raw"] for sample in train_samples])
        fixed_mean_ = fixed_train.mean(axis=0)
        fixed_std_ = fixed_train.std(axis=0)
        fixed_std_[fixed_std_ < 1e-8] = 1.0

        obs_values = [[], []]
        for sample in train_samples:
            for channel in range(OBSERVATION_FEATURE_DIM):
                valid = sample["channel_mask"][:, channel]
                if np.any(valid):
                    obs_values[channel].append(sample["obs_raw"][valid, channel])
        obs_mean_ = np.zeros(OBSERVATION_FEATURE_DIM)
        obs_std_ = np.ones(OBSERVATION_FEATURE_DIM)
        for channel in range(OBSERVATION_FEATURE_DIM):
            if obs_values[channel]:
                values = np.concatenate(obs_values[channel])
                obs_mean_[channel] = float(values.mean())
                std_value = float(values.std())
                obs_std_[channel] = 1.0 if std_value < 1e-8 else std_value
        return fixed_mean_, fixed_std_, obs_mean_, obs_std_

    def _pack_rollout_samples(samples, normalizer):
        sequence_length = max(len(sample["sat_ids"]) for sample in samples)
        count = len(samples)
        fixed_array = np.stack([sample["fixed_raw"] for sample in samples])
        observations_array = np.zeros(
            (count, sequence_length, OBSERVATION_FEATURE_DIM), dtype=float
        )
        channel_mask_array = np.zeros(
            (count, sequence_length, OBSERVATION_FEATURE_DIM), dtype=bool
        )
        innovation_array = np.zeros((count, sequence_length), dtype=float)
        target_array = np.stack([sample["target_state_9"] for sample in samples])
        eta_sensitivity_array = np.stack([
            sample["eta_forward_sensitivity_9"] for sample in samples
        ])
        eta_target_array = np.stack([
            sample["eta_forward_target_9"] for sample in samples
        ])

        for row, sample in enumerate(samples):
            n = len(sample["sat_ids"])
            observations_array[row, :n] = sample["obs_raw"]
            channel_mask_array[row, :n] = sample["channel_mask"]
            innovation_array[row, :n] = sample["innovation"]

        fixed_mean_, fixed_std_, obs_mean_, obs_std_ = normalizer
        fixed_normalized = (fixed_array - fixed_mean_) / fixed_std_
        observations_normalized = (
            observations_array - obs_mean_.reshape(1, 1, 2)
        ) / obs_std_.reshape(1, 1, 2)
        observations_normalized = np.where(
            channel_mask_array, observations_normalized, 0.0
        )
        satellite_mask_array = channel_mask_array[:, :, 1]

        if (
            not np.all(np.isfinite(fixed_normalized))
            or not np.all(np.isfinite(observations_normalized))
            or not np.all(np.isfinite(innovation_array))
            or not np.all(np.isfinite(target_array))
            or not np.all(np.isfinite(eta_sensitivity_array))
            or not np.all(np.isfinite(eta_target_array))
        ):
            raise FloatingPointError("packed recursive training arrays are non-finite")
        ...
        dataset_ = TensorDataset(
            ...
            torch.tensor(satellite_mask_array, dtype=torch.bool),
            torch.tensor(channel_mask_array, dtype=torch.bool),
            torch.tensor(innovation_array, dtype=torch.float32),
            torch.tensor(eta_sensitivity_array, dtype=torch.float32),
            torch.tensor(target_array, dtype=torch.float64),
            torch.tensor(eta_target_array, dtype=torch.float64),
        )
        return dataset_, {
            "sequence_length": sequence_length,
            "fixed_raw": fixed_array,
            "observations_raw": observations_array,
            "channel_mask": channel_mask_array,
            "satellite_mask": satellite_mask_array,
            "innovation_padded": innovation_array,
            "target_state_9": target_array,
            "eta_forward_state_sensitivity_9": eta_sensitivity_array,
            "eta_forward_target_state_9": eta_target_array,
            "time_gpst_s": np.asarray([sample["time"] for sample in samples]),
            "eta_future_end_time_gpst_s": np.asarray([
                sample["eta_future_end_time"] for sample in samples
            ]),
            "posterior_position_ecef_m": np.stack([
                sample["posterior_position"] for sample in samples
            ]),
            "truth_position_ecef_m": np.stack([
                sample["truth_position"] for sample in samples
            ]),
        }

    # First frozen rollout: both output heads are exactly zero, so this is a
    # deterministic INS-only bootstrap generated by KalmanNet itself. It exists
    # solely to fit input scaling from Data01-train; no classical filter is used.
    bootstrap_samples = _generate_recursive_data01_samples(normalizer=None)
    fixed_mean, fixed_std, obs_mean, obs_std = _fit_train_only_normalizer(
        bootstrap_samples
    )
    ...
    def _online_feature_arrays(
        ...
        """Shared causal feature builder for recursive validation/diagnostic/test."""
        (
            fixed_raw,
            obs_raw,
            current_mask,
        ...
        fixed_k, obs_k = _normalize_causal_feature_arrays(
            fixed_raw, obs_raw, channel_mask, normalizer
        )
        return (
            fixed_k,
            obs_k,
    ...
    np.savez(
        ...
        validation_start_fusion_index=validation_start_fusion_index,
        validation_start_time_gpst_s=validation_start_time,
        source=np.asarray("Data01_train_zero_output_KNet_self_rollout_bootstrap"),
    )

    # =========================================================================
    # 7. YAN EQ. (30)/(32) + REF. [15] ALTERNATING OPTIMIZATION
    #    WITH PER-EPOCH KALMANNET SELF-ROLLOUT REGENERATION
    # =========================================================================
    ...
    def _state_prediction_loss(predicted_update, true_error_state_9):
        ...
        if predicted_update.shape[1] == SUPERVISED_STATE_DIM:
            predicted_update = F.pad(
                predicted_update, (0, INS_STATE_DIM - SUPERVISED_STATE_DIM)
            )
        ...
        linear_error_6, attitude_angle = fig8_post_update_error_state_9(
            predicted_update, true_error_state_9
        )
        per_sample_squared_norm = (
            torch.sum(linear_error_6**2, dim=1) + attitude_angle**2
        )
    ...
    def _regularized_temporal_state_objective(
        ...
        following_eq30_loss, following_position_rmse = _state_prediction_loss(
            eta_forward_state_update_9, eta_forward_target_9
        )
    ...
    def _run_training_phase(
        ...
        for batch in epoch_batches:
            ...
            eta_forward_state_update_9 = fig2_eta_forward_state_update_9(
                output, eta_forward_sensitivity_b
            )
            (
                objective,
                temporal_state_loss,
            ...
            temporal_state_sum += float(
                temporal_state_loss.detach().cpu()
            ) * batch_size_now
            current_eq30_sum += float(
                current_eq30_loss.detach().cpu()
            ) * batch_size_now
            following_eq30_sum += float(
                following_eq30_loss.detach().cpu()
            ) * batch_size_now
    ...
    def _recursive_validation_metrics(samples):
        _, val_samples = _split_rollout_samples(samples)
        corrections = torch.tensor(
            np.stack([sample["correction_15"] for sample in val_samples]),
            dtype=torch.float64,
            device=DEVICE,
        )
        targets = torch.tensor(
            np.stack([sample["target_state_9"] for sample in val_samples]),
            dtype=torch.float64,
            device=DEVICE,
        )
        eta_values = torch.tensor(
            np.stack([sample["eta_6"] for sample in val_samples]),
            dtype=torch.float64,
            device=DEVICE,
        )
        eta_sensitivity = torch.tensor(
            np.stack([
                sample["eta_forward_sensitivity_9"] for sample in val_samples
            ]),
            dtype=torch.float64,
            device=DEVICE,
        )
        eta_target = torch.tensor(
            np.stack([
                sample["eta_forward_target_9"] for sample in val_samples
            ]),
            dtype=torch.float64,
            device=DEVICE,
        )
        with torch.inference_mode():
            current_eq30, current_position_rmse = _state_prediction_loss(
                corrections, targets
            )
            eta_forward_update = torch.bmm(
                eta_sensitivity,
                eta_values.unsqueeze(-1),
            ).squeeze(-1)
            following_eq30, following_position_rmse = _state_prediction_loss(
                eta_forward_update, eta_target
            )
            temporal_state_loss = torch.stack(
                [current_eq30, following_eq30]
            ).mean()
            l2_value = sum(
                torch.sum(parameter.double() * parameter.double())
                for parameter in model.parameters()
            )
            objective = temporal_state_loss + GAMMA_L2 * l2_value
        return {
            "objective": float(objective.cpu()),
            "temporal_state_loss": float(temporal_state_loss.cpu()),
            "current_eq30": float(current_eq30.cpu()),
            "following_eq30_completion": float(following_eq30.cpu()),
            "current_position_rmse_m": float(current_position_rmse.cpu()),
            "following_position_rmse_m": float(following_position_rmse.cpu()),
            "eta_gyro_rms_radps": float(
                torch.sqrt(torch.mean(eta_values[:, ETA_GYRO_SLICE] ** 2)).cpu()
            ),
            "eta_accel_rms_mps2": float(
                torch.sqrt(torch.mean(eta_values[:, ETA_ACCEL_SLICE] ** 2)).cpu()
            ),
    ...
    print(
        ...
        "WITH KALMANNET SELF-ROLLOUT (DATA01) ==="
    )

    # Baseline recursive validation before any optimization. Because the output
    # heads are zero, this is an INS-only starting point, not a classical filter.
    bootstrap_validation = _recursive_validation_metrics(bootstrap_samples)
    best_val_joint_state_loss = bootstrap_validation["temporal_state_loss"]
    best_val_eq30 = bootstrap_validation["current_eq30"]
    best_val_eta_forward = bootstrap_validation["following_eq30_completion"]
    best_val_objective = bootstrap_validation["objective"]
    best_epoch = 0
    best_state = {
    ...
    epochs_ran = 0
    max_training_nmax_seen = nmax
    last_rollout_samples = bootstrap_samples

    print(
        "bootstrap recursive validation: "
        f"val_Eq30mean={best_val_joint_state_loss:.6g}, "
        f"val_current={best_val_eq30:.6g}, "
        f"val_posRMSE={bootstrap_validation['current_position_rmse_m']:.3f} m"
    )

    for epoch in range(1, TRAINING_EPOCHS + 1):
        epochs_ran = epoch

        # On-policy/self-rollout regeneration with frozen current weights.
        rollout_samples = _generate_recursive_data01_samples(normalizer)
        train_samples, _ = _split_rollout_samples(rollout_samples)
        train_dataset, packed_train = _pack_rollout_samples(
            train_samples, normalizer
        )
        max_training_nmax_seen = max(
            max_training_nmax_seen,
            int(packed_train["sequence_length"]),
        )
        ...
        # Recursive validation is regenerated with the UPDATED model. Validation
        # truth is used only for the metric/loss and never for feedback/reset.
        validation_rollout_samples = _generate_recursive_data01_samples(normalizer)
        validation = _recursive_validation_metrics(validation_rollout_samples)
        last_rollout_samples = validation_rollout_samples
        ...
        training_history.append({
            "stage": "recursive_self_rollout_ref15_alternating_train_validation",
            "epoch": epoch,
            "training_rollout_samples": len(rollout_samples),
            "training_samples_after_boundary_control": len(train_samples),
            ...
            "representation_phase_temporal_state_loss": representation_phase["temporal_state_loss"],
            "train_current_eq30_after_representation_phase": representation_phase["current_eq30"],
            "train_following_eq30_eta_completion_after_representation_phase": representation_phase["following_eq30_completion"],
            ...
            "val_following_eq30_eta_completion": validation["following_eq30_completion"],
            "val_current_position_rmse_m": validation["current_position_rmse_m"],
            "val_following_position_rmse_m": validation["following_position_rmse_m"],
        ...
        print(
            ...
            f"recursive_val_Eq30mean={validation['temporal_state_loss']:.6g}, "
        ...
        if epochs_without_improvement >= EARLY_STOP_PATIENCE:
            print(
                "early stopping: recursive validation Eq.(30)-type state loss "
                f"did not improve for {EARLY_STOP_PATIENCE} complete alternating "
                f"cycles; best epoch={best_epoch}, "
                f"best recursive val Eq30mean={best_val_joint_state_loss:.6g}"
            )
            break
    ...
    # Regenerate one final fixed offline dataset with the selected checkpoint for
    # auditability and reproducibility. It is not used to further update weights.
    final_rollout_samples = _generate_recursive_data01_samples(normalizer)
    final_train_samples, final_val_samples = _split_rollout_samples(
        final_rollout_samples
    )
    _, packed_final = _pack_rollout_samples(final_rollout_samples, normalizer)
    max_training_nmax_seen = max(
        max_training_nmax_seen,
        int(packed_final["sequence_length"]),
    )
    nmax = int(max_training_nmax_seen)

    feature_time = packed_final["time_gpst_s"]
    eta_future_end_time = packed_final["eta_future_end_time_gpst_s"]
    fixed = packed_final["fixed_raw"]
    observations = packed_final["observations_raw"]
    channel_masks = packed_final["channel_mask"]
    satellite_masks = packed_final["satellite_mask"]
    innovation_padded = packed_final["innovation_padded"]
    target_states_9 = packed_final["target_state_9"]
    eta_forward_state_sensitivities_9 = packed_final[
        "eta_forward_state_sensitivity_9"
    ]
    eta_forward_target_states_9 = packed_final[
        "eta_forward_target_state_9"
    ]
    residual_padded = observations[:, :, 0]
    prior_positions = np.stack([
        sample["posterior_position"] - sample["correction_15"][:3]
        for sample in final_rollout_samples
    ])
    truth_positions = packed_final["truth_position_ecef_m"]
    ...
    np.savez(
        ...
        posterior_position_ecef_m=packed_final["posterior_position_ecef_m"],
        truth_position_ecef_m=truth_positions,
        measurement_mode=np.asarray("pseudorange_only"),
        feature_timing=np.asarray("causal_lagged_zero_history_first_epoch"),
        training_history_source=np.asarray("KNet_recursive_self_rollout_no_classical_teacher_forcing"),
        truth_feedback_used=np.asarray(False),
    )

    print(
        f"Data01 recursive self-rollout: train={len(final_train_samples)}, "
        f"validation={len(final_val_samples)}, Nmax_seen={nmax}"
    )

    selected_training_stage = "recursive_self_rollout_ref15_alternating_best_checkpoint"
    ...
    torch.save(
        {
            ...
            "loss": "mean_raw_state_Eq30_current_plus_eta_following_completion_plus_Eq32_L2",
            "training_features": "recursive_KNet_self_rollout_Yan_Eqs10_to_21",
            "truth_usage": "Eq30_supervision_only_no_navigation_state_reset",
            "classical_teacher_forcing": False,
            "first_fusion_context": "zero_previous_context_KNet_update_from_first_usable_epoch",
            ...
            "recursive_training_status": "paper_consistent_completion_history_generation_unpublished",
    ...
    # -------------------------------------------------------------------------
    # After restoring the best recursive-validation checkpoint, run one full Data01
    # closed-loop stability diagnostic before the independent Data02 run. This
    # diagnostic never changes model weights or normalization statistics. It uses
    # the same zero-history first-epoch convention as training and Data02.
    def _recursive_data01_diagnostic_rollout():
        ...
        for event in build_exact_fusion_timeline(
            ...
            # Zero-history context was initialized before the first fusion epoch;
            # every usable Data01 diagnostic update is therefore produced by KNet.
    ...
    torch.save(
        {
            ...
            "classical_bias_gain_rows_forced_zero": False,
            ...
            "training_history_generator": "KNet_recursive_self_rollout_no_classical_KF",
            ...
            "offline_training_dataset": "Yan_Eqs18_to_20_regenerated_each_outer_epoch_from_recursive_KNet_self_rollout",
            ...
            "self_rollout_regenerated_each_outer_epoch": REGENERATE_SELF_ROLLOUT_EVERY_EPOCH,
    ...
    summary = {
        ...
        "masked_cla_protocol": {
            ...
            "training_features": "recursive_KNet_self_rollout_regenerated_before_each_outer_epoch",
            "recursive_training_claimed_by_paper": False,
            "recursive_training_status": "paper_consistent_completion_history_generation_unpublished",
            "classical_teacher_forcing": False,
            "truth_state_feedback_during_training": False,
            ...
            "internal_validation_status": "fixed_chronological_time_boundary; recursive_validation_truth_used_only_for_model_selection",
            "self_rollout_regenerated_each_outer_epoch": REGENERATE_SELF_ROLLOUT_EVERY_EPOCH,
                        "recursive_data01_diagnostic": recursive_data01_diagnostic,
        },
        ...
        "limitations": [
            ...
            "Fig. 8 outputs eta_k as inertial-navigation measurement-error estimation and Fig. 2 feeds IMU-error correction back to Error compensation. Eq. (8) is the only explicit compensation equation, so this run uses eta_k=[epsilon_g(3),epsilon_a(3)] and applies it to the following IMU interval. The six-dimensional interpretation, zero-order hold timing, and self-rollout-conditioned linearized forward eta loss remain paper-guided completions because the paper does not publish eta_k dimension, timing, or a separate eta target/loss.",
            "Fig. 8 is implemented as KG_k -> State Update -> Eq. (30) state loss. To match the online closed-loop input distribution, the offline Eqs. (18)-(20) dataset is regenerated from the frozen current KNet trajectory before each outer alternating epoch. Yan et al. do not publish this history-generation detail, so self-rollout regeneration is a paper-consistent completion; BPTT across navigation epochs is not used or attributed to the paper.",
            "The eta branch has no direct pseudo-label. Its following-interval navigation consequence is linearized around the current KNet posterior and treated as another supervised state instant in the same raw Eq. (30)-type temporal state objective. The future-state construction remains a paper-guided completion because Yan et al. do not publish eta_k timing/supervision; samples whose eta target crosses the fixed train/validation time boundary are excluded from optimization.",
