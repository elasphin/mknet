...
if __name__ == '__main__':
    ...
    # Bias-gradient ablation experiment.
    #
    # Modes:
    #   none       : A - no learned IMU-bias feedback
    #   accel_only : B - accelerometer-bias feedback only
    #   gyro_only  : C - gyro-bias feedback only
    #   both       : D - original 15-state feedback
    ...
    if BIAS_ABLATION_MODE not in BIAS_ABLATION_MASKS:
        raise ValueError(
            f'Unknown BIAS_ABLATION_MODE={BIAS_ABLATION_MODE!r}; '
            f'expected one of {tuple(BIAS_ABLATION_MASKS)}'
        )

    OUTPUT_DIR = Path('/kaggle/working/bias_ablation_phase6') / BIAS_ABLATION_MODE
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    MAX_FUSION_EPOCHS = int(os.environ.get('ABLATION_MAX_FUSION_EPOCHS', '1601'))
    MAX_TEST_FUSION_EPOCHS = 300
    TRAINING_EPOCHS = int(os.environ.get('ABLATION_TRAINING_EPOCHS', '1'))
    DIAGNOSTIC_ONLY = os.environ.get('BIAS_ABLATION_DIAGNOSTIC_ONLY', '1') != '0'
    LEARNING_RATE = 1e-4
    ...
    BIAS_UPDATE_MASK_T = torch.tensor(
        BIAS_ABLATION_MASKS[BIAS_ABLATION_MODE],
        dtype=torch.float64,
        device=DEVICE,
    )
    ...
    nav = NavState(initial_nav.position_ecef_m.copy(), initial_nav.velocity_ecef_mps.copy(), initial_nav.body_to_ecef_dcm.copy(), initial_nav.accel_bias_body_mps2.copy(), initial_nav.gyro_bias_body_radps.copy())
    P = P0.copy()
    history_rows = []
    ...
    for event_kind, event_start, event_end, event_index in fusion_timeline(imr_time, fusion_time, through_last_fusion=True):
        ...
        # Yan Eq. (7): [delta_p, delta_v, delta_theta, b_a, b_g].
        # The first 9 closed-loop error states are reset after feedback, while
        # b_a and b_g persist as the current estimated IMU biases.
        ...
        history_rows.append({
            ...
            'gyro': last_gyro.copy(),
            'posterior_nav': NavState(nav.position_ecef_m.copy(), nav.velocity_ecef_mps.copy(), nav.body_to_ecef_dcm.copy(), nav.accel_bias_body_mps2.copy(), nav.gyro_bias_body_radps.copy()),
    ...
    # Per-window diagnostics.  The gain head is laid out as
    # [15 state rows, Nmax measurement columns], so its gradient can be split
    # into navigation, accelerometer-bias and gyro-bias output rows.
    gradient_diagnostics_path = OUTPUT_DIR / 'bias_gradient_diagnostics.jsonl'
    gradient_diagnostics_path.write_text('', encoding='utf-8')
    (OUTPUT_DIR / 'ablation_config.json').write_text(
        json.dumps({
            'mode': BIAS_ABLATION_MODE,
            'mask': BIAS_ABLATION_MASKS[BIAS_ABLATION_MODE],
            'training_epochs': TRAINING_EPOCHS,
            'max_fusion_epochs': MAX_FUSION_EPOCHS,
    ...
    def _gain_head_gradient_groups():
        groups = {
            'navigation_0_8': (0, 9),
            'accel_bias_9_11': (9, 12),
            'gyro_bias_12_14': (12, 15),
        }
        result = {}
        ...
        weight_view = weight_grad.reshape(INS_STATE_DIM, network_nmax, -1)
        bias_view = None if bias_grad is None else bias_grad.reshape(INS_STATE_DIM, network_nmax)

        for name, (start_row, stop_row) in groups.items():
            tensors = [weight_view[start_row:stop_row].reshape(-1)]
            if bias_view is not None:
                tensors.append(bias_view[start_row:stop_row].reshape(-1))
            values = torch.cat(tensors).detach().double()
    ...
    def _gradient_snapshot():
        ...
        return {
            'all_gradients_finite': not nonfinite_parameters,
            'finite_parameter_count': finite_parameter_count,
            'total_l2_if_finite': (
                math.sqrt(total_sq) if not nonfinite_parameters else None
            ),
            'nonfinite_parameters': nonfinite_parameters,
            'gain_head_groups': _gain_head_gradient_groups(),
        }

    def _append_gradient_diagnostic(record):
        with gradient_diagnostics_path.open('a', encoding='utf-8') as stream:
    ...
    def _rollout(start_sample: int, stop_sample: int, *, phase: str | None=None, initial_state=None, loss_start_sample: int | None=None):
        ...
        metric_count = 0
        rollout_diag = {
            'max_abs_innovation_m': 0.0,
            'max_abs_raw_navigation_correction': 0.0,
            'max_abs_raw_accel_bias_correction_mps2': 0.0,
            'max_abs_raw_gyro_bias_correction_radps': 0.0,
            'max_abs_applied_accel_bias_correction_mps2': 0.0,
            'max_abs_applied_gyro_bias_correction_radps': 0.0,
        }
        ...
        with (torch.enable_grad() if training else torch.inference_mode()):
            for sample_index in range(start_sample, stop_sample):
                ...
                if innovation_now.numel():
                    rollout_diag['max_abs_innovation_m'] = max(
                        rollout_diag['max_abs_innovation_m'],
                ...
                raw_correction = state_update(
                    output,
                    innovation_nn.float().unsqueeze(0),
                )[0].double().reshape(INS_STATE_DIM)

                rollout_diag['max_abs_raw_navigation_correction'] = max(
                    rollout_diag['max_abs_raw_navigation_correction'],
                    float(torch.max(torch.abs(raw_correction[0:9].detach())).cpu()),
                )
                rollout_diag['max_abs_raw_accel_bias_correction_mps2'] = max(
                    rollout_diag['max_abs_raw_accel_bias_correction_mps2'],
                    float(torch.max(torch.abs(raw_correction[9:12].detach())).cpu()),
                )
                rollout_diag['max_abs_raw_gyro_bias_correction_radps'] = max(
                    rollout_diag['max_abs_raw_gyro_bias_correction_radps'],
                    float(torch.max(torch.abs(raw_correction[12:15].detach())).cpu()),
                )

                # Ablation is applied only to the bias-feedback rows.  The
                # 15-state architecture is unchanged in all four experiments.
                correction = raw_correction * BIAS_UPDATE_MASK_T

                rollout_diag['max_abs_applied_accel_bias_correction_mps2'] = max(
                    rollout_diag['max_abs_applied_accel_bias_correction_mps2'],
                    float(torch.max(torch.abs(correction[9:12].detach())).cpu()),
                )
                rollout_diag['max_abs_applied_gyro_bias_correction_radps'] = max(
                    rollout_diag['max_abs_applied_gyro_bias_correction_radps'],
                    float(torch.max(torch.abs(correction[12:15].detach())).cpu()),
                )
    ...
    def _train_block(phase: str, optimizer: torch.optim.Optimizer):
        ...
        for window_start in range(0, training_sample_count, OPTIMIZER_WINDOW_SIZE):
            ...
            # _rollout() has already executed mean_loss.backward().  Inspect
            # the Eq. (30) gradient before adding L2 regularization so we can
            # distinguish a navigation/BPTT failure from a regularization issue.
            eq30_gradient = _gradient_snapshot()
            _append_gradient_diagnostic({
                'mode': BIAS_ABLATION_MODE,
                'phase': phase,
            ...
            if not eq30_gradient['all_gradients_finite']:
                bad = ', '.join(
                    item['name'] for item in eq30_gradient['nonfinite_parameters']
                )
                raise FloatingPointError(
                    f'non-finite Eq.(30) gradient | mode={BIAS_ABLATION_MODE} '
                    f'phase={phase} window={window_start}:{window_stop} '
                    f'parameters={bad}'
                )
            ...
            regularized_gradient = _gradient_snapshot()
            _append_gradient_diagnostic({
                'mode': BIAS_ABLATION_MODE,
                'phase': phase,
            ...
            if not regularized_gradient['all_gradients_finite']:
                bad = ', '.join(
                    item['name']
                    for item in regularized_gradient['nonfinite_parameters']
                )
                raise FloatingPointError(
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        try:
            ...
            training_stop_reason = str(error)
    ...
    experiment_result = {
        'mode': BIAS_ABLATION_MODE,
        'completed_training_epochs': int(len(training_history)),
        'requested_training_epochs': int(TRAINING_EPOCHS),
        'stopped_on_nonfinite_gradient': training_stop_reason is not None,
        'stop_reason': training_stop_reason,
        'best_selection_epoch': int(best_epoch),
        'best_validation_loss': float(best_val_state_loss),
        'best_validation_position_rmse_m': float(best_val_position_rmse_m),
        'gradient_diagnostics_file': str(gradient_diagnostics_path),
    }
    (OUTPUT_DIR / 'bias_ablation_result.json').write_text(
        json.dumps(experiment_result, indent=2),
        encoding='utf-8',
    )
    print(
        'Bias ablation result | '
        f"mode={BIAS_ABLATION_MODE} | "
        f"nonfinite={experiment_result['stopped_on_nonfinite_gradient']} | "
        f"completed_epochs={experiment_result['completed_training_epochs']}"
    )

    if DIAGNOSTIC_ONLY:
        print(
            'Diagnostic-only run finished before Data02 test. '
