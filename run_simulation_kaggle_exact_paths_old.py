...
def anchor_imr_tow_to_gpst_seconds(
    ...
    This is the stronger synchronization guard used by orchestration.py. The
    paper requires synchronized multisensor fusion but does not publish this
    dataset-specific week-anchoring rule, so it remains a STANDARD-COMPLETION.
    """
    ...
    time_gpst_s = np.empty_like(tow_s)
    time_gpst_s[0] = first
    current_week = int(math.floor(first / GPS_WEEK_S))
    previous_tow_s = float(tow_s[0])

    for index in range(1, len(tow_s)):
        current_tow_s = float(tow_s[index])
        if current_tow_s + 0.5 * GPS_WEEK_S < previous_tow_s:
            current_week += 1
        elif current_tow_s - 0.5 * GPS_WEEK_S > previous_tow_s:
            current_week -= 1
        time_gpst_s[index] = current_week * GPS_WEEK_S + current_tow_s
        previous_tow_s = current_tow_s
...
def build_exact_fusion_timeline(
    ...
    *,
...
) -> tuple[PropagationSegment | FusionMarker, ...]:
    """Split IMU intervals at every exact GNSS/LEO fusion epoch.

    The scheduler is adapted from orchestration.py. Within each IMU interval the
    current project keeps the same current-sample zero-order-hold convention as
    the preceding run implementation; only the scheduling is centralized and
    validated here.
    """
    ...
    timeline: list[PropagationSegment | FusionMarker] = []
    fusion_index = 0
    ...
    for imu_index in range(1, len(imu_time)):
        ...
        while (
            ...
            if t > segment_start + 1e-12:
                timeline.append(
                    PropagationSegment(segment_start, t, imu_index)
                )
            timeline.append(FusionMarker(t, fusion_index))
            segment_start = t
            fusion_index += 1

        if interval_end > segment_start + 1e-12:
            timeline.append(
                PropagationSegment(segment_start, interval_end, imu_index)
            )
...
def elevation_azimuth_from_ecef_los(
    ...
    receiver_to_satellite_ned = c_ecef_to_ned(lat, lon) @ -np.asarray(
...
class RINEXObservationFile:
    ...
    def _select_signal_and_pseudorange(
        ...
        """
        index_by_type = {obs_type: i for i, obs_type in enumerate(obs_types)}
    ...
    def iter_epochs(self, allowed_constellations: set[str] | None = None):
        with self.path.open("r", encoding="ascii", errors="replace") as stream:
            ...
            for line in stream:
                ...
                for _ in range(satellite_count):
                    ...
                    cn0 = None
                    if snr_type in obs_types:
                        raw_snr = fields[obs_types.index(snr_type)].ljust(16)[:14].strip()
                        if raw_snr:
...
class SP3Orbit:
    ...
    def state(self, sat_id: str, time_gpst_s: float):
        ...
        finite_v = np.all(np.isfinite(native_velocity), axis=1)
        if finite_v.sum() >= 2:
            velocity, _ = self._interpolate(times[w][finite_v], native_velocity[finite_v], time_gpst_s)
        else:
            velocity = position_derivative
        finite_clock = np.isfinite(s["clock"])
        clock = None if not finite_clock.any() else float(np.interp(time_gpst_s, times[finite_clock], s["clock"][finite_clock]))
        finite_drift = np.isfinite(s["drift"])
        drift = None if not finite_drift.any() else float(np.interp(time_gpst_s, times[finite_drift], s["drift"][finite_drift]))
...
def read_imr(path: str | Path, scaling_mode: str = "cpp_exact") -> IMRData:
    path = Path(path)
    ...
    record_format = endian + IMR_RECORD_FORMAT_BODY
    record_bytes = struct.calcsize(record_format)
    with path.open("rb") as stream:
        stream.seek(512)
        body = stream.read()
    record_count = len(body) // record_bytes
    body = body[:record_count * record_bytes]

    tow = np.empty(record_count)
    gyro_counts = np.empty((record_count, 3), dtype=np.int32)
    accel_counts = np.empty((record_count, 3), dtype=np.int32)
    for i, record in enumerate(struct.iter_unpack(record_format, body)):
        tow[i] = record[0]
        gyro_counts[i] = record[1:4]
        accel_counts[i] = record[4:7]
...
def compensate_imu(
    ...
    gyro = np.linalg.solve(
        I + (S_g if S_g is not None else Z) + (M_g if M_g is not None else Z),
        np.asarray(measured_angular_rate_body_radps) - np.asarray(gyroscope_bias_body_radps),
    )
    accel = np.linalg.solve(
        I + (S_a if S_a is not None else Z) + (M_a if M_a is not None else Z),
        np.asarray(measured_specific_force_body_mps2) - np.asarray(accelerometer_bias_body_mps2),
    )
    return gyro, accel
...
def mechanize_ecef(
    ...
    C1 = _rotation(
        expm(-OMEGA_IE_SKEW * dt)
        @ C0
        @ expm(_skew(angular_rate_body_radps) * dt)
    )
    Cmid = 0.5 * (C0 + C1)
    acceleration = (
...
class GNSSPreprocessor:
    ...
    def _state(self, sat_id: str, time_gpst_s: float):
        position_ecef_m, _, clock_bias_s, _ = self.orbit.state(sat_id, time_gpst_s)
        if sat_id in self.clock.satellites:
            clock_bias_s = self.clock.bias(sat_id, time_gpst_s)
        if clock_bias_s is None:
            raise KeyError(sat_id)
        return position_ecef_m, float(clock_bias_s)
    ...
    def prepare_measurement(
        ...
        try:
            initial_position, _ = self._state(raw.sat_id, reception_time_gpst_s)
        except (KeyError, ValueError):
            return None

        initial_range, _, _ = geometric_range(receiver_position_ecef_m, initial_position, 0.0)
        try:
            transmit_time, transit_s, _ = _iterate_transmit_time(
                lambda sat_id, t: self._state(sat_id, t)[0],
                raw.sat_id,
        ...
        satellite_position_tx, satellite_clock_bias_s = self._state(raw.sat_id, transmit_time)
        rho, los, satellite_position_rx = geometric_range(
            receiver_position_ecef_m, satellite_position_tx, transit_s
        )
        elevation, azimuth = elevation_azimuth_from_ecef_los(receiver_position_ecef_m, los)
        ...
        if self.use_ionosphere:
            ...
            lat, lon, height_m = ecef_to_llh(receiver_position_ecef_m)
        ...
        if self.use_troposphere:
            if height_m is None:
                _, _, height_m = ecef_to_llh(receiver_position_ecef_m)
    ...
    def prepare_epoch(
        ...
        for raw in epoch.measurements:
            measurement = self.prepare_measurement(epoch.time_gpst_s, raw, antenna_position)
...
def build_measurement_model(
    ...
    """Build the 15-state pseudorange model with GPS/BDS clock nuisance elimination.

    For each GNSS constellation, the WLS receiver-clock offset is removed by the
    weighted residual-maker matrix

        P_c = I - 1 (1^T R_c^-1) / (1^T R_c^-1 1).

    The same transform is applied to innovation, H, and R. This preserves the
    original per-satellite row layout used by Masked CLA while making the
    measurement statistics consistent. The projected covariance is singular by
    one rank per GNSS constellation; the classical update therefore uses the
    Moore-Penrose inverse on the innovation covariance.
    """
    ...
    projector = np.eye(n)
    receiver_clock = estimate_receiver_clock_biases_wls_m(
...
def inject_error_state(nav: NavigationState, dx: Array) -> NavigationState:
    ...
    out.body_to_ecef_dcm = _rotation(
        expm(ATTITUDE_FEEDBACK_SIGN * _skew(dx[6:9])) @ out.body_to_ecef_dcm
    )
...
class TLESGP4Provider:
    ...
    def state_at(self, time_gpst_s: float, sat_id: str) -> LEOSatelliteState:
        ...
        index = bisect_right([e.epoch_gpst_s for e in elements], time_gpst_s) - 1
        if index < 0:
        ...
        utc = Time(time_gpst_s, format="gps").utc
...
class LEODownlinkSimulator:
    ...
    def simulate_one(self, receive_time_gpst_s: float, receiver_position_ecef_m: Array, sat_id: str):
        initial_state = self.provider.state_at(receive_time_gpst_s, sat_id)
        initial_range = np.linalg.norm(initial_state.position_ecef_m - receiver_position_ecef_m)
        transmit_time, transit_s, _ = _iterate_transmit_time(
        ...
        state_tx = self.provider.state_at(transmit_time, sat_id)
        rho, los, sat_rx = geometric_range(
            receiver_position_ecef_m, state_tx.position_ecef_m, transit_s
        )
        elevation, azimuth = elevation_azimuth_from_ecef_los(receiver_position_ecef_m, los)
        if elevation < self.minimum_elevation_rad:
            return None

        receiver_lat, _, receiver_height_m = ecef_to_llh(receiver_position_ecef_m)
    ...
    def simulate_epoch(self, receive_time_gpst_s: float, receiver_position_ecef_m: Array):
        ...
        for sat_id in self.provider.satellite_ids:
            try:
                measurement = self.simulate_one(receive_time_gpst_s, receiver_position_ecef_m, sat_id)
            except ValueError:
                continue
            if measurement is not None:
...
class MaskedConv1d(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        local_count = F.conv1d(
            m, ones, stride=1, padding=self.padding
        )
...
if __name__ == "__main__":
    ...
    imr = read_imr(IMR_PATH, scaling_mode="cpp_exact")
    rinex = RINEXObservationFile.open(RINEX_OBS_PATH)
    gnss_epochs = tuple(rinex.iter_epochs(allowed_constellations={"G", "C"}))
    ...
    print("IMU samples:", len(imr.tow_s))
    print("RINEX epochs:", len(gnss_epochs))

    # =========================================================================
    # 2. GPST SYNCHRONIZATION
    # =========================================================================
    # IMR stores TOW; orchestration.py's stronger helper validates finite values,
    # week unwrapping, and strict monotonicity.
    anchor_time = float(gnss_epochs[0].time_gpst_s)
    imr_time = anchor_imr_tow_to_gpst_seconds(imr.tow_s, anchor_time)
    ...
    # Trim to the common IMU + antenna-truth + IMU-truth span.
    common_start = max(float(imr_time[0]), float(antenna_truth_time[0]), float(imu_truth_time[0]))
    common_end = min(float(imr_time[-1]), float(antenna_truth_time[-1]), float(imu_truth_time[-1]))
    start = int(np.searchsorted(imr_time, common_start, side="left"))
    stop = min(int(np.searchsorted(imr_time, common_end, side="left")) + 1, len(imr_time))
    imr = IMRData(
        imr.header,
        imr.tow_s[start:stop].copy(),
        imr.angular_rate_body_radps[start:stop].copy(),
        imr.acceleration_body_mps2[start:stop].copy(),
        imr.raw_gyro_counts[start:stop].copy(),
        imr.raw_accel_counts[start:stop].copy(),
    )
    imr_time = imr_time[start:stop].copy()

    fusion_end = min(float(imr_time[-1]), float(antenna_truth_time[-1]), float(imu_truth_time[-1]))
    gnss_epochs = tuple(
        epoch for epoch in gnss_epochs
        if epoch.measurements and imr_time[0] <= epoch.time_gpst_s <= fusion_end
    )
    if MAX_FUSION_EPOCHS is not None:
        gnss_epochs = gnss_epochs[:MAX_FUSION_EPOCHS]
    ...
    for event in training_timeline:
        ...
        posterior_residual = build_measurement_model(
            nav, measurements, lever_arm_b_m
        ).innovation
    ...
    test_imr = read_imr(test_files.imr, scaling_mode="cpp_exact")
    test_rinex = RINEXObservationFile.open(test_files.rinex_obs)
    test_gnss_epochs = tuple(test_rinex.iter_epochs(allowed_constellations={"G", "C"}))
    ...
    test_anchor_time = float(test_gnss_epochs[0].time_gpst_s)
    test_imr_time = anchor_imr_tow_to_gpst_seconds(
        test_imr.tow_s,
        test_anchor_time,
    )
    ...
    test_common_start = max(
        float(test_imr_time[0]),
        float(test_antenna_truth_time[0]),
        float(test_imu_truth_time[0]),
    )
    test_common_end = min(
        float(test_imr_time[-1]),
        float(test_antenna_truth_time[-1]),
        float(test_imu_truth_time[-1]),
    )
    test_start = int(np.searchsorted(test_imr_time, test_common_start, side="left"))
    test_stop = min(
        int(np.searchsorted(test_imr_time, test_common_end, side="left")) + 1,
        len(test_imr_time),
    )
    test_imr = IMRData(
        test_imr.header,
        test_imr.tow_s[test_start:test_stop].copy(),
        test_imr.angular_rate_body_radps[test_start:test_stop].copy(),
        test_imr.acceleration_body_mps2[test_start:test_stop].copy(),
        test_imr.raw_gyro_counts[test_start:test_stop].copy(),
        test_imr.raw_accel_counts[test_start:test_stop].copy(),
    )
    test_imr_time = test_imr_time[test_start:test_stop].copy()
    ...
    test_gnss_epochs = tuple(
        epoch for epoch in test_gnss_epochs
        if epoch.measurements and test_imr_time[0] <= epoch.time_gpst_s <= test_fusion_end
    )
    if MAX_TEST_FUSION_EPOCHS is not None:
        test_gnss_epochs = test_gnss_epochs[:MAX_TEST_FUSION_EPOCHS]
    ...
    print("test IMU samples:", len(test_imr.tow_s))
    print("test RINEX epochs:", len(test_gnss_epochs))
    ...
    for event in test_timeline:
        ...
        if previous_online is None:
            ...
            warm_posterior_residual = build_measurement_model(
