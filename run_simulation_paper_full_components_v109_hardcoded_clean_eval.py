...
# Fixed LEO prefilter guard; final elevation mask remains 10 deg.
...
if __name__ == '__main__':
    ...
    # --- DATA01 TRAIN/VALIDATION SETUP ---
    # Nmax is fixed from Data01; Yan features are built causally inside _network_input.
    ...
    # Groupwise L2 normalization used by the project.
    ...
    # One alternating optimization block over Data01.
    def _train_block(phase: str, optimizer: torch.optim.Optimizer):
        ...
        for window_start in range(0, training_sample_count, OPTIMIZER_WINDOW_SIZE):
            ...
            regularization = GAMMA_L2 * sum(torch.sum(p.double() * p.double()) for p in all_network_parameters)
    ...
    # Validation is a causal continuation through the Data01 prefix.
    ...
    # Zero-gain initialization remains the fallback checkpoint.
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        train_grad = max(filter_phase['gradient_l2_norm'], representation_phase['gradient_l2_norm'])
        training_history.append({
            'epoch': epoch,
            'train_loss': representation_phase['loss'],
            'train_gradient_l2_norm': train_grad,
            'train_position_rmse_m': representation_phase['position_rmse_m'],
            'validation_loss': validation_loss,
            'validation_position_rmse_m': validation_rmse,
        })
        print(
            f"Epoch {epoch:02d}/{TRAINING_EPOCHS} | "
            f"loss={representation_phase['loss']:.6g} | grad={train_grad:.3g} | "
            f"train RMSE={representation_phase['position_rmse_m']:.3f} m | "
            f"val loss={validation_loss:.6g} | val RMSE={validation_rmse:.3f} m"
        )
    ...
    def _online_features(previous_online, current_sat_ids, innovation_now, current_accel, current_gyro):
        ...
        fixed_k = _normalize_fixed(np.concatenate([
            np.asarray(current_accel, dtype=float).reshape(3) - previous_online['accel'],
            np.asarray(current_gyro, dtype=float).reshape(3) - previous_online['gyro'],
        ...
        return fixed_k, _normalize_obs(np.stack([residual_k, innovation_k], axis=1), channel_k), current_mask, channel_k, innovation_k

    # Pad one online epoch to Data01 Nmax.
    def _pad_epoch(fixed_k, obs_k, current_mask, channel_k, innovation_k):
        ...
        innovation_padded_now = np.zeros(network_nmax, dtype=float)
    ...
    test_P0 = np.diag(np.concatenate((test_imu_model_values['ISDV_Pos'], test_imu_model_values['ISDV_Vel'], test_imu_model_values['ISDV_Att'] * (np.pi / 180.0))) ** 2)
    ...
    for event_kind, event_start, event_end, event_index in fusion_timeline(test_imr_time, test_fusion_time, through_last_fusion=True):
        ...
        fixed_k, obs_k, current_mask, channel_k, innovation_k = _online_features(previous_online, meas.sat_ids, meas.innovation, last_feature_accel, last_feature_gyro)
        fixed_nn, obs_nn, mask_nn, channel_nn, innovation_nn = _pad_epoch(fixed_k, obs_k, current_mask, channel_k, innovation_k)
        with torch.inference_mode():
            output = model(
                torch.tensor(fixed_nn[None], dtype=torch.float32, device=DEVICE),
                torch.tensor(obs_nn[None], dtype=torch.float32, device=DEVICE),
                torch.tensor(mask_nn[None], dtype=torch.bool, device=DEVICE),
                torch.tensor(channel_nn[None], dtype=torch.bool, device=DEVICE),
                recurrent_state=recurrent_state,
            )
            recurrent_state = output.recurrent_state
            correction = state_update(output, torch.tensor(innovation_nn[None], dtype=torch.float32, device=DEVICE))[0].cpu().numpy().astype(float)
