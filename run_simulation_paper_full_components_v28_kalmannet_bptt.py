...
#     is introduced. KalmanNet-style external-recurrence BPTT allows these rows to
#     receive only physically mediated future-state supervision through Eq. (8).
#   - Fig. 8 also outputs eta_k for inertial-navigation measurement-error
#     estimation. Its direct target/timing are unpublished. The Eq. (8) interface
#     is retained and eta receives only indirect future-state supervision through
#     the differentiable closed-loop recurrence; no direct eta label is invented.
...
class MaskedCLA(nn.Module):
    ...
    Eq. (8)+INS mechanization used online, so estimator-dependent features are
    generated from the learned trajectory.  Network parameters remain fixed within
    each short trajectory and are updated only after a mini-batch of completed
    trajectories. The representation/filter parameter blocks are optimized
    alternately following Latent-KalmanNet. No synthetic noisy-prior augmentation
    is added. Exact pooling hyperparameters,
    ...
    def __init__(
        ...
        # does not publish a direct eta target/timing. During Data01 training the
        # KalmanNet-guided differentiable external recurrence lets future Eq. (30)
        # losses supervise eta only through its physical Eq. (8) effect.
        self.imu_error_head = nn.Linear(
            64,
            IMU_ERROR_DIM,
        )
        # Zero-output initialization prevents arbitrary IMU feedback before
        # training. No direct auxiliary eta loss is invented; future-state gradient
        # arrives only through the Eq. (8)+INS recurrence. Initialization remains an
        # explicit reproducibility completion.
        ...
        *,
        ...
        with grad_context:
            for sample_index in range(start_sample, stop_sample):
                ...
                recurrent_capacity = max(recurrent_capacity, len(current_sat_ids))
                (
                    fixed_nn,
                    obs_nn,
                    mask_nn,
    ...
    torch.save(
        {
            ...
            "checkpoint_schema": "full_15_state_gain_v28_kalmannet_external_bptt",
            "nmax": nmax,
