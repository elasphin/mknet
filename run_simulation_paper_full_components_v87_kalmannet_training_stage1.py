...
if __name__ == '__main__':
    ...
    MAX_FUSION_EPOCHS = _env_optional_int('MKNET_MAX_FUSION_EPOCHS', 500)
    ...
    print(f"training setup: samples={training_sample_count}, Nmax={network_nmax} (Data01={data01_sequence_nmax}, Data02={data02_sequence_nmax}), mode=KalmanNet_V2_short_trajectories, Tshort={KNET_V2_TRAJECTORY_LENGTH}, trajectories={trajectory_count}, trajectory_batch={TRAJECTORY_BATCH_SIZE}, batching=fixed_random_sample_without_replacement_one_step_per_phase, init=ground_truth_navigation_state, V2_warmup_epochs={KNET_V2_WARMUP_EPOCHS}, V1_finetune_epochs={KNET_V1_FINE_TUNE_EPOCHS}, lr_theta={FILTER_BLOCK_LEARNING_RATE:g}, lr_psi={REPRESENTATION_BLOCK_LEARNING_RATE:g}, FDE={('ON' if FDE_DIA_ON else 'OFF')}")
    # --- MASKED-CLA TRAINING: KALMANNET V2 WARM-UP -> V1 FULL BPTT ---
    model = MaskedCLA(nmax=network_nmax, dropout=0.2).to(DEVICE)
    # Stage-1 controlled ablation: retain the existing neutral zero-K gain-head
    # initialization so batching/dropout/checkpoint effects are isolated first.
    # Yan et al. do not publish this initialization; official KalmanNet uses
    # ordinary framework initialization. Revisit this in Stage 2 if needed.
    with torch.no_grad():
