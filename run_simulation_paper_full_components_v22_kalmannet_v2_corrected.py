...
- Across navigation fusion epochs, training uses KalmanNet-style truncated
  ...
  fusion epochs inside the same short trajectory.  The requested chunk length is
  treated as a maximum: when a shortened Data01 run would otherwise create too
  few trajectories for KalmanNet's mini-batch condition M < N, the effective
  chunk length is reduced using only the available Data01 sample count and the
  configured trajectory-batch size.  No test-data statistic is used.
...
This revision also hardens diagnostic bookkeeping without changing any navigation
or neural-network equation:
...
if __name__ == "__main__":
    ...
    print(
        ...
        f"max-chunk={TBPTT_CHUNK_LENGTH_REQUESTED}; requested chunk-batch={BATCH_SIZE}; "
    ...
    # navigation hyperparameter.  Eq. (14) additionally defines a mini-batch of
    # M trajectories selected from N trajectories with M < N.  Therefore the
    # requested length is treated as a maximum.  If a shortened diagnostic Data01
    # run would produce N <= requested batch size, reduce the chunk length using
    # only Data01 sample count so that the configured M can remain smaller than N.
    # This prevents the previous 99-sample/100-step run from silently becoming V1.
    if training_sample_count < 2:
        raise ValueError("KalmanNet V2 requires at least two training samples")

    if training_sample_count > BATCH_SIZE:
        max_chunk_for_requested_batch = max(
            1, training_sample_count // (BATCH_SIZE + 1)
        )
    else:
        # Not enough samples to form BATCH_SIZE+1 trajectories; use the shortest
        # legal chunks and reduce the effective trajectory batch below N instead.
        max_chunk_for_requested_batch = 1

    TBPTT_CHUNK_LENGTH_EFFECTIVE = min(
        TBPTT_CHUNK_LENGTH_REQUESTED,
        max_chunk_for_requested_batch,
    )
    training_chunks = [
        (
            start,
    ...
    if len(training_chunks) < 2:
        raise RuntimeError(
            "KalmanNet V2 must produce multiple short trajectories; "
            "increase Data01 coverage or reduce MKNET_TBPTT_CHUNK_LENGTH"
        )
    ...
    print(
        ...
        len(training_chunks),
        f"(requested max length={TBPTT_CHUNK_LENGTH_REQUESTED}, "
        f"effective max length={TBPTT_CHUNK_LENGTH_EFFECTIVE}, "
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        training_history.append({
            "stage": "yan_kalmannet_v2_tbptt_temporal_corrected_latent_alternating",
            "epoch": epoch,
    ...
    summary = {
        ...
        "unavoidable_unpublished_completions": [
            ...
            "KalmanNet_V2_chunk_length_uses_Revach_explicit_T100_example_Yan_does_not_publish_length",
