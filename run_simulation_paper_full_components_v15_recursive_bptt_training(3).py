...
- full 15-row classical and learned measurement corrections in the Eq. (7)
  ...
  zero-initialized for safety but remain trainable through recursive state loss;
...
This revision keeps the non-invasive divergence diagnostics and also corrects
the train/test recurrence mismatch exposed by comparison with the original
KalmanNet training procedure:
...
- a Data01-only affine input standardization is applied as numerical conditioning
  because the raw closed-loop features were observed to leave the offline range
  by many orders of magnitude. This scaling changes neither the physical feature
  definitions nor any navigation/FDE/state-update equation;
- Yan's published fixed offline X/M/Y training stage is retained unchanged as a
  warm start.  It is followed by a Data01-only KalmanNet-style recursive
  trajectory refinement in which the network's own previous correction/residual
  regenerates the causal Eqs. (11)-(13) features, while the current innovation is
  updated through the locally linearized TC measurement model.  The complete
  Data01 trajectory is unfolded and differentiated end-to-end (full-trajectory
  BPTT) with the same Eq. (30)/(32) state objective and the same Ref. [15]-style
  alternating parameter blocks.  Yan et al. do not publish this BPTT schedule,
  so this second stage is explicitly a KalmanNet-reference-guided completion and
...
#   - the six bias-gain rows are conservatively zero-initialized, then remain
#     trainable through their propagated effect on future labeled navigation states
#     in the recursive 15-state trajectory objective
...
class MaskedCLA(nn.Module):
    ...
    recursively regenerates the causal state/residual features on Data01 while
    leaving this per-epoch network architecture unchanged. Exact pooling
    hyperparameters, FC-head tensorization, eta_k supervision/timing and behavior
    when N_test>Nmax_train remain explicitly documented project completions.
    """
    ...
    def __init__(
        ...
        # prevent arbitrary pretraining feedback. They are not frozen: recursive
        # BPTT trains them through the 15-state transition into future Eq. (30)
        # navigation-state errors. This initialization is a declared completion.
        with torch.no_grad():
...
def fig8_post_update_error_state_9(
    ...
    update itself remains 15-dimensional; its bias rows receive temporal
    supervision through the recursive 15-state transition into later labeled
    navigation errors.
    """
...
if __name__ == "__main__":
    ...
    # KalmanNet-reference-guided trajectory refinement.
    #
    # Yan et al. explicitly publish the fixed X/M/Y construction (Eqs. 18-20),
    # the end-to-end state loss (Eqs. 30-32), and alternating optimization, but
    # not whether their optimizer unfolds the external KalmanNet recursion through
    # successive fusion epochs.  The original KalmanNet training algorithm does
    # use trajectory-level BPTT.  To remove the observed train/test mismatch
    # without replacing any published Yan component, the fixed offline stage below
    # is kept as the warm start and a second Data01-only recursive refinement is
    # applied afterwards.  Its epoch count is an explicit reproducibility
    # completion and can be overridden for runtime studies.
    RECURSIVE_TRAJECTORY_TRAINING_ON = True
    RECURSIVE_TRAINING_EPOCHS = _env_int(
        "MKNET_RECURSIVE_TRAINING_EPOCHS",
        min(TRAINING_EPOCHS, 20),
    )
    RECURSIVE_FILTER_LEARNING_RATE = FILTER_BLOCK_LEARNING_RATE
    RECURSIVE_REPRESENTATION_LEARNING_RATE = REPRESENTATION_BLOCK_LEARNING_RATE
    ...
    print(
        "KalmanNet-style recursive trajectory refinement:",
        (
            f"ON ({RECURSIVE_TRAINING_EPOCHS} full-trajectory BPTT epochs, Data01 only)"
            if RECURSIVE_TRAJECTORY_TRAINING_ON
            else "OFF"
        ),
    )
    ...
    # Linearized 15-state transition from the previous usable fusion epoch to
    # the current one.  It is retained for the later KalmanNet-style recursive
    # BPTT refinement only; the published fixed X/M/Y warm-start stage does not
    # use it.
    transition_since_previous_usable_fusion = np.eye(INS_STATE_DIM)
    ...
    # This partition is explicitly a paper-guided completion. The separate
    # Ref. [15] encoder warm-start is not copied because Yan provides no
    # intermediate representation target.
    # -------------------------------------------------------------------------
    ...
    print(
        "Kalman gain output:",
        "full 15-state Eq.(7) rows; b_a/b_g rows zero-initialized and trainable "
        "through recursive future-state loss",
    )
    ...
    def _state_prediction_loss(predicted_update, true_error_state_9):
        ...
        # are supervised by their future navigation effect during recursive BPTT.
    ...
    def _evaluate_state_objective(loader):
        ...
        with torch.inference_mode():
            for batch in loader:
                ...
                batch_size_now = len(fixed_b)
                valid_future_now = int(
                    torch.count_nonzero(eta_valid_b).item()
                )
                objective_sum += (
                    float(objective.detach().cpu()) * batch_size_now
                )
                temporal_state_sum += (
                    float(temporal_state_loss.detach().cpu())
                    * batch_size_now
                )
                current_eq30_sum += (
                    float(current_eq30_loss.detach().cpu()) * batch_size_now
                ...
                if valid_future_now:
                    future_eq30_weighted_sum += (
                        float(future_eq30_loss.detach().cpu())
                        * valid_future_now
                    )
                    future_position_rmse_squared_sum += (
                        float(future_position_rmse_b.detach().cpu()) ** 2
    ...
    print(
        ...
        "ON FIXED OFFLINE DATA01 (Eqs. 18-20; KG + Eq.8 ETA) ==="
    )
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        # Ref. [15] Algorithm 2 ordering: theta first, then psi. The same
        # fixed Eqs. (18)-(20) Data01 dataset is used throughout training, as
        # explicitly constructed in Yan et al.; it is NOT regenerated from the
        # current network trajectory.
        epoch_batches = list(train_loader)
        filter_phase = _run_training_phase(
            "filter", filter_optimizer, epoch_batches, epoch
        )
        representation_phase = _run_training_phase(
            "representation", representation_optimizer, epoch_batches, epoch
        )

        final_training_metrics = _evaluate_state_objective(evaluation_loader)
        ...
        training_history.append({
            "stage": "yan_fixed_offline_Data01_ref15_alternating",
            "epoch": epoch,
    ...
    # not report an internal validation checkpoint rule.  Keep the final
    # published-style warm-start parameters here; the reference-guided recursive
    # refinement below starts from this exact state.
    model.eval()
    ...
    # =========================================================================
    # 8B. KALMANNET-STYLE RECURSIVE DATA01 TRAJECTORY REFINEMENT
    # =========================================================================
    # Why this stage exists:
    #   Yan et al. explicitly publish fixed offline X/M/Y (Eqs. 18-20), Eq. (30)
    #   end-to-end state supervision, Eq. (32), and alternating optimization.
    #   The exact training-time treatment of the *external* KalmanNet recursion
    #   is not published.  The original KalmanNet training algorithm, however,
    #   unfolds complete state-estimation trajectories and applies BPTT.
    #
    # Paper-preserving policy used here:
    #   1) The published Yan fixed-X/M/Y training above is retained unchanged as
    #      the warm start.
    #   2) No architecture, Eq. (30)/(32), FDE rule, INS equation, measurement
    #      equation, pseudorange-only assumption, or TLE-only assumption is
    #      replaced.
    #   3) A second Data01-only refinement regenerates the causal Eqs. (11)-(13)
    #      quantities from the network's own previous update, exactly as occurs
    #      online.  The current innovation is updated by the local linearized TC
    #      model nu_rec ~= nu_ref - H * delta_prior.
    #   4) The network correction is fed forward through the accumulated 15-state
    #      transition matrix; eta_k is fed forward through the already-declared
    ...
    recursive_fixed_mean_t = torch.tensor(
        fixed_mean,
        dtype=torch.float32,
        device=DEVICE,
    )
    recursive_fixed_std_t = torch.tensor(
        fixed_std,
        dtype=torch.float32,
        device=DEVICE,
    )
    recursive_obs_mean_t = torch.tensor(
        obs_mean,
        dtype=torch.float32,
        device=DEVICE,
    )
    recursive_obs_std_t = torch.tensor(
        obs_std,
        dtype=torch.float32,
        device=DEVICE,
    )

    # One ordinary classical fusion row is used as the same causal warm context
    # used by the online Data02 pass.  It is not a learned training target.
    recursive_warm_previous_correction = torch.tensor(
        history_rows[0]["x_post"],
        dtype=torch.float32,
        device=DEVICE,
    )
    recursive_warm_previous_residual = torch.tensor(
        history_rows[0]["residual"],
        dtype=torch.float32,
        device=DEVICE,
    )
    recursive_warm_previous_sat_ids = tuple(history_rows[0]["sat_ids"])

    def _recursive_trajectory_objective():
        """Unfold Data01 in chronological order and return one Eq. (32) objective.

        This routine keeps the full computation graph across fusion epochs.  It
        uses the paper's current-epoch state loss at every usable epoch; the
        dependence of later losses on earlier learned corrections is what supplies
        the trajectory-level BPTT signal.
        """
        prior_deviation_15 = torch.zeros(
            INS_STATE_DIM,
            dtype=torch.float32,
            device=DEVICE,
        )
        previous_correction_15 = recursive_warm_previous_correction
        ...
        max_prior_position_deviation_m = 0.0
        max_abs_recursive_innovation_m = 0.0
        max_accel_bias_correction_norm_mps2 = 0.0
        max_gyro_bias_correction_norm_radps = 0.0

        for sample_index in range(len(feature_time)):
            # feature sample s corresponds to history row k=s+1.
            history_index = sample_index + 1
            current_row = history_rows[history_index]
            current_sat_ids = tuple(current_row["sat_ids"])
            current_count = len(current_sat_ids)
            if current_count <= 0 or current_count > nmax:
                raise RuntimeError(
                    "recursive Data01 training encountered an invalid "
                    f"measurement count {current_count}"
                )
            ...
            reference_innovation = torch.tensor(
                current_row["innovation"],
                dtype=torch.float32,
                device=DEVICE,
            )
            if H_current.shape != (current_count, INS_STATE_DIM):
                raise RuntimeError(
                    "recursive Data01 H dimension is inconsistent with sat_ids"
                )

            # First-order closed-loop innovation about the stable classical
            # Data01 reference trajectory.  H already contains the same GPS/BDS
            # clock projection used by the online estimator.
            innovation_current = (
                reference_innovation
                - H_current @ prior_deviation_15
            )

            # Paper Eq. (11): previous posterior residual, aligned only to
            # satellites still visible at the current epoch.
            previous_slot = {
                sat_id: slot
                for slot, sat_id in enumerate(previous_sat_ids)
            }
            residual_values = []
            residual_valid = []
            zero_scalar = innovation_current.new_zeros(())
            for sat_id in current_sat_ids:
                slot = previous_slot.get(sat_id)
                if slot is None:
                    residual_values.append(zero_scalar)
                    residual_valid.append(False)
                else:
                    residual_values.append(previous_residual[slot])
                    residual_valid.append(True)
            residual_current_slots = torch.stack(residual_values)

            # Paper Eqs. (12)-(13), generated from the learned completed
            # correction history rather than the classical history used by the
            # fixed warm-start dataset.
            previous_state_innovation = previous_correction_15
            previous_state_residual = (
                torch.zeros_like(previous_correction_15)
                if previous_previous_correction_15 is None
                else (
                    previous_correction_15
                    - previous_previous_correction_15
                )
            )

            delta_imu = torch.tensor(
                fixed[sample_index, :6],
                dtype=torch.float32,
                device=DEVICE,
            )
            fixed_raw = torch.cat(
            ...
            if fixed_raw.shape != (FIXED_FEATURE_DIM,):
                raise RuntimeError(
                    "recursive Data01 fixed feature has the wrong dimension"
                )
            fixed_network = (
                (fixed_raw - recursive_fixed_mean_t)
                / recursive_fixed_std_t
            )

            # Preserve Yan's Nmax zero-padding and masking during recursive
            # training rather than silently changing the network tensorization.
            observation_raw = torch.zeros(
                (nmax, OBSERVATION_FEATURE_DIM),
                dtype=torch.float32,
                device=DEVICE,
            )
            ...
            satellite_mask = torch.zeros(
                nmax,
                dtype=torch.bool,
                device=DEVICE,
            )
            innovation_padded_recursive = torch.zeros(
                nmax,
                dtype=torch.float32,
                device=DEVICE,
            )

            observation_raw[:current_count, 0] = residual_current_slots
            observation_raw[:current_count, 1] = innovation_current
            channel_mask[:current_count, 0] = torch.tensor(
                residual_valid,
                dtype=torch.bool,
                device=DEVICE,
            )
            channel_mask[:current_count, 1] = True
            satellite_mask[:current_count] = True
            innovation_padded_recursive[:current_count] = innovation_current

            observation_network = (
                observation_raw
                - recursive_obs_mean_t.view(1, OBSERVATION_FEATURE_DIM)
            ) / recursive_obs_std_t.view(1, OBSERVATION_FEATURE_DIM)
            observation_network = torch.where(
                channel_mask,
                observation_network,
                torch.zeros_like(observation_network),
            )

            output = model(
                fixed_network.unsqueeze(0),
            ...
            max_accel_bias_correction_norm_mps2 = max(
                max_accel_bias_correction_norm_mps2,
                float(
                    torch.linalg.vector_norm(
                        correction_15[ACCELEROMETER_BIAS_STATE_SLICE].detach()
                    ).cpu()
                ),
            )
            max_gyro_bias_correction_norm_radps = max(
                max_gyro_bias_correction_norm_radps,
                float(
                    torch.linalg.vector_norm(
                        correction_15[GYROSCOPE_BIAS_STATE_SLICE].detach()
                    ).cpu()
                ),
            )

            # The postprocessed Yan label is referenced to the classical prior.
            # Shift it by the learned-vs-classical prior deviation so Eq. (30)
            # remains a loss on the state reached by the recursive learned prior.
            reference_target_9 = torch.tensor(
                target_states_9[sample_index],
                dtype=torch.float64,
                device=DEVICE,
            )
            recursive_target_9 = (
                reference_target_9
                - prior_deviation_15[:SUPERVISED_STATE_DIM].double()
            )
            eq30_loss, _ = _state_prediction_loss(
                correction_15.unsqueeze(0),
                recursive_target_9.unsqueeze(0),
            )
            eq30_losses.append(eq30_loss)

            post_update_residual_9 = fig8_post_update_error_state_9(
                correction_15.unsqueeze(0),
                recursive_target_9.unsqueeze(0),
            )[0]
            position_squared_errors.append(
                torch.sum(post_update_residual_9[:3] ** 2)
            )

            # First-order posterior residual for the next Eq. (11) input.
            posterior_residual = (
                innovation_current
                - H_current @ correction_15
            )

            # Difference between the learned and classical posterior nominal
            # trajectories at this fusion epoch.
            classical_correction_15 = torch.tensor(
                current_row["x_post"],
                dtype=torch.float32,
                device=DEVICE,
            )
            posterior_deviation_15 = (
                prior_deviation_15
                + correction_15
                - classical_correction_15
            )

            max_prior_position_deviation_m = max(
                max_prior_position_deviation_m,
                float(
                    torch.linalg.vector_norm(
                        prior_deviation_15[:3].detach()
                    ).cpu()
                ),
            )
            max_abs_recursive_innovation_m = max(
                max_abs_recursive_innovation_m,
                float(
                    torch.max(
                        torch.abs(innovation_current.detach())
                    ).cpu()
                ),
            )

            # Propagate the learned-vs-classical trajectory difference to the
            # next usable fusion epoch.  The transition belongs to the next
            # history row because it spans current -> next.
            if sample_index + 1 < len(feature_time):
                next_history_row = history_rows[history_index + 1]
                Phi_next = torch.tensor(
                    next_history_row["preceding_state_transition"],
                    dtype=torch.float32,
                    device=DEVICE,
                )
                if Phi_next.shape != (INS_STATE_DIM, INS_STATE_DIM):
                    raise RuntimeError(
                        "recursive Data01 transition matrix has wrong shape"
                    )

                eta_sensitivity = torch.tensor(
                    eta_forward_state_sensitivities_9[sample_index],
                    dtype=torch.float32,
                    device=DEVICE,
                )
                eta_effect_9 = eta_sensitivity @ output.imu_error[0]
                eta_effect_15 = F.pad(
                    eta_effect_9,
                    (0, INS_STATE_DIM - SUPERVISED_STATE_DIM),
                )
                prior_deviation_15 = (
                    Phi_next @ posterior_deviation_15
                    + eta_effect_15
                )

            previous_previous_correction_15 = previous_correction_15
            previous_correction_15 = correction_15
            previous_residual = posterior_residual
            previous_sat_ids = current_sat_ids

        if not eq30_losses:
            raise RuntimeError(
                "recursive Data01 trajectory training produced no supervised step"
            )

        mean_eq30 = torch.stack(eq30_losses).mean()
        position_rmse = torch.sqrt(
            torch.stack(position_squared_errors).mean()
        )
        if GAMMA_L2:
            l2_all = sum(
                torch.sum(parameter * parameter)
            ...
            objective = mean_eq30 + GAMMA_L2 * l2_all
        else:
            objective = mean_eq30

        if not torch.isfinite(objective):
            raise FloatingPointError(
                "non-finite recursive Data01 trajectory objective"
            )

        metrics = {
            "objective": float(objective.detach().cpu()),
            "eq30": float(mean_eq30.detach().cpu()),
            "position_rmse_m": float(position_rmse.detach().cpu()),
            "max_prior_position_deviation_m": float(
                max_prior_position_deviation_m
            ),
            "max_abs_recursive_innovation_m": float(
                max_abs_recursive_innovation_m
            ),
    ...
    def _run_recursive_training_phase(
        phase: str,
        optimizer: torch.optim.Optimizer,
        epoch: int,
    ) -> dict:
        _set_alternating_phase(phase)
        optimizer.zero_grad(set_to_none=True)
        objective, metrics = _recursive_trajectory_objective()
        objective.backward()
        optimizer.step()
    ...
    def _evaluate_recursive_training_trajectory() -> dict:
        for parameter in model.parameters():
            parameter.requires_grad_(True)
        model.eval()
        with torch.no_grad():
            _, metrics = _recursive_trajectory_objective()
        return metrics

    if RECURSIVE_TRAJECTORY_TRAINING_ON:
        print(
            "\n=== KALMANNET-STYLE RECURSIVE DATA01 REFINEMENT "
            "(FULL-TRAJECTORY BPTT; REFERENCE-GUIDED COMPLETION) ==="
        )
        print(
            "recursive training epochs:",
            RECURSIVE_TRAINING_EPOCHS,
            "| FDE excluded from training (paper online-only placement)",
        )

        # Fresh Adam states are used for the refinement while retaining Yan's
        # published initial learning rate.  This prevents fixed-offline optimizer
        # moments from encoding the shuffled independent-sample regime.
        recursive_filter_optimizer = torch.optim.Adam(
            filter_parameters,
            lr=RECURSIVE_FILTER_LEARNING_RATE,
        )
        recursive_representation_optimizer = torch.optim.Adam(
            representation_parameters,
            lr=RECURSIVE_REPRESENTATION_LEARNING_RATE,
        )

        for recursive_epoch in range(
            1,
            RECURSIVE_TRAINING_EPOCHS + 1,
        ):
            recursive_filter_phase = _run_recursive_training_phase(
                "filter",
                recursive_filter_optimizer,
                recursive_epoch,
            )
            recursive_representation_phase = _run_recursive_training_phase(
                "representation",
                recursive_representation_optimizer,
                recursive_epoch,
            )
            recursive_training_metrics = (
                _evaluate_recursive_training_trajectory()
            )

            training_history.append({
                "stage": (
                    "kalmannet_reference_guided_recursive_Data01_"
                    "full_trajectory_BPTT"
                ),
                "epoch": int(recursive_epoch),
                "filter_phase_objective": recursive_filter_phase["objective"],
                "filter_phase_eq30": recursive_filter_phase["eq30"],
                "representation_phase_objective": (
                    recursive_representation_phase["objective"]
                ),
                "representation_phase_eq30": (
                    recursive_representation_phase["eq30"]
                ),
                "recursive_train_objective": (
                    recursive_training_metrics["objective"]
                ),
                "recursive_train_eq30": recursive_training_metrics["eq30"],
                "recursive_train_position_rmse_m": (
                    recursive_training_metrics["position_rmse_m"]
                ),
                "recursive_max_prior_position_deviation_m": (
                    recursive_training_metrics[
                        "max_prior_position_deviation_m"
                    ]
                ),
                "recursive_max_abs_innovation_m": (
                    recursive_training_metrics[
                        "max_abs_recursive_innovation_m"
                    ]
                ),
                "recursive_max_accel_bias_correction_norm_mps2": (
                    recursive_training_metrics[
                        "max_accel_bias_correction_norm_mps2"
                    ]
                ),
                "recursive_max_gyro_bias_correction_norm_radps": (
                    recursive_training_metrics[
                        "max_gyro_bias_correction_norm_radps"
                    ]
                ),
                "filter_block_learning_rate": (
                    RECURSIVE_FILTER_LEARNING_RATE
                ),
                "representation_block_learning_rate": (
                    RECURSIVE_REPRESENTATION_LEARNING_RATE
                ),
                "bptt_scope": "entire_Data01_usable_fusion_trajectory",
                "paper_status": (
                    "KalmanNet_reference_guided_completion_"
                    "Yan_exact_BPTT_schedule_unpublished"
                ),
            })

            print(
                f"recursive epoch {recursive_epoch:03d}/"
                f"{RECURSIVE_TRAINING_EPOCHS}: "
                f"theta_obj={recursive_filter_phase['objective']:.6g}, "
                f"psi_obj={recursive_representation_phase['objective']:.6g}, "
                f"Eq30={recursive_training_metrics['eq30']:.6g}, "
                f"posRMSE={recursive_training_metrics['position_rmse_m']:.3f} m, "
                f"maxPriorPosDev="
                f"{recursive_training_metrics['max_prior_position_deviation_m']:.3f} m, "
                f"max|innovation|="
                f"{recursive_training_metrics['max_abs_recursive_innovation_m']:.3f} m, "
                f"max|db_a|="
                f"{recursive_training_metrics['max_accel_bias_correction_norm_mps2']:.3e} m/s^2, "
                f"max|db_g|="
                f"{recursive_training_metrics['max_gyro_bias_correction_norm_radps']:.3e} rad/s"
            )

        selected_training_stage = (
            "yan_fixed_offline_warmstart_plus_"
            "kalmannet_recursive_full_trajectory_BPTT_completion"
        )
        best_val_state_loss = float(
            recursive_training_metrics["eq30"]
        )
        best_val_position_rmse_m = float(
            recursive_training_metrics["position_rmse_m"]
        )
        ...
        print(
            "\nKalmanNet-style recursive Data01 refinement: DISABLED"
        )
    ...
    torch.save(
        {
            ...
            "bias_gain_training": (
                "zero_initialized_not_frozen; no_fabricated_direct_bias_labels; "
                "recursive_15_state_future_navigation_supervision"
            ),
            "fixed_mean": fixed_mean,
            "fixed_std": fixed_std,
            "obs_mean": obs_mean,
            ...
            "eta_training": (
                "fixed_warmstart_future_state_Eq30_plus_recursive_BPTT_"
                "future_state_consequence_no_direct_eta_MSE"
            ),
            "alternating_partition": (
                "psi=conv+lstm+attention;theta=gain_head+eta_head"
            ),
            "training_stage_selected": selected_training_stage,
            "fixed_offline_warmstart_epochs": best_epoch,
            "recursive_trajectory_training_enabled": bool(
                RECURSIVE_TRAJECTORY_TRAINING_ON
            ),
            "recursive_trajectory_training_epochs": int(
                RECURSIVE_TRAINING_EPOCHS
                if RECURSIVE_TRAJECTORY_TRAINING_ON else 0
            ),
    ...
    # =========================================================================
    # DIAGNOSTIC ONLY. The published Yan fixed-X/M/Y warm start and the optional
    # KalmanNet-reference-guided recursive refinement have both finished before
    # this point. This replay uses model.eval()+inference_mode(), never updates a
    ...
    summary = {
        ...
        "paper_components": {
            ...
            "Eq7_Fig8_state_update": {
                ...
                "bias_supervision": (
                    "recursive_15_state_transition_to_future_labeled_"
                    "navigation_errors_no_invented_bias_truth"
                ),
            },
        ...
        "unavoidable_unpublished_completions": [
            ...
            "recursive_navigation_consequence_used_instead",
        ],
        "dataset_protocol": {
        ...
        "training": {
            ...
            "batch_size_completion": int(BATCH_SIZE),
            ...
            "recursive_trajectory_refinement": {
                "enabled": bool(RECURSIVE_TRAJECTORY_TRAINING_ON),
                "epochs": int(
                    RECURSIVE_TRAINING_EPOCHS
                    if RECURSIVE_TRAJECTORY_TRAINING_ON else 0
                ),
                ...
                "innovation_recursion": (
                    "local_TC_linearization_nu_reference_minus_H_delta_prior"
                ),
                "state_recursion": (
                    "delta_prior_next=Phi_delta_post_plus_Jeta_eta"
                ),
                "paper_status": (
                    "original_KalmanNet_reference_guided_completion_"
                    "Yan_exact_BPTT_schedule_unpublished"
                ),
                "final_metrics": recursive_training_metrics,
            },
