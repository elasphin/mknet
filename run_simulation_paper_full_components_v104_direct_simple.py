...
def validate_strict_time_axis(name: str, time_gpst_s: np.ndarray) -> np.ndarray:
    ...
    return time_gpst_s
class PropagationSegment(NamedTuple):
    start_time_gpst_s: float
    end_time_gpst_s: float
    imu_index: int
class FusionMarker(NamedTuple):
    time_gpst_s: float
    fusion_index: int
...
class IMUNoiseModelSI(NamedTuple):
    imu_type: str
...
# --- SmartPNT README.xml ---
class RoverMetadata(NamedTuple):
    imu_type: str
...
class GNSSPreprocessor:
    ...
    def prepare_measurement(self, reception_time_gpst_s: float, raw: SatelliteMeasurement, receiver_position_ecef_m: Array, receiver_llh: tuple[float, float, float] | None=None, c_ecef_ned: Array | None=None) -> PseudorangeMeasurement | None:
        ...
        if self.use_ionosphere:
            ...
            reception_week = int(math.floor(float(reception_time_gpst_s) / GPS_WEEK_S))
            tow = float(reception_time_gpst_s) - reception_week * GPS_WEEK_S
...
class _TLEElement(NamedTuple):
    epoch_gpst_s: float
    satrec: Satrec
class LEOSatelliteState(NamedTuple):
    position_ecef_m: Array
    velocity_ecef_mps: Array
...
if __name__ == '__main__':
    ...
    MAX_FUSION_EPOCHS = _env_optional_int('MKNET_MAX_FUSION_EPOCHS', 51)
    MAX_TEST_FUSION_EPOCHS = _env_optional_int('MKNET_MAX_TEST_FUSION_EPOCHS', 51)
    ...
    lever_arm_b_m = c_vehicle_to_body_zxy(*rover.mounting_xyz_deg) @ np.asarray(rover.lever_arm_vehicle_m, dtype=float).reshape(3)
    ...
    def _set_training_phase(phase: str) -> list[torch.nn.Parameter]:
        """Activate exactly one Latent-style block while preserving autograd paths."""
        ...
        # Frozen LSTM must execute in training mode for cuDNN backward to its
        # input; dropout is disabled so the frozen map stays deterministic.
        ...
        for parameter in representation_parameters:
            parameter.requires_grad_(True)
        return representation_parameters

    def _eq32_regularization() -> torch.Tensor:
        if not GAMMA_L2:
            return torch.zeros((), dtype=torch.float64, device=DEVICE)
        return GAMMA_L2 * sum(torch.sum(p.double() * p.double()) for p in all_network_parameters)
    ...
    def _stored_context(history_row_index: int):
        """Causal predecessor context used only at the physical sequence start."""
        if not 0 <= history_row_index < len(history_rows):
            raise ValueError('invalid stored Data01 context row')
        row = history_rows[history_row_index]
        x_pred = np.asarray(row['x_pred'], dtype=float).reshape(INS_STATE_DIM)
        x_post = np.asarray(row['x_post'], dtype=float).reshape(INS_STATE_DIM)
        ...
        context = {
            'sat_ids': tuple(row['sat_ids']),
            'residual': torch.as_tensor(np.asarray(row['residual'], dtype=float), dtype=torch.float64, device=DEVICE),
            ...
            'state_innovation': torch.as_tensor(state_innovation, dtype=torch.float64, device=DEVICE),
            'state_residual': torch.as_tensor(state_residual, dtype=torch.float64, device=DEVICE),
            'accel': torch.as_tensor(np.asarray(row['accel'], dtype=float), dtype=torch.float64, device=DEVICE),
            'gyro': torch.as_tensor(np.asarray(row['gyro'], dtype=float), dtype=torch.float64, device=DEVICE),
        }
        if len(context['sat_ids']) != int(context['residual'].numel()):
            raise RuntimeError('stored Data01 context satellite/residual mismatch')
        return context

    def _detach_rollout_state(nav_state, context, feature_accel, feature_gyro, recurrent_state):
        """TBPTT boundary: keep numerical beliefs, sever only autograd history."""
        detached_context = {
            key: value.detach() if isinstance(value, torch.Tensor) else value
            for key, value in context.items()
        }
        detached_recurrent = None if recurrent_state is None else tuple(value.detach() for value in recurrent_state)
        return {
            ...
            'recurrent_state': detached_recurrent,
        }

    def _prepare_measurements(measurements):
        measurements = tuple(retain_clock_observable_measurements(measurements))
        if not measurements:
        ...
        return (
            ...
            torch.as_tensor(projector, dtype=torch.float64, device=DEVICE),
        )
    ...
    def _rollout(start_sample: int, stop_sample: int, *, phase: str | None=None, initial_state=None, loss_start_sample: int | None=None):
        ...
        if initial_state is None:
            ...
            previous_context = _stored_context(start_sample)
        ...
        with grad_context:
            for sample_index in range(start_sample, stop_sample):
                ...
                prepared = _prepare_measurements(measurements)
        ...
        if training:
            ...
            mean_loss.backward()
        carried = _detach_rollout_state(nav_state, previous_context, feature_accel, feature_gyro, recurrent_state) if training else None
        return {
    ...
    def _clip_gradient(parameters) -> tuple[float, float]:
        ...
        total_sq_after = sum(float(torch.sum(p.grad.detach().double() ** 2).cpu()) for p in active)
        after = math.sqrt(total_sq_after)
    ...
    def _train_phase(phase: str, optimizer: torch.optim.Optimizer, epoch: int):
        """One chronological pass; TBPTT detaches only at optimizer-window boundaries."""
        active_parameters = _set_training_phase(phase)
        carried = None
        total_state_sum = 0.0
        ...
        for window_start in range(0, training_sample_count, OPTIMIZER_WINDOW_SIZE):
            ...
            before, after = _clip_gradient(active_parameters)
    ...
    test_lever_arm_b_m = c_vehicle_to_body_zxy(*test_rover.mounting_xyz_deg) @ np.asarray(test_rover.lever_arm_vehicle_m, dtype=float).reshape(3)
    ...
    test_initial_sigma = np.concatenate([test_imu_noise.isdv_pos_m, test_imu_noise.isdv_vel_mps, test_imu_noise.isdv_att_rad])
    test_P0 = np.diag(test_initial_sigma ** 2)
    test_process_density = np.concatenate([test_imu_noise.pnsd_pos_m_sqrt_s, test_imu_noise.pnsd_vel_mps_sqrt_s, test_imu_noise.pnsd_att_rad_sqrt_s])
