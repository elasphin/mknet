"""Single-file GNSS/LEO/INS + Masked KalmanNet reproduction/simplification.

Scientific policy
-----------------
The implementation follows Yan et al. (2026) and its cited references wherever
the papers publish enough information to reproduce a step. Two pre-existing
project substitutions are preserved: pseudorange-only measurements (no
pseudorange-rate/Doppler) and TLE/SGP4 LEO orbits instead of STK/HPOP.

User-requested 9-state simplification (carried into v32)
---------------------------------
This branch intentionally removes the six IMU-bias states b_a,b_g from the
classical/learned navigation error state and removes the Fig. 8 eta_k head and
feedback completely. The estimated error state is therefore

    x = [delta-p, delta-v, delta-theta]^T,  dim(x)=9,

and the learned Kalman gain has shape [9,Nmax]. IMU mechanization receives the
measured angular rate and specific force directly (apart from the optional generic
scale/misalignment interface, which is zero in the current project). This is NOT
Yan Eq. (7)/Eq. (8) exactly; it is an explicit user-requested ablation designed to
isolate the directly supervised navigation states and remove the unstable/
unidentifiable bias/eta branches observed in v29.

The Eq. (15) fixed feature block consequently changes from 36 to 24 values:
6 IMU differences + 9 state residual + 9 state innovation. In pseudorange-only
mode the padded network vector has length 24 + 2*Nmax. The v29 correction that
removes per-satellite broadcasting of fixed features is retained.

Causality of the observation-residual feature
---------------------------------------------
Yan Eq. (11) explicitly defines Delta-y_{k-1}; Eq. (15)/Fig. 8 notation is less
consistent and can be read as Delta-y_k. The user's requested change to same-epoch
Delta-y_k is intentionally NOT applied because Delta-y_k requires the posterior
xhat_{k,k}, while xhat_{k,k} itself requires the Kalman gain being predicted from
the network input at epoch k. Feeding that residual to the same K_k would create
a circular/non-causal dependency (or require a separate provisional estimator).
The implementation therefore preserves the latest completed posterior residual
Delta-y_{k-1}; after epoch k is completed, Delta-y_k is stored and becomes the
lagged residual input at epoch k+1.

Alternating-optimization correction in v32
------------------------------------------
Yan explicitly states that its network parameters are optimized alternately and cites
Latent-KalmanNet.  Following Latent-KalmanNet's encoder-vs-filter decomposition, this
revision maps psi to the masked CNN feature extractor and theta to the recurrent masked
LSTM + attention + Kalman-gain head.  The previously used theta=gain-head-only split is
removed.  During each coordinate update, the frozen counterpart is also put in eval mode
so dropout cannot change a block that is meant to be fixed.  Latent-KalmanNet's optional
Eq. (9) encoder warm start is not fabricated because Yan publishes no supervised target
for the intermediate CNN feature representation.

Retained pipeline components
----------------------------
- real GNSS pseudoranges, precise GNSS orbit/clock products, two-pass GNSS
  transmit-time correction, and TLE/SGP4 LEO pseudorange simulation;
- 9-state ECEF INS error propagation and tightly-coupled pseudorange measurement
  update with lever arm and clock-nuisance projection;
- Yan Eqs. (10)-(21) feature ordering adapted for the 9-state dimension, with the
  published Delta-y_{k-1} residual retained.  Because same-epoch posterior state
  innovation/residual terms would also be circular inputs to K_k, the implementation
  uses the immediately preceding completed state context for those terms as an explicit
  causal completion; zero-padding/masking and Table III network sizes are retained;
- Yan Eq. (30)/(32) supervised state-estimation objective, with all nine learned
  state rows directly supervised by available postprocessed position/velocity/
  attitude truth;
- KalmanNet-guided differentiable external recurrence through the same ECEF INS
  mechanization used online, within short Data01 trajectories;
- FDE/DIA and integrity bookkeeping retained as in v29;
- v29 mask semantics restored: the CNN keeps the Eq. (15) feature-position axis,
  Eq. (23) mask gates every LSTM position update, and Eqs. (26)-(29) attention
  excludes padded positions in the current fusion epoch. Only the final valid
  LSTM h/c is carried across fusion epochs.

Yan et al. do not publish the exact pooling operator, exact FC tensorization,
feature scaling, full training batching/BPTT details, or several simulation/FDE
parameters. Existing compact completions remain explicitly labelled in code.
"""
...
#
# Keep True to retain Yan's online FDE/DIA path. Use False only as a controlled
# FDE ablation. The overall v30 branch is still a 9-state/no-eta simplification.
FDE_DIA_ON = True

# =============================================================================
# DIVERGENCE-DIAGNOSTIC CONTROLS
# =============================================================================
# USER-REQUESTED SIMPLIFICATION (v30): learned IMU bias states and Fig. 8 eta_k
# are removed.  The navigation filter/KalmanNet estimates only [delta-p,delta-v,
# delta-theta].  This is an explicit deviation from Yan Eq. (7)/Fig. 8 used to
# isolate the 9-state navigation-update path and eliminate the unidentifiable
# bias/eta feedback branches observed in v29.
...
# =============================================================================
Array = np.ndarray
# USER-REQUESTED SIMPLIFICATION (v30): the learned/classical navigation error state
# is restricted to position, velocity, and attitude only.  Yan Eq. (7) is 15-state,
# so this 9-state branch is an explicit project simplification, not attributed to Yan.
INS_STATE_DIM = 9
...
class TransmitTimeConvergenceError(RuntimeError):
    """One-satellite light-time fixed-point iteration did not converge.

    This is a per-measurement numerical/availability failure, not a valid
    pseudorange solution.  Callers may therefore reject that satellite for the
    current epoch, but the detailed residuals are retained for diagnostics so a
    navigation divergence is not silently hidden.
    """
...
def _gnss_transmit_time_two_pass(
    ...
    """Deterministic GNSS emission-time correction without fixed-point iteration.

    GNSS satellite coordinates must be evaluated at signal transmission time; using
    reception-time coordinates would leave O(10^2 m) satellite-motion errors over
    the roughly 70--90 ms MEO signal flight time.  For a geometric light-time map,
    however, the fixed-point contraction factor is only O(v_sat/c) ~ 1e-5.  One
    corrected travel-time update therefore removes essentially all of the residual
    iteration error while avoiding a convergence loop.

    Pass 1: use the receive-time satellite position to estimate tau_0.
    Pass 2: evaluate the satellite at t_rx-tau_0, recompute the Earth-rotation-aware
            geometric range, and evaluate once more at the corrected t_rx-tau_1.

    The returned ``transit_s`` is tau_1 and is also used by ``geometric_range`` for
    the reception-frame Earth-rotation (Sagnac) correction.  This helper is only for
    GNSS.  The LEO simulator keeps Yan et al. Eq. (5)'s published iteration.
    """
...
def build_error_state_dynamics(nav: NavigationState, specific_force_body_mps2: Array) -> Array:
    """9-state ECEF error dynamics for [delta-p, delta-v, delta-theta].

    This is the user-requested reduced-state branch.  The position/velocity/attitude
    blocks retain the same signs and deterministic ECEF dynamics used by v29; the
    accelerometer- and gyro-bias states/couplings from Yan Eq. (7) are omitted.
    """
...
def discretize_process_noise_van_loan(F: Array, Qc: Array, dt_s: float) -> tuple[Array, Array]:
    ...
    Taylor series is much cheaper than a general-purpose (2*n)x(2*n) matrix exponential.
...
def build_measurement_model(
    ...
    """Build the 9-state pseudorange model with clock-nuisance projection.
...
def kalman_measurement_update(P: Array, innovation: Array, H: Array, R: Array):
    """Conventional reduced 9-state TC measurement update.
...
def protection_levels_from_covariance(
    ...
    """HPL/VPL from Ref. [35] Eqs. (57)-(58), with overflow-safe algebra.

    The horizontal major-axis variance is mathematically unchanged.  ``hypot``
    evaluates sqrt(a^2+b^2) without forming overflow-prone intermediate squares.
    A non-finite covariance/position represents an unusable integrity solution, so
    the protection levels are reported as +inf rather than terminating the run.
    """
...
def closed_loop_spectral_radius(gain: Array, H: Array) -> float:
    ...
    used I-KH. That matrix is not a valid reproduction here: this 9-state
...
def reset_error_state_covariance(
    ...
    Position and velocity feedback are additive, so their reset Jacobian is identity.  The attitude block is the exact SO(3) left Jacobian evaluated
...
def _torch_compensate_imu(
    ...
    """9-state/no-bias training branch: no learned bias or eta compensation."""
    return (
...
def _torch_mechanize_ecef(
    ...
    """Differentiable equivalent of mechanize_ecef() for BPTT training."""
    dt = float(dt_s)
    C0 = nav.body_to_ecef_dcm
...
# MASKED CLA NETWORK
# =============================================================================
# Paper-guided architecture (with explicitly documented project simplifications):
...
#   Yan Eqs. (24)-(25): the Eq. (23) output mask gates EVERY LSTM position update.
#   Yan Eqs. (26)-(29): masked attention excludes padded positions in the CURRENT
#                        fusion epoch.
...
#                  5 hidden layers, dropout 0.2.
#
# MASK-FIDELITY CORRECTION (ported from the verified v29 implementation):
#   The network input is one Eq. (15)/(16) vector per fusion epoch,
#
#     X_bar[k] = [ fixed_24,
#                  residual_{k-1,1:Nk},
#                  innovation_{k,1:Nk},
#                  zeros_{2*(Nmax-Nk)} ]
#
#   with total length D = 24 + 2*Nmax. The 24 fixed IMU/state features appear once.
#
#   Crucially, the CNN output keeps the Eq. (15) feature-position axis D:
#
#       [B,D] -> Masked CNN/Pooling -> [B,D,24]
#
#   The LSTM then scans those D positions. At every position t, M_out[k,t]=0
#   retains the preceding hidden/cell states exactly as Yan Eq. (25) prescribes.
#   Attention is also computed over the same CURRENT-EPOCH D positions and masks
#   padded positions before softmax, following Yan Eqs. (26)-(29).
#
#   Only the FINAL valid LSTM h/c is carried from fusion epoch k to k+1. There is
#   no separate temporal-attention history over fusion epochs. This restores the
#   mask semantics of v29 while preserving the user-requested v30 9-state/no-bias/
#   no-eta navigation simplification.
#
#   Fig. 8 shows Pooling -> Flatten but does not publish the exact pooling operator
#   or tensor layout. The retained completion uses same-length masked max-pooling.
#   "Flatten" is interpreted as flattening the CNN map dimension at EACH Eq. (15)
#   position, without collapsing the masked position axis required by Eqs. (23)-(29).
#
...
# to the full [9,Nmax] KG matrix, then the current observation mask zeroes padded
# columns.
...
class MaskedCLAOutput:
    ...
    Shapes
    ------
    kalman_gain : [B, 9, Nmax]
    attention   : [B, D], D = 24 + 2*Nmax
    recurrent_state : (h, c)
        h,c: [layers,B,64]
    """
...
class MaskedConv1d(nn.Module):
    """Masked Conv1D on the single Yan Eq. (15)/(16) vector.

    Input
    -----
    x    : [B,D], D = 24 + 2*Nmax
    mask : [B,D], True for physical values and False for unavailable/padded
           observation entries.

    Output
    ------
    [B,D,24], preserving the masked Eq. (15) feature-position axis for the LSTM.
    """
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        # Fig. 8 "Flatten" completion from v29: flatten the CNN-map dimension
        # at each Eq. (15) position, NOT the masked position axis itself.
        return feature_maps.transpose(1, 2)            # [B,D,24]


class MaskedStackedLSTM(nn.Module):
    """Yan Eqs. (24)-(25) using one packed LSTM sequence per fusion epoch.

    Eq. (25) states that a masked position must *skip* the LSTM state update and
    retain the preceding hidden/cell state.  Therefore the state trajectory is
    unchanged if masked positions are removed, the remaining valid positions are
    processed in their original order, and the latest valid hidden state is copied
    back across skipped positions.  This implementation performs exactly that
    compression/reconstruction, avoiding one separate ``nn.LSTM`` call per Eq. (15)
    position while preserving the paper's masked-state recurrence.

    The final valid h/c is returned for fusion epoch k+1.  PyTorch's standard
    stacked-LSTM dropout is consequently applied within one real sequence call,
    instead of being re-instantiated at every feature-position call boundary.
    """
...
class MaskedCLA(nn.Module):
    """Masked CNN-LSTM-attention KG estimator with Yan Eq. (15)-structured input.

    One padded Eq. (15) vector is processed per fusion epoch. Its feature-position
    mask gates both the LSTM and current-epoch attention. The final valid LSTM h/c
    is carried across fusion epochs within a trajectory.
    """
...
if __name__ == "__main__":
    ...
    # (e.g. 3000 -> 100 samples).  We therefore use short Data01 trajectories only
    # as a training-implementation completion.  Their forward navigation remains
    # the same 9-state ECEF INS loop used online; there is no Phi/H surrogate and no
    ...
    print(
        "FDE/DIA block:",
        "ON (Yan online FDE path)" if FDE_DIA_ON else "OFF (FDE ablation)",
    )
    print("IMU bias states / eta_k:", "REMOVED (user-requested 9-state simplification)")
    print(
    ...
    print(
        ...
        "KalmanNet-guided short Data01 trajectories with actual closed-loop forward "
        "navigation: learned posterior -> ECEF INS mechanization -> next GNSS/LEO "
        "innovation/features; "
    ...
    print(
        ...
        "KalmanNet-guided BPTT through BOTH the Masked-CLA h/c recurrence and the "
        "external 9-state correction -> ECEF INS -> next innovation recurrence; "
        "satellite/orbit/atmosphere preprocessing is the only detached boundary",
    )
    print(
        "Training augmentation:",
        "OFF -- no synthetic noisy-prior perturbation; recursive priors/features are "
        "generated by the actual learned navigation forward loop",
    )
    ...
    print(
        "GNSS transmit-time model: two-pass geometric emission-time correction "
        "+ reception-frame Earth rotation; no GNSS fixed-point iteration"
    )
    ...
    # Yan Eq. (16) uses one fixed Nmax for zero-padding/alignment. The observed
    # Data01 maximum is the minimum admissible value. An optional larger value may
    # be supplied before training so the same fixed network shape can be used when
    # an independent test epoch contains more observations; it does not use labels
    # or fit any statistic from Data02.
    nmax = max(len(row["sat_ids"]) for row in history_rows[1:])
    network_nmax = _env_int("MKNET_NETWORK_NMAX", nmax)
    if network_nmax < nmax:
        raise ValueError(
            "MKNET_NETWORK_NMAX cannot be smaller than the Data01 observed Nmax "
            f"({network_nmax} < {nmax})"
        )
    feature_time = []
    fixed = []
    ...
    # KalmanNet trains recursive estimators on trajectories.  Yan does not publish
    # how its single long Data01 sequence is batched for optimization, so we use the
    # conservative KalmanNet-V2 principle only for segmentation: each short
    # trajectory starts from a stored causal Data01 posterior/context, then every
    # subsequent prior/innovation/feature inside that trajectory is generated by
    # the learned estimator and the actual reduced 9-state INS forward loop.  No
    # network parameter is changed until an entire trajectory has completed.
    training_trajectories = [
        (start, min(start + TRAINING_TRAJECTORY_LENGTH, training_sample_count))
        for start in range(0, training_sample_count, TRAINING_TRAJECTORY_LENGTH)
    ]
    trajectory_count = len(training_trajectories)
    if trajectory_count < 2:
        raise ValueError(
            "short-trajectory training requires at least two Data01 trajectories. "
            f"Only {training_sample_count} samples are available with "
            f"MKNET_TRAINING_TRAJECTORY_LENGTH={TRAINING_TRAJECTORY_LENGTH}. "
            "Increase MKNET_MAX_FUSION_EPOCHS or reduce the trajectory length."
        )
    # Latent-KalmanNet Algorithm 2 randomly partitions the complete training set
    # into mini-batches; it does not require artificially reserving one trajectory
    # outside a batch.  The former ``trajectory_count - 1`` rule forced an
    # unnecessary final singleton/undersized Adam step in short diagnostic runs.
    TRAJECTORY_BATCH_SIZE = min(BATCH_SIZE, trajectory_count)
    if TRAJECTORY_BATCH_SIZE <= 0:
        raise ValueError("trajectory mini-batch size must be positive")

    print(
        f"Data01 training samples={training_sample_count}, "
        f"observed_Nmax={nmax}, network_Nmax={network_nmax}"
    )
    print(
        "Closed-loop training trajectories:",
        f"N={trajectory_count}, configured length={TRAINING_TRAJECTORY_LENGTH}, "
        f"trajectory mini-batch M={TRAJECTORY_BATCH_SIZE} <= N={trajectory_count}; "
        f"expected optimizer steps/phase={math.ceil(trajectory_count / TRAJECTORY_BATCH_SIZE)}; "
        "chronological inside each trajectory, shuffled only at trajectory level; "
        "no optimizer step inside a trajectory",
    )
    if MAX_FUSION_EPOCHS is not None:
        print(
            "DIAGNOSTIC TRAINING NOTICE: fusion cap is active, so only "
            f"{training_sample_count} Data01 fusion samples are available. "
            "This is useful for a short code/stability check but must not be "
            "interpreted as the full-paper training experiment. For the "
            "scientific run set MKNET_MAX_FUSION_EPOCHS=none."
        )
    print(
        "NN input standardization:",
    ...
    #   (3) Latent-KalmanNet: alternating optimization of an observation encoder psi
    #       and a recurrent KalmanNet filter theta using the same state-estimation
    #       objective.  In this Masked-CLA adaptation psi is the masked CNN and theta
    #       is the masked LSTM+attention+gain head; the exact Yan partition is unpublished.
    #
    # The Masked-CLA final valid h/c is carried across fusion epochs within each
    # short trajectory and participates in BPTT. Following the external recurrence
    # of KalmanNet, the learned 9-state posterior and ECEF INS mechanization also
    # remain differentiable inside the short trajectory.  This is explicitly a
    # KalmanNet-guided training implementation completion.  In this v30 ablation,
    # Yan's six bias states and Fig. 8 eta branch are intentionally absent; no Phi/H
    ...
    # Eq. (30) train the gain from the first causal Data01 samples.  This replaces
    # v23's random first-nine-row gain initialization, which could corrupt the
    # physical forward trajectory before the first optimizer step.
    with torch.no_grad():
        model.gain_head.weight.zero_()
        model.gain_head.bias.zero_()
    print("Kalman gain initialization: neutral zero-output completion (paper unpublished)")
    print(
        "Masked CLA input/CNN:",
        "Yan Eq.(15)/(16) structure adapted to 9-state: fixed 24 features appear once; "
        "Conv1D 24 filters, kernel width 3, stride 1; masked same-length pool; "
        "Eq.(15) position axis preserved for Masked LSTM",
    )
    print(
        "Masked LSTM/attention:",
        "64 units x 5 layers, dropout=0.2; valid Eq.(15) positions are compressed "
        "in original order and processed by one packed LSTM sequence call; Eq.(25) "
        "hidden-state carry is reconstructed across masked positions; attention is "
        "over valid current-epoch positions per Eqs.(26)-(29)",
    )
    print(
    ...
    print(
        "Kalman gain output:",
        "9 rows [delta-p, delta-v, delta-theta]; IMU bias rows and eta_k removed",
    )

    # Latent-KalmanNet Algorithm 2 alternates an encoder/representation block psi
    # and a KalmanNet filter block theta.  Mapping theta to the final FC head alone
    # is too narrow: in Latent-KalmanNet the recurrent KalmanNet that computes K_t
    # belongs to theta, whereas the encoder that extracts observation features is psi.
    # For Yan's Masked-CLA architecture, the closest source-supported decomposition is
    # therefore:
    #   psi   = masked CNN feature extractor
    #   theta = masked LSTM + attention + FC Kalman-gain head
    # Yan does not publish this exact partition, so it remains an explicitly labelled
    # Ref.[15]-guided implementation completion rather than a claimed paper detail.
    ...
    print(
        "Latent-KalmanNet alternating partition:",
        "psi=masked CNN feature extractor; theta=masked LSTM+attention+gain head",
    )
    print(
        "Latent-KalmanNet Eq.(9) warm start:",
        "NOT APPLIED -- Yan provides no supervised target for the intermediate CNN "
        "feature representation, so adding an auxiliary encoder head/loss would be "
        "an unsupported training objective",
    )
    ...
    def _set_alternating_phase(phase: str) -> None:
        """Latent-KalmanNet Algorithm-2 parameter freezing for one coordinate step.

        Algorithm 2 alternates by freezing the *parameters* of the counterpart block;
        it does not prescribe switching that block to inference/evaluation mode.
        Keep the complete network in training mode so Yan's published LSTM dropout=0.2
        remains a training regularizer in both coordinate steps and, critically, so
        cuDNN can backpropagate through the frozen LSTM with respect to its inputs
        during the psi/CNN update.  ``requires_grad=False`` is sufficient to keep the
        frozen block's weights unchanged while still allowing dL/d(input) to pass
        through it to the active block.
        """
    ...
    def _stored_training_context(history_row_index: int):
        """Causal stored context used only to initialize one short trajectory."""
    ...
    def _torch_measurement_innovation(
        ...
        """Differentiable counterpart of build_innovation_only().

        Satellite availability, transmit-time coordinates and atmosphere terms are
        treated as exogenous preprocessing constants for the current epoch.  The
        receiver state -> range -> clock-projected innovation map remains in Torch,
        which is the path required by KalmanNet external-recurrence BPTT.
        """
    ...
    def _rollout_actual_closed_loop_trajectory(
        ...
        *,
        ...
        """KalmanNet-style unroll of one Yan GNSS/LEO/INS training trajectory.

        The navigation posterior, estimator-dependent features and Masked-CLA h/c
        state remain connected across all fusion epochs in this
        short trajectory.  Only satellite/orbit/atmosphere preprocessing receives
        a detached snapshot because it is exogenous, discrete/non-differentiable
        preprocessing.  No optimizer step occurs until the trajectory has finished.
        """
        if not (0 <= start_sample < stop_sample <= training_sample_count):
            raise ValueError("invalid short-trajectory bounds")
        ...
        with grad_context:
            for sample_index in range(start_sample, stop_sample):
                ...
                if len(current_sat_ids) > recurrent_capacity:
                    raise RuntimeError(
                        "current Data01 observation count exceeds the fixed Yan Eq.(16) Nmax: "
                        f"count={len(current_sat_ids)}, network_Nmax={recurrent_capacity}. "
                        "Rerun with MKNET_NETWORK_NMAX set to a larger fixed value."
                    )
                (
                    fixed_nn,
                    obs_nn,
        ...
        if total_count <= 0:
            raise RuntimeError(
                "differentiable short training trajectory produced no learned updates: "
                f"trajectory={trajectory_id}, bounds=[{start_sample},{stop_sample})"
            )
    ...
    def _run_training_phase(
        ...
        """One alternating phase using batches of independent short trajectories.

        The same parameter values are used throughout every trajectory.  An Adam
        step occurs only after all trajectories in the current mini-batch have
        completed and their mean trajectory losses have been backpropagated.
        """
        ...
        if total_count <= 0 or not trajectory_eq30_values:
            raise RuntimeError(
                "short-trajectory actual-forward training produced no updates"
            )
    ...
    def _evaluate_actual_closed_loop_training_objective():
        """Frozen-weight continuous Data01 rollout with no short-trajectory resets."""
        for parameter in model.parameters():
            parameter.requires_grad_(False)
        model.eval()
        metrics = _rollout_actual_closed_loop_trajectory(
    ...
    # Read-only baseline before any optimizer step.  Because the gain head is
    # exactly zero-initialized, this is the continuous reduced 9-state INS forward
    # navigation with K_k=0 throughout the Data01 training span.  It separates
    # mechanization drift from instability introduced by learned corrections.
    ...
    print(
        "\n=== YAN EQ.30/EQ.32 + KALMANNET-GUIDED SHORT TRAJECTORIES + "
        "ACTUAL CLOSED-LOOP GNSS/LEO/INS FORWARD + LATENT-KALMANNET "
        "ALTERNATING OPTIMIZATION (DATA01 ONLY) ==="
    )
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        # Latent-KalmanNet Algorithm-2 principle: one random partition/order of
        # the training set is used for the theta pass and then the psi pass.  Here
        # the training items are KalmanNet-guided short Data01 trajectories.  Every
        # trajectory is chronological internally and starts from its own stored
        # causal Data01 context; Data02 is never consulted.
        trajectory_order = np.arange(trajectory_count, dtype=int)
        epoch_rng = np.random.default_rng(SEED + epoch)
        epoch_rng.shuffle(trajectory_order)
        ...
        training_history.append({
            "stage": "yan_actual_closed_loop_short_trajectory_latent_alternating",
            "epoch": epoch,
            "trajectory_length": int(TRAINING_TRAJECTORY_LENGTH),
            "trajectory_count": int(trajectory_count),
            "trajectory_batch_size": int(TRAJECTORY_BATCH_SIZE),
        ...
        print(
            ...
            f"theta_trajEq30={filter_phase['trajectory_mean_eq30']:.6g}, "
            f"theta_posRMSE={filter_phase['position_rmse_m']:.3f} m, "
            f"psi_obj={representation_phase['objective']:.6g}, "
            f"psi_trajEq30={representation_phase['trajectory_mean_eq30']:.6g}, "
            f"psi_posRMSE={representation_phase['position_rmse_m']:.3f} m, "
            f"steps(theta/psi)={filter_phase['optimizer_steps']}/"
            f"{representation_phase['optimizer_steps']}, "
            f"maxK(theta/psi)={filter_phase['max_gain_fro_norm']:.3g}/"
            f"{representation_phase['max_gain_fro_norm']:.3g}, "
    ...
    print(
        "final frozen-weight actual Data01 closed-loop training diagnostic: "
        f"Eq30={final_training_metrics['eq30']:.6g}, "
        f"posRMSE={final_training_metrics['position_rmse_m']:.3f} m, "
        f"updates={final_training_metrics['learned_updates']}, "
        f"skipped={final_training_metrics['skipped_no_measurements']}"
    )
    ...
    print(
        "closed-loop training effect vs zero-K baseline: "
        f"delta_posRMSE={trained_rmse - zero_gain_rmse:+.3f} m, "
        f"ratio={trained_rmse / max(zero_gain_rmse, 1e-12):.3f}x"
    )

    final_teacher_forced_metrics = _evaluate_teacher_forced_eq30()
    print(
        "final clean Yan X/M/Y one-step diagnostic: "
        f"Eq30={final_teacher_forced_metrics['eq30']:.6g}, "
        f"posRMSE={final_teacher_forced_metrics['position_rmse_m']:.3f} m"
    )
    ...
    def _online_feature_arrays(
        ...
        The immediately preceding *completed* fusion context supplies Yan Eq. (11)
        and the causal-completion versions of Eqs. (12)-(13).  The latter are printed
        with index k in the paper, but their current values depend on the posterior
        produced using K_k and therefore cannot be inputs to that same K_k without a
        circular/provisional-estimator construction.  This matches the offline
        training convention and prevents current-posterior leakage.
        """
        n = len(current_sat_ids)
    ...
    torch.save(
        {
            ...
            "sequence_training": (
                "Yan_Fig7_Fig8_order_adapted_to_user_requested_9_state_branch_inside_short_Data01_"
                "trajectories;learned_posterior_to_ECEF_INS_to_next_features;"
                "KalmanNet_V2_guided_external_plus_hc_BPTT;"
            ...
            "external_filter_state": (
                ...
                "final_frozen_Data01_diagnostic_is_continuous_no_reset"
            ),
    ...
    if RUN_RECURSIVE_DATA01_DIAGNOSTIC:
        ...
        print("IMU bias states / eta_k: REMOVED")
    ...
    for event in test_timeline:
        ...
        if n > test_recurrent_capacity:
            raise RuntimeError(
                "Data02 observation count exceeds the fixed Yan Eq.(16) network Nmax: "
                f"count={n}, network_Nmax={test_recurrent_capacity}. "
                "This architecture must keep one fixed Nmax in train and test; "
                "rerun training with MKNET_NETWORK_NMAX set to a larger value."
            )
        (
            fixed_nn,
            obs_nn,
    ...
    summary = {
        ...
        "training": {
            ...
            "sequence_training": {
                ...
                "training_trajectory_count": int(trajectory_count),
                "trajectory_batch_size": int(TRAJECTORY_BATCH_SIZE),
                "trajectory_order": "shuffled_each_epoch_same_partition_for_theta_and_psi",
                "trajectory_reset": "stored_causal_Data01_posterior_only_at_short_trajectory_boundary",
                "optimizer_step_inside_trajectory": False,
                "optimizer_step_after_trajectory_batch": True,
                "external_filter_recurrence": (
                    "learned_posterior_physically_propagated_to_next_prior_and_features_"
                    "inside_each_short_training_trajectory"
                ),
