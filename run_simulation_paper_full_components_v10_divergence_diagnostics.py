...
# =============================================================================
# These switches do not alter the default paper-reproduction path.  Keep
# ETA_FEEDBACK_ON=True for the paper run.  Set it False only in a diagnostic
# rerun to isolate whether the learned eta_k -> Eq. (8) feedback is responsible
# for closed-loop divergence.  The network still predicts eta_k when feedback
# is disabled; only its application to the next IMU interval is bypassed.
ETA_FEEDBACK_ON = True

# Independent classical TC reference on Data02.  This does not feed the learned
# estimator and cannot change its trajectory; it is executed only after the
# learned online pass to determine whether the same preprocessing/INS/FDE path
# remains stable under a conventional Kalman update.
RUN_CLASSICAL_TEST_BASELINE = True
