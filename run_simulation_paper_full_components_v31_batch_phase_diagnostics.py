...
User-requested v30 simplification
---------------------------------
...
v31 training-stability correction
---------------------------------
This revision does not change the 9-state scientific model introduced in v30.
It fixes the diagnostic short-run trajectory batching bug that forced
M=min(BATCH_SIZE,N-1), which could create a highly unbalanced final mini-batch
(e.g. N=10 -> 9+1) and give the final singleton trajectory a full Adam step.
The corrected rule is M=min(BATCH_SIZE,N), so the current N=10 diagnostic uses
one 10-trajectory mini-batch per alternating phase. Frozen continuous Data01
closed-loop diagnostics are also evaluated immediately after theta and psi in
short diagnostic runs, allowing the phase that destabilizes recursion to be
identified without changing weights or consulting Data02.
...
# FDE ablation. The overall v31 branch is still a 9-state/no-eta simplification.
FDE_DIA_ON = True

# =============================================================================
# DIVERGENCE-DIAGNOSTIC CONTROLS
# =============================================================================
# USER-REQUESTED SIMPLIFICATION (v31): learned IMU bias states and Fig. 8 eta_k
...
# =============================================================================
Array = np.ndarray
# USER-REQUESTED SIMPLIFICATION (v31): the learned/classical navigation error state
...
#
# IMPORTANT INPUT-FIDELITY CORRECTION (v29):
#   The paper defines ONE X_k vector, not one 38-D token per satellite. Therefore
...
#
#   with shape [B, 24 + 2*Nmax]. The 24 fixed IMU/state features appear exactly
#   once. They are never broadcast or repeated over satellite slots.
#
#   The paper labels the layer Conv1D but Table III writes a filter size (3 x 3),
#   without publishing a tensor layout that would make a literal 2-D 3x3 kernel
#   uniquely reproducible. The minimal non-invented Conv1D completion used here is
#   therefore 24 filters with kernel width 3, stride 1 and ReLU, applied to the
#   single Eq. (15)/(16) feature axis. This removes the unsupported 38-channel
#   convolution while preserving the explicitly published Conv1D interpretation.
#
...
class MaskedConv1d(nn.Module):
    ...
    -----
    x    : [B,D], D = 24 + 2*Nmax
    mask : [B,D], True for physical values and False only for unavailable/padded
           observation entries.

    The 24 fixed features are present once. No per-satellite replication exists.
    """
    ...
    def __init__(
        ...
        # Standard Conv1D tensorization: [Cout,Cin=1,K=3].  The former
        # [24,38,3] weight tensor was caused by the unsupported satellite-token
        # representation and is intentionally removed.
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        # Eq. (22)-style unbiased masked normalization over the local window.
        ...
        # Eq. (23): preserve the input validity pattern at the CNN output.
        feature_maps = feature_maps * m

        # Fig. 8 Pooling. Exact pooling operator is unpublished; preserve length
        # so masked feature positions remain aligned before Flatten.
...
class MaskedCLA(nn.Module):
    """Masked CNN-LSTM-attention KG estimator with Eq. (15)-structured input.
    ...
    def __init__(
        ...
        # masked. This avoids the former unsupported shared satellite-token head.
    ...
    def forward(
        ...
        # Causal Eq. (11)/(15)-structured pseudorange-only input:
        # [fixed_24, previous completed residual Nmax, current innovation Nmax].
        # Same-epoch posterior residual is not available before K_k is predicted.
        x_bar = torch.cat(
...
if __name__ == "__main__":
    ...
    # Training-batching completion: use every available trajectory in the requested
    # mini-batch capacity. v30 used ``trajectory_count - 1`` solely to force at
    # least two optimizer steps in tiny diagnostic runs; for N=10,B=16 that made
    # batches 9+1 and let the singleton receive a full Adam step. Yan does not
    # publish such a constraint, and Latent-KalmanNet only requires random batches.
    # The corrected diagnostic therefore uses one batch containing all 10
    # trajectories when B>=N. No trajectory is duplicated or dropped.
    ...
    EXPECTED_OPTIMIZER_STEPS_PER_PHASE = int(
        math.ceil(trajectory_count / TRAJECTORY_BATCH_SIZE)
    )
    ...
    print(
        ...
        f"expected optimizer steps/phase={EXPECTED_OPTIMIZER_STEPS_PER_PHASE}; "
    ...
    # KalmanNet-guided training implementation completion.  In this v31 ablation,
    ...
    filter_modules = (model.gain_head,)
    ...
    # Read-only training-stability diagnostic. A full continuous Data01 replay is
    # expensive, so phase-by-phase replay is enabled only for shortened diagnostic
    # experiments with a small requested epoch count. It never calls backward(),
    # never steps an optimizer, and never accesses Data02.
    RUN_PHASE_STABILITY_DIAGNOSTIC = (
        MAX_FUSION_EPOCHS is not None and TRAINING_EPOCHS <= 5
    )
    print(
        "alternating-phase frozen continuous diagnostic:",
        "ON" if RUN_PHASE_STABILITY_DIAGNOSTIC else "OFF",
    )
    ...
    def _set_alternating_phase(phase: str) -> None:
        """Latent-KalmanNet-style alternating minimization."""
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        theta_frozen_continuous = (
            _evaluate_actual_closed_loop_training_objective()
            if RUN_PHASE_STABILITY_DIAGNOSTIC
            else None
        )
        ...
        psi_frozen_continuous = (
            _evaluate_actual_closed_loop_training_objective()
            if RUN_PHASE_STABILITY_DIAGNOSTIC
            else None
        )

        # The phase metrics are reset-at-trajectory training objectives. The
        # optional frozen diagnostics above are continuous no-reset replays and
        # expose closed-loop deterioration that a short trajectory objective can
        # hide. One mandatory frozen replay is still run at the end of training.
        ...
        training_history.append({
            ...
            "theta_frozen_continuous_eq30": (
                None if theta_frozen_continuous is None
                else float(theta_frozen_continuous["eq30"])
            ),
            "theta_frozen_continuous_position_rmse_m": (
                None if theta_frozen_continuous is None
                else float(theta_frozen_continuous["position_rmse_m"])
            ),
            "psi_frozen_continuous_eq30": (
                None if psi_frozen_continuous is None
                else float(psi_frozen_continuous["eq30"])
            ),
            "psi_frozen_continuous_position_rmse_m": (
                None if psi_frozen_continuous is None
                else float(psi_frozen_continuous["position_rmse_m"])
            ),
        ...
        if RUN_PHASE_STABILITY_DIAGNOSTIC:
            print(
                "  frozen continuous no-reset Data01 after theta: "
                f"Eq30={theta_frozen_continuous['eq30']:.6g}, "
                f"posRMSE={theta_frozen_continuous['position_rmse_m']:.3f} m, "
                f"maxK={theta_frozen_continuous['max_gain_fro_norm']:.3g}, "
                f"maxPosCorr={theta_frozen_continuous['max_position_correction_norm_m']:.3f} m"
            )
            print(
                "  frozen continuous no-reset Data01 after psi:   "
                f"Eq30={psi_frozen_continuous['eq30']:.6g}, "
                f"posRMSE={psi_frozen_continuous['position_rmse_m']:.3f} m, "
                f"maxK={psi_frozen_continuous['max_gain_fro_norm']:.3g}, "
                f"maxPosCorr={psi_frozen_continuous['max_position_correction_norm_m']:.3f} m"
            )
    ...
    torch.save(
        {
            ...
            "checkpoint_schema": "v31_9_state_no_bias_no_eta_balanced_batch_phase_diagnostics",
            ...
            "alternating_partition": (
                "psi=conv+lstm+attention;theta=gain_head"
            ),
            ...
            "trajectory_batch_size": int(TRAJECTORY_BATCH_SIZE),
            "trajectory_batch_policy": "min(requested_batch_size,trajectory_count);no_forced_singleton",
            "expected_optimizer_steps_per_phase": int(EXPECTED_OPTIMIZER_STEPS_PER_PHASE),
            "phase_stability_diagnostic_on": bool(RUN_PHASE_STABILITY_DIAGNOSTIC),
    ...
    summary = {
        ...
        "unavoidable_unpublished_completions": [
            ...
            "temporal_LSTM_attention_interpretation_across_fusion_epochs_Fig8_tensorization_unpublished",
        ...
        "training": {
            ...
            "alternating_partition": {
                ...
                "theta_filter_outputs": "gain_head_only",
