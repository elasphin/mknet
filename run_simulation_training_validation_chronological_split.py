...
if __name__ == '__main__':
    ...
    # Chronological holdout: first 80% for training, final 20% for validation.
    # No shuffling is allowed because every network sample belongs to a causal sequence.
    ...
    print(
        ...
        f'Nmax={network_nmax}, epochs={TRAINING_EPOCHS}, '
        f'optimizer=Adam, lr={LEARNING_RATE:g}, weight_decay={WEIGHT_DECAY:g}, '
        f'loss=MSE, random_shuffle=False'
    )

    model = MaskedCLA(nmax=network_nmax, dropout=0.2).to(DEVICE)

    # Yan states alternating optimization. Latent_KalmanNet_TSP trains one
    # block while the other is frozen. For this architecture:
    #   psi   -> masked convolutional representation block
    #   theta -> LSTM + attention + Kalman-gain head
    ...
    # Preserve the already-generated sequential inputs exactly as they are.
    fixed_all_t = torch.as_tensor(fixed_network, dtype=torch.float32, device=DEVICE)
    observations_all_t = torch.as_tensor(observations_network, dtype=torch.float32, device=DEVICE)
    masks_all_t = torch.as_tensor(satellite_masks, dtype=torch.bool, device=DEVICE)
    channel_masks_all_t = torch.as_tensor(channel_masks, dtype=torch.bool, device=DEVICE)
    innovations_all_t = torch.as_tensor(innovation_padded, dtype=torch.float32, device=DEVICE)
    targets_all_t = torch.as_tensor(target_states_9, dtype=torch.float32, device=DEVICE)

    # Views only: no copy, randomization, re-ordering, or feature regeneration.
    fixed_train_t = fixed_all_t[:training_sample_count]
    observations_train_t = observations_all_t[:training_sample_count]
    masks_train_t = masks_all_t[:training_sample_count]
    channel_masks_train_t = channel_masks_all_t[:training_sample_count]
    innovations_train_t = innovations_all_t[:training_sample_count]
    targets_train_t = targets_all_t[:training_sample_count]

    fixed_validation_t = fixed_all_t[validation_start_sample:]
    observations_validation_t = observations_all_t[validation_start_sample:]
    masks_validation_t = masks_all_t[validation_start_sample:]
    channel_masks_validation_t = channel_masks_all_t[validation_start_sample:]
    innovations_validation_t = innovations_all_t[validation_start_sample:]
    targets_validation_t = targets_all_t[validation_start_sample:]

    def _forward_sequential_sequence(
        fixed_seq,
        observations_seq,
        masks_seq,
        channel_masks_seq,
        ...
        # Hidden state is reset at the beginning of each train/CV sequence, as in
        # the reference KalmanNet pipelines where train and CV sequences are separate.
        recurrent_state = None
        corrections = []
        for sample_index in range(fixed_seq.shape[0]):
            output = model(
                fixed_seq[sample_index:sample_index + 1],
                observations_seq[sample_index:sample_index + 1],
                masks_seq[sample_index:sample_index + 1],
                channel_masks_seq[sample_index:sample_index + 1],
                recurrent_state=recurrent_state,
            )
            recurrent_state = output.recurrent_state
            correction = fig8_state_update(
                output,
                innovations_seq[sample_index:sample_index + 1],
            )[0, :SUPERVISED_STATE_DIM]
    ...
    print('\n=== DATA01 TRAINING + CHRONOLOGICAL VALIDATION ===')
    for epoch in range(1, TRAINING_EPOCHS + 1):
        # ---------------------------------------------------------------
        # Phase 1: theta / filtering block
        # ---------------------------------------------------------------
        ...
        theta_output = _forward_sequential_sequence(
            fixed_train_t,
            observations_train_t,
            masks_train_t,
            channel_masks_train_t,
            innovations_train_t,
        )
        theta_loss = loss_fn(theta_output, targets_train_t)
        theta_loss.backward()
        optimizer.step()

        # ---------------------------------------------------------------
        # Phase 2: psi / representation block
        # ---------------------------------------------------------------
        for parameter in model.parameters():
            parameter.requires_grad_(False)
        ...
        # Keep the frozen recurrent/filter block deterministic while gradients
        # still flow through it into the representation block.
        model.eval()
        optimizer.zero_grad(set_to_none=True)
        psi_output = _forward_sequential_sequence(
            fixed_train_t,
            observations_train_t,
            masks_train_t,
            channel_masks_train_t,
        ...
        psi_loss = loss_fn(psi_output, targets_train_t)
        psi_loss.backward()
        optimizer.step()

        # ---------------------------------------------------------------
        # Deterministic train and validation evaluation AFTER both updates.
        # These are the two curves requested for loss-vs-epoch.
        # ---------------------------------------------------------------
        model.eval()
        with torch.no_grad():
            train_eval_output = _forward_sequential_sequence(
                fixed_train_t,
                observations_train_t,
                masks_train_t,
                channel_masks_train_t,
            ...
            validation_output = _forward_sequential_sequence(
                fixed_validation_t,
                observations_validation_t,
                masks_validation_t,
                channel_masks_validation_t,
                innovations_validation_t,
            )

            train_mse = float(loss_fn(train_eval_output, targets_train_t).cpu())
            validation_mse = float(loss_fn(validation_output, targets_validation_t).cpu())

            train_position_rmse_m = float(
                torch.sqrt(torch.mean((train_eval_output[:, :3] - targets_train_t[:, :3]) ** 2)).cpu()
            )
            validation_position_rmse_m = float(
                torch.sqrt(
                    torch.mean(
                        (validation_output[:, :3] - targets_validation_t[:, :3]) ** 2
                    )
                ).cpu()
            )

        # Reference-repository behavior: select checkpoint by minimum CV loss.
        ...
        training_history.append({
            'epoch': int(epoch),
            'theta_optimization_mse': float(theta_loss.detach().cpu()),
            'psi_optimization_mse': float(psi_loss.detach().cpu()),
            'train_mse': train_mse,
            ...
            'alternating_order': 'theta_filter_then_psi_representation',
            'split': 'chronological',
            'validation_available': True,
        })

        if epoch == 1 or epoch % 10 == 0 or epoch == TRAINING_EPOCHS:
            print(
                f'epoch {epoch:03d}/{TRAINING_EPOCHS}: '
                f'train_MSE={train_mse:.6g}, '
                f'validation_MSE={validation_mse:.6g}, '
                f'train_pos_RMSE={train_position_rmse_m:.3f} m, '
                f'validation_pos_RMSE={validation_position_rmse_m:.3f} m, '
                f'best_epoch={best_epoch}'
            )

    # Restore the checkpoint selected exclusively by the chronological CV tail.
    ...
    with torch.inference_mode():
        final_training_output = _forward_sequential_sequence(
            fixed_train_t,
            observations_train_t,
            masks_train_t,
            channel_masks_train_t,
        ...
        final_validation_output = _forward_sequential_sequence(
            fixed_validation_t,
            observations_validation_t,
            masks_validation_t,
            channel_masks_validation_t,
        ...
        final_training_mse = float(loss_fn(final_training_output, targets_train_t).cpu())
        final_validation_mse = float(
            loss_fn(final_validation_output, targets_validation_t).cpu()
        )
        final_training_position_rmse_m = float(
            torch.sqrt(
                torch.mean(
                    (final_training_output[:, :3] - targets_train_t[:, :3]) ** 2
                )
            ).cpu()
        )
        final_validation_position_rmse_m = float(
            torch.sqrt(
                torch.mean(
                    (final_validation_output[:, :3] - targets_validation_t[:, :3]) ** 2
                )
            ).cpu()
        )
    ...
    selected_training_stage = 'best_chronological_validation_checkpoint'

    training_performance_summary = {
        'training': {
            'samples': int(training_sample_count),
            'range': [0, int(training_sample_count - 1)],
            'sequence_count': 1,
        ...
        'validation': {
            ...
            'range': [int(validation_start_sample), int(data01_total_sample_count - 1)],
            'sequence_count': 1,
            'sequence_order': 'chronological_preserved',
    ...
    print('Data01 training/validation:', json.dumps(training_performance_summary, indent=2))

    # Save numerical loss history.
    history_epoch = np.asarray([row['epoch'] for row in training_history], dtype=int)
    history_train_mse = np.asarray([row['train_mse'] for row in training_history], dtype=float)
    ...
    # Loss-vs-epoch figure requested for presentation.
    fig = plt.figure(figsize=(8, 5))
    plt.plot(history_epoch, history_train_mse, label='Train MSE')
    ...
    plt.legend()
    plt.tight_layout()
    fig.savefig(OUTPUT_DIR / 'loss_epoch_train_validation.png', dpi=180)
    plt.close(fig)

    # Log-scale version is useful when loss spans several orders of magnitude.
    positive_train = np.all(history_train_mse > 0.0)
    positive_validation = np.all(history_validation_mse > 0.0)
    if positive_train and positive_validation:
        fig = plt.figure(figsize=(8, 5))
        plt.semilogy(history_epoch, history_train_mse, label='Train MSE')
        plt.semilogy(history_epoch, history_validation_mse, label='Validation MSE')
        plt.xlabel('Epoch')
        ...
        plt.legend()
        plt.tight_layout()
        fig.savefig(OUTPUT_DIR / 'loss_epoch_train_validation_log.png', dpi=180)
        plt.close(fig)
    ...
    torch.save({
        ...
        'sequential_input_generation_changed': False,
        'training_sequence_length': int(training_sample_count),
        'training_sequence_count': 1,
        'training_sequence_order': 'chronological_preserved',
        ...
        'weight_decay': float(WEIGHT_DECAY),
        'alternating_order': 'theta_filter_then_psi_representation',
        'theta_filter': 'masked_lstm+masked_attention+gain_head',
        'psi_representation': 'masked_conv',
        'validation_available': True,
        ...
        'removed_short_trajectory_chunking': True,
        'removed_truth_restarts': True,
        'removed_tbptt_schedule': True,
        ...
        'removed_closed_loop_feature_regeneration_during_training': True,
    ...
    summary = {
        ...
        'training_validation_audit': {
            ...
            'alternating_optimization': {
                ...
                'theta_filter': 'masked_lstm+masked_attention+gain_head',
                'psi_representation': 'masked_conv',
                'order': 'theta_then_psi_per_epoch',
            },
            'validation': {
                'available': True,
            ...
            'removed_assumptions': [
                'short_trajectory_length_50',
                'random_short_trajectory_batch_size_8',
                'periodic_ground_truth_restart',
                'optimizer_window_size_4',
                'TBPTT_recurrent_detach_step_2',
                'navigation_detach_training_schedule',
                'gradient_clipping_norm_1',
                'extra_KalmanNet_V1_finetune_stage',
                'custom_gain_head_scale_0.001',
                'project_gamma_1e-6',
                'closed_loop_recomputation_of_network_inputs_during_training',
                'validation_checkpoint_selection_without_independent_CV_data',
            ],
            'retained_because_sourced': {
                'MSE': 'Yan_Eq30_and_both_repositories',
                'Adam': 'Yan_TableIII_and_both_repositories',
                'learning_rate_0.01': 'Yan_Fig15_text',
                'training_epochs_300': 'Latent_KalmanNet_TSP_config_file',
                'weight_decay_0.01': 'Latent_KalmanNet_TSP_config_file',
                'alternating_block_training': 'Yan_Eq32_text_plus_Latent_KalmanNet_freeze_train_pattern',
                'full_sequence_recurrence': 'both_repositories_process_complete_sequences_in_temporal_order',
                'CV_best_checkpoint_rule_if_CV_exists': 'both_repositories_save_lowest_CV_MSE_model',
            },
            'final_training_mse': float(final_training_metrics['mse']),
            'final_training_position_component_rmse_m': float(final_training_metrics['position_component_rmse_m']),
            'final_validation_mse': float(final_validation_metrics['mse']),
