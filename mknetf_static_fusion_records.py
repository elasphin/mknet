...
TRAINING_STRATEGY = 'joint_end_to_end_single_optimizer'
...
def _load_test_only_model(
    ...
    *,
    ...
    model = MaskedCLA(nmax=checkpoint_nmax, dropout=0.2).to(device)
...
def state_update(network_output: CLAOutput, innovation: torch.Tensor) -> torch.Tensor:
    # Keep the CLA itself in float32, but perform K_k @ innovation in the
    # innovation/navigation dtype (float64 in this project).  This avoids an
    # avoidable float32 overflow at the learned correction boundary.
...
if __name__ == '__main__':
    ...
    # Cold-start validation state.  Unlike the former continuation evaluation,
    # this starts at the train/validation boundary with its own one-time
    # classical context and an empty recurrent state, matching the independent
    # Data02 lifecycle without constructing a full classical history.
    validation_boundary_record = fusion_records[validation_start_sample]
    validation_boundary_index = int(validation_boundary_record['fusion_index'])
    validation_seed_nav = NavState(
        fusion_imu_truth_position[validation_boundary_index].copy(),
        fusion_imu_truth_velocity[validation_boundary_index].copy(),
        fusion_truth_body_to_ecef[validation_boundary_index].copy(),
        np.zeros(3, dtype=float),
        np.zeros(3, dtype=float),
    )
    validation_seed_measurements = prepare_fusion_measurements(
        gnss_preprocessor,
        validation_boundary_record['gnss_epoch'],
        validation_boundary_record['leo_measurements'],
        validation_seed_nav,
        lever_arm_b_m,
        capacity=network_nmax,
    )
    if not validation_seed_measurements:
        raise ValueError('validation boundary has no usable measurements for cold start')
    validation_seed_model = measurement_model(
        validation_seed_nav,
        validation_seed_measurements,
        lever_arm_b_m,
    )
    validation_seed_correction, _, _ = kf_update(
        P0.copy(),
        validation_seed_model.innovation,
        validation_seed_model.H,
        validation_seed_model.R,
    )
    validation_nav = inject_error(validation_seed_nav, validation_seed_correction)
    validation_x_pred = np.zeros(INS_STATE_DIM, dtype=float)
    validation_x_post = validation_seed_correction.copy()
    validation_x_post[9:12] = validation_nav.accel_bias_body_mps2
    validation_x_post[12:15] = validation_nav.gyro_bias_body_radps
    validation_warm_start = {
        'nav': validation_nav,
        'context': {
            'sat_ids': tuple(validation_seed_model.sat_ids),
            'residual': innovation_only(
                validation_nav,
                validation_seed_measurements,
                lever_arm_b_m,
            ),
            'x_post': validation_x_post,
            'state_innovation': validation_x_post - validation_x_pred,
            'state_residual': np.zeros(INS_STATE_DIM, dtype=float),
            'accel': validation_boundary_record['accel'].copy(),
            'gyro': validation_boundary_record['gyro'].copy(),
        },
        'feature_accel': validation_boundary_record['accel'].copy(),
        'feature_gyro': validation_boundary_record['gyro'].copy(),
    }
    ...
    if TEST_ONLY:
        ...
        # User-requested training deviation from Yan et al.: remove alternating
        # optimization and train the complete Masked-CLA network end-to-end.
        # This matches the reference GitHub Stateful KalmanNet training lifecycle
        # while preserving Yan's Adam optimizer, Eq. (30), Eq. (32), architecture,
        # masking, and published initial learning rate.
    ...
    def _train_epoch(optimizer: torch.optim.Optimizer):
        # Reference-GitHub style joint end-to-end optimization: every trainable
        # Masked-CLA parameter participates in the same forward/backward/update.
        ...
        for window_start in range(0, training_sample_count, OPTIMIZER_WINDOW_SIZE):
            ...
            # _rollout() has already backpropagated Yan Eq. (30).
            ...
            if not eq30_diag['all_finite']:
                ...
                raise FloatingPointError(
                    ...
                    f'mode={BIAS_ABLATION_MODE} | phase=train | '
                    f'window={window_start}:{window_stop} | parameters={bad}'
                )

            # Keep Yan Eq. (32) explicit instead of replacing it with AdamW
            # weight_decay, because the paper specifies an L2 regularizer.
            ...
            # Same standard clipping primitive used by the reference GitHub
            # trainer; the returned value is the pre-clipping total norm.
            ...
            data_loss = float(metrics['eq30'])
            loss = data_loss + regularization_value
    ...
    if TEST_ONLY:
        ...
        for epoch in range(1, TRAINING_EPOCHS + 1):
            try:
                train_metrics = _train_epoch(optimizer)
            ...
            training_history.append({
                ...
                'gradient_l2_norm': train_metrics['gradient_l2_norm'],
            ...
            print(
                ...
                f"(data={train_metrics['data_loss']:.6g}, "
                f"reg={train_metrics['regularization_loss']:.3g}, "
                f"lr={train_metrics['learning_rate']:.3g}) | "
                f"grad={train_metrics['gradient_l2_norm']:.3g} | "
    ...
    if TEST_ONLY:
        ...
        checkpoint_payload = {
            ...
            'paper_training_deviation': 'alternating optimization intentionally removed by user request',
