...
TRAINING_STRATEGY = 'paper_aligned_alternating_filter_then_representation_v1'
...
def resolve_alternating_learning_rates(
    paper_initial_learning_rate: float,
    environment=None,
) -> dict[str, float]:
    """Resolve the two Algorithm-2 learning rates with paper defaults."""
    environment = os.environ if environment is None else environment
    rates = {
        'filter': float(environment.get(
            'MKNET_FILTER_LR',
            str(paper_initial_learning_rate),
        )),
...
def run_alternating_cycle(train_block) -> dict[str, dict]:
    """Run one complete Algorithm-2 cycle in its published block order."""
    return {
...
if __name__ == '__main__':
    ...
    def _train_block(phase: str):
        ...
        for window_start in range(0, training_sample_count, OPTIMIZER_WINDOW_SIZE):
            # Clear the complete model, rather than only the active optimizer,
            # so stale gradients from the previous alternating phase cannot be
            # mistaken for current gradients or leak into diagnostics.
    ...
    if TEST_ONLY:
        ...
        for epoch in range(1, TRAINING_EPOCHS + 1):
            try:
                ...
                # theta over every chronological TBPTT window, then optimize
                # psi over the same windows from the same causal warm start.
                cycle_metrics = run_alternating_cycle(_train_block)
