...
if __name__ == '__main__':
    ...
    for event_kind, event_start, event_end, event_index in fusion_timeline(
        ...
        if event_kind == 'imu':
            ...
            # INS-only reference used only to prepare fixed measurement records.
            # It is never corrected by a full classical-KF trajectory.
    ...
    def _rollout(start_sample: int, stop_sample: int, *, phase: str | None=None, initial_state=None, loss_start_sample: int | None=None):
        ...
        if initial_state is None:
            ...
            start_nav = training_warm_start['nav']
            nav_state = TorchNavState(
            ...
            warm_context = training_warm_start['context']
            previous_context = {
                'sat_ids': tuple(warm_context['sat_ids']),
                'residual': torch.as_tensor(
                    warm_context['residual'],
                    dtype=torch.float64,
                    device=DEVICE,
                ),
                ...
                'state_innovation': torch.as_tensor(
                    warm_context['state_innovation'],
                    dtype=torch.float64,
                    device=DEVICE,
                ),
                'state_residual': torch.as_tensor(
                    warm_context['state_residual'],
                    dtype=torch.float64,
                    device=DEVICE,
                ),
            ...
            feature_accel = torch.as_tensor(
                training_warm_start['feature_accel'],
                dtype=torch.float64,
                device=DEVICE,
            )
            feature_gyro = torch.as_tensor(
                training_warm_start['feature_gyro'],
                dtype=torch.float64,
                device=DEVICE,
            )
