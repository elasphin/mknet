...
def resolve_alternating_learning_rates(
    ...
    *,
    ...
    """Resolve independent Algorithm-2 rates without inventing a paper value.

    Yan et al. and Latent-KalmanNet publish two learning-rate symbols but do
    not publish a transferable numerical pair for this implementation.  Fresh
    training therefore requires both values explicitly; the caller can select
    them with a held-out-validation sweep.  The reference value is retained
    only for non-training compatibility paths.
    """
    environment = os.environ if environment is None else environment
    variable_names = {
        'filter': 'MKNET_FILTER_LR',
        'representation': 'MKNET_REPRESENTATION_LR',
    }
    if require_explicit:
        missing = [
            variable_name
            for variable_name in variable_names.values()
            if not str(environment.get(variable_name, '')).strip()
        ]
        if missing:
            raise ValueError(
                'fresh alternating training requires explicit '
                'MKNET_FILTER_LR and MKNET_REPRESENTATION_LR values selected '
                'using held-out validation; missing: ' + ', '.join(missing)
            )
    rates = {
        phase: float(environment.get(variable_name, str(base_learning_rate)))
        for phase, variable_name in variable_names.items()
    }
...
def resolve_batched_training_config(
    ...
    environment=None,
    *,
    require_explicit_learning_rates: bool=True,
) -> dict:
    """Resolve documented mini-batching and explicit per-phase rates.
    ...
    with Adam learning rates in the 1e-3 regime.  These values are reference
    anchors, not a published numerical pair for the two blocks here; fresh
    training therefore requires both rates explicitly and applies no unsupported
    linear-scaling rule.
    """
    environment = os.environ if environment is None else environment
    ...
    learning_rates = resolve_alternating_learning_rates(
        ...
        environment,
        require_explicit=require_explicit_learning_rates,
    )
    return {
    ...
    *,
    ...
    return {
        ...
        'learning_rate_policy': (
            'explicit_per_phase_values_for_held_out_validation_sweep'
        ),
        'learning_rate_environment_variables': {
            'filter': 'MKNET_FILTER_LR',
            'representation': 'MKNET_REPRESENTATION_LR',
        },
...
def alternating_checkpoint_contract(learning_rates: dict[str, float]) -> dict:
    ...
    return {
        ...
        'alternating_forward_mode': 'deterministic_eval_both_phases',
        'alternating_recurrent_dropout': ALTERNATING_RECURRENT_DROPOUT,
        'phase_diagnostics_policy': (
            'fixed_first_training_sequence_before_after_each_phase'
        ),
    }
...
class MaskedLSTM(nn.Module):
    ...
    def __init__(
        ...
        hidden_size: int = 64,
        num_layers: int = 5,
        dropout: float = ALTERNATING_RECURRENT_DROPOUT,
    ) -> None:
        super().__init__()
...
def configure_alternating_phase(
    ...
    active_parameters = parameter_groups[phase]
    inactive_phase = 'representation' if phase == 'filter' else 'filter'
    inactive_parameters = parameter_groups[inactive_phase]

    # Both blocks use the same deterministic forward operator.  ``eval`` does
    # not disable autograd; ``requires_grad`` alone selects the optimized block.
    # Yan does not publish a recurrent-dropout schedule, so no phase receives
    # extra stochastic noise absent from the other phase or from validation.
...
@torch.no_grad()
def summarize_parameter_update(before, parameters) -> dict[str, float | int]:
    """Return the actual net L2 displacement of one optimized parameter block."""
    parameter_list = list(parameters)
    if len(before) != len(parameter_list):
        raise ValueError('parameter count changed while measuring optimizer update')
    update_squared = 0.0
    baseline_squared = 0.0
    parameter_count = 0
    for old, parameter in zip(before, parameter_list):
        current = parameter.detach().to(dtype=torch.float64)
        old64 = old.detach().to(device=current.device, dtype=torch.float64)
        if old64.shape != current.shape:
            raise ValueError('parameter shape changed while measuring optimizer update')
        update_squared += float(torch.sum((current - old64) ** 2).cpu())
        baseline_squared += float(torch.sum(old64 ** 2).cpu())
        parameter_count += int(current.numel())
    update_l2 = math.sqrt(update_squared)
    baseline_l2 = math.sqrt(baseline_squared)
    relative_l2 = update_l2 / max(baseline_l2, torch.finfo(torch.float64).eps)
    return {
        'parameter_update_l2': update_l2,
        'parameter_relative_update_l2': relative_l2,
        'parameter_count': parameter_count,
    }


def summarize_forward_norm_samples(norm_samples) -> dict[str, float | int]:
    """Summarize gain, innovation, and correction norms on one fixed rollout."""
    expected = ('kalman_gain_l2', 'innovation_l2', 'correction_l2')
    if set(norm_samples) != set(expected):
        raise ValueError(f'forward norm samples must contain exactly {expected}')
    raw_values = {name: tuple(norm_samples[name]) for name in expected}
    lengths = {len(items) for items in raw_values.values()}
    if len(lengths) != 1 or not lengths or next(iter(lengths)) <= 0:
        raise ValueError('forward norm samples must have one shared positive length')
    summary = {'sample_count': next(iter(lengths))}
    for name in expected:
        if all(isinstance(value, torch.Tensor) for value in raw_values[name]):
            values = torch.stack([
                value.detach().to(dtype=torch.float64).reshape(())
                for value in raw_values[name]
            ])
        else:
            values = torch.as_tensor(raw_values[name], dtype=torch.float64)
        if not bool(torch.isfinite(values).all().cpu()) or bool((values < 0.0).any().cpu()):
            raise ValueError('forward norm samples must be finite and non-negative')
        summary[f'{name}_mean'] = float(values.mean().cpu())
        summary[f'{name}_max'] = float(values.max().cpu())
    return summary
...
def run_alternating_cycle(
    ...
    *,
    ...
    """Run one Algorithm-2 cycle and bracket each phase with a fixed probe."""
    results = {}
    for phase in ALTERNATING_PHASE_ORDER:
        before = None if phase_probe is None else phase_probe(phase, 'before')
        if training_batches is None:
            phase_metrics = train_block(phase)
        else:
            phase_metrics = train_block(phase, training_batches)
        after = None if phase_probe is None else phase_probe(phase, 'after')
        if phase_probe is not None:
            phase_metrics = dict(phase_metrics)
            phase_metrics['deterministic_probe_before'] = before
            phase_metrics['deterministic_probe_after'] = after
        results[phase] = phase_metrics
    return results
...
def _load_test_only_model(
    ...
    *,
    ...
    required = {
        ...
        'learning_rates',
        'learning_rate_policy',
        'alternating_forward_mode',
        'alternating_recurrent_dropout',
        'phase_diagnostics_policy',
        'torch_float_dtype',
    }
    ...
    if any(
        ...
        raise ValueError('checkpoint alternating learning rates are incompatible')
    if payload['learning_rate_policy'] != (
        'explicit_per_phase_values_for_held_out_validation_sweep'
    ):
        raise ValueError('checkpoint learning-rate policy is incompatible')
    if payload['alternating_forward_mode'] != 'deterministic_eval_both_phases':
        raise ValueError('checkpoint alternating forward mode is incompatible')
    if float(payload['alternating_recurrent_dropout']) != (
        ALTERNATING_RECURRENT_DROPOUT
    ):
        raise ValueError('checkpoint alternating recurrent dropout is incompatible')
    if payload['phase_diagnostics_policy'] != (
        'fixed_first_training_sequence_before_after_each_phase'
    ):
        raise ValueError('checkpoint alternating phase diagnostics are incompatible')
    ...
    model = MaskedCLA(
        nmax=checkpoint_nmax,
        dropout=ALTERNATING_RECURRENT_DROPOUT,
    ).to(
        device=device,
...
if __name__ == '__main__':
    ...
    # provenance.  Yan and Latent-KalmanNet do not publish a transferable
    # numerical pair for the two alternating blocks, so fresh training requires
    # both environment values and leaves their selection to held-out validation.
    ...
    batched_training_config = resolve_batched_training_config(
        ...
        require_explicit_learning_rates=not TEST_ONLY,
    )
    ...
    print(
        'Sequence mini-batching | phase LR source=explicit validation sweep | '
    ...
    def _rollout(
        ...
        *,
        ...
        collect_forward_norms: bool=False,
    ):
        ...
        with (torch.enable_grad() if training else torch.inference_mode()):
            for sample_index in range(start_sample, stop_sample):
                ...
                if collect_forward_norms:
                    forward_norm_samples['kalman_gain_l2'].append(
                        torch.linalg.vector_norm(
                            output.kalman_gain.detach().to(dtype=torch.float64)
                        )
                    )
                    forward_norm_samples['innovation_l2'].append(
                        torch.linalg.vector_norm(
                            innovation_nn.detach().to(dtype=torch.float64)
                        )
                    )
                    forward_norm_samples['correction_l2'].append(
                        torch.linalg.vector_norm(
                            correction.detach().to(dtype=torch.float64)
                        )
                    )
                _check_forward_finite(
                    sample_index,
        ...
        return {
            ...
            'forward_norms': (
                summarize_forward_norm_samples(forward_norm_samples)
                if collect_forward_norms
                else None
            ),
        }
    ...
    def _train_block(phase: str, training_batches):
        ...
        active_before = snapshot_parameter_values(active_parameters)
        ...
        parameter_update = summarize_parameter_update(
            active_before,
            active_parameters,
        )
        for parameter in model.parameters():
    ...
    # A single declared sequence and its own fixed warm start bracket both
    # alternating phases.  This endpoint diagnostic is deterministic and is
    # deliberately not used for gradient computation or checkpoint selection.
    fixed_phase_probe_sequence = training_sequence_plan[0]

    def _fixed_training_phase_probe(phase: str, stage: str) -> dict:
        for parameter in model.parameters():
            parameter.requires_grad_(False)
        model.eval()
        probe_metrics = _rollout(
            fixed_phase_probe_sequence.start,
            fixed_phase_probe_sequence.stop,
            phase=None,
            initial_state=_warm_start_to_rollout_state(
                training_sequence_warm_starts[fixed_phase_probe_sequence.start]
            ),
            loss_start_sample=fixed_phase_probe_sequence.start,
            collect_forward_norms=True,
        )
        with torch.no_grad():
            objective, regularization = documented_alternating_objective(
                torch.as_tensor(
        ...
        return {
            'phase': phase,
            'stage': stage,
            'sequence_start': int(fixed_phase_probe_sequence.start),
            'sequence_stop': int(fixed_phase_probe_sequence.stop),
            'eq30': float(probe_metrics['eq30']),
            'regularization': float(regularization.detach().cpu()),
            'objective': float(objective.detach().cpu()),
            'position_rmse_m': float(probe_metrics['position_rmse_m']),
            'count': int(probe_metrics['count']),
            **probe_metrics['forward_norms'],
        }
        ...
        for epoch in range(1, TRAINING_EPOCHS + 1):
            try:
                ...
                cycle_metrics = run_alternating_cycle(
                    ...
                    phase_probe=_fixed_training_phase_probe,
                )
            ...
            print(
                ...
                f"update={filter_phase['parameter_update_l2']:.3g}, "
                f"probe={filter_phase['deterministic_probe_before']['objective']:.3g}"
                f"->{filter_phase['deterministic_probe_after']['objective']:.3g}, "
                f"Kmax={filter_phase['deterministic_probe_after']['kalman_gain_l2_max']:.3g}, "
                f"corrmax={filter_phase['deterministic_probe_after']['correction_l2_max']:.3g}) | "
                ...
                f"update={representation_phase['parameter_update_l2']:.3g}, "
                f"probe={representation_phase['deterministic_probe_before']['objective']:.3g}"
                f"->{representation_phase['deterministic_probe_after']['objective']:.3g}, "
                f"Kmax={representation_phase['deterministic_probe_after']['kalman_gain_l2_max']:.3g}, "
                f"corrmax={representation_phase['deterministic_probe_after']['correction_l2_max']:.3g}) | "
