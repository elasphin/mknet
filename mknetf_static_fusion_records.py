...
if __name__ == '__main__':
    ...
    def _train_block(phase: str):
        ...
        # Reference lifecycle: initialize once per independent training
        # subsequence, then carry/detach state only across optimizer windows
        # inside that subsequence.  Validation/test remain full-trajectory.
        # Sources: KalmanNet_TSP Pipeline_EKF.NNTrain and
        # KalmanNet4SensorFusion LitFusionKalmanNet.training_step.
        # https://github.com/KalmanNet/KalmanNet_TSP/blob/main/Pipelines/Pipeline_EKF.py
        # https://github.com/SongJgit/KalmanNet4SensorFusion/blob/main/Net/trainer/fusion_trainer.py
        for training_sequence in training_sequence_plan:
            carried = _warm_start_to_rollout_state(
                training_sequence_warm_starts[training_sequence.start]
            )
            for window_start, window_stop in training_sequence.windows:
                ...
                if not eq30_diag['all_finite']:
                    ...
                    raise FloatingPointError(
                        ...
                        f'sequence={training_sequence.start}:{training_sequence.stop} | '
    ...
    if TEST_ONLY:
        ...
        for epoch in range(1, TRAINING_EPOCHS + 1):
            try:
                ...
                # theta over every independent training sequence, then optimize
                # psi over the same sequence plan and boundary warm starts.
