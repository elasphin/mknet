...
# Direct run configuration.  Edit these values in this block for each run;
# there is intentionally no environment-variable or profile resolver layer.
TEST_ONLY = False
RUN_PROFILE = 'direct_1001_sweep'
...
FILTER_LEARNING_RATE = 1e-5
REPRESENTATION_LEARNING_RATE = 1e-4
LEARNING_RATES = {
    'filter': FILTER_LEARNING_RATE,
    'representation': REPRESENTATION_LEARNING_RATE,
}
    ...
    *,
    ...
    return {
        ...
        'learning_rate_policy': (
            'direct_per_phase_values_for_held_out_validation_sweep'
        ),
        'learning_rate_configuration': 'direct_code_constants',
    ...
    *,
    ...
    if not path.is_file():
        raise FileNotFoundError(
            ...
            'Edit CHECKPOINT_PATH in the direct run-configuration block.'
        )
...
if __name__ == '__main__':
    ...
    if TRAINING_EPOCHS <= 0 and not TEST_ONLY:
        raise ValueError('TRAINING_EPOCHS must be positive during fresh training')
    if MAX_FUSION_EPOCHS is not None and MAX_FUSION_EPOCHS <= 0:
        raise ValueError('MAX_FUSION_EPOCHS must be positive or None')
    if MAX_TEST_FUSION_EPOCHS is not None and MAX_TEST_FUSION_EPOCHS <= 0:
        raise ValueError('MAX_TEST_FUSION_EPOCHS must be positive or None')
    if REQUESTED_TRAIN_BATCH_SIZE <= 0:
        raise ValueError('REQUESTED_TRAIN_BATCH_SIZE must be positive')
    if any(
        not math.isfinite(value) or value <= 0.0
        for value in LEARNING_RATES.values()
    ):
        raise ValueError('direct alternating learning rates must be positive finite values')
    # Yan et al. Table III / Fig. 15 report 0.01.  Keep that paper value as
    # provenance.  The two direct rates above are validation-sweep candidates,
    # not values claimed to be numerically published by Yan or Latent-KalmanNet.
    ...
    if TEST_ONLY:
        ...
        print(f'New checkpoint will be saved to: {CHECKPOINT_PATH}')
    ...
    print(
        'Sequence mini-batching | phase LR source=direct code constants | '
    ...
    if TEST_ONLY:
        ...
        print(f'Reloaded generated checkpoint for Data02 test: {CHECKPOINT_PATH}')
