...
#     attitude truth are supervised by the partial-state Eq. (30) reproduction
...
class MaskedCLA(nn.Module):
    ...
    The paper-baseline training uses Yan et al. Eq. (30)/(32): supervised
    state-error loss plus L2 regularization. Because the supplied truth does not
    contain IMU-bias labels, only the 9 position/velocity/attitude correction rows
    are supervised and the six unlabelled bias rows are padded with exact zeros.
    Any recursive fine-tuning is kept as an optional, disabled stability completion
    rather than being mixed into the paper baseline. Because the paper does not
    specify how to handle N_test > Nmax_train, the gain head is shared across
    satellite slots rather than flattened over a fixed Nmax.
    """
...
if __name__ == "__main__":
    ...
    # Optional non-paper stability completion retained only for experiments.
    # It is disabled by default and is never part of the paper-baseline result.
    ...
    if split < 1 or validation_count < 1:
        raise ValueError(
            "Not enough feature samples for a non-empty chronological "
    ...
    # =========================================================================
    # 8. MASKED CLA TRAINING: PAPER LOSS + VALIDATION/EARLY-STOP SAFEGUARD
    # =========================================================================
    # Paper-supported training facts:
    #   - supervised/end-to-end KalmanNet training
    #   - Eq. (30): ||x_k - xhat_k||_2^2
    #   - Eq. (32): average prediction error + gamma*||Theta||_2^2
    #   - Adam, initial learning rate 0.01
    #   - Fig. 15 shows an approximately 500-epoch learning-curve horizon
    #
    # Project completions retained deliberately:
    #   - chronological Data01 train/validation split
    #   - early stopping / best-validation checkpoint
    #   - batch size and gamma
    #
    # No target normalization, LR scheduler, baseline gradient clipping, or
    # recursive fine-tuning is mixed into the paper-loss baseline.
    model = MaskedCLA(nmax=nmax, dropout=0.2).to(DEVICE)
    optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)
    training_history = []

    def _regularized_state_loss(correction, target):
        # Partial-state reproduction of Yan et al. Eq. (30):
        #     L_k = ||x_k^(9) - xhat_k^(9)||_2^2
        # Eq. (32) averages L_k and adds gamma*||Theta||_2^2.
        state_error = (
            correction[:, :SUPERVISED_STATE_DIM].double() - target
        )
        per_sample_squared_norm = torch.sum(state_error**2, dim=1)
        eq30_loss = torch.mean(per_sample_squared_norm)
        ...
        if GAMMA_L2:
            ...
            objective = eq30_loss + GAMMA_L2 * l2
        else:
            objective = eq30_loss
        return objective, eq30_loss

    print(
        "\n=== SUPERVISED TRAINING WITH CHRONOLOGICAL VALIDATION "
        "(DATA01) ==="
    )
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        for batch in train_loader:
            ...
            correction = torch.bmm(
                output.kalman_gain,
                innovation_b.unsqueeze(-1),
            ).squeeze(-1)

            loss, eq30_loss = _regularized_state_loss(correction, target_b)
            ...
            train_eq30_sum += float(eq30_loss.detach().cpu()) * batch_size_now
            train_count += batch_size_now

        train_objective = train_objective_sum / train_count
        train_eq30 = train_eq30_sum / train_count
        ...
        with torch.inference_mode():
            for batch in val_loader:
                ...
                val_objective_b, val_eq30_b = _regularized_state_loss(
                    correction, target_b
                )
                if (
                    not torch.isfinite(val_objective_b)
                    or not torch.isfinite(val_eq30_b)
                ):
                    raise FloatingPointError(
                ...
                val_eq30_sum += (
                    float(val_eq30_b.detach().cpu()) * batch_size_now
                )
                val_count += batch_size_now

        val_objective = val_objective_sum / val_count
        val_eq30 = val_eq30_sum / val_count
        current_lr = float(optimizer.param_groups[0]["lr"])

        training_history.append({
            "stage": "supervised_train_validation",
            "epoch": epoch,
            "train_objective_eq32": train_objective,
            "train_eq30_state_loss": train_eq30,
            "val_objective_eq32": val_objective,
            "val_eq30_state_loss": val_eq30,
            "learning_rate": current_lr,
        })

        print(
            f"epoch {epoch:03d}/{TRAINING_EPOCHS}: "
            f"train_Eq30={train_eq30:.6g}, "
            f"val_Eq30={val_eq30:.6g}, "
            f"train_Eq32={train_objective:.6g}, "
            f"val_Eq32={val_objective:.6g}, "
            f"lr={current_lr:.3g}"
        )

        # Select by held-out Eq. (30), the predictive state-estimation loss.
        if val_eq30 < best_val_eq30 - EARLY_STOP_MIN_DELTA:
            best_val_eq30 = val_eq30
        ...
        if epochs_without_improvement >= EARLY_STOP_PATIENCE:
            print(
                "early stopping: validation Eq30 did not improve for "
                f"{EARLY_STOP_PATIENCE} consecutive epochs; "
                f"best epoch={best_epoch}, best val Eq30={best_val_eq30:.6g}"
            )
            break
    ...
    torch.save(
        {
            ...
            "stage": "supervised_train_validation_best_checkpoint",
            "nmax": nmax,
            "fixed_mean": fixed_mean,
            "fixed_std": fixed_std,
            "obs_mean": obs_mean,
            ...
            "loss": "partial_9_state_Yan_Eq30_plus_Eq32_L2",
    ...
    # and the post-training recursive Data01 diagnostic.
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # OPTIONAL NON-PAPER recursive closed-loop stability completion on Data01.
    # The network update from epoch k is actually injected into navigation and
    # therefore affects the features/innovation seen at epoch k+1.  This is the
    # key correction to the former train/test mismatch.
    # -------------------------------------------------------------------------
        ...
        for event in build_exact_fusion_timeline(
            ...
            if previous_online is None:
                ...
                previous_online = {
                    ...
                    "accel": current_accel.copy(),
                    "gyro": current_gyro.copy(),
                }
                continue
            ...
            loss, _ = _regularized_state_loss(correction_t, target_t)
            if not torch.isfinite(loss):
                raise FloatingPointError(
    ...
    selected_training_stage = (
        ...
        if ENABLE_STABILITY_COMPLETION
        else "supervised_train_validation_best_checkpoint"
    )
    ...
    torch.save(
        {
            ...
            "paper_baseline_loss": "partial_9_state_Yan_Eq30_plus_Eq32_L2",
            "max_training_epochs": TRAINING_EPOCHS,
            "epochs_ran": epochs_ran,
            "best_epoch": best_epoch,
            "best_val_eq30": best_val_eq30,
    ...
    summary = {
        ...
        "masked_cla_protocol": {
            ...
            "training_status": "paper_baseline_supervised_Eq30_Eq32",
            "loss": "partial_9_state_Yan_Eq30_plus_Eq32_L2",
        ...
        "limitations": [
            ...
            "The paper states supervised end-to-end training and Eq. (30)/(32), but does not publish the alternating-optimization schedule; the paper-baseline path therefore uses direct supervised Adam optimization without inventing an alternating schedule.",
