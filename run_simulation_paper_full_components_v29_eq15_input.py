...
- Raw Data01 observations, truth, timestamps, masks, and simulated LEO pseudoranges
  ...
  network sees the same class of recursively generated inputs that it sees online.
- Between usable fusion epochs, the training forward pass uses the same Eq. (8)
  IMU compensation and the same ECEF strapdown mechanization used by the online
...
- Yan et al. publish an epoch-wise supervised state loss and recurrent LSTM state,
  but do not publish the implementation of cross-fusion differentiation through
  the complete INS mechanization. KalmanNet (Revach et al., 2022), which Yan builds
  upon conceptually, explicitly trains a recursive estimator by unfolding both its
  external filter recurrence and internal RNN with BPTT. This revision therefore
  uses a training-only differentiable Torch port of the SAME Eq. (8)+ECEF INS
  forward equations inside each short Data01 trajectory. No Phi/H surrogate is
  introduced. Satellite availability/transmit-time/atmosphere preprocessing remains
  a detached exogenous boundary, but receiver-state range prediction, innovation,
  correction, eta feedback and the next navigation prior remain in the autograd graph.
- To avoid changing network parameters in the middle of one physical navigation
  ...
  trajectory starts from a stored causal Data01 posterior/context, then uses the
  actual Eq.(8)+INS forward loop recursively. Network weights stay fixed throughout
  that trajectory and Adam steps only after a mini-batch of completed trajectories.
  The default length 100 follows KalmanNet's published example, not a Yan-specific
  hyperparameter. No truncated Phi/H BPTT surrogate is reintroduced.
...
- Latent-KalmanNet's alternating-minimization principle is retained: the filter
  ...
  using the same Eq. (30)/(32) state-estimation objective.
- Yan et al. do not publish a direct eta_k target or full 15-state postprocessed
  bias truth. No fabricated direct auxiliary target is introduced. Under the
  KalmanNet-guided BPTT completion, however, eta_k and the six bias-gain rows can
  receive indirect supervision from later Yan Eq. (30) state losses through the
  published Fig. 8/Eq. (8) closed-loop navigation path.
...
- GNSS satellite coordinates are still evaluated at signal transmit time, but the
  iterative fixed-point solver is removed from the GNSS path. A deterministic
  two-pass geometric light-time correction is used instead: receive-time satellite
  position -> first travel-time estimate -> transmit-time satellite position -> one
  corrected travel-time update -> final transmit-time satellite position. For MEO
  GNSS this retains the physically necessary transmit-time and Earth-rotation
  corrections while eliminating an unnecessary convergence loop. Yan Eq. (5)'s
...
class TorchNavigationState:
    ...
    accelerometer_bias_body_mps2: torch.Tensor
    gyroscope_bias_body_radps: torch.Tensor
    ...
    def from_numpy(
        ...
        *,
        ...
        return cls(
            ...
            torch.as_tensor(
                nav.accelerometer_bias_body_mps2, dtype=dtype, device=device
            ).clone(),
            torch.as_tensor(
                nav.gyroscope_bias_body_radps, dtype=dtype, device=device
            ).clone(),
        )

    def detached_numpy(self) -> NavigationState:
        # This conversion is used only by non-differentiable satellite availability,
        # transmit-time and atmosphere preprocessing.  The actual state recurrence
        # used to form innovations/losses remains in Torch.
        return NavigationState(
            ...
            self.accelerometer_bias_body_mps2.detach().cpu().numpy().copy(),
            self.gyroscope_bias_body_radps.detach().cpu().numpy().copy(),
        )
...
def _torch_compensate_imu(
    ...
    if eta is None:
        epsilon_g = gyroscope_bias_body_radps.new_zeros(3)
        epsilon_a = accelerometer_bias_body_mps2.new_zeros(3)
    else:
        eta = eta.reshape(IMU_ERROR_DIM)
        epsilon_g = eta[ETA_GYRO_SLICE]
        epsilon_a = eta[ETA_ACCEL_SLICE]
    gyro = (
        measured_angular_rate_body_radps.reshape(3)
        - gyroscope_bias_body_radps.reshape(3)
        - epsilon_g
    )
    accel = (
        measured_specific_force_body_mps2.reshape(3)
        - accelerometer_bias_body_mps2.reshape(3)
        - epsilon_a
    )
    return gyro, accel
...
def _torch_inject_error_state(
    ...
    return TorchNavigationState(
        ...
        nav.accelerometer_bias_body_mps2
        + dx[ACCELEROMETER_BIAS_STATE_SLICE],
        nav.gyroscope_bias_body_radps + dx[GYROSCOPE_BIAS_STATE_SLICE],
    )
...
class MaskedConv1d(nn.Module):
    ...
    The 36 fixed features are present once. No per-satellite replication exists.
    """
...
class MaskedCLA(nn.Module):
    ...
    def __init__(
        ...
        with torch.no_grad():
            weight = self.gain_head.weight.view(INS_STATE_DIM, self.nmax, 64)
            bias = self.gain_head.bias.view(INS_STATE_DIM, self.nmax)
            weight[SUPERVISED_STATE_DIM:, :, :].zero_()
            bias[SUPERVISED_STATE_DIM:, :].zero_()

        self.imu_error_head = nn.Linear(64, IMU_ERROR_DIM)
...
if __name__ == "__main__":
    ...
    # the exact Eq.(8)+INS loop used online; there is no Phi/H surrogate and no
    ...
    print(
        ...
        "navigation: learned posterior -> Eq.(8) IMU compensation -> ECEF INS "
    ...
    print(
        ...
        "external correction -> Eq.(8) -> ECEF INS -> next innovation recurrence; "
    ...
    # the same Eq. (8)+ECEF INS mechanization used online, rather than by a stored
    # linearized Phi/H surrogate.
    ...
    # the learned estimator and the actual Eq.(8)+INS forward loop.  No network
    # parameter is changed until an entire trajectory has completed.
    ...
    TRAJECTORY_BATCH_SIZE = min(BATCH_SIZE, trajectory_count - 1)
    if TRAJECTORY_BATCH_SIZE <= 0:
        raise ValueError("trajectory mini-batch size must be positive and smaller than N")
    ...
    # of KalmanNet, the learned posterior, eta feedback, Eq. (8) compensation and
    # ECEF INS mechanization also remain differentiable inside the short trajectory.
    # This is explicitly a KalmanNet-guided training implementation completion; the
    # Yan equations/architecture are unchanged and no Phi/H surrogate is introduced.
    model = MaskedCLA(
        nmax=network_nmax,
        dropout=0.2,
    ...
    def _navigation_state_finite(nav_state: NavigationState) -> bool:
        return bool(
            ...
            and np.all(np.isfinite(nav_state.accelerometer_bias_body_mps2))
            and np.all(np.isfinite(nav_state.gyroscope_bias_body_radps))
        )
    ...
    # Eqs. (10)-(15), Eq. (8), INS mechanization, Fig. 8 state update and Eq. (30)
    # loss, but implement their estimator-dependent recurrence in Torch so future
    # losses can assign credit to earlier KG/eta corrections.
    ...
    def _torch_navigation_state_finite(nav_state: TorchNavigationState) -> bool:
        tensors = (
            ...
            nav_state.accelerometer_bias_body_mps2,
            nav_state.gyroscope_bias_body_radps,
        )
        return all(
    ...
    def _propagate_torch_interval(
        ...
        *,
        ...
        """Propagate one fusion interval while keeping the external BPTT path."""
        if not segments:
            feature_gyro = (
                training_imr_gyro_t[0] - nav_state.gyroscope_bias_body_radps
            )
            feature_accel = (
                training_imr_accel_t[0] - nav_state.accelerometer_bias_body_mps2
            )
            return nav_state, feature_gyro, feature_accel

        segment_tuple = tuple((int(i), float(dt)) for i, dt in segments)

        def interval_core(position, velocity, dcm, accel_bias, gyro_bias, eta_k):
            state = TorchNavigationState(
                position, velocity, dcm, accel_bias, gyro_bias
            )
            feature_gyro = training_imr_gyro_t[segment_tuple[0][0]] - gyro_bias
            feature_accel = training_imr_accel_t[segment_tuple[0][0]] - accel_bias
            for imu_index, dt in segment_tuple:
                ...
                propagation_gyro, propagation_accel = _torch_compensate_imu(
                    measured_gyro,
                    measured_accel,
                    state.gyroscope_bias_body_radps,
                    state.accelerometer_bias_body_mps2,
                    eta_k if ETA_FEEDBACK_ON else None,
                )
                # Yan Eq. (14) feature uses bias compensation without eta, matching
                # the online Data02 path in this project.
                feature_gyro, feature_accel = _torch_compensate_imu(
                    measured_gyro,
                    measured_accel,
                    state.gyroscope_bias_body_radps,
                    state.accelerometer_bias_body_mps2,
        ...
        propagated = TorchNavigationState(
            position,
            velocity,
            dcm,
            nav_state.accelerometer_bias_body_mps2,
    ...
    def _rollout_actual_closed_loop_trajectory(
        ...
        *,
        ...
        The navigation posterior, eta feedback, estimator-dependent features and
        Masked-CLA h/c state remain connected across all fusion epochs in this
        ...
        current_eta = torch.zeros(
            IMU_ERROR_DIM, dtype=torch.float64, device=DEVICE
        )
        ...
        max_bias_gain_rows_fro_norm = 0.0
        ...
        with grad_context:
            for sample_index in range(start_sample, stop_sample):
                ...
                max_bias_gain_rows_fro_norm = max(
                    max_bias_gain_rows_fro_norm,
                    float(
                        torch.linalg.matrix_norm(
                            valid_gain[SUPERVISED_STATE_DIM:, :]
                        ).detach().cpu()
                    ),
                )
                ...
                # No detach here. This is the key correction from v27 and mirrors
                # KalmanNet_nn.py: posterior(t) is the state from which prior(t+1)
                # is generated. Bias-gain rows and eta receive only indirect future-
                # state supervision; no unpublished direct target is fabricated.
        ...
        return {
            ...
            "max_bias_gain_rows_fro_norm": float(max_bias_gain_rows_fro_norm),
            ...
            "max_eta_norm": float(max_eta_norm),
        }
    ...
    torch.save(
        {
            ...
            "bias_gain_training": (
                "no_direct_bias_truth_labels; zero-initialized bias rows receive "
                "indirect future-state supervision only through KalmanNet-guided "
                "Eq8+INS external-recurrence BPTT"
            ),
            "fixed_mean": fixed_mean,
            "fixed_std": fixed_std,
            "obs_mean": obs_mean,
            ...
            "eta_training": (
                "no_direct_eta_label_published; exact Eq8 interface retained; "
                "eta receives indirect future-state supervision through the "
                "KalmanNet-guided differentiable external recurrence"
            ),
            ...
            "sequence_training": (
                ...
                "to_next_features;KalmanNet_V2_guided_external_plus_hc_BPTT;"
            ...
            "cross_fusion_navigation_gradient": (
                "KalmanNet_guided_BPTT_through_correction_Eq8_ECEF_INS_and_innovation"
            ),
    ...
    for event in test_timeline:
        ...
        # Preserve the complete Eq. (7) state-update interface, including b_a and
        # b_g. The public dataset has no direct bias truth, so no direct auxiliary
        # bias loss is fabricated. During training these rows are supervised only
        # indirectly by later Eq. (30) losses through the Eq. (8)+INS recurrence.
        if (
            not np.all(np.isfinite(active_gain))
    ...
    summary = {
        ...
        "paper_components": {
            ...
            "Fig8_eta": {
                ...
                "training": "no_direct_eta_label_published; exact_Eq8_forward_interface_retained; indirect_future_state_supervision_via_KalmanNet_guided_external_BPTT",
            },
            "FDE_Eq33_to_Eq34": {
        ...
        "training": {
            ...
            "bias_gain_training": (
                "no_direct_bias_labels; no fabricated bias loss; indirect future-state "
                "supervision only through KalmanNet-guided Eq8+INS BPTT"
            ),
            "eta_training": (
                "no_direct_eta_loss_published; Eq8 forward interface retained; "
                "indirect future-state supervision through external-recurrence BPTT"
            ),
