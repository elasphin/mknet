...
class MaskedCLA(nn.Module):
    ...
    def __init__(self, nmax: int, dropout: float=0.2, gain_init_scale: float=1e-3) -> None:
        super().__init__()
        if nmax <= 0:
            raise ValueError('nmax must be positive')
        if not 0.0 <= dropout < 1.0:
            raise ValueError('dropout must be in [0,1)')
        if not math.isfinite(gain_init_scale) or gain_init_scale <= 0.0:
            raise ValueError('gain_init_scale must be finite and positive')
        self.nmax = int(nmax)
        ...
        # SensorFusion-inspired initialization adapted to this model's direct
        # raw-innovation update.  SensorFusion applies Kaiming inside a deeper
        # ReLU head; applying its full scale to this single linear K head made
        # the initial gain norm about 1.9 and destabilized long closed-loop
        # rollouts.  Retain Kaiming directions but start close to the finite
        # zero-gain baseline.  This changes neither Masked-CLA nor Yan Eq. (30).
...
if __name__ == '__main__':
    ...
    # KalmanNet4SensorFusion stability schedule.  Navigation/filter state is
    # stop-gradient after every learned update; only the neural recurrent
    # state is retained for two updates.  Optimizer updates occur after four
    # learned updates while the numerical forward state continues.
    RECURRENT_TBPTT_DETACH_STEP = _env_nonnegative_int('MKNET_RECURRENT_TBPTT_DETACH_STEP', 2)
    OPTIMIZER_WINDOW_SIZE = _env_int('MKNET_OPTIMIZER_WINDOW_SIZE', 4)
    if RECURRENT_TBPTT_DETACH_STEP <= 0:
        raise ValueError('MKNET_RECURRENT_TBPTT_DETACH_STEP must be positive')
    ...
    # Audit the project-specific short-trajectory ground-truth restarts.
    # Latent-KalmanNet Algorithm 2 does not prescribe periodic truth resets;
    # this project uses them to make short BPTT trajectories tractable.  The
    # diagnostic below does NOT claim to measure the learned model's restart
    # effect.  It quantifies (i) how often the training exposure is restarted
    # and (ii) the causal classical-posterior navigation error that a truth
    # restart would remove at those boundaries, as a transparent proxy for the
    ...
    _interchunk_context_only_samples = int(sum(
        max(0, int(training_trajectories[i][0] - training_trajectories[i - 1][1]))
        for i in range(1, trajectory_count)
    ))
    _tail_unused_samples = int(max(0, training_sample_count - training_trajectories[-1][1]))
    if _interchunk_context_only_samples + _tail_unused_samples != v2_unused_sample_count:
        raise RuntimeError('short-trajectory coverage accounting is inconsistent')
    ...
    for _sample_index in _periodic_restart_starts:
        ...
        _restart_pos_proxy_m.append(float(np.linalg.norm(
            fusion_imu_truth_position[_fusion_index] - _causal_nav.position_ecef_m
        )))
        _restart_vel_proxy_mps.append(float(np.linalg.norm(
            fusion_imu_truth_velocity[_fusion_index] - _causal_nav.velocity_ecef_mps
        )))
        _restart_att_proxy_deg.append(float(np.degrees(np.linalg.norm(
            attitude_error_state_target(
                _causal_nav.body_to_ecef_dcm,
                fusion_truth_body_to_ecef[_fusion_index],
            )
        ))))
    ...
    short_trajectory_restart_diagnostic = {
        'paper_status': 'project_training_adaptation_not_LatentKalmanNet_Algorithm2_and_not_published_by_Yan',
        'trajectory_length_samples': int(KNET_V2_TRAJECTORY_LENGTH),
        'trajectory_count': int(trajectory_count),
        'periodic_truth_restarts_after_initialization': int(max(0, trajectory_count - 1)),
        'periodic_restart_fraction_of_training_samples': float(max(0, trajectory_count - 1) / max(training_sample_count, 1)),
        'trained_samples_per_phase': int(v2_trained_sample_count),
        'interchunk_boundary_context_samples_excluded_from_loss': int(_interchunk_context_only_samples),
        'tail_samples_not_used_by_short_stage': int(_tail_unused_samples),
        'total_samples_excluded_from_short_stage_loss': int(v2_unused_sample_count),
        'deployment_has_periodic_truth_restart': False,
        'causal_classical_posterior_proxy_position_error_m': _restart_stats(_restart_pos_proxy_m),
        'causal_classical_posterior_proxy_velocity_error_mps': _restart_stats(_restart_vel_proxy_mps),
        'causal_classical_posterior_proxy_attitude_error_deg': _restart_stats(_restart_att_proxy_deg),
        'interpretation': 'proxy_only_not_a_learned_rollout_effect;large_values_indicate_truth_restarts_remove_nontrivial_causal_state_error_at_chunk_boundaries',
    }
    print(
        'short-trajectory truth-restart audit: '
        f"periodic_restarts={short_trajectory_restart_diagnostic['periodic_truth_restarts_after_initialization']}, "
        f"context_only={_interchunk_context_only_samples}, unused_tail={_tail_unused_samples}, "
        f"causal_proxy_pos_median={short_trajectory_restart_diagnostic['causal_classical_posterior_proxy_position_error_m']['median']:.3f}m, "
        f"causal_proxy_pos_max={short_trajectory_restart_diagnostic['causal_classical_posterior_proxy_position_error_m']['max']:.3f}m"
    )
    training_history.append({
        'stage': 'short_trajectory_ground_truth_restart_audit',
        **short_trajectory_restart_diagnostic,
    })

    target_base_t = torch.tensor(target_states_9, dtype=torch.float64, device=DEVICE)
    def _set_alternating_phase(phase: str) -> None:
        """Freeze the inactive parameter block while keeping required autograd paths valid.

        In the representation/CNN phase gradients must propagate through the
        frozen LSTM to reach the CNN. CUDA/cuDNN cannot backpropagate through an
        RNN whose forward pass ran in eval mode, even when the RNN parameters
        themselves do not require gradients. Therefore the LSTM remains in
        training mode in both alternating phases. During the representation
        phase its inter-layer dropout is set to zero so the frozen filter block
        remains deterministic; its parameters stay requires_grad=False.
        """
        ...
        # Default all modules to deterministic/evaluation behaviour first.
        model.eval()

        if phase == 'filter':
            # Theta/filter block is active, including the LSTM with its
            # configured training dropout.
            ...
            # Psi/representation block is active. The LSTM is frozen but must
            # run in training mode so cuDNN builds the backward reserve needed
            # for d(loss)/d(CNN output). Disable only its dropout to preserve a
            # deterministic frozen mapping.
    ...
    def _rollout_actual_closed_loop_trajectory(start_sample: int, stop_sample: int, *, phase: str | None, backward_scale: float | None=None, trajectory_id: int | None=None, optimizer_step_index: int | None=None, initialization_mode: str='stored_causal', recurrent_detach_step: int=0, initial_rollout_state=None, allow_empty_training_window: bool=False):
        ...
        if recurrent_detach_step < 0:
            raise ValueError('recurrent_detach_step must be non-negative')
        if not training and recurrent_detach_step:
            raise ValueError('TBPTT detachment is only valid during training')
        if initial_rollout_state is not None:
            if not training:
                raise ValueError('carried rollout state is only valid during training')
            ...
            for sample_index in range(start_sample, stop_sample):
                ...
                if not bool(torch.isfinite(state_loss).detach().cpu()):
                    ...
                    raise FloatingPointError(
                        f'non-finite Yan Eq.(30) loss inside KalmanNet-style BPTT '
                        ...
                        f'max_position_correction={float(max_position_correction_t.cpu()):.6g}, '
                        f'max_velocity_correction={float(max_velocity_correction_t.cpu()):.6g}, '
                        f'max_attitude_correction={float(max_attitude_correction_t.cpu()):.6g}'
                    )
                ...
                # SensorFusion-style stop-gradient separation:
                #   * physical navigation/filter state and lagged causal
                #     context are detached after every learned update;
                #   * LSTM hidden/cell state keeps a short temporal graph and
                #     is detached only at recurrent_detach_step.
                # Numerical forward recursion is preserved in both cases.
                if training:
                ...
                if (
                    training
                    and recurrent_detach_step > 0
                    and learned_updates_since_detach >= recurrent_detach_step
                    and sample_index < stop_sample - 1
                ):
                    # Truncated neural BPTT: preserve the LSTM's numerical
                    # memory while severing its autograd graph.
                    recurrent_state = _detach_recurrent_state(recurrent_state)
                    learned_updates_since_detach = 0
                    tbptt_detach_count += 1
        if total_count <= 0:
            if not (training and allow_empty_training_window):
        ...
        rollout_state = None
        if training:
            # Every call is one optimizer window.  Detach all carried values
            # before optimizer.step() changes parameter versions, then reuse
            # the same numerical state in the next window.
            rollout_state = {
                'nav_train': _detach_torch_navigation_state(nav_train),
                'previous_context': _detach_training_context(previous_context),
                'last_feature_accel': last_feature_accel.detach(),
    ...
    def _run_training_phase(
        ...
        *,
        ...
        """Train with short optimizer windows and continuous numerical state."""
        ...
        if sequence_count <= 0:
            raise ValueError('windowed training requires at least one sequence')
        ...
        for batch_index, batch_ids in enumerate(epoch_batches):
            ...
            if np.any(batch_ids < 0) or np.any(batch_ids >= sequence_count):
                raise ValueError('training batch contains an out-of-range sequence id')
            ...
            max_sequence_length = max(
                bounds[int(sequence_id)][1] - bounds[int(sequence_id)][0]
                for sequence_id in batch_ids.tolist()
            )
            ...
            for window_index in range(window_count):
                ...
                for sequence_id in active_ids:
                    ...
                    metrics = _rollout_actual_closed_loop_trajectory(
                        ...
                        window_stop,
                        phase=phase,
                        backward_scale=1.0 / max(len(gradient_ids), 1),
                        trajectory_id=sequence_id,
                        optimizer_step_index=optimizer_step_count,
                        initialization_mode='kalmannet_v2_truth',
                        recurrent_detach_step=RECURRENT_TBPTT_DETACH_STEP,
                        initial_rollout_state=carried_states[sequence_id],
                        allow_empty_training_window=True,
                    )
                ...
                if not learned_window_metrics:
                    # Continue the same numerical states across observation
                    # gaps, but do not create a data-free optimizer update.
                    continue

                regularization = _eq32_regularization()
                if not torch.isfinite(regularization):
                    raise FloatingPointError(
                        f'non-finite {phase} Eq.(32) regularization in optimizer window {optimizer_step_count}'
                    )
                if GAMMA_L2:
                    regularization.backward()
                if not _active_gradient_finite(phase):
                    raise FloatingPointError(
                        f'non-finite {phase} gradient before optimizer window {optimizer_step_count}'
                    )
                ...
                if not math.isfinite(window_objective):
                    raise FloatingPointError(
                        f'non-finite {phase} objective before optimizer window {optimizer_step_count}'
                    )
                ...
                if not _active_gradient_finite(phase):
                    raise FloatingPointError(
                        f'non-finite {phase} gradient after clipping before optimizer window {optimizer_step_count}'
                    )
        ...
        if total_count <= 0:
            raise RuntimeError(f'windowed {phase} phase produced no learned updates')
        all_trajectory_eq30 = [
            sequence_state_sums[sequence_id] / sequence_update_counts[sequence_id]
            for sequence_id in range(sequence_count)
            if sequence_update_counts[sequence_id] > 0
        ]
        if len(all_trajectory_eq30) != sequence_count:
            raise RuntimeError(f'windowed {phase} phase did not train every sequence')

        flattened = [v for batch in batch_partitions for v in batch]
        if sorted(flattened) != list(range(sequence_count)):
            raise RuntimeError(
                f'windowed {phase} phase did not cover every sequence exactly once'
            )
        ...
        out = {
            ...
            'available_trajectory_count': int(sequence_count),
            'sequence_batch_size': int(max(len(batch) for batch in batch_partitions)),
            'sampled_trajectory_ids': [int(v) for v in flattened],
            'batch_partitions': batch_partitions,
            'epoch_dataset_coverage_fraction': float(len(set(flattened)) / sequence_count),
            'optimizer_window_size': int(OPTIMIZER_WINDOW_SIZE),
            'navigation_detach_step': 1,
            'recurrent_detach_step': int(RECURRENT_TBPTT_DETACH_STEP),
            'recurrent_detach_count': int(recurrent_detach_count),
    ...
    def _run_v1_training_phase(phase: str, optimizer: torch.optim.Optimizer, epoch: int):
        """Full chronological Data01 pass with the stable window schedule."""
        return _run_training_phase(
            phase,
            optimizer,
            epoch,
            [np.asarray([0], dtype=int)],
            sequence_bounds=[(0, training_sample_count)],
        )
    def _evaluate_actual_closed_loop_training_objective():
        """Frozen-weight recursive rollout on the chronological Data01 training partition."""
        for parameter in model.parameters():
            parameter.requires_grad_(False)
        model.eval()
    ...
    def _evaluate_actual_closed_loop_validation_objective():
        """KalmanNet-style held-out validation: full recursive sequence, no gradients.

        The last chronological fraction of Data01 is never used by optimizer
        updates.  As in KalmanNet4SensorFusion validation, recurrent/filter
        beliefs are initialized at the start of the validation sequence and the
        entire held-out sequence is then rolled out recursively in eval mode.
        The validation metric excludes Eq.(32) regularization and is used only
    ...
    def _safe_recursive_monitor(scope: str, evaluator):
        """Reject a divergent checkpoint without aborting the training run.

        Yan's checkpoint criterion is meaningful only for finite recursive
        validation.  A non-finite rollout therefore receives +inf and cannot
        become the best checkpoint, while later epochs remain evaluable.
        """
        try:
            metrics = evaluator()
    ...
    def _evaluate_true_zero_gain_closed_loop_baseline():
        ...
        the original nonzero Kaiming initialization is restored exactly.
    ...
    pretraining_initialized_metrics = _safe_recursive_monitor(
        'pre-training initialized-model Data01 rollout',
        _evaluate_actual_closed_loop_training_objective,
    )
    print(
        f"pre-training initialized-model Data01 rollout: "
        f"Eq30={pretraining_initialized_metrics['eq30']:.6g}, "
        f"posRMSE={pretraining_initialized_metrics['position_rmse_m']:.3f} m, "
        f"maxK={pretraining_initialized_metrics.get('max_gain_fro_norm', float('nan')):.6g}, "
        f"maxPosCorr={pretraining_initialized_metrics.get('max_position_correction_norm_m', float('nan')):.6g} m, "
        f"diverged={pretraining_initialized_metrics['diverged']}"
    )
    ...
    print('\n=== DATA01 TRAINING ===')
    training_history.append({
        'stage': 'pretraining_scaled_kaiming_data01_diagnostic_only',
        'eligible_for_checkpoint_selection': False,
        'gain_head_initial_scale': float(GAIN_HEAD_INITIAL_SCALE),
        'eq30': float(pretraining_initialized_metrics['eq30']),
        'position_rmse_m': float(pretraining_initialized_metrics['position_rmse_m']),
        'diverged': bool(pretraining_initialized_metrics['diverged']),
        'divergence_reason': pretraining_initialized_metrics['divergence_reason'],
    })
    training_history.append({
    ...
    for epoch in range(1, KNET_V2_ALTERNATING_EPOCHS + 1):
        ...
        train_recursive_monitor = _safe_recursive_monitor(
            f'ALT epoch {epoch} Data01 training monitor',
            _evaluate_actual_closed_loop_training_objective,
        )
        validation_recursive_monitor = _safe_recursive_monitor(
            f'ALT epoch {epoch} Data01 validation monitor',
            _evaluate_actual_closed_loop_validation_objective,
        )
        ...
        if (
            ...
            and math.isfinite(validation_rmse_m)
    ...
    for epoch in range(1, KNET_V1_FINE_TUNE_EPOCHS + 1):
        ...
        current_representation_lr = float(representation_optimizer.param_groups[0]['lr'])
        train_recursive_monitor = _safe_recursive_monitor(
            f'V1 epoch {epoch} Data01 training monitor',
            _evaluate_actual_closed_loop_training_objective,
        )
        validation_recursive_monitor = _safe_recursive_monitor(
            f'V1 epoch {epoch} Data01 validation monitor',
            _evaluate_actual_closed_loop_validation_objective,
        )
        validation_eq30 = float(validation_recursive_monitor['eq30'])
        validation_rmse_m = float(validation_recursive_monitor['position_rmse_m'])
    ...
    print(
        f"restored best TRAINED Data01 validation checkpoint: "
        f"stage={best_selection_stage}, epoch={best_selection_epoch}, "
        f"val_Eq30={best_selection_eq30:.6g}, val_posRMSE={best_selection_rmse_m:.3f} m"
    )
    ...
    training_performance_summary = {
        ...
        'validation_closed_loop': {
            ...
            'validation_minus_train_rmse_m': validation_rmse - trained_rmse,
        ...
        'selected_checkpoint': {
            ...
            'selection_metric': 'Data01_validation_recursive_Eq30',
    ...
    # train and validation curves are directly comparable across V2 and V1 epochs.
    epoch_loss_rows = [
    ...
    if epoch_loss_rows:
        ...
        # Show the transition from the short-trajectory alternating stage to V1.
        if KNET_V1_FINE_TUNE_EPOCHS > 0 and KNET_V2_ALTERNATING_EPOCHS > 0:
            ax.axvline(
                KNET_V2_ALTERNATING_EPOCHS + 0.5,
                linestyle='--',
                linewidth=1.0,
                alpha=0.7,
                label='_nolegend_',
