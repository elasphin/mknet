...
TEST_SP3_FILENAME = 'WUM0MGXFIN_20220680000_01D_05M_ORB.SP3'
TEST_CLK_FILENAME = 'WUM0MGXFIN_20220680000_01D_30S_CLK.CLK'
...
if __name__ == '__main__':
    ...
    def _network_measurements_from_current_state(source_row, nav_state):
        """Prepare observations using the current learned/INS navigation state."""
        fusion_index = int(source_row['fusion_index'])
        epoch_now = gnss_epochs[fusion_index]

        # GNSS satellite geometry, elevation screening and modeled corrections are
        # recalculated from the current estimator state, never from classical history.
        ...
        # The LEO pseudoranges are exogenous simulated measurements generated once
        # from truth. Their measurement values must not change with estimator state.
        return retain_clock_observable_measurements(
            tuple(gnss_now) + tuple(source_row['leo_measurements'])
        )

    def _build_network_feature_arrays(
        previous_context,
        current_sat_ids,
        innovation_now,
        current_accel,
        ...
        """Yan Eq. (10)-(15) features from the learned causal trajectory."""
        current_sat_ids = tuple(current_sat_ids)
        innovation_now = np.asarray(innovation_now, dtype=float).reshape(-1)
        ...
        obs_k = np.stack([residual_k, innovation_now], axis=1)
        channel_k = np.stack([residual_mask, current_mask], axis=1)
        obs_k = np.where(channel_k, obs_k, 0.0)
        return fixed_k, obs_k, current_mask, channel_k, innovation_now

    def _pad_network_epoch(
        fixed_k,
        obs_k,
        current_mask,
        channel_k,
        ...
        if count > network_nmax:
            raise RuntimeError(
                f'Network capacity exceeded in Data01: '
                f'count={count}, Nmax={network_nmax}'
            )
        ...
        return (
            np.asarray(fixed_k, dtype=float),
            obs_padded,
            mask_padded,
            channel_padded,
    ...
    def _truth_navigation_at_source_row(source_row):
        fusion_index = int(source_row['fusion_index'])
    ...
    def _initial_learned_context(source_row):
        """Initial sequence context without a classical Kalman correction."""
        nav_state = _truth_navigation_at_source_row(source_row)
        measurements = _network_measurements_from_current_state(
            source_row, nav_state
        )
        if not measurements:
            return nav_state, None
        ...
        context = {
            ...
            'accel': np.asarray(source_row['accel'], dtype=float).reshape(3).copy(),
            'gyro': np.asarray(source_row['gyro'], dtype=float).reshape(3).copy(),
        }
        return nav_state, context
    ...
    def _run_learned_recursive_sequence(
        ...
        *,
        ...
        nav_state, previous_context = _initial_learned_context(boundary_row)
        ...
        with grad_context:
            for sample_index in range(start_sample, stop_sample):
                ...
                correction = fig8_state_update(output, innovation_t)[0]
                ...
                losses.append(loss_fn(correction[:SUPERVISED_STATE_DIM], target_t))
                position_squared_errors.append(
                    torch.mean((correction[:3] - target_t[:3]) ** 2)
                )
        ...
        sequence_loss = torch.stack(losses).mean()
        position_rmse = torch.sqrt(torch.stack(position_squared_errors).mean())
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        theta_metrics = _run_learned_recursive_sequence(
            0,
            training_sample_count,
            require_grad=True,
        )
        theta_loss = theta_metrics['loss']
        theta_loss.backward()
        optimizer.step()
        ...
        # Keep frozen recurrent/filter modules deterministic, while preserving
        # gradient flow through them to the representation block.
        model.eval()
        optimizer.zero_grad(set_to_none=True)
        psi_metrics = _run_learned_recursive_sequence(
            0,
            training_sample_count,
            require_grad=True,
        )
        psi_loss = psi_metrics['loss']
        psi_loss.backward()
        optimizer.step()

        # Evaluate both chronological segments after both alternating updates.
        model.eval()
        train_eval = _run_learned_recursive_sequence(
            0,
            training_sample_count,
            require_grad=False,
        )
        ...
        train_mse = float(train_eval['loss'].detach().cpu())
        validation_mse = float(validation_eval['loss'].detach().cpu())
        train_position_rmse_m = float(
            train_eval['position_rmse_m'].detach().cpu()
        )
    ...
    final_training_eval = _run_learned_recursive_sequence(
        0,
        training_sample_count,
        require_grad=False,
    ...
    # Loss-vs-epoch outputs requested for presentation.
    ...
    if np.all(history_train_mse > 0.0) and np.all(
        ...
        fig = plt.figure(figsize=(8, 5))
        plt.semilogy(
            history_epoch, history_train_mse, label='Train MSE'
        )
        plt.semilogy(
            history_epoch,
