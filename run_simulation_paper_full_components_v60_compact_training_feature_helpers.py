...
if __name__ == "__main__":
    ...
    for event in training_timeline:
        ...
        history_rows.append({
            ...
            # Hold simulated LEO observations fixed; rebuild predicted ranges from current learned navigation.
    ...
    for epoch in range(1, KNET_V2_WARMUP_EPOCHS + 1):
        ...
        # KalmanNet V2: independent short trajectories are shuffled; temporal
        # chronology is preserved inside every short trajectory.
        ...
        training_history.append({
            "stage": "kalmannet_v2_warmup_yan_masked_cla_latent_alternating",
            "epoch": epoch,
            "global_training_epoch": epochs_ran,
            "sequence_length": int(training_sample_count),
            "kalmannet_v2_short_trajectory_length": int(KNET_V2_TRAJECTORY_LENGTH),
            "kalmannet_v2_initialization": "ground_truth_navigation_state",
            "kalmannet_v2_trained_sample_count": int(v2_trained_sample_count),
            "kalmannet_v2_initialization_sample_count": int(v2_initialization_sample_count),
            "kalmannet_v2_unused_tail_sample_count": int(v2_unused_sample_count),
        ...
        print(
            f"V2 epoch {epoch:03d}/{KNET_V2_WARMUP_EPOCHS}: "
    ...
    for epoch in range(1, KNET_V1_FINE_TUNE_EPOCHS + 1):
        ...
        filter_phase = _run_v1_training_phase(
            "filter", filter_optimizer, epoch
        )
        representation_phase = _run_v1_training_phase(
        ...
        training_history.append({
            "stage": "kalmannet_v1_full_sequence_finetune_yan_masked_cla_latent_alternating",
            "epoch": epoch,
            "global_training_epoch": epochs_ran,
            "sequence_length": int(training_sample_count),
            "initialization": "ground_truth_navigation_state",
            "trajectory_count": 1,
        ...
        print(
            f"V1 epoch {epoch:03d}/{KNET_V1_FINE_TUNE_EPOCHS}: "
            f"theta_Eq30={filter_phase['trajectory_mean_eq30']:.6g}, "
            f"theta_RMSE={filter_phase['position_rmse_m']:.3f}m, "
            f"theta_maxK={filter_phase['max_gain_fro_norm']:.3g}, "
            f"theta_maxInnov={filter_phase['max_innovation_abs_m']:.3g}m, "
    ...
    if RUN_RECURSIVE_DATA01_DIAGNOSTIC:
        ...
        # Fresh simulator + training seed reproduces LEO realization without changing other RNG states.
        ...
        try:
            for event in build_exact_fusion_timeline(
                ...
                # First usable row is classical causal context only; exclude it from learned metrics/recurrent state.
                ...
                # If FDE leaves no usable observation, keep INS propagation and do not feed suspect innovation to Masked-CLA.
                if FDE_DIA_ON and not diag_measurements:
                    ...
                    diag_rows.append({
                        ...
                        # No learned update here; keep the diagnostic row schema unchanged.
        ...
        # Enforce one recursive-Data01 diagnostic schema across all branches.
        diag_fields = (
            "time",
            "prior_error_3d_m",
    ...
    for event in test_timeline:
        ...
        # Data02 first uses one ordinary TC/KF update for lagged context; exclude it from learned metrics/state.
        ...
        # Remove identified faults before Masked-CLA; unresolved/empty cases keep INS-only state and remain in metrics.
        if FDE_DIA_ON and not measurements:
            if previous_online is not None:
                posterior_position = gnss_antenna_position(
                    nav, test_lever_arm_b_m
                )
                # No DIA/learned correction here; P stays paired with the propagated INS state.
                ...
                online_rows.append({
                    ...
                    # No learned update: NaN gain norms, zero corrections, propagated-state diagnostics.
        ...
        # First learned Data02 update uses completed lagged context; neural h/c starts from zero here.

        # Data01 fixes normalization/Nmax; Data02 only masks valid columns and never refits/retrains.
        n = len(measurements)
        ...
        if n > test_recurrent_capacity:
            raise RuntimeError(
                "Data02 measurement selection exceeded the Data01-defined "
        ...
        # FDE uses raw INS innovation; learned update receives only post-exclusion measurements.
    ...
    summary = {
        ...
        "unavoidable_unpublished_completions": [
            ...
            "V2_and_V1_epoch_counts_are_explicit_overridable_completion_not_published_by_KalmanNet_or_Yan",
            "V1_continues_same_network_and_Adam_states_after_V2_no_reinitialization",
            "V1_full_sequence_initial_state_is_ground_truth_navigation_state_not_classical_KF_posterior",
            "V2_chunk_boundary_Yan_lagged_feature_context_is_project_adaptation_no_classical_KF_state_reset",
