...
Dataset protocol:
- Data01 is the complete offline training dataset, matching Yan et al.'s experimental
  design of one training dataset and one distinct testing dataset.
- Data02 is used only for online testing/evaluation and never enters training or
  preprocessing fitted from Data01.
- No unpublished validation split, early stopping, or recursive training-data
  regeneration is used in the paper-faithful baseline.
...
- the supplied IE truth exports do not contain accelerometer/gyro bias truth;
  learned updates therefore correct only the 9 position/velocity/attitude states
  and keep the 6 nominal bias states frozen
- Eqs. (10)-(15) are implemented causally: current innovation/IMU increments are
  ...
  Offline training uses the previous completed reference-pass row for lagged
  residual/state features. Online testing uses an all-zero previous context at the
  first usable fusion epoch and KNet history thereafter.
...
- Yan et al. explicitly construct one fixed offline training dataset X, M, Y in
  Eqs. (18)-(20). This reproduction therefore constructs the Data01 feature history
  once and keeps it fixed during Masked-CLA optimization. The paper does not publish
  the estimator used to generate Eqs. (10)-(14) history quantities; a conventional
  tightly-coupled reference pass is used only for this offline feature construction
  and is not the online estimator.
...
- Fig. 8 explicitly shows a second network output eta_k for inertial-navigation
  measurement-error estimation, and Fig. 2 feeds that quantity back to the IMU
  ``Error compensation`` block. Eq. (8) identifies the corresponding physical terms
  as residual gyro/accelerometer measurement errors epsilon_g and epsilon_a. However,
  Yan et al. publish neither an eta_k supervision target nor a differentiable temporal
  training rule that connects eta_k to a later Eq. (30) loss. The previous v5
  one-step teacher-forced sensitivity loss was therefore removed: it was a reasonable
  completion, but it was not Eq. (30)/(32) and could not be claimed as paper-faithful.
  The eta head is retained structurally, initialized to zero, and frozen in this
  paper-faithful baseline; consequently the online Eq. (8) residual-error feedback is
  zero. This is an explicit unreproduced paper detail, NOT a claim that eta_k is zero
  in Yan et al. Re-enabling a learned eta branch requires additional author-level
  information about its target/timing or an explicitly separate completion experiment.

- Yan et al. state that the network parameters are optimized alternately and cite
  Ref. [15]. Ref. [15] defines psi as the representation/encoder block and theta as
  the recurrent KalmanNet/filter block, and alternates theta first, then psi, using
  the same final state-estimation loss. Mapping that decomposition to Fig. 8 gives the
  closest reproducible split:
      representation block psi = Masked CNN
      filter block theta = Masked LSTM + Masked Attention + KG FC head
  because the LSTM is the recurrent component that computes the learned filtering
  mapping, while the CNN is the feature extractor preceding it. The eta head is not
  placed in either trainable block because its training criterion is unpublished.
  Each outer epoch performs theta then psi, matching Algorithm 2 ordering in Ref. [15].
  Both phases minimize the same Yan Eq. (30) state-estimation objective plus the
  Eq. (32) L2 penalty on the fixed Data01 dataset of Eqs. (18)-(20). The paper does
  not publish recursive regeneration of X,M,Y or a BPTT schedule, so neither is
  introduced. This theta/psi partition remains paper-guided rather than paper-exact,
  because Yan et al. do not explicitly publish their alternating parameter groups.
...
#     but the paper does not publish its supervision/timing. The eta head is retained
#     structurally and frozen at its zero initialization in the paper-faithful baseline
#     rather than training it with an invented auxiliary loss.
...
class MaskedCLA(nn.Module):
    ...
    The training set is constructed once as the fixed offline labeled dataset
    X, M, Y of Yan et al. Eqs. (18)-(20).  The paper does not state that these
    training arrays are regenerated from the network's online trajectory.
    ...
    bias-gain rows are padded with exact zeros. Fig. 8 also shows eta_k, interpreted
    physically from Eq. (8) as six residual IMU measurement-error terms, but Yan et
    al. do not publish a reproducible supervision/timing rule for that branch. The
    eta head is therefore retained but frozen at zero in this paper-faithful baseline
    instead of being trained by an invented future-state auxiliary objective.
    ...
    def __init__(
        ...
        # Fig. 8 explicitly produces eta_k in addition to KG_k, and Eq. (8) gives
        # its physical interpretation as residual gyro/accelerometer errors. However,
        # the paper does not publish an eta_k label or temporal training equation.
        # Keep the branch structurally present, but freeze a zero initialization so
        # the paper-faithful baseline does not inject a learned signal trained by an
        # unpublished auxiliary objective.
...
if __name__ == "__main__":
    ...
    # Yan et al. Eqs. (15)-(21) specify the raw feature vector, zero-padding,
    # and mask, but do not publish feature-wise z-score standardization. Keep the
    # published feature values unchanged; masked padding remains exact zero.
    ...
    np.savez(
        ...
        obs_mean=obs_mean,
        obs_std=obs_std,
        source=np.asarray("identity_no_zscore_Yan_Eqs15_to_21"),
    )
    ...
    #
    # Fig. 8 also depicts eta_k / IMU-update output, but the paper publishes no
    # eta target, timing, or temporal backpropagation rule. The v5 auxiliary
    # future-state loss is therefore removed from the paper-faithful baseline.
    # The eta head remains zero-initialized and frozen so it cannot contaminate
    # the published Eq. (30)/(32) objective.
    #
    # Ref. [15] Algorithm 2 alternates theta first and psi second with the SAME
    # final state-estimation objective. Ref. [15] identifies psi as the encoder
    # and theta as the recurrent KalmanNet/filter block. The closest Fig. 8 mapping
    # used here is therefore:
    #   psi   : Masked CNN
    #   theta : Masked LSTM + Masked Attention + KG FC head
    # Yan et al. do not explicitly publish this partition, so it remains a
    # paper-guided completion rather than a paper-exact architectural claim.
    # A separate Ref. [15] encoder warm-start is not copied because Yan et al. do
    # not provide an independent target for the Masked CNN representation.
    # -------------------------------------------------------------------------
    model = MaskedCLA(nmax=nmax, dropout=0.2).to(DEVICE)

    # Ref. [15] decomposes the trainable system into a representation/encoder
    # block psi and a recurrent KalmanNet/filter block theta. Yan et al. do not
    # publish the exact Masked-CLA partition, so we map the CNN feature extractor
    # to psi and the recurrent LSTM + attention + KG head to theta. The eta head
    # is excluded because its training criterion is not published.
    ...
    frozen_unpublished_parameters = [
        parameter
        for module in frozen_unpublished_modules
        for parameter in module.parameters()
    ]
    ...
    frozen_parameter_ids = {id(parameter) for parameter in frozen_unpublished_parameters}

    if representation_parameter_ids & filter_parameter_ids:
        raise RuntimeError("alternating parameter blocks overlap")
    if (representation_parameter_ids | filter_parameter_ids) & frozen_parameter_ids:
        raise RuntimeError("frozen eta parameters overlap trainable blocks")
    if (
        representation_parameter_ids
        | filter_parameter_ids
        | frozen_parameter_ids
        != all_parameter_ids
    ):
        raise RuntimeError(
            "trainable/frozen parameter blocks do not cover all MaskedCLA parameters"
        )
    if any(parameter.requires_grad for parameter in frozen_unpublished_parameters):
        raise RuntimeError("unpublished eta head must remain frozen")
    ...
    def _regularized_eq32_objective(kg_state_update, current_target_9):
        """Yan Eq. (30) state loss plus Eq. (32) L2 regularization."""
        eq30_loss, position_rmse = _state_prediction_loss(
            kg_state_update,
            current_target_9,
        )
        ...
        if GAMMA_L2:
            ...
            objective = eq30_loss + GAMMA_L2 * l2_theta
        else:
            objective = eq30_loss
        return objective, eq30_loss, position_rmse
    ...
    def _run_training_phase(
        ...
        for batch in epoch_batches:
            ...
            objective, eq30_loss, position_rmse_b = _regularized_eq32_objective(
                kg_state_update,
                target_b,
            )
            ...
            eq30_sum += float(eq30_loss.detach().cpu()) * batch_size_now
            position_rmse_squared_sum += (
                float(position_rmse_b.detach().cpu()) ** 2 * batch_size_now
            )
        ...
        return {
            ...
            "eq30": eq30_sum / count,
            "position_rmse_m": math.sqrt(position_rmse_squared_sum / count),
        }

    def _evaluate_state_objective(loader):
        # Restore the two published-objective blocks as trainable for checkpoint
        # consistency; the eta head remains frozen because its criterion is unknown.
        for parameter in model.parameters():
            parameter.requires_grad_(False)
        for parameter in trainable_eq32_parameters:
            parameter.requires_grad_(True)
        ...
        with torch.inference_mode():
            for batch in loader:
                ...
                if not torch.isfinite(objective) or not torch.isfinite(eq30_loss):
                    raise FloatingPointError(
                        "non-finite Data01 state-estimation objective"
                    )
    ...
    print(
        ...
        "ON FIXED OFFLINE DATA01 (Eqs. 18-20; ETA HEAD FROZEN) ==="
    )
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        training_history.append({
            ...
            "train_objective_eq32": final_training_metrics["objective"],
        ...
        print(
            ...
            f"train_Eq32={final_training_metrics['objective']:.6g}, "
    ...
    # Yan et al. publish a fixed offline training stage followed by testing; no
    # internal validation checkpoint selection is reported. Continue with the
    ...
    for event in test_timeline:
        ...
        learned_error_state_post = learned_error_state_pred + correction
        nav = inject_error_state(nav, correction)
        # Fig. 2 feedback path is retained structurally, but the unpublished eta
        # branch is frozen at its zero initialization in this paper-faithful
        # baseline. Thus Eq. (8) receives zero residual-error feedback here.
    ...
    summary = {
        ...
        "feature_protocol": {
            ...
            "first_epoch_context": "all_zero_previous_residual_state_context_no_classical_warm_start",
        ...
        "masked_cla_protocol": {
            ...
            "imu_error_output": "6D_eta_[epsilon_g,epsilon_a]_head_retained_but_frozen_zero_unpublished_training_rule",
            "eta_online_timing": "structural_feedback_path_retained_but_zero_output_in_paper_faithful_baseline",
            "eta_online_application": "Eq8_subtraction_receives_zero_eta_in_paper_faithful_baseline",
            "eta_same_epoch_state_update": False,
            "eta_training_completion": "removed_v5_future_state_auxiliary_loss_not_published_in_Yan_Eq30_Eq32",
            "eta_direct_sensor_error_labels": False,
            "training_status": "fixed_offline_Data01_Yan_Eqs18_to_20_Eq30_Eq32_with_Ref15_style_alternating_blocks",
            "training_features": "fixed_offline_Data01_Yan_Eqs10_to_21_then_Eqs18_to_20",
            "recursive_training_claimed_by_paper": False,
            "recursive_training_status": "not_used_in_offline_training",
            "offline_feature_history_generator": "conventional_TC_reference_pass_unpublished_by_Yan_but_required_to_instantiate_Eqs10_to_14",
            "truth_state_feedback_during_training": False,
            "bptt_across_navigation_epochs": False,
            "loss": "Yan_Eq30_current_state_squared_error_plus_Eq32_L2",
            "alternating_optimization": "Ref15_Algorithm2_order_theta_then_psi_same_objective",
            "alternating_theta_block": "Masked_LSTM_plus_Masked_Attention_plus_KG_FC_head",
            "alternating_psi_block": "Masked_CNN",
            ...
            "paper_baseline_early_stopping": False,
            "early_stopping_status": "not_used_paper_does_not_publish_it",
            "paper_baseline_gradient_clipping": False,
            "internal_validation_split": False,
            "internal_validation_status": "not_used_Yan_designates_full_Data01_for_training",
            "self_rollout_regenerated_each_outer_epoch": False,
        },
        "test_causal_initialization": {
            "method": "zero_previous_context_then_KNet_from_first_usable_fusion_epoch",
            "classical_KF_warm_start": False,
            "first_KNet_update_included_in_reported_test_metrics": True,
        },
        "orchestration_audit": {
            "adopted": [
        ...
        "online_update_protocol": {
            ...
            "eta_feedback": "structural_Eq8_path_present_but_eta_head_frozen_zero_due_unpublished_training_rule",
            ...
            "standalone_online_zero_history_first_epoch_adopted": True,
        },
        ...
        "limitations": [
            ...
            "The IE postprocessed truth provides position, velocity, and attitude but not accelerometer/gyro bias truth; Masked CLA measurement updates therefore keep nominal bias states frozen while propagating the full 15-state covariance.",
            "Yan et al. do not publish the initialization of lagged Eqs. (11)-(13) at the first usable fusion epoch. This run initializes the missing previous context to exact zeros and applies KNet from the first usable epoch in both training and testing; no classical warm-start is used.",
            "The first usable training/test fusion epoch uses zero lagged history features because no previous completed fusion state exists; this causal initialization is an explicit completion of the paper's unpublished boundary condition.",
            ...
            "Fig. 8 outputs eta_k as inertial-navigation measurement-error estimation and Fig. 2 feeds IMU-error correction back to Error compensation, but Yan et al. publish no eta_k target, timing, or temporal training equation. The previous v5 following-interval sensitivity loss has been removed because it was not Eq. (30)/(32). The eta head is retained structurally but frozen at zero; this is an explicit unreproduced paper detail, not a claim that eta_k is zero in the original method.",
            "Yan et al. explicitly define a fixed offline training dataset X, M, Y in Eqs. (18)-(20). This run keeps that dataset fixed during optimization. The paper does not publish how the estimated innovation/residual history of Eqs. (10)-(14) is generated; a conventional TC reference pass is therefore used only as the minimal offline history-generation completion.",
            "Because the eta branch has no published supervision rule, it is excluded from the trainable Eq. (30)/(32) parameter set in this paper-faithful baseline. A learned eta feedback experiment should be reported separately as a completion unless author code/details become available.",
            "Yan et al. explicitly state alternating optimization and cite Ref. [15]. This run follows Ref. [15] Algorithm 2 ordering (theta then psi) with the same final state-estimation objective in both phases. Ref. [15] defines theta as the recurrent filter and psi as the encoder; the closest Fig. 8 mapping used here is theta=Masked LSTM+Attention+KG head and psi=Masked CNN. Yan et al. do not publish the exact partition, so this remains a declared paper-guided completion.",
            ...
            "Yan et al. do not publish feature-wise z-score standardization; this baseline therefore uses the raw Eqs. (15)-(21) features with only the paper-specified zero-padding/masking. Eq. (30) state targets are likewise left in physical units.",
