"""Single-file GNSS/LEO/INS + Masked KalmanNet simulation.

The file contains the complete training and independent-test pipeline in one script.
Core processing follows Yan et al. (2026): tightly coupled INS/GNSS/LEO navigation,
Masked CNN-LSTM-attention Kalman-gain estimation, eta_k IMU-error feedback, and
online FDE before the learned measurement update.

Retained project constraints required by the available data/resources:
- pseudorange-only measurements,
- TLE/SGP4 LEO propagation in place of unavailable STK/HPOP data,
- 9 supervised navigation-error states with 15-state covariance propagation,
- explicit GPS/BDS receiver-clock nuisance elimination,
- paper-guided choices only where the paper leaves implementation details open.

Diagnostic-only experiments and redundant intermediate outputs were removed.

Numerical-alignment revision:
- Eq. (30)-type target loss is kept in raw physical state units; the extra
  position/velocity/attitude RMS reweighting from clean_v1 was removed.
- checkpoint selection monitors the same validation Eq. (30)-type state loss.
- the Masked-FC gain head uses the standard seeded PyTorch initialization rather
  than an unpublished forced-zero initialization.
- the more physically complete analytic ECEF gravity gradient from clean_v1 is
  retained; the older radial-only approximation is intentionally not restored.
"""
...
def discretize_process_noise_van_loan(F: Array, Qc: Array, dt_s: float) -> tuple[Array, Array]:
    """Van Loan discretization for the 15-state error model."""
    ...
    # High-rate IMU steps are small; a guarded Taylor expansion is numerically
    # equivalent here and avoids a costly 30x30 expm at every sample.
    if np.linalg.norm(B, 1) <= VAN_LOAN_TAYLOR_MAX_NORM_1:
        E = np.eye(30, dtype=float)
        term = np.eye(30, dtype=float)
...
def learned_gain_covariance_update(
    ...
    - ``R`` appears here only to propagate a covariance estimate for subsequent propagation and FDE. Yan et al. do not publish an explicit
...
if __name__ == "__main__":
    ...
    _, imr_tow_all, imr_record_count = read_imr_tow_only(IMR_PATH)
    ...
    def _state_prediction_loss(predicted_update, true_error_state_9):
        ...
        # Yan et al. Eq. (30): raw squared state-estimation error.
        # Input features remain normalized; state targets are not rescaled.
    ...
    def _regularized_temporal_state_objective(
        ...
        # The same raw physical-state criterion is used at both supervised
        # instants. Equal averaging is the time-sample mean analogue
    ...
    best_val_state_loss = float("inf")
    best_val_position_rmse_m = float("inf")
    best_epoch = 0
    best_state = None
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        # Use the same Eq. (30)-type validation state criterion for checkpoint
        # selection. Position RMSE is retained only as a reporting metric.
        # Early stopping itself remains an unpublished reproducibility safeguard.
        val_state_loss = validation["temporal_state_loss"]
        if val_state_loss < best_val_state_loss - EARLY_STOP_MIN_DELTA:
            best_val_state_loss = val_state_loss
            best_val_position_rmse_m = validation["current_position_rmse_m"]
            best_epoch = epoch
            best_state = {
    ...
    torch.save(
        {
            ...
            "best_val_eq30_state_loss": best_val_state_loss,
            "best_val_position_rmse_m": best_val_position_rmse_m,
    ...
    _, test_imr_tow_all, test_imr_record_count = read_imr_tow_only(
        test_files.imr
    )
    ...
    with (OUTPUT_DIR / "test_trajectory.csv").open(
    ...
    summary = {
        ...
        "measurement_mode": "pseudorange_only",
        "leo_orbit_source": "TLE/SGP4",
        "best_training_epoch": int(best_epoch),
        "best_validation_eq30_state_loss": float(best_val_state_loss),
        ...
        "test_online_epochs": int(len(online_rows)),
        "fde": test_fde_stats,
    }
