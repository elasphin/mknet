...
if __name__ == "__main__":
    ...
    # =========================================================================
    # =========================================================================
    # 7. YAN Eqs. (18)-(20): DATA01 BASE SET + NUMERICAL CONDITIONING
    #    + SIMPLE PRIOR-ROBUST TRAINING AUGMENTATION
    # =========================================================================
    ...
    # Data01-only prior perturbation bank. Centering removes any deterministic
    # bias and preserves empirical cross-axis structure without imposing a
    # Gaussian prior-noise model. The original clean Yan sample is always kept.
    prior_perturbation_bank_9 = (
        target_states_9 - np.mean(target_states_9, axis=0, keepdims=True)
    )
    if len(prior_perturbation_bank_9) <= 1:
        prior_perturbation_bank_9 = np.zeros_like(target_states_9)
    prior_perturbation_rms_9 = np.sqrt(
        np.mean(prior_perturbation_bank_9**2, axis=0)
    )
    ...
    dataset = TensorDataset(
        ...
        torch.tensor(measurement_jacobian_padded, dtype=torch.float32),
    )
    ...
    if MAX_FUSION_EPOCHS is not None:
        print(
            "DIAGNOSTIC TRAINING NOTICE: fusion cap is active, so these "
            f"{len(dataset)} fixed Data01 samples are reused for "
            f"{TRAINING_EPOCHS} training epochs. This is useful for a short "
            "code/stability check but must not be interpreted as the full-paper "
            "training experiment. For the scientific run set "
            "MKNET_MAX_FUSION_EPOCHS=none (and normally "
            "MKNET_MAX_TEST_FUSION_EPOCHS=none)."
        )
    print(
        "NN input standardization:",
    ...
    print(
        "prior-robust perturbation RMS before scale [dp(3), dv(3), dtheta(3)]:",
        prior_perturbation_rms_9,
    )
    ...
    # the SAME final state-estimation objective. The added prior-robust view keeps
    # that objective unchanged and modifies only the Data01 training prior/input.
    # Yan et al. do not publish the
    ...
    prior_perturbation_bank_device = torch.tensor(
        prior_perturbation_bank_9, dtype=torch.float64, device=DEVICE
    )
    fixed_std_device = torch.tensor(
        fixed_std, dtype=torch.float32, device=DEVICE
    )
    obs_std_device = torch.tensor(
        obs_std, dtype=torch.float32, device=DEVICE
    )

    def _prior_robust_training_pair(
        fixed_b,
        obs_b,
        mask_b,
        channel_b,
        ...
        """Pair a clean sample with one first-order prior-perturbed Data01 view."""
        if not PRIOR_ROBUST_TRAINING_ON:
            return (
                fixed_b, obs_b, mask_b, channel_b, innovation_b, target_b,
                eta_sensitivity_b, eta_future_target_b, eta_valid_b
            )

        delta9 = (
            PRIOR_PERTURBATION_SCALE
            * prior_perturbation_bank_device[int(perturbation_index)]
        ).unsqueeze(0)
        delta15_64 = torch.zeros(
            (1, INS_STATE_DIM), dtype=torch.float64, device=DEVICE
        )
        delta15_64[:, :SUPERVISED_STATE_DIM] = delta9
        delta15 = delta15_64.to(dtype=torch.float32)

        # First-order consistency for y-h(x): nu' ~= nu - H*delta_x.
        delta_innovation = torch.bmm(
            H_b.to(dtype=torch.float32), delta15.unsqueeze(-1)
        ).squeeze(-1)
        delta_innovation = delta_innovation * mask_b.to(dtype=torch.float32)

        fixed_aug = fixed_b.clone()
        # fixed = [dAccel(3), dGyro(3), prev_state_residual(15),
        #          prev_state_innovation(15)].
        fixed_aug[:, 6:21] += delta15 / fixed_std_device[6:21].unsqueeze(0)
        fixed_aug[:, 21:36] += delta15 / fixed_std_device[21:36].unsqueeze(0)

        obs_aug = obs_b.clone()
        innovation_shift_normalized = (
            delta_innovation / obs_std_device[1].clamp_min(1e-12)
        )
        obs_aug[:, :, 1] = torch.where(
            channel_b[:, :, 1],
            obs_b[:, :, 1] - innovation_shift_normalized,
            obs_b[:, :, 1],
        )
        innovation_aug = innovation_b - delta_innovation
        target_aug = target_b - delta9

        return (
            torch.cat([fixed_b, fixed_aug], dim=0),
            torch.cat([obs_b, obs_aug], dim=0),
            torch.cat([mask_b, mask_b], dim=0),
            torch.cat([channel_b, channel_b], dim=0),
            torch.cat([innovation_b, innovation_aug], dim=0),
            torch.cat([target_b, target_aug], dim=0),
            torch.cat([eta_sensitivity_b, eta_sensitivity_b], dim=0),
            torch.cat([eta_future_target_b, eta_future_target_b], dim=0),
            torch.cat([eta_valid_b, eta_valid_b], dim=0),
        )
    ...
    def _run_training_phase(
        ...
        """One alternating pass over the Yan Data01 base plus robust paired view.

        Each X_bar[k] remains one intra-epoch masked sequence. BATCH_SIZE controls
        gradient accumulation across physical fusion-epoch samples; it does not
        create cross-fusion recurrent state.
        """
        _set_alternating_phase(phase)

        # Same deterministic perturbation draw for theta and psi within an epoch.
        perturb_rng = np.random.default_rng(SEED + 1009 * int(epoch))
        perturbation_indices = perturb_rng.integers(
            0, len(prior_perturbation_bank_9), size=len(dataset)
        )
        ...
        for sample_index in range(len(dataset)):
            ...
            (
                ...
                eta_valid_b,
                H_b,
                int(perturbation_indices[sample_index]),
            )
    ...
    print(
        ...
        "ON DATA01 BASE X/M/Y + SIMPLE PRIOR-ROBUST AUGMENTATION "
        "(INTRA-EPOCH MASKED LSTM) ==="
    )
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        # Ref. [15] Algorithm 2 ordering: theta first, then psi. Yan's Eqs.
        # (18)-(20) samples remain the clean base set. The extra paired prior-
        # perturbed view is a declared robustness completion using Data01 only.
        ...
        training_history.append({
            "stage": "yan_Data01_ref15_intra_epoch_prior_robust_alternating",
            "epoch": epoch,
    ...
    # Yan et al. do not report an internal validation checkpoint rule. Keep the
    # final epoch. Clean Data01 Eq. (30) remains the reported training metric;
    # prior-robust augmentation affects optimization only and never uses Data02.
    model.eval()
    ...
    selected_training_stage = "yan_Data01_ref15_prior_robust_alternating_eq30_final_epoch"
    ...
    # =========================================================================
    # No second recursive/full-trajectory training stage is applied. Yan's Data01
    # X/M/Y remains the sole measured/labeled data source; the only additional
    # training view is the Data01-only first-order prior perturbation above.
    recursive_training_metrics = None
    selected_training_stage = (
        "yan_Data01_ref15_prior_robust_intra_epoch_masked_LSTM"
    )
    model.eval()
    ...
    torch.save(
        {
            ...
            "sequence_training": (
                "Yan_Data01_XMY_base_intra_epoch_masked_LSTM; "
                "clean_plus_first_order_prior_perturbed_training_view; "
                "hidden_state_local_to_each_fusion_epoch"
            ),
            "prior_robust_training": {
                "enabled": bool(PRIOR_ROBUST_TRAINING_ON),
                "scale": float(PRIOR_PERTURBATION_SCALE),
                "source": "centered_Data01_state_error_labels_only",
                "linearized_innovation_update": "innovation_minus_H_delta_x",
    ...
    # =========================================================================
    # DIAGNOSTIC ONLY. The Yan-base + prior-robust Data01 training has finished
    ...
    summary = {
        ...
        "paper_components": {
            ...
            "training_Eq30_to_Eq32": "Yan_Data01_base_intra_epoch_masked_LSTM_ref15_alternating_plus_prior_robust_completion",
            "Fig8_eta": {
                "enabled": True,
        ...
        "unavoidable_unpublished_completions": [
            ...
            "Data01_only_first_order_prior_robust_training_augmentation_inspired_by_related_KalmanNet_work",
        ...
        "training": {
            ...
            "prior_robust_training": {
                "enabled": bool(PRIOR_ROBUST_TRAINING_ON),
                "paired_clean_and_augmented_views": bool(PRIOR_ROBUST_TRAINING_ON),
                "scale": float(PRIOR_PERTURBATION_SCALE),
                "perturbation_source": "centered_Data01_target_state_errors",
                "innovation_mapping": "first_order_innovation_minus_H_delta_x",
                "lagged_state_feature_mapping": "add_delta_x_to_two_15_state_history_blocks",
                "Data02_used": False,
                "paper_status": "explicit_training_robustness_completion_not_published_by_Yan",
            },
            "sequence_training": {
                "scope": "Yan_Data01_Eqs18_to_20_base_plus_Data01_only_prior_robust_view",
                ...
                "feature_recursion": "no_full_recursive_training_or_BPTT",
                "paper_status": (
                    "Yan_base_training_plus_related_KalmanNet_prior_robust_completion"
                ),
            },
