...
# True enables the Yan/Ref.[33] online FDE/DIA path; False bypasses it with the same raw measurements.
FDE_DIA_ON = True

# PROJECT SIMPLIFICATION: 9-state [delta-p,delta-v,delta-theta], no bias states or Fig.8 eta_k.

# Read-only Data01 closed-loop replay compares learned vs classical TC/KF; Data02 always runs independently.
...
# ECEF -> geodetic conversion; numerical method unchanged.
...
# Saastamoinen troposphere correction; formula unchanged.
...
# Klobuchar ionosphere correction; formula unchanged.
...
# IMU error-model unit conversion to SI.
def imu_model_to_si(model: IMUNoiseModel) -> IMUNoiseModelSI:
    d2r = np.pi / 180.0
    return IMUNoiseModelSI(model.imu_type, model.isdv_pos_m.copy(), model.isdv_vel_mps.copy(), model.isdv_att_deg * d2r, model.isdv_accel_bias_mps2.copy(), model.isdv_gyro_bias_deg_s * d2r, model.pnsd_pos_m_sqrt_s.copy(), model.pnsd_vel_mps_sqrt_s.copy(), model.pnsd_att_deg_sqrt_s * d2r, model.pnsd_accel_bias_mps2_sqrt_s.copy(), model.pnsd_gyro_bias_deg_s_sqrt_s * d2r)

# IMU error-model reader; units/conversions unchanged.
def read_imu_error_models(path: str | Path) -> dict[str, IMUNoiseModel]:
    ...
    for match in re.finditer('IMU\\s*\\{(.*?)\\}', text, flags=re.S):
        ...
        models[imu_type] = IMUNoiseModel(imu_type, values['ISDV_Pos'], values['ISDV_Vel'], values['ISDV_Att'], values['ISDV_AccelBias'], values['ISDV_GyrosBias'], values['PNSD_Pos'], values['PNSD_Vel'], values['PNSD_Att'], values['PNSD_AccelBias'], values['PNSD_GyrosBias'])
    return models
...
class RINEXObservationFile:
    ...
    def _read_header(self) -> None:
        ...
        self._index_by_type = {system: {obs_type: i for i, obs_type in enumerate(values)} for system, values in self.observation_types.items()}

    def _select_signal_and_pseudorange(self, constellation: str, obs_types: tuple[str, ...], fields: list[str]) -> tuple[SelectedSignal, float] | None:
            ...
            for line in stream:
                ...
                for _ in range(satellite_count):
                    ...
                    if allowed_constellations and constellation not in allowed_constellations:
                        continue
                    selected = self._select_signal_and_pseudorange(constellation, obs_types, fields)
                    if selected is None:
                        continue
                    ...
                    measurements.append(SatelliteMeasurement(sat_id, constellation, signal, pseudorange, cn0))
                if start_time_gpst_s is not None and time_gpst_s < float(start_time_gpst_s):
                    continue
                if require_measurements and (not measurements):
                    continue
                yield ObservationEpoch(time_gpst_s, gps_time.week, gps_time.tow_s, tuple(measurements))
                yielded += 1
...
class RINEXClock:
    ...
    def _parse(self):
        ...
        for sat_id, (times, bias, drift) in records.items():
            ...
            out[sat_id] = (np.asarray(times)[order], np.asarray(bias)[order], np.asarray(drift)[order])
        return out
...
# --- SP3 precise orbit ---
# SP3 orbit interpolation; interpolation equations and units unchanged.
class SP3Orbit:
    ...
    def _parse(self):
        ...
        for sat_id, (times, positions, clocks) in temporary.items():
            ...
            velocity = np.vstack([velocities.get((sat_id, float(t)), [np.nan] * 3) for t in times])
            drift = np.asarray([drifts.get((sat_id, float(t)), np.nan) for t in times])
            finite_clock = np.isfinite(clock)
            finite_drift = np.isfinite(drift)
            series[sat_id] = {'time': times, 'position': np.asarray(positions, dtype=float)[order], 'clock': clock, 'velocity': velocity, 'velocity_finite': np.all(np.isfinite(velocity), axis=1), 'drift': drift, 'clock_time': times[finite_clock], 'clock_value': clock[finite_clock], 'drift_time': times[finite_drift], 'drift_value': drift[finite_drift]}
        return series
    ...
    def state(self, sat_id: str, time_gpst_s: float):
        ...
        clock = None if s['clock_time'].size == 0 else float(np.interp(time_gpst_s, s['clock_time'], s['clock_value']))
        drift = None if s['drift_time'].size == 0 else float(np.interp(time_gpst_s, s['drift_time'], s['drift_value']))
...
def read_imr(path: str | Path, scaling_mode: str='cpp_exact', *, start_record: int=0, stop_record: int | None=None) -> IMRData:
    ...
    if scaling_mode == 'cpp_exact':
        ...
        if header.delta_velocity:
            accel *= header.data_rate_hz
    return IMRData(header, tow, np.deg2rad(gyro), accel, gyro_counts, accel_counts)
...
def resolve_dataset_files(dataset_dir: str | Path, rover_id: str='01') -> tuple[DatasetFiles, RoverMetadata]:
    ...
    files = DatasetFiles(root=root, readme_xml=readme, rover_ground_truth=rover_truth, imu_ground_truth=imu_truth, rinex_obs=rover_observation, imr=imr, sp3=_unique_file(root, ('*.SP3', '*.sp3'), 'SP3 precise-orbit'), clk=_unique_file(root, ('*.CLK', '*.clk'), 'RINEX clock'), nav=_unique_file(root, ('brdm*.*p', 'brdm*.*P', 'brdm*.rnx', 'BRDM*.RNX'), 'broadcast navigation'))
    required = [files.readme_xml, files.rover_ground_truth, files.imu_ground_truth, files.rinex_obs, files.imr, files.sp3, files.clk, files.nav]
...
def load_ie_ground_truth(path: str | Path) -> GroundTruth:
    ...
    return GroundTruth(a[:, 0].astype(int), a[:, 1], a[:, 2:5], a[:, 5:8], a[:, 8], a[:, 9], a[:, 10])

def interpolate_ground_truth(truth: GroundTruth, query_time_gpst_s: np.ndarray, max_gap_s: float):
...
# IE heading/pitch/roll -> body-to-ECEF attitude conversion.
...
# 9-state/no-bias branch: omit Yan Eq.(8) bias/residual terms; zero calibration matrices leave IMU unchanged.
# 9-state branch IMU compensation/calibration interface.
def compensate_imu(measured_angular_rate_body_radps: Array, measured_specific_force_body_mps2: Array, S_g: Array | None=None, M_g: Array | None=None, S_a: Array | None=None, M_a: Array | None=None) -> tuple[Array, Array]:
    ...
    if S_g is None and M_g is None:
        ...
        gyro = np.linalg.solve(np.eye(3) + (S_g if S_g is not None else 0.0) + (M_g if M_g is not None else 0.0), gyro_rhs)
    if S_a is None and M_a is None:
        accel = accel_rhs
    else:
        accel = np.linalg.solve(np.eye(3) + (S_a if S_a is not None else 0.0) + (M_a if M_a is not None else 0.0), accel_rhs)
    return (gyro, accel)
...
# 9-state ECEF INS mechanization; propagation equations unchanged.
def mechanize_ecef(nav: NavigationState, angular_rate_body_radps: Array, specific_force_body_mps2: Array, dt_s: float) -> NavigationState:
    ...
    Cmid = 0.5 * (C0 + C1)
    acceleration = Cmid @ np.asarray(specific_force_body_mps2) + effective_gravity_ecef(nav.position_ecef_m) - 2.0 * np.cross(OMEGA_IE_E, nav.velocity_ecef_mps)
...
# Existing iterative transmit-time/light-time solution.
...
# Existing two-pass GNSS transmit-time correction.
...
# GNSS pseudorange preprocessing; corrections and selection logic unchanged.
class GNSSPreprocessor:
    ...
    def prepare_measurement(self, reception_time_gpst_s: float, raw: SatelliteMeasurement, receiver_position_ecef_m: Array, receiver_llh: tuple[float, float, float] | None=None, c_ecef_ned: Array | None=None) -> PseudorangeMeasurement | None:
        try:
            ...
            return None
        try:
            transmit_time, transit_s, state_tx_position = _gnss_transmit_time_two_pass(self._position, raw.sat_id, reception_time_gpst_s, receiver_position_ecef_m, initial_position)
        except (KeyError, ValueError):
            return None
        ...
        if self.use_ionosphere:
            ...
            ionosphere = klobuchar_delay_m(tow, lat, lon, elevation, azimuth, alpha, beta) * (reference_frequency_hz / raw.signal.frequency_hz) ** 2
        troposphere = 0.0
        ...
        iono_sigma = ref35_ionosphere_sigma_m(elevation, receiver_llh[0]) * (iono_reference_frequency_hz / raw.signal.frequency_hz) ** 2 if self.use_ionosphere else 0.0
        ...
        return PseudorangeMeasurement(raw.sat_id, raw.constellation, float(raw.pseudorange_m), satellite_position_rx, satellite_clock_bias_s, float(ionosphere), float(troposphere), float(sigma_code_m), float(elevation), raw.cn0_dbhz)
...
# Initial 9-state covariance from the IMU model.
...
# Continuous 9-state process covariance from the IMU model.
...
# Van Loan continuous-to-discrete process-noise mapping; mathematics unchanged.
...
# Existing ECEF pseudorange prediction model.
def predict_pseudorange(antenna_position_ecef_m: Array, measurement: PseudorangeMeasurement, receiver_clock_bias_m_by_system: dict[str, float]) -> tuple[float, Array]:
    ...
    receiver_clock_m = 0.0 if measurement.constellation == 'L' else float(receiver_clock_bias_m_by_system.get(measurement.constellation, 0.0))
    predicted = geometric_range + receiver_clock_m - SPEED_OF_LIGHT_MPS * measurement.satellite_clock_bias_s + measurement.ionosphere_delay_m + measurement.troposphere_delay_m
    return (float(predicted), los)

# Remove singleton G/C observations that vanish under clock projection.
...
# Existing epoch-wise receiver-clock nuisance projection.
def _clock_projector_and_biases(measurements, variances: Array, raw_innovation: Array) -> tuple[Array, dict[str, float]]:
    n = len(measurements)
    projector = np.eye(n)
    ...
    for system in ('G', 'C'):
        ...
        receiver_clock[system] = float(np.sum(weights * raw_innovation[index]) / weight_sum)
...
# TC pseudorange y/H/R construction; state convention unchanged.
def build_measurement_model(nav: NavigationState, measurements, lever_arm_b_m: Array) -> TCMeasurementModel:
    ...
    n = len(measurements)
    if n == 0:
        return TCMeasurementModel(np.empty(0), np.empty(0), np.empty(0), np.zeros((0, INS_STATE_DIM)), np.zeros((0, 0)), (), {}, np.zeros((0, 0)))
    y = np.empty(n)
    ...
    for i, m in enumerate(measurements):
        y_pred_zero_clock[i], los = predict_pseudorange(antenna_position, m, zero_clock)
        y[i] = m.pseudorange_m
        H_raw[i, 0:3] = los
    ...
    projector, receiver_clock = _clock_projector_and_biases(measurements, variances, raw_innovation)
    ...
    return TCMeasurementModel(y, y_pred, innovation, H, R, tuple(sat_ids), receiver_clock, projector)

# Recompute pseudorange innovation/residual for the current navigation state.
def build_innovation_only(nav: NavigationState, measurements, lever_arm_b_m: Array) -> Array:
    ...
    for i, m in enumerate(measurements):
        ...
        if not math.isfinite(variance) or variance <= 0.0:
            raise ValueError('pseudorange variance must be finite and positive')
        variances[i] = variance
    projector, _ = _clock_projector_and_biases(measurements, variances, raw_innovation)
...
# Ref.[33] PSD pseudoinverse/rank helper.
...
# Ref.[33] FDE detection statistics.
def _ref33_detection_terms(prior_covariance: Array, measurement_model: TCMeasurementModel, alpha: float) -> tuple[float, float, int, Array, Array]:
...
# Ref.[33] single-fault identification logic.
def _ref33_identify_single_pseudorange_fault(measurement_model: TCMeasurementModel, Q_pinv: Array):
    ...
    for j in range(len(nu)):
        ...
        tolerance = 100.0 * eps * max(qpinv_norm_2 * float(C_i @ C_i), np.finfo(float).tiny)
        ...
        candidates.append({'index': int(j), 'C_i': C_i.copy(), 'denominator': denominator, 'numerator': numerator, 'local_statistic': local_statistic, 'local_score': float(chi2.cdf(local_statistic, df=1)), 'estimated_fault_m': float(numerator / denominator)})
    if not candidates:
        return None
    ...
    for candidate in candidates:
        ...
        cosine = abs(float(best_C @ Q_pinv @ C_j)) / math.sqrt(max(best_den * den_j, np.finfo(float).tiny))
...
# Yan Eq.(34) DIA covariance adaptation.
def _yan_eq34_dia_adaptation(prior_covariance: Array, model: TCMeasurementModel, Q_nu_nu: Array, Q_pinv: Array, fault_direction: Array | None) -> tuple[Array, Array]:
...
# Online Ref.[33]/Yan FDE-DIA decision path; thresholds/statistics unchanged.
def ref33_fde_dia_decision(nav: NavigationState, prior_covariance: Array, measurements, lever_arm_b_m: Array, alpha: float) -> Ref33FDEResult:
    ...
    if not current:
        return Ref33FDEResult((), tested_model, (), empty_model, False, (), (), (), (), 0.0, float('inf'), 0, False, 0.0, 0.0, np.zeros((0, 0)), zero_dx, P0)
    statistic, threshold, dof, Q_nu_nu, Q_pinv = _ref33_detection_terms(prior_covariance, tested_model, alpha)
    nominal_dx, nominal_P = _yan_eq34_dia_adaptation(prior_covariance, tested_model, Q_nu_nu, Q_pinv, None)
    if dof == 0 or statistic <= threshold:
        return Ref33FDEResult(current, tested_model, current, tested_model, False, (), (), (), (), statistic, threshold, dof, False, 0.0, 0.0, Q_nu_nu, nominal_dx, nominal_P)
    identification = _ref33_identify_single_pseudorange_fault(tested_model, Q_pinv)
    if identification is None:
        return Ref33FDEResult(current, tested_model, (), empty_model, True, (), (), tuple((m.sat_id for m in current)), (), statistic, threshold, dof, True, 0.0, 0.0, Q_nu_nu, nominal_dx, nominal_P)
    ...
    dia_dx, dia_P = _yan_eq34_dia_adaptation(prior_covariance, tested_model, Q_nu_nu, Q_pinv, np.asarray(identification['C_i'], dtype=float))
    return Ref33FDEResult(current, tested_model, usable, filtered_model, True, (selected.sat_id,), (selected.constellation,), ambiguous_sat_ids, excluded_sat_ids, statistic, threshold, dof, not bool(usable), float(identification['local_score']), float(identification['estimated_fault_m']), Q_nu_nu, dia_dx, dia_P, 0.0, float('inf'), 0, True)

def _new_fde_stats() -> dict:
    return {'epochs_checked': 0, 'epochs_detected': 0, 'identified_fault_modes': 0, 'hard_exclusion_epochs': 0, 'excluded_measurements': 0, 'unresolved_epochs': 0, 'post_exclusion_failed_epochs': 0, 'max_statistic_to_threshold_ratio': 0.0, 'max_abs_estimated_fault_m': 0.0, 'max_local_identification_score': 0.0, 'identified_by_constellation': {'G': 0, 'C': 0, 'L': 0}}

def _fde_export_fields(result: Ref33FDEResult | None) -> dict:
    """Diagnostic fields only; None means FDE/DIA was not executed."""
    if result is None:
        return {'fde_detected': False, 'fde_identified_sat_ids': (), 'fde_excluded_sat_ids': (), 'fde_ambiguous_sat_ids': (), 'fde_statistic': float('nan'), 'fde_threshold': float('nan'), 'fde_dof': 0, 'fde_post_exclusion_statistic': float('nan'), 'fde_post_exclusion_threshold': float('nan'), 'fde_post_exclusion_dof': 0, 'fde_post_exclusion_consistent': True, 'fde_local_identification_score': float('nan'), 'fde_estimated_fault_m': float('nan'), 'fde_hard_exclusion_applied': False, 'fde_state_correction_norm': 0.0, 'fde_unresolved': False}
    return {'fde_detected': bool(result.detected), 'fde_identified_sat_ids': tuple(result.identified_sat_ids), 'fde_excluded_sat_ids': tuple(result.excluded_sat_ids), 'fde_ambiguous_sat_ids': tuple(result.ambiguous_sat_ids), 'fde_statistic': float(result.statistic), 'fde_threshold': float(result.threshold), 'fde_dof': int(result.dof), 'fde_post_exclusion_statistic': float(result.post_exclusion_statistic), 'fde_post_exclusion_threshold': float(result.post_exclusion_threshold), 'fde_post_exclusion_dof': int(result.post_exclusion_dof), 'fde_post_exclusion_consistent': bool(result.post_exclusion_consistent), 'fde_local_identification_score': float(result.local_identification_score), 'fde_estimated_fault_m': float(result.estimated_fault_m), 'fde_hard_exclusion_applied': bool(result.excluded_sat_ids), 'fde_state_correction_norm': float(np.linalg.norm(result.dia_state_correction)), 'fde_unresolved': bool(result.unresolved)}
...
def _accumulate_fde_stats(stats: dict, result: Ref33FDEResult) -> None:
    ...
    stats['post_exclusion_failed_epochs'] += int(bool(result.excluded_sat_ids) and (not result.post_exclusion_consistent))
    if math.isfinite(result.threshold) and result.threshold > 0.0:
        stats['max_statistic_to_threshold_ratio'] = max(stats['max_statistic_to_threshold_ratio'], float(result.statistic / result.threshold))
    stats['max_abs_estimated_fault_m'] = max(stats['max_abs_estimated_fault_m'], abs(float(result.estimated_fault_m)))
    stats['max_local_identification_score'] = max(stats['max_local_identification_score'], float(result.local_identification_score))
...
def _validate_ref33_statistical_core(alpha: float, *, sample_count: int=100000, seed: int=24681357) -> dict:
    ...
    positive = eigenvalues > 100.0 * np.finfo(float).eps * max(Q.shape) * max(np.max(np.abs(eigenvalues)), 1.0)
    basis = eigenvectors[:, positive]
    ...
    standard_error = math.sqrt(float(alpha) * (1.0 - float(alpha)) / float(sample_count))
    z_score = (empirical_false_alarm - float(alpha)) / max(standard_error, 1e-15)
    if abs(z_score) > 8.0:
        raise RuntimeError(f'projected Ref. [33] chi-square self-check failed: alpha={alpha}, empirical={empirical_false_alarm}, z={z_score}')
    ...
    denominators = np.einsum('ij,ij->j', candidate_directions, weighted_directions)
    ...
    correct_identification_rate = float(np.mean(detected_fault & (identified_index == 0)))
    wrong_identification_rate = float(np.mean(detected_fault & (identified_index != 0)))
    if correct_identification_rate < 0.99:
        raise RuntimeError(f'FDE strong-fault identification self-check failed: CI={correct_identification_rate}, MD={missed_detection_rate}, WI={wrong_identification_rate}')
    ...
    dummy_model = TCMeasurementModel(y=np.zeros(2), y_pred=np.zeros(2), innovation=np.array([5.0, -5.0]), H=np.zeros((2, INS_STATE_DIM)), R=Q2, sat_ids=('G01', 'G02'), receiver_clock_bias_m_by_system={'G': 0.0}, clock_projector=projector2)
    ambiguity = _ref33_identify_single_pseudorange_fault(dummy_model, Q2_pinv)
    if rank2 != 1 or ambiguity is None or bool(ambiguity['unique']) or (tuple(ambiguity['ambiguous_indices']) != (0, 1)):
        raise RuntimeError('FDE ambiguity self-check failed for two projected pseudoranges')
    return {'sample_count': int(sample_count), 'requested_alpha': float(alpha), 'empirical_false_alarm': empirical_false_alarm, 'false_alarm_z_score': float(z_score), 'projected_test_rank': int(rank), 'strong_fault_injected_m': injected_fault_m, 'strong_fault_missed_detection_rate': missed_detection_rate, 'strong_fault_correct_identification_rate': correct_identification_rate, 'strong_fault_wrong_identification_rate': wrong_identification_rate, 'two_measurement_ambiguity_detected': True, 'scope': 'synthetic_core_only_not_real_data_integrity_validation'}

# Classical Kalman measurement update; equations unchanged.
...
# Covariance update associated with learned Kalman gain; equations unchanged.
...
# Protection-level computation from covariance; formulas unchanged.
def protection_levels_from_covariance(covariance_ecef: Array, position_ecef_m: Array) -> tuple[float, float]:
...
# Error-state covariance reset after navigation-state feedback.
...
# Inject 9-state error correction into navigation state.
...
# Differentiable mirror of the same 9-state ECEF INS mechanization.
...
# Differentiable mirror of the same 9-state state injection.
...
# Existing elevation-dependent MP/NLOS sigma model.
...
# Ref.[35] receiver-noise sigma model.
...
# LEO pseudorange simulation; TLE/SGP4 and noise model unchanged.
class LEODownlinkSimulator:
    ...
    def simulate_one(self, receive_time_gpst_s: float, receiver_position_ecef_m: Array, sat_id: str, receiver_llh: tuple[float, float, float] | None=None, c_ecef_ned: Array | None=None):
        ...
        return PseudorangeMeasurement(sat_id, 'L', float(pseudorange_m), sat_rx, 0.0, float(ionosphere_m), float(troposphere_m), float(sigma_code_m), float(elevation), None)
...
def _scan_test_sequence_capacity(dataset_dir: str | Path, max_epochs: int | None, tle_provider: TLESGP4Provider, leo_seed: int) -> int:
    ...
    antenna_time = validate_strict_time_axis('test antenna truth', antenna_truth.week.astype(float) * GPS_WEEK_S + antenna_truth.tow_s)
    imu_truth_time = validate_strict_time_axis('test IMU truth', imu_truth.week.astype(float) * GPS_WEEK_S + imu_truth.tow_s)
    ...
    antenna_position, _, _, _, _ = interpolate_ground_truth(antenna_truth, fusion_time, MAX_TRUTH_INTERPOLATION_GAP_S)
...
# --- MASKED CLA NETWORK ---
# Yan Eqs. (10)-(29); 9-state/pseudorange-only. Pooling and [9,Nmax] FC reshape are project completions.
...
class MaskedStackedLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor, recurrent_state: tuple[torch.Tensor, torch.Tensor] | None=None) -> tuple[torch.Tensor, tuple[torch.Tensor, torch.Tensor]]:
        ...
        if x.shape[2] != self.input_size:
            raise ValueError(f'MaskedStackedLSTM feature size must be {self.input_size}, got {x.shape[2]}')
        ...
        if recurrent_state is not None:
            ...
            for name, state in zip(('hidden', 'cell'), recurrent_state):
                if tuple(state.shape) != state_shape:
                    raise ValueError(f'recurrent {name} must have shape {state_shape}, got {tuple(state.shape)}')
        ...
        packed = nn.utils.rnn.pack_padded_sequence(compressed, valid_lengths.detach().cpu(), batch_first=True, enforce_sorted=False)
        packed_output, next_state = self.lstm(packed, (initial_hidden, initial_cell))
        compressed_output, _ = nn.utils.rnn.pad_packed_sequence(packed_output, batch_first=True, total_length=max_valid)
        valid_rank = torch.cumsum(valid.to(torch.long), dim=1) - 1
        gather_index = valid_rank.clamp_min(0).unsqueeze(-1).expand(-1, -1, self.hidden_size)
        gathered = torch.gather(compressed_output, 1, gather_index)
        initial_top = initial_hidden[-1].unsqueeze(1).expand(-1, position_count, -1)
        output = torch.where((valid_rank >= 0).unsqueeze(-1), gathered, initial_top)
...
class MaskedCLA(nn.Module):
    ...
    def forward(self, fixed: torch.Tensor, observations: torch.Tensor, mask: torch.Tensor, channel_mask: torch.Tensor, recurrent_state: tuple[torch.Tensor, torch.Tensor] | None=None) -> MaskedCLAOutput:
        ...
        for batch_index in range(batch_size):
            ...
            variable_values = torch.cat((observations[batch_index, :count, 0], observations[batch_index, :count, 1], observations.new_zeros(suffix_length)), dim=0)
        ...
        lstm_sequence, next_lstm_state = self.lstm(conv_sequence, feature_mask, recurrent_state)
...
# Fig. 8 learned Kalman-gain state correction.
...
if __name__ == '__main__':
    ...
    KNET_V1_FINE_TUNE_EPOCHS = _env_int('MKNET_V1_FINE_TUNE_EPOCHS', TRAINING_EPOCHS)
    if KNET_V2_WARMUP_EPOCHS <= 0:
        raise ValueError('MKNET_V2_WARMUP_EPOCHS must be positive')
    if KNET_V1_FINE_TUNE_EPOCHS <= 0:
        raise ValueError('MKNET_V1_FINE_TUNE_EPOCHS must be positive')
    LEARNING_RATE = 0.01
    ...
    FDE_STATISTICAL_SELF_CHECK = _validate_ref33_statistical_core(FDE_SIGNIFICANCE_ALPHA) if FDE_DIA_ON else None
    SEED = 0
    ...
    antenna_truth_time = validate_strict_time_axis('training antenna truth', antenna_truth.week.astype(float) * GPS_WEEK_S + antenna_truth.tow_s)
    imu_truth_time = validate_strict_time_axis('training IMU truth', imu_truth.week.astype(float) * GPS_WEEK_S + imu_truth.tow_s)
    ...
    antenna_position, _, _, _, _ = interpolate_ground_truth(antenna_truth, query_time, MAX_TRUTH_INTERPOLATION_GAP_S)
    ...
    for event in training_timeline:
        ...
        history_rows.append({'time': t, 'sat_ids': measurement_model.sat_ids, 'innovation': measurement_model.innovation.copy(), 'residual': posterior_residual.copy(), 'H': measurement_model.H.copy(), 'x_pred': error_state_pred.copy(), 'x_post': error_state_post.copy(), 'accel': last_accel.copy(), 'gyro': last_gyro.copy(), 'target_state_9': target_state_9.copy(), 'posterior_nav': nav.copy(), 'fusion_index': int(fusion_index), 'leo_measurements': tuple(leo_measurements), 'preceding_interval_segments': tuple(interval_segments_since_previous_usable_fusion)})
        interval_segments_since_previous_usable_fusion = []
    ...
    capacity_leo_simulator = LEODownlinkSimulator(tle_provider, leo_klobuchar, seed=LEO_SEED, tx_epsilon_position_m=LEO_TX_EPSILON_POSITION_M, tx_max_iterations=LEO_TX_MAX_ITERATIONS, minimum_elevation_deg=LEO_MIN_ELEVATION_DEG, prefilter_guard_deg=LEO_PREFILTER_GUARD_DEG, use_ionosphere=USE_IONOSPHERE, use_troposphere=USE_TROPOSPHERE)
    data01_sequence_nmax = 0
    for epoch, receiver_position in zip(gnss_epochs, fusion_antenna_truth_position):
        leo_count = len(capacity_leo_simulator.simulate_epoch(float(epoch.time_gpst_s), receiver_position))
        data01_sequence_nmax = max(data01_sequence_nmax, len(epoch.measurements) + leo_count)
    ...
    if FEATURE_STANDARDIZATION_ON:
        ...
        for channel in range(OBSERVATION_FEATURE_DIM):
            values = observations[:, :, channel][channel_masks[:, :, channel]]
            if values.size:
                obs_mean[channel] = float(values.mean())
                std = float(values.std())
                obs_std[channel] = 1.0 if std < 1e-08 else std
        fixed_normalized = (fixed - fixed_mean) / fixed_std
        observations_normalized = (observations - obs_mean.reshape(1, 1, OBSERVATION_FEATURE_DIM)) / obs_std.reshape(1, 1, OBSERVATION_FEATURE_DIM)
        observations_normalized = np.where(channel_masks, observations_normalized, 0.0)
        normalizer_source = 'Data01_only_zscore_numerical_conditioning_paper_feature_information_unchanged'
    else:
        ...
        observations_normalized = np.where(channel_masks, observations, 0.0)
        normalizer_source = 'identity_ablation'
    normalizer = (fixed_mean, fixed_std, obs_mean, obs_std)
    np.savez(OUTPUT_DIR / 'normalizer.npz', fixed_mean=fixed_mean, fixed_std=fixed_std, obs_mean=obs_mean, obs_std=obs_std, source=np.asarray(normalizer_source), feature_standardization_on=np.asarray(FEATURE_STANDARDIZATION_ON))
    ...
    if trajectory_count < 2:
        raise ValueError(f'KalmanNet V2 requires at least two complete short trajectories so the trajectory mini-batch satisfies M < N. Data01 samples={training_sample_count}, MKNET_V2_TRAJECTORY_LENGTH={KNET_V2_TRAJECTORY_LENGTH}. For the current capped diagnostic run, reduce MKNET_V2_TRAJECTORY_LENGTH (for example to 10); for the scientific run, remove the fusion cap and retain the paper-example value 100.')
    TRAJECTORY_BATCH_SIZE = min(KNET_V2_BATCH_SIZE, trajectory_count - 1)
    ...
    print(f"training setup: samples={training_sample_count}, Nmax={network_nmax} (Data01={data01_sequence_nmax}, Data02={data02_sequence_nmax}), mode=KalmanNet_V2_short_trajectories, Tshort={KNET_V2_TRAJECTORY_LENGTH}, trajectories={trajectory_count}, trajectory_batch={TRAJECTORY_BATCH_SIZE}, init=ground_truth_navigation_state, V2_warmup_epochs={KNET_V2_WARMUP_EPOCHS}, V1_finetune_epochs={KNET_V1_FINE_TUNE_EPOCHS}, lr_theta={FILTER_BLOCK_LEARNING_RATE:g}, lr_psi={REPRESENTATION_BLOCK_LEARNING_RATE:g}, FDE={('ON' if FDE_DIA_ON else 'OFF')}")
    ...
    training_history = []
    satellite_masks_t = torch.tensor(satellite_masks, dtype=torch.bool, device=DEVICE)
    channel_masks_t = torch.tensor(channel_masks, dtype=torch.bool, device=DEVICE)
    innovation_base_t = torch.tensor(innovation_padded, dtype=torch.float64, device=DEVICE)
    ...
    def _training_context_from_completed_epoch(sat_ids, posterior_residual, x_pred, x_post, accel, gyro, previous_context=None):
    ...
    def _training_feature_arrays(previous_context, current_sat_ids, innovation_now, current_accel, current_gyro):
        ...
        delta_accel = np.asarray(current_accel, dtype=float).reshape(3) - previous_context['accel']
        delta_gyro = np.asarray(current_gyro, dtype=float).reshape(3) - previous_context['gyro']
        previous_state_innovation = previous_context['state_innovation']
        previous_state_residual = previous_context['state_residual']
        previous_residual_by_sat = dict(zip(previous_context['sat_ids'], previous_context['residual']))
        for slot, sat_id in enumerate(current_sat_ids):
            if sat_id in previous_residual_by_sat:
                residual_k[slot] = previous_residual_by_sat[sat_id]
                residual_mask[slot] = True
        fixed_raw = np.concatenate([delta_accel, delta_gyro, previous_state_residual, previous_state_innovation])
        ...
        obs_nn = (obs_raw - obs_mean.reshape(1, OBSERVATION_FEATURE_DIM)) / obs_std.reshape(1, OBSERVATION_FEATURE_DIM)
        obs_nn = np.where(channel_mask, obs_nn, 0.0)
        if fixed_raw.shape != (FIXED_FEATURE_DIM,) or obs_raw.shape != (n, OBSERVATION_FEATURE_DIM) or (not np.all(np.isfinite(fixed_nn))) or (not np.all(np.isfinite(obs_nn))) or (not np.all(np.isfinite(innovation_k))):
            raise FloatingPointError('non-finite feature generated by actual Data01 closed-loop training')
        return (fixed_nn, obs_nn, current_mask, channel_mask, innovation_k)

    def _pad_training_epoch(fixed_nn, obs_nn, current_mask, channel_mask, innovation_k, slot_capacity: int):
        n = len(current_mask)
        if slot_capacity < n:
        ...
        return (np.asarray(fixed_nn, dtype=float), obs_padded, mask_padded, channel_padded, innovation_padded_now)
    ...
    def _stored_training_context(history_row_index: int):
        ...
        previous = None if history_row_index == 0 else {'x_post': history_rows[history_row_index - 1]['x_post']}
        return _training_context_from_completed_epoch(warm['sat_ids'], warm['residual'], warm['x_pred'], warm['x_post'], warm['accel'], warm['gyro'], previous_context=previous)

    def _navigation_state_finite(nav_state: NavigationState) -> bool:
        return bool(np.all(np.isfinite(nav_state.position_ecef_m)) and np.all(np.isfinite(nav_state.velocity_ecef_mps)) and np.all(np.isfinite(nav_state.body_to_ecef_dcm)))
    ...
    fixed_mean_t = torch.as_tensor(fixed_mean, dtype=torch.float64, device=DEVICE)
    fixed_std_t = torch.as_tensor(fixed_std, dtype=torch.float64, device=DEVICE)
    obs_mean_t = torch.as_tensor(obs_mean, dtype=torch.float64, device=DEVICE)
    obs_std_t = torch.as_tensor(obs_std, dtype=torch.float64, device=DEVICE)

    def _torch_navigation_state_finite(nav_state: TorchNavigationState) -> bool:
        tensors = (nav_state.position_ecef_m, nav_state.velocity_ecef_mps, nav_state.body_to_ecef_dcm)
        return all((bool(torch.all(torch.isfinite(value)).detach().cpu()) for value in tensors))

    def _torch_training_context_from_stored(history_row_index: int):
        base = _stored_training_context(history_row_index)
        return {'sat_ids': tuple(base['sat_ids']), 'residual': torch.as_tensor(base['residual'], dtype=torch.float64, device=DEVICE), 'x_pred': torch.as_tensor(base['x_pred'], dtype=torch.float64, device=DEVICE), 'x_post': torch.as_tensor(base['x_post'], dtype=torch.float64, device=DEVICE), 'state_innovation': torch.as_tensor(base['state_innovation'], dtype=torch.float64, device=DEVICE), 'state_residual': torch.as_tensor(base['state_residual'], dtype=torch.float64, device=DEVICE), 'accel': torch.as_tensor(base['accel'], dtype=torch.float64, device=DEVICE), 'gyro': torch.as_tensor(base['gyro'], dtype=torch.float64, device=DEVICE)}
    ...
    def _torch_training_context_from_completed_epoch(sat_ids, posterior_residual: torch.Tensor, x_pred: torch.Tensor, x_post: torch.Tensor, accel: torch.Tensor, gyro: torch.Tensor, previous_context):
        ...
        finite_tensors = (context['residual'], context['state_innovation'], context['state_residual'], context['accel'], context['gyro'])
        if len(context['sat_ids']) != int(context['residual'].numel()) or not all((bool(torch.all(torch.isfinite(value)).detach().cpu()) for value in finite_tensors)):
            raise FloatingPointError('invalid differentiable Data01 training context')
        return context

    def _torch_measurement_innovation(nav_state: TorchNavigationState, measurements) -> tuple[torch.Tensor, tuple[str, ...]]:
        ...
        satellite_position = torch.as_tensor(np.stack([m.satellite_position_reception_ecef_m for m in measurements], axis=0), dtype=torch.float64, device=DEVICE)
        observed = torch.as_tensor([m.pseudorange_m for m in measurements], dtype=torch.float64, device=DEVICE)
        satellite_clock = torch.as_tensor([m.satellite_clock_bias_s for m in measurements], dtype=torch.float64, device=DEVICE)
        ionosphere = torch.as_tensor([m.ionosphere_delay_m for m in measurements], dtype=torch.float64, device=DEVICE)
        troposphere = torch.as_tensor([m.troposphere_delay_m for m in measurements], dtype=torch.float64, device=DEVICE)
        ...
        projector, _ = _clock_projector_and_biases(measurements, variances, np.zeros(n, dtype=float))
        projector_t = torch.as_tensor(projector, dtype=torch.float64, device=DEVICE)
    ...
    def _torch_training_feature_tensors(previous_context, current_sat_ids, innovation_now: torch.Tensor, current_accel: torch.Tensor, current_gyro: torch.Tensor):
        ...
        if not bool(torch.all(torch.isfinite(fixed_nn)).detach().cpu()) or not bool(torch.all(torch.isfinite(obs_nn)).detach().cpu()):
    ...
    def _rollout_actual_closed_loop_trajectory(start_sample: int, stop_sample: int, *, phase: str | None, backward_scale: float | None=None, trajectory_id: int | None=None, optimizer_step_index: int | None=None, initialization_mode: str='stored_causal'):
        ...
        if initialization_mode == 'kalmannet_v2_truth':
            ...
            boundary_epoch = gnss_epochs[boundary_fusion_index]
            boundary_gnss = gnss_preprocessor.prepare_epoch(boundary_epoch, nav_train.detached_numpy(), lever_arm_b_m)
            boundary_measurements = retain_clock_observable_measurements(tuple(boundary_gnss) + tuple(boundary_row['leo_measurements']))
            boundary_residual, boundary_sat_ids = _torch_measurement_innovation(nav_train, boundary_measurements)
            zero_state = torch.zeros(INS_STATE_DIM, dtype=torch.float64, device=DEVICE)
            previous_context = {'sat_ids': tuple(boundary_sat_ids), 'residual': boundary_residual, 'x_pred': zero_state.clone(), 'x_post': zero_state.clone(), 'state_innovation': zero_state.clone(), 'state_residual': zero_state.clone(), 'accel': stored_boundary_context['accel'], 'gyro': stored_boundary_context['gyro']}
        else:
        ...
        with grad_context:
            for sample_index in range(start_sample, stop_sample):
                ...
                max_prior_position_error_m = max(max_prior_position_error_m, float(torch.linalg.vector_norm(prior_position_error).detach().cpu()))
                epoch = gnss_epochs[fusion_index]
                gnss_measurements = gnss_preprocessor.prepare_epoch(epoch, nav_train.detached_numpy(), lever_arm_b_m)
                measurements = retain_clock_observable_measurements(tuple(gnss_measurements) + tuple(row['leo_measurements']))
                if not measurements:
                    skipped_no_measurements += 1
                    continue
                innovation_now, current_sat_ids = _torch_measurement_innovation(nav_train, measurements)
                ...
                total_count += 1
                max_position_correction_norm_m = max(max_position_correction_norm_m, float(torch.linalg.vector_norm(correction64[0:3]).detach().cpu()))
                max_velocity_correction_norm_mps = max(max_velocity_correction_norm_mps, float(torch.linalg.vector_norm(correction64[3:6]).detach().cpu()))
                max_attitude_correction_norm_rad = max(max_attitude_correction_norm_rad, float(torch.linalg.vector_norm(correction64[6:9]).detach().cpu()))
                nav_train = _torch_inject_error_state(nav_train, correction64)
                posterior_residual, posterior_sat_ids = _torch_measurement_innovation(nav_train, measurements)
                if posterior_sat_ids != current_sat_ids:
                    raise RuntimeError('training satellite order changed within one fusion update')
                previous_context = _torch_training_context_from_completed_epoch(current_sat_ids, posterior_residual, torch.zeros(INS_STATE_DIM, dtype=torch.float64, device=DEVICE), correction64, last_feature_accel, last_feature_gyro, previous_context=previous_context)
        if total_count <= 0:
        ...
        return {'trajectory_id': None if trajectory_id is None else int(trajectory_id), 'start_sample': int(start_sample), 'stop_sample': int(stop_sample), 'trajectory_eq30': trajectory_eq30, 'state_sum': float(total_state_sum), 'position_sum': float(total_position_sum), 'learned_updates': int(total_count), 'skipped_no_measurements': int(skipped_no_measurements), 'max_prior_position_error_m': float(max_prior_position_error_m), 'max_innovation_abs_m': float(max_innovation_abs_m), 'max_gain_fro_norm': float(max_gain_fro_norm), 'max_position_correction_norm_m': float(max_position_correction_norm_m), 'max_velocity_correction_norm_mps': float(max_velocity_correction_norm_mps), 'max_attitude_correction_norm_rad': float(max_attitude_correction_norm_rad)}

    def _active_gradient_finite(phase: str) -> bool:
        parameters = filter_parameters if phase == 'filter' else representation_parameters
        return all((parameter.grad is None or bool(torch.all(torch.isfinite(parameter.grad)).detach().cpu()) for parameter in parameters))

    def _run_training_phase(phase: str, optimizer: torch.optim.Optimizer, epoch: int, trajectory_order: np.ndarray):
        """Run one alternating-optimization phase over KalmanNet-V2 short trajectories."""
        ...
        for batch_start in range(0, trajectory_count, TRAJECTORY_BATCH_SIZE):
            batch_ids = trajectory_order[batch_start:batch_start + TRAJECTORY_BATCH_SIZE]
            ...
            for trajectory_id_raw in batch_ids:
                ...
                metrics = _rollout_actual_closed_loop_trajectory(start_sample, stop_sample, phase=phase, backward_scale=1.0 / actual_batch_size, trajectory_id=trajectory_id, optimizer_step_index=optimizer_steps, initialization_mode='kalmannet_v2_truth')
            ...
            if not torch.isfinite(regularization):
                raise FloatingPointError(f'non-finite {phase} Eq.(32) regularization')
            if GAMMA_L2:
                regularization.backward()
            if not _active_gradient_finite(phase):
                raise FloatingPointError(f'non-finite {phase} gradient before optimizer step {optimizer_steps}')
            ...
            if not math.isfinite(batch_objective):
                raise FloatingPointError(f'non-finite {phase} trajectory-batch objective before step {optimizer_steps}')
            optimizer.step()
            optimizer_steps += 1
        ...
        if total_count <= 0 or not trajectory_eq30_values:
            raise RuntimeError('KalmanNet-V2 Data01 actual-forward training produced no updates')
        metrics = {'eq30': float(total_state_sum / total_count), 'position_rmse_m': math.sqrt(total_position_sum / total_count), 'trajectory_mean_eq30': float(np.mean(trajectory_eq30_values)), 'objective': float(batch_objective_weighted_sum / batch_objective_weight), 'learned_updates': int(total_count), 'skipped_no_measurements': int(total_skipped), 'optimizer_steps': int(optimizer_steps), 'trajectory_count': int(trajectory_count), 'sequence_batch_size': 1}
        metrics.update(maxima)
        metrics['epoch'] = int(epoch)
        return metrics

    def _run_v1_training_phase(phase: str, optimizer: torch.optim.Optimizer, epoch: int):
        """KalmanNet V1: BPTT through the complete chronological Data01 trajectory."""
        _set_alternating_phase(phase)
        optimizer.zero_grad(set_to_none=True)
        metrics = _rollout_actual_closed_loop_trajectory(0, training_sample_count, phase=phase, backward_scale=1.0, trajectory_id=0, optimizer_step_index=0, initialization_mode='kalmannet_v2_truth')
        regularization = _eq32_regularization()
        if not torch.isfinite(regularization):
    ...
    def _evaluate_teacher_forced_eq30():
        ...
        with torch.inference_mode():
            for sample_index in range(training_sample_count):
                fixed_eval, obs_eval, mask_eval, channel_eval, innovation_eval = _pad_training_epoch(fixed_normalized[sample_index], observations_normalized[sample_index], satellite_masks[sample_index], channel_masks[sample_index], innovation_padded[sample_index], network_nmax)
                output = model(torch.tensor(fixed_eval[None], dtype=torch.float32, device=DEVICE), torch.tensor(obs_eval[None], dtype=torch.float32, device=DEVICE), torch.tensor(mask_eval[None], dtype=torch.bool, device=DEVICE), torch.tensor(channel_eval[None], dtype=torch.bool, device=DEVICE), recurrent_state=recurrent_state)
                recurrent_state = output.recurrent_state
                correction = fig8_state_update(output, torch.tensor(innovation_eval[None], dtype=torch.float32, device=DEVICE))[0].double()
    ...
    print(f"pre-training continuous zero-K Data01 baseline: Eq30={pretraining_zero_gain_metrics['eq30']:.6g}, posRMSE={pretraining_zero_gain_metrics['position_rmse_m']:.3f} m, maxInnovation={pretraining_zero_gain_metrics['max_innovation_abs_m']:.3f} m, maxPosCorr={pretraining_zero_gain_metrics['max_position_correction_norm_m']:.3f} m")
    print('\n=== DATA01 TRAINING ===')
    training_history.append({'stage': 'pretraining_continuous_zero_gain_data01_baseline', 'eq30': float(pretraining_zero_gain_metrics['eq30']), 'position_rmse_m': float(pretraining_zero_gain_metrics['position_rmse_m']), 'max_innovation_abs_m': float(pretraining_zero_gain_metrics['max_innovation_abs_m']), 'max_gain_fro_norm': float(pretraining_zero_gain_metrics['max_gain_fro_norm']), 'max_position_correction_norm_m': float(pretraining_zero_gain_metrics['max_position_correction_norm_m']), 'learned_updates': int(pretraining_zero_gain_metrics['learned_updates'])})
    final_training_metrics = None
    epochs_ran = 0
    print('\n--- KalmanNet V2 warm-up ---')
    for epoch in range(1, KNET_V2_WARMUP_EPOCHS + 1):
        epochs_ran += 1
        trajectory_order = np.random.permutation(trajectory_count).astype(int)
        filter_phase = _run_training_phase('filter', filter_optimizer, epoch, trajectory_order)
        representation_phase = _run_training_phase('representation', representation_optimizer, epoch, trajectory_order)
        ...
        training_history.append({'stage': 'kalmannet_v2_warmup_yan_masked_cla_latent_alternating', 'epoch': epoch, 'global_training_epoch': epochs_ran, 'sequence_length': int(training_sample_count), 'kalmannet_v2_short_trajectory_length': int(KNET_V2_TRAJECTORY_LENGTH), 'kalmannet_v2_initialization': 'ground_truth_navigation_state', 'kalmannet_v2_trained_sample_count': int(v2_trained_sample_count), 'kalmannet_v2_initialization_sample_count': int(v2_initialization_sample_count), 'kalmannet_v2_unused_tail_sample_count': int(v2_unused_sample_count), 'trajectory_count': int(trajectory_count), 'trajectory_batch_size': int(TRAJECTORY_BATCH_SIZE), 'filter_phase_total_objective': filter_phase['objective'], 'filter_phase_eq30': filter_phase['eq30'], 'filter_phase_trajectory_mean_eq30': filter_phase['trajectory_mean_eq30'], 'filter_phase_position_rmse_m': filter_phase['position_rmse_m'], 'filter_phase_learned_updates': filter_phase['learned_updates'], 'filter_phase_optimizer_steps': filter_phase['optimizer_steps'], 'filter_phase_skipped_no_measurements': filter_phase['skipped_no_measurements'], 'filter_phase_max_innovation_abs_m': filter_phase['max_innovation_abs_m'], 'filter_phase_max_gain_fro_norm': filter_phase['max_gain_fro_norm'], 'filter_phase_max_position_correction_norm_m': filter_phase['max_position_correction_norm_m'], 'representation_phase_total_objective': representation_phase['objective'], 'representation_phase_eq30': representation_phase['eq30'], 'representation_phase_trajectory_mean_eq30': representation_phase['trajectory_mean_eq30'], 'representation_phase_position_rmse_m': representation_phase['position_rmse_m'], 'representation_phase_learned_updates': representation_phase['learned_updates'], 'representation_phase_optimizer_steps': representation_phase['optimizer_steps'], 'representation_phase_skipped_no_measurements': representation_phase['skipped_no_measurements'], 'representation_phase_max_innovation_abs_m': representation_phase['max_innovation_abs_m'], 'representation_phase_max_gain_fro_norm': representation_phase['max_gain_fro_norm'], 'representation_phase_max_position_correction_norm_m': representation_phase['max_position_correction_norm_m'], 'filter_block_learning_rate': current_filter_lr, 'representation_block_learning_rate': current_representation_lr})
        print(f"V2 epoch {epoch:03d}/{KNET_V2_WARMUP_EPOCHS}: theta_Eq30={filter_phase['trajectory_mean_eq30']:.6g}, theta_RMSE={filter_phase['position_rmse_m']:.3f}m, theta_maxK={filter_phase['max_gain_fro_norm']:.3g}, theta_maxInnov={filter_phase['max_innovation_abs_m']:.3g}m, theta_maxCorr={filter_phase['max_position_correction_norm_m']:.3g}m; psi_Eq30={representation_phase['trajectory_mean_eq30']:.6g}, psi_RMSE={representation_phase['position_rmse_m']:.3f}m, psi_maxK={representation_phase['max_gain_fro_norm']:.3g}, psi_maxInnov={representation_phase['max_innovation_abs_m']:.3g}m, psi_maxCorr={representation_phase['max_position_correction_norm_m']:.3g}m")
    ...
    best_val_position_rmse_m = float(final_training_metrics['position_rmse_m'])
    selected_training_stage = 'kalmannet_V2_warmup_then_V1_full_sequence_tuning_yan_eq30_latent_alternating_final_epoch'
    ...
    def _online_feature_arrays(previous_online, current_sat_ids, innovation_now, current_accel, current_gyro):
        ...
        obs_k = (obs_k_raw - obs_mean.reshape(1, 2)) / obs_std.reshape(1, 2)
    ...
    model.eval()
    torch.save({'model_state_dict': {key: value.detach().cpu().clone() for key, value in model.state_dict().items()}, 'checkpoint_schema': 'v34_9_state_packed_masked_lstm_latent_alternating', 'nmax': int(network_nmax), 'training_observed_nmax': int(nmax), 'data01_sequence_nmax': int(data01_sequence_nmax), 'data02_sequence_nmax': int(data02_sequence_nmax), 'nmax_source': 'max_observed_sequence_length_across_selected_Data01_Data02', 'state_order': '[delta_p,delta_v,delta_theta]', 'kalman_gain_state_dimension': INS_STATE_DIM, 'direct_state_label_dimension': SUPERVISED_STATE_DIM, 'fixed_mean': fixed_mean, 'fixed_std': fixed_std, 'obs_mean': obs_mean, 'obs_std': obs_std, 'feature_standardization_on': bool(FEATURE_STANDARDIZATION_ON), 'normalizer_source': normalizer_source, 'alternating_partition': 'psi=masked_conv;theta=masked_lstm+attention+gain_head', 'latent_kalmannet_warm_start': 'not_applied_no_published_supervised_target_for_intermediate_Yan_CNN_features', 'training_stage_selected': selected_training_stage, 'training_epochs': best_epoch, 'requires_recursive_Data01_gate_before_Data02': True, 'sequence_training': 'Yan_Fig7_Fig8_order_adapted_to_user_requested_9_state_branch;one_continuous_Data01_learned_closed_loop_sequence_per_alternating_phase;learned_posterior_to_ECEF_INS_to_next_features;no_internal_classical_KF_reset;no_Phi_H_surrogate;satellite_preprocessing_only_detached', 'training_sequence_length': int(training_sample_count), 'training_sequence_count': 1, 'sequence_batch_size': 1, 'optimizer_step_inside_sequence': False, 'cross_fusion_navigation_gradient': 'KalmanNet_guided_BPTT_through_9_state_correction_ECEF_INS_and_innovation', 'cross_fusion_neural_gradient': 'LSTM_h_c_BPTT_across_continuous_Data01_sequence', 'synthetic_prior_augmentation': False, 'lstm_sequence_state': 'final_valid_hc_carried_between_all_Xbar_k_calls_in_Data01_sequence', 'external_filter_state': 'recursive_across_complete_Data01_training_sequence_via_actual_INS;no_internal_classical_KF_reset', 'final_epoch': best_epoch, 'final_train_recursive_eq30_state_loss': best_val_state_loss, 'final_train_recursive_position_rmse_m': best_val_position_rmse_m, 'final_clean_teacher_forced_eq30': float(final_teacher_forced_metrics['eq30']), 'final_clean_teacher_forced_position_rmse_m': float(final_teacher_forced_metrics['position_rmse_m']), 'state_target_scaling': 'none_raw_physical_units_per_Yan_Eq30', 'measurement_mode': 'pseudorange_only', 'observation_residual_feature': 'Yan_Eq11_Delta_y_k_minus_1;same_epoch_Delta_y_k_not_used_because_it_requires_the_current_posterior_and_would_make_K_k_input_circular'}, OUTPUT_DIR / 'best_model.pt')
    ...
    if RUN_RECURSIVE_DATA01_DIAGNOSTIC:
        ...
        try:
            for event in build_exact_fusion_timeline(imr_time, fusion_time, through_last_fusion=True):
                ...
                if diag_previous is None:
                    ...
                    continue
                if FDE_DIA_ON:
                    diag_fde = ref33_fde_dia_decision(diag_nav, diag_P, diag_measurements, lever_arm_b_m, FDE_SIGNIFICANCE_ALPHA)
                ...
                if FDE_DIA_ON and (not diag_measurements):
                    ...
                    diag_rows.append({'time': t, 'position': posterior_position.copy(), 'truth': truth_position_now.copy(), 'prior_error_3d_m': prior_error_3d_m, 'posterior_error_3d_m': prior_error_3d_m, 'classical_posterior_error_3d_m': classical_error_3d_m, 'spectral_radius_knet': float('nan'), 'gain_fro_norm': float('nan'), 'correction_position_norm_m': 0.0, 'prefde_innovation_max_abs_m': float(np.max(np.abs(diag_prefde_model.innovation))) if len(diag_prefde_model.innovation) else 0.0, 'obs_max_abs_train_ratio': float('nan')})
                ...
                rho_knet = closed_loop_spectral_radius(active_gain, diag_model.H)
        ...
        diag_fields = ('time', 'prior_error_3d_m', 'posterior_error_3d_m', 'classical_posterior_error_3d_m', 'spectral_radius_knet', 'gain_fro_norm', 'correction_position_norm_m', 'prefde_innovation_max_abs_m', 'obs_max_abs_train_ratio')
        ...
        diag_rho = np.asarray([row['spectral_radius_knet'] for row in diag_rows], dtype=float)
    ...
    if first_test_rinex_epoch is None:
        raise ValueError('Test RINEX file contains no GPS/BDS epochs')
    test_imr_header, test_imr_tow_all, test_imr_record_count = read_imr_tow_only(test_files.imr)
    ...
    test_antenna_truth_time = validate_strict_time_axis('test antenna truth', test_antenna_truth.week.astype(float) * GPS_WEEK_S + test_antenna_truth.tow_s)
    test_imu_truth_time = validate_strict_time_axis('test IMU truth', test_imu_truth.week.astype(float) * GPS_WEEK_S + test_imu_truth.tow_s)
    ...
    test_antenna_position, _, _, _, _ = interpolate_ground_truth(test_antenna_truth, test_query_time, MAX_TRUTH_INTERPOLATION_GAP_S)
    ...
    for event in test_timeline:
        ...
        if previous_online is None:
            ...
            previous_online = _make_online_context(warm_measurement_model.sat_ids, warm_posterior_residual, np.zeros(INS_STATE_DIM), warm_correction, last_feature_accel, last_feature_gyro, previous_context=None)
            ...
            continue
        if FDE_DIA_ON:
            fde_result = ref33_fde_dia_decision(nav, P, measurements, test_lever_arm_b_m, FDE_SIGNIFICANCE_ALPHA)
        ...
        Q_classical = measurement_model.H @ P_prior_update @ measurement_model.H.T + measurement_model.R
        K_classical = P_prior_update @ measurement_model.H.T @ np.linalg.pinv(Q_classical, rcond=1e-12)
        ...
        P = learned_gain_covariance_update(P_prior_update, active_gain, measurement_model.H, measurement_model.R, correction)
        learned_error_state_post = learned_error_state_pred + correction
        nav = inject_error_state(nav, correction)
        posterior_position = gnss_antenna_position(nav, test_lever_arm_b_m)
        posterior_error_3d_m = float(np.linalg.norm(posterior_position - truth_position_now))
        ...
        online_rows.append({'time': t, 'position': posterior_position.copy(), 'truth': test_fusion_antenna_truth_position[fusion_index].copy(), 'hpl_m': hpl_m, 'vpl_m': vpl_m, 'spectral_radius_knet': rho_knet, 'spectral_radius_classical': rho_classical, 'n_input': len(fde_result.tested_measurements) if FDE_DIA_ON else len(measurements), 'n_total': n, 'n_gnss': counts['G'] + counts['C'], 'n_leo': counts['L'], 'sat_ids': current_sat_ids, 'prior_error_3d_m': prior_error_3d_m, 'posterior_error_3d_m': posterior_error_3d_m, 'prefde_innovation_rms_m': prefde_innovation_rms_m, 'prefde_innovation_max_abs_m': prefde_innovation_max_abs_m, 'knet_innovation_rms_m': knet_innovation_rms_m, 'knet_innovation_max_abs_m': knet_innovation_max_abs_m, 'gain_fro_norm': gain_fro_norm, 'gain_max_abs': gain_max_abs, 'gain_navigation_rows_fro_norm': gain_navigation_rows_fro_norm, 'correction_position_norm_m': correction_position_norm_m, 'correction_velocity_norm_mps': correction_velocity_norm_mps, 'correction_attitude_norm_rad': correction_attitude_norm_rad, **feature_shift, **_fde_export_fields(fde_result), 'fusion_mode': 'MaskedCLA_post_FDE_DIA' if FDE_DIA_ON else 'MaskedCLA_no_FDE_DIA'})
    if not online_rows:
    ...
    spectral_radius_knet = np.asarray([row['spectral_radius_knet'] for row in online_rows], dtype=float)
    spectral_radius_classical = np.asarray([row['spectral_radius_classical'] for row in online_rows], dtype=float)
    ...
    divergence_diagnostics = {'spectral_radius_status': 'unavailable_Yan_Fig10_square_operator_not_published', 'first_spectral_radius_gt_1': _first_true_event(spectral_radius_knet > 1.0), 'first_error_gt_100m': _first_true_event(error_3d > 100.0), 'first_error_gt_1km': _first_true_event(error_3d > 1000.0), 'first_error_gt_100km': _first_true_event(error_3d > 100000.0), 'first_error_growth_gt_10x': _first_true_event(growth_ratio > 10.0), 'first_obs_feature_gt_10x_training_absmax': _first_true_event(diagnostic_arrays['obs_max_abs_train_ratio'] > 10.0), 'max_prefde_innovation_abs_m': float(np.nanmax(diagnostic_arrays['prefde_innovation_max_abs_m'])), 'max_knet_innovation_abs_m': float(np.nanmax(diagnostic_arrays['knet_innovation_max_abs_m'])), 'max_gain_fro_norm': float(np.nanmax(diagnostic_arrays['gain_fro_norm'])), 'max_position_correction_norm_m': float(np.nanmax(diagnostic_arrays['correction_position_norm_m'])), 'max_fixed_feature_abs_training_ratio': float(np.nanmax(diagnostic_arrays['fixed_max_abs_train_ratio'])), 'max_observation_feature_abs_training_ratio': float(np.nanmax(diagnostic_arrays['obs_max_abs_train_ratio']))}
    np.savez(OUTPUT_DIR / 'test_evaluation.npz', dataset_dir=np.asarray(str(test_dataset_dir)), time_gpst_s=online_time, estimate_ecef_m=estimate, truth_ecef_m=truth_aligned, ned_error_m=ned_error, error_3d_m=error_3d, horizontal_error_m=horizontal_error, vertical_error_m=vertical_error, hpl_m=hpl, vpl_m=vpl, spectral_radius_knet=spectral_radius_knet, spectral_radius_classical=spectral_radius_classical, **diagnostic_arrays, error_growth_ratio=growth_ratio, rmse_north_east_down_3d_m=rmse, cdf_probability=cdf_probability, north_cdf_absolute_error_m=np.sort(np.abs(ned_error[:, 0])), east_cdf_absolute_error_m=np.sort(np.abs(ned_error[:, 1])), down_cdf_absolute_error_m=np.sort(np.abs(ned_error[:, 2])), three_d_cdf_error_m=np.sort(error_3d))
    with (OUTPUT_DIR / 'test_trajectory.csv').open('w', newline='', encoding='utf-8') as stream:
        writer = csv.writer(stream)
        writer.writerow(['time_gpst_s', 'n_input_before_fde', 'n_k_used_after_fde', 'n_gnss', 'n_leo', 'sat_ids_used', 'fde_detected', 'fde_identified_sat_ids', 'fde_excluded_sat_ids', 'fde_ambiguous_sat_ids', 'fde_statistic', 'fde_threshold', 'fde_dof', 'fde_post_exclusion_statistic', 'fde_post_exclusion_threshold', 'fde_post_exclusion_dof', 'fde_post_exclusion_consistent', 'fde_local_identification_score', 'fde_estimated_fault_m', 'fde_hard_exclusion_applied', 'fde_state_correction_norm', 'fde_unresolved', 'fusion_mode', 'hpl_m', 'vpl_m', 'spectral_radius_knet', 'spectral_radius_classical', 'x_ecef_m', 'y_ecef_m', 'z_ecef_m'])
        for row in online_rows:
            writer.writerow([row['time'], row['n_input'], row['n_total'], row['n_gnss'], row['n_leo'], ';'.join(row['sat_ids']), int(row['fde_detected']), ';'.join(row['fde_identified_sat_ids']), ';'.join(row['fde_excluded_sat_ids']), ';'.join(row['fde_ambiguous_sat_ids']), row['fde_statistic'], row['fde_threshold'], row['fde_dof'], row['fde_post_exclusion_statistic'], row['fde_post_exclusion_threshold'], row['fde_post_exclusion_dof'], int(row['fde_post_exclusion_consistent']), row['fde_local_identification_score'], row['fde_estimated_fault_m'], int(row['fde_hard_exclusion_applied']), row['fde_state_correction_norm'], int(row['fde_unresolved']), row['fusion_mode'], row['hpl_m'], row['vpl_m'], row['spectral_radius_knet'], row['spectral_radius_classical'], *row['position'].tolist()])
    with (OUTPUT_DIR / 'test_diagnostics.csv').open('w', newline='', encoding='utf-8') as stream:
        writer = csv.writer(stream)
        writer.writerow(['time_gpst_s', *diagnostic_fields, 'spectral_radius_knet', 'fde_detected', 'fde_excluded_sat_ids'])
        for row in online_rows:
            writer.writerow([row['time'], *[row[name] for name in diagnostic_fields], row['spectral_radius_knet'], int(row['fde_detected']), ';'.join(row['fde_excluded_sat_ids'])])
    ...
    if RUN_CLASSICAL_TEST_BASELINE:
        ...
        for baseline_event in build_exact_fusion_timeline(test_imr_time, test_fusion_time, through_last_fusion=True):
            ...
            if baseline_measurements:
                if FDE_DIA_ON:
                    baseline_fde = ref33_fde_dia_decision(baseline_nav, baseline_P, baseline_measurements, test_lever_arm_b_m, FDE_SIGNIFICANCE_ALPHA)
