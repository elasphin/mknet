...
# in the learned measurement updates; the full covariance still propagates online.
...
def kalman_measurement_update(P: Array, innovation: Array, H: Array, R: Array):
    """Reference TC update used only to instantiate offline Eqs. (10)-(14).
    ...
    pass is therefore a minimal feature-history completion and is never used as the
    online estimator. The GPS/BDS clock projection makes S singular in removed clock
    directions, so a Moore-Penrose inverse is used.
    """
    PHt = P @ H.T
    S = H @ PHt + R
    S = 0.5 * (S + S.T)
...
def fig8_post_update_error_state_9(
    ...
    Yan et al. define the navigation error state with position, velocity and
    attitude-error components and train with a squared Euclidean state error.
    ``true_error_state_9`` and the first nine entries of the learned state update
    use that same error-state parameterization, so the paper-faithful loss is the
    direct component-wise residual rather than an additional SO(3) geodesic loss.
    """
...
if __name__ == "__main__":
    ...
    def _state_prediction_loss(predicted_update, true_error_state_9):
        ...
        # coordinates. The target and learned update share the same 9-state
        # error-state parameterization here.
    ...
    diagnostic_fields = (
        ...
        "correction_attitude_norm_rad", "eta_gyro_norm_radps",
    ...
    summary = {
        ...
        "unavoidable_unpublished_completions": [
            ...
            "bias_truth_for_the_six_IMU_bias_states",
        ],
        "dataset_protocol": {
