...
This revision keeps the non-invasive divergence diagnostics but corrects two
training-side issues exposed by the recursive Data01 runs:
- eta_k is no longer trained with an extra direct sensor-error MSE absent from
  Yan Eq. (32); its unpublished timing is completed through the next-interval
...
- a Data01-only affine input standardization is applied as numerical conditioning
  ...
  definitions nor any navigation/FDE/state-update equation, and is explicitly
  labelled as an unpublished numerical completion.

This CNN/LSTM revision additionally aligns the Masked-CLA block more closely with
Yan Eqs. (22)-(25), Fig. 8 and Table III: the explicit Pooling/Flatten stages are
restored, and the five-layer masked LSTM now carries h/c chronologically from
fusion epoch k-1 to k while M[k,t] gates every padded observation slot t. Training
therefore uses consecutive temporal chunks rather than shuffled independent
epochs. The exact pooling operator remains unpublished; the implemented same-
length masked max-pool is explicitly labelled as a figure-guided completion.

The recursive Data01 rollout remains diagnostic only and never updates a network
parameter or feeds information into Data02.
...
#   Fig. 8: convolution -> pooling -> flatten -> masked LSTM-attention -> masked FC
#   Table III: 24 Conv1D filters, stride 1, ReLU, 64 LSTM units, 5 layers,
#              dropout 0.2 and sigmoid LSTM gates.
#
# Important notation/alignment used here:
#   - k is the fusion-epoch/time index.
#   - t is the padded observation-slot index inside an epoch.
#   - The CNN operates across the padded observation-slot axis t at each epoch.
#   - The LSTM state is propagated across successive fusion epochs k and is gated
#     independently for every observation slot t by M[k,t], matching Eq. (25).
#     This avoids the previous implementation error where the LSTM was reset at
#     every fusion epoch and instead recurred only across satellite slots.
#
...
#   - Fig. 8 shows pooling but gives no type/kernel/stride.  A shape-preserving
#     masked max-pool (kernel 3, stride 1) is used so Eq. (23) can still keep
#     M_out == M_in; this is a declared figure-guided completion, not author data
#   - Fig. 8's "Flatten" is implemented as the per-slot flattening of the 24 CNN
#     feature maps into a 24-element vector before the LSTM; no observation slot
#     is collapsed or removed, so the mask remains one-to-one with the input
...
@dataclass(frozen=True)
class MaskedLSTMState:
    """Recurrent Masked-LSTM state carried between consecutive fusion epochs.

    hidden/cell shapes are [num_layers, slot_count, hidden_size].
    The slot axis corresponds to the padded observation index t in M[k,t].
    """

    hidden: torch.Tensor
    cell: torch.Tensor

    def detached(self) -> "MaskedLSTMState":
        return MaskedLSTMState(
            self.hidden.detach(),
            self.cell.detach(),
        )
...
class MaskedCLAOutput:
    """One chronological Masked-CLA chunk.

    The first tensor dimension is a chronological fusion-epoch chunk, not a set
    of exchangeable independent samples.  This is required so the LSTM can carry
    h/c from epoch k-1 to epoch k.

    Shapes
    ------
    kalman_gain : [T, 15, N]
    imu_error   : [T, 6] -- eta_k = [epsilon_g(3) rad/s, epsilon_a(3) m/s^2]
    attention   : [T, N]
    lstm_state  : recurrent state after the final epoch in the chunk
    """

    kalman_gain: torch.Tensor
    imu_error: torch.Tensor
    attention: torch.Tensor
    lstm_state: MaskedLSTMState


class MaskedConv1d(nn.Module):
    """Yan Eqs. (22)-(23) plus the Pooling/Flatten stages drawn in Fig. 8.

    The convolution is the paper-specified Conv1D with 24 filters, kernel width
    3, stride 1 and ReLU.  The valid-window normalization is mask-aware, so zero
    padding cannot bias boundary features.

    Fig. 8 additionally shows Pooling followed by Flatten, but the paper does not
    publish the pooling operator or its hyperparameters.  The minimal completion
    used here is a shape-preserving masked max-pool with kernel=3, stride=1.  It
    keeps the observation-slot axis unchanged, which is necessary for the
    published Eq. (23), M_out[k,t] = M_in[k,t].  "Flatten" is the final transpose
    from CNN feature maps [T,C,N] to per-slot vectors [T,N,C].
    """
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        if x.ndim != 3 or mask.ndim != 2:
            raise ValueError("MaskedConv1d expects x=[T,N,C] and mask=[T,N]")
        if x.shape[:2] != mask.shape:
            raise ValueError("MaskedConv1d x/mask sequence shapes do not match")

        m = mask.to(dtype=x.dtype).unsqueeze(1)  # [T,1,N]

        # Eq. (22): first suppress padded inputs, then convolve only the valid
        # observation window.  Batch dimension here is the chronological epoch
        # chunk; Conv1D itself is applied independently at each epoch.
        x_masked = x.transpose(1, 2) * m         # [T,Cin,N]
        z = F.conv1d(
            x_masked,
            self.weight,
            bias=None,
        ...
        # Fig. 8 Pooling.  Type/kernel/stride are not published.  A same-length
        # masked max-pool is the least invasive explicit stage: padded values are
        # never candidates and the published mask length is preserved.
        ...
        # Fig. 8 Flatten: each slot t receives the 24 pooled CNN feature maps as
        # one feature vector.  No slot is removed, so M_out == M_in (Eq. 23).
        return feature_maps.transpose(1, 2)       # [T,N,24]


class MaskedStackedLSTM(nn.Module):
    """Yan Eqs. (24)-(25): five-layer temporal masked LSTM.

    In Yan's notation k is the fusion epoch and t is the padded observation-slot
    index.  Eq. (25) therefore gates the recurrent state at every slot t while
    carrying that state from epoch k-1 to k.  The previous implementation reset
    h/c for every epoch and iterated the LSTM across satellite slots; this class
    corrects that axis.

    Input
    -----
    x    : [T,N,C] chronological fusion-epoch chunk
    mask : [T,N]   M[k,t]
    state: optional h/c from the immediately preceding chronological chunk

    Output
    ------
    h_out      : [T,N,H], top-layer hidden state for every epoch/slot
    next_state : h/c after the last epoch, carried to the next chunk or online
                 fusion epoch.
    """
    ...
    def __init__(
        ...
        # nn.LSTMCell implements the standard Eq. (24) sigmoid input/forget/output
        # gates and tanh candidate/cell output.  Five cells are stacked to match
        # Table III's "5 hidden layers".
    ...
    def _initial_or_resized_state(
        self,
        x: torch.Tensor,
        slot_count: int,
        state: MaskedLSTMState | None,
    ) -> tuple[list[torch.Tensor], list[torch.Tensor], int]:
        if state is None:
            working_slots = int(slot_count)
            hidden = x.new_zeros(
                self.num_layers, working_slots, self.hidden_size
            )
            cell = x.new_zeros(
                self.num_layers, working_slots, self.hidden_size
            )
        else:
            hidden = state.hidden.to(dtype=x.dtype, device=x.device)
            cell = state.cell.to(dtype=x.dtype, device=x.device)
            if (
                hidden.ndim != 3
                or cell.ndim != 3
                or hidden.shape != cell.shape
                or hidden.shape[0] != self.num_layers
                or hidden.shape[2] != self.hidden_size
            ):
                raise ValueError(
                    "MaskedLSTMState must have shape "
                    f"[{self.num_layers},N,{self.hidden_size}]"
                )

            working_slots = max(int(slot_count), int(hidden.shape[1]))
            if hidden.shape[1] < working_slots:
                pad_slots = working_slots - int(hidden.shape[1])
                hidden = F.pad(hidden, (0, 0, 0, pad_slots))
                cell = F.pad(cell, (0, 0, 0, pad_slots))

        return list(hidden.unbind(0)), list(cell.unbind(0)), working_slots

    def forward(
        self,
        x: torch.Tensor,
        mask: torch.Tensor,
        state: MaskedLSTMState | None = None,
    ) -> tuple[torch.Tensor, MaskedLSTMState]:
        if x.ndim != 3 or mask.ndim != 2:
            raise ValueError(
                "MaskedStackedLSTM expects x=[T,N,C] and mask=[T,N]"
            )
        if x.shape[:2] != mask.shape:
            raise ValueError(
        ...
        time_steps, slot_count = x.shape[:2]
        if time_steps <= 0 or slot_count <= 0:
            raise ValueError("MaskedStackedLSTM requires T>0 and N>0")

        h, c, working_slots = self._initial_or_resized_state(
            x, slot_count, state
        )

        # If online N temporarily shrinks, preserve the hidden states of the
        # trailing slots by appending masked zero inputs.  When N later grows,
        # those states are still available.  Training uses fixed Nmax, so this
        # branch is only a variable-measurement-count completion.
        if working_slots > slot_count:
            pad_slots = working_slots - int(slot_count)
            x_work = F.pad(x, (0, 0, 0, pad_slots))
            mask_work = F.pad(mask, (0, pad_slots), value=False)
        else:
            x_work = x
            mask_work = mask
        ...
        # Temporal recurrence is across fusion epoch k.  All observation slots t
        # are evaluated in parallel as the LSTMCell batch dimension.
        for k in range(time_steps):
            mk = mask_work[k].to(dtype=x.dtype).unsqueeze(-1)  # [N,1]
            keep = 1.0 - mk
            layer_input = x_work[k]                            # [N,C]

            for layer, cell_module in enumerate(self.cells):
                h_candidate, c_candidate = cell_module(
                    layer_input,
                    (h[layer], c[layer]),
                )

                # Paper Eq. (25): padded positions retain their previous h/c.
                h[layer] = mk * h_candidate + keep * h[layer]
                c[layer] = mk * c_candidate + keep * c[layer]
            ...
            outputs.append(h[-1][:slot_count])

        next_state = MaskedLSTMState(
            torch.stack(h, dim=0),
            torch.stack(c, dim=0),
        )
        return torch.stack(outputs, dim=0), next_state
...
class MaskedAttention(nn.Module):
    ...
    def forward(
        ...
        if h.ndim != 3 or mask.ndim != 2:
            raise ValueError(
                "MaskedAttention expects h=[T,N,H] and mask=[T,N]"
            )
...
class MaskedCLA(nn.Module):
    ...
    The feature extractor follows Yan et al. Eqs. (22)-(29), Fig. 8 and Table III:

        zero-padding/mask
            -> masked Conv1D
            -> pooling
            -> per-slot flatten
            -> 5-layer masked temporal LSTM
            -> masked attention
            -> FC heads -> KG_k and eta_k.

    The first tensor dimension passed to ``forward`` is a chronological sequence
    of fusion epochs.  CNN feature extraction is performed independently at each
    epoch, while the LSTM state is propagated from epoch k-1 to k and gated by
    M[k,t] at every padded observation slot t according to Eq. (25).

    The training set itself remains the fixed offline labeled X/M/Y dataset of
    Yan Eqs. (18)-(20); only the neural recurrence is corrected so that the LSTM
    is trained/used as a temporal model rather than being reset every epoch.

    Exact pooling hyperparameters, FC-head tensorization, eta_k supervision and
    behavior when N_test > Nmax_train are not published.  Those project
    completions are kept explicit and separate from paper-supported structure.
    """
    ...
    def __init__(
        ...
        # Yan et al. do not publish the exact Masked-FC tensorization.  A shared
        # per-slot head keeps the KG independent of a fixed output N and therefore
        # does not require truncating a test epoch with N > Nmax_train.
        self.gain_head = nn.Linear(
            2 * 64,
            SUPERVISED_STATE_DIM,
        )

        # Fig. 8 explicitly outputs eta_k in parallel with KG_k.  Its exact target
        # and timing are unpublished; the existing state-consequence supervision
        # is intentionally left unchanged in this architecture-only revision.
    ...
    def forward(
        ...
        lstm_state: MaskedLSTMState | None = None,
    ) -> MaskedCLAOutput:
        if (
            fixed.ndim != 2
        ...
        time_steps = fixed.shape[0]
        slot_count = observations.shape[1]
        if time_steps <= 0:
            raise ValueError("Mask-CLA input must contain at least one fusion epoch")
        if slot_count <= 0:
        ...
        if tuple(fixed.shape) != (
            ...
            raise ValueError(
                f"fixed must have shape {(time_steps, FIXED_FEATURE_DIM)}, "
        ...
        if tuple(mask.shape) != (
            ...
            raise ValueError(
                f"mask must have shape {(time_steps, slot_count)}, "
        ...
        # Paper Eq. (15) information is represented per padded observation slot.
        # This broadcast is an existing project tensorization completion and is
        # intentionally not changed in the CNN/LSTM architecture revision.
        token = torch.cat(
            [
        ...
        # Eqs. (24)-(25): temporal recurrence is along fusion epoch k.  State from
        # the preceding chronological chunk/online epoch is explicitly supplied.
        lstm, next_lstm_state = self.lstm(
            conv_flat,
            mask_values,
            state=lstm_state,
        )

        # Eqs. (26)-(29): attention fuses only the valid slots of each epoch.
        context, attention = self.attention(
            lstm,
            mask_values,
        )

        context_per_slot = context.unsqueeze(1).expand(
            -1, slot_count, -1
        )
        gain_features = torch.cat(
            [lstm, context_per_slot],
            dim=-1,
        )
        learned_gain = self.gain_head(
            gain_features
        ).transpose(1, 2)
        learned_gain = (
            learned_gain
...
if __name__ == "__main__":
    ...
    # With the corrected temporal LSTM, this is the truncated-BPTT chunk length
    # (consecutive fusion epochs), not a shuffled independent-sample minibatch.
    # Yan et al. do not publish this length; 16 preserves the previous runtime
    # setting while changing only the chronology/recurrence semantics.
    ...
    # The first DataLoader dimension is now a CHRONOLOGICAL temporal chunk for
    # the LSTM, not an exchangeable set of independent epochs.  Keeping shuffle
    # disabled is therefore required by Yan Eq. (24)-(25).  BATCH_SIZE acts as
    # the truncated-BPTT chunk length; Yan does not publish that length.
    ...
    print(
        "Masked LSTM:",
        "64 units x 5 layers, dropout=0.2; Eq.(25) stateful across fusion epochs",
    )
    print(
        "training chronology:",
        f"consecutive chunks of {BATCH_SIZE} epochs (truncated BPTT; no epoch shuffle)",
    )
    ...
    def _run_training_phase(
        ...
        # Carry the masked-LSTM state chronologically across consecutive
        # Data01 chunks.  The state is detached after each optimizer step, giving
        # truncated BPTT while preserving the forward temporal recursion.
        lstm_state = None
        ...
        for batch in epoch_batches:
            ...
            objective.backward()
            optimizer.step()
            lstm_state = output.lstm_state.detached()
    ...
    torch.save(
        {
            ...
            "eta_training": (
                "future_state_Eq30_through_Eq8_INS_sensitivity_no_direct_eta_MSE"
            ),
            "alternating_partition": (
                "psi=masked_conv+fig8_pool_flatten+temporal_masked_lstm+attention;"
                "theta=gain_head+eta_head"
            ),
            "masked_cla_recurrence": (
                "Eq25_stateful_across_fusion_epochs_k_per_observation_slot_t"
            ),
            "fig8_pooling_completion": (
                "shape_preserving_masked_max_pool_kernel3_stride1_unpublished"
            ),
    ...
    # =========================================================================
    # DIAGNOSTIC ONLY. Yan et al. train on fixed offline X/M/Y (Eqs. 18-20) and
    # then perform online estimation. This replay does not alter that training
    # protocol: model.eval()+inference_mode() are used throughout and no optimizer
    # step is possible. The purpose is only to test whether the final trained
    ...
    if RUN_RECURSIVE_DATA01_DIAGNOSTIC:
        ...
        # Stateful Eq. (24)-(25) LSTM memory for the chronological online replay.
        diag_lstm_state = None
            ...
            for event in build_exact_fusion_timeline(
                ...
                with torch.inference_mode():
                    diag_output = model(
                        ...
                        lstm_state=diag_lstm_state,
                    )
                    diag_lstm_state = diag_output.lstm_state
    ...
    # Eq. (24)-(25) LSTM state persists across Data02 fusion epochs.
    test_lstm_state = None
    ...
    for event in test_timeline:
        ...
        with torch.inference_mode():
            output = model(
                ...
                lstm_state=test_lstm_state,
            )
            test_lstm_state = output.lstm_state
    ...
    summary = {
        ...
        "paper_components": {
            ...
            "MaskedCLA_Eq10_to_Eq29": "implemented_with_Fig8_pool_flatten_and_Eq25_temporal_state",
            "training_Eq30_to_Eq32": "fixed_Data01_supervised_Adam_LR_0p01_ref15_alternating_chronological_TBPTT",
            "Fig8_eta": {
                "enabled": True,
