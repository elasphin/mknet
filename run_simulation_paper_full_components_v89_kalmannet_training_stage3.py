...
# KalmanNet reference implementation L2-normalizes each semantic recurrent
# feature at every time step. Yan et al. do not publish an input-scaling rule,
# so this is a KalmanNet-derived numerical completion, not a paper claim.
KALMANNET_L2_FEATURE_NORMALIZATION_ON = True
KALMANNET_FEATURE_NORM_EPS = 1e-12
...
class MaskedStackedLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor, recurrent_state: tuple[torch.Tensor, torch.Tensor] | None=None) -> tuple[torch.Tensor, tuple[torch.Tensor, torch.Tensor]]:
        ...
        valid_lengths_cpu = valid_lengths.detach().cpu()
        max_valid = int(valid_lengths_cpu.max())
        compressed = x.new_zeros((batch_size, max_valid, self.input_size))
        for batch_index in range(batch_size):
            length = int(valid_lengths_cpu[batch_index])
            compressed[batch_index, :length] = x[batch_index, valid[batch_index], :]
        packed = nn.utils.rnn.pack_padded_sequence(compressed, valid_lengths_cpu, batch_first=True, enforce_sorted=False)
...
if __name__ == '__main__':
    ...
    MAX_FUSION_EPOCHS = _env_optional_int('MKNET_MAX_FUSION_EPOCHS', 250)
    ...
    KNET_V2_WARMUP_EPOCHS = _env_int('MKNET_V2_WARMUP_EPOCHS', 1)
    KNET_V1_FINE_TUNE_EPOCHS = _env_nonnegative_int('MKNET_V1_FINE_TUNE_EPOCHS', 0)
    ...
    for event in training_timeline:
        ...
        history_rows.append({'time': t, 'sat_ids': measurement_model.sat_ids, 'innovation': measurement_model.innovation.copy(), 'residual': posterior_residual.copy(), 'x_pred': error_state_pred.copy(), 'x_post': error_state_post.copy(), 'accel': last_accel.copy(), 'gyro': last_gyro.copy(), 'target_state_9': target_state_9.copy(), 'posterior_nav': nav.copy(), 'fusion_index': int(fusion_index), 'leo_measurements': tuple(leo_measurements), 'preceding_interval_segments': tuple(interval_segments_since_previous_usable_fusion)})
        interval_segments_since_previous_usable_fusion = []
    ...
    def _np_l2_normalize_rows(values: np.ndarray, eps: float=KALMANNET_FEATURE_NORM_EPS) -> np.ndarray:
        values = np.asarray(values, dtype=float)
        norms = np.linalg.norm(values, axis=1, keepdims=True)
        return values / np.maximum(norms, eps)

    if KALMANNET_L2_FEATURE_NORMALIZATION_ON:
        # Original KalmanNet normalizes its four semantic difference features
        # independently at every time step. Adapt that rule to Yan Eq. (15):
        # delta-a, delta-w, state residual, state innovation, observation
        # residual, and observation innovation are each normalized separately.
        fixed_normalized = np.zeros_like(fixed, dtype=float)
        fixed_normalized[:, 0:3] = _np_l2_normalize_rows(fixed[:, 0:3])
        fixed_normalized[:, 3:6] = _np_l2_normalize_rows(fixed[:, 3:6])
        fixed_normalized[:, 6:6 + INS_STATE_DIM] = _np_l2_normalize_rows(
            fixed[:, 6:6 + INS_STATE_DIM]
        )
        fixed_normalized[:, 6 + INS_STATE_DIM:6 + 2 * INS_STATE_DIM] = _np_l2_normalize_rows(
            fixed[:, 6 + INS_STATE_DIM:6 + 2 * INS_STATE_DIM]
        )

        observations_normalized = np.zeros_like(observations, dtype=float)
        for sample_index in range(expected_t):
            for channel in range(OBSERVATION_FEATURE_DIM):
                valid = channel_masks[sample_index, :, channel]
                values = observations[sample_index, valid, channel]
                if values.size:
                    norm = max(float(np.linalg.norm(values)), KALMANNET_FEATURE_NORM_EPS)
                    observations_normalized[sample_index, valid, channel] = values / norm

        # Identity values are saved because normalization is now causal/per-epoch,
        # not a fitted dataset statistic.
        ...
        normalizer_source = 'KalmanNet_per_epoch_L2_semantic_blocks_eps_1e-12'
    elif FEATURE_STANDARDIZATION_ON:
        ...
        observations_normalized[~channel_masks] = 0.0
    ...
    print(f"training setup: samples={training_sample_count}, Nmax={network_nmax} (Data01={data01_sequence_nmax}, Data02={data02_sequence_nmax}), mode=KalmanNet_V2_short_trajectories, Tshort={KNET_V2_TRAJECTORY_LENGTH}, trajectories={trajectory_count}, trajectory_batch={TRAJECTORY_BATCH_SIZE}, batching=fixed_random_sample_without_replacement_one_step_per_phase, gain_init=zero_K_safe_warm_start, gain_head_lr=1e-4, feature_norm=KalmanNet_L2, init=ground_truth_navigation_state, V2_warmup_epochs={KNET_V2_WARMUP_EPOCHS}, V1_finetune_epochs={KNET_V1_FINE_TUNE_EPOCHS}, lr_theta={FILTER_BLOCK_LEARNING_RATE:g}, lr_psi={REPRESENTATION_BLOCK_LEARNING_RATE:g}, FDE={('ON' if FDE_DIA_ON else 'OFF')}")
    # --- MASKED-CLA TRAINING: KALMANNET V2 WARM-UP -> V1 FULL BPTT ---
    model = MaskedCLA(nmax=network_nmax, dropout=0.2).to(DEVICE)
    # Yan et al. do not publish the FC/KG-head initialization. Direct ordinary
    # nn.Linear initialization was empirically non-finite in this pseudorange
    # implementation before any optimizer step (v88). Use a neutral zero-K
    # output warm start, while controlling the first Adam updates with a
    # KalmanNet-scale learning rate for the gain head below.
    with torch.no_grad():
        model.gain_head.weight.zero_()
        model.gain_head.bias.zero_()
    representation_modules = (model.conv,)
    filter_core_modules = (model.lstm, model.attention)
    representation_parameters = [parameter for module in representation_modules for parameter in module.parameters()]
    filter_core_parameters = [parameter for module in filter_core_modules for parameter in module.parameters()]
    gain_head_parameters = list(model.gain_head.parameters())
    filter_parameters = filter_core_parameters + gain_head_parameters
    ...
    # Table III of Yan et al. gives Adam with initial LR=0.01 for the network,
    # but does not publish per-layer rates. The official KalmanNet code uses
    # 1e-4 in difficult recurrent experiments. Because this project's direct
    # FC head emits the physical K matrix and a zero head otherwise jumps by
    # O(1) in parameter norm at LR=0.01, use 1e-4 only for the gain head.
    GAIN_HEAD_LEARNING_RATE = 1e-4
    filter_optimizer = torch.optim.Adam(
        [
            {'params': filter_core_parameters, 'lr': FILTER_BLOCK_LEARNING_RATE},
            {'params': gain_head_parameters, 'lr': GAIN_HEAD_LEARNING_RATE},
        ]
    )
    ...
    def _torch_training_feature_tensors(previous_context, current_sat_ids, innovation_now: torch.Tensor, current_accel: torch.Tensor, current_gyro: torch.Tensor):
        ...
        if KALMANNET_L2_FEATURE_NORMALIZATION_ON:
            def _torch_norm_block(block: torch.Tensor) -> torch.Tensor:
                return F.normalize(block.reshape(1, -1), p=2, dim=1, eps=KALMANNET_FEATURE_NORM_EPS).reshape(-1)
            fixed_nn = torch.cat((
                _torch_norm_block(fixed_raw[0:3]),
                _torch_norm_block(fixed_raw[3:6]),
                _torch_norm_block(fixed_raw[6:6 + INS_STATE_DIM]),
                _torch_norm_block(fixed_raw[6 + INS_STATE_DIM:6 + 2 * INS_STATE_DIM]),
            ))
            obs_nn = torch.zeros_like(obs_raw)
            for channel in range(OBSERVATION_FEATURE_DIM):
                valid = channel_mask[:, channel]
                if bool(valid.any().detach().cpu()):
                    values = obs_raw[valid, channel]
                    normalized = F.normalize(values.reshape(1, -1), p=2, dim=1, eps=KALMANNET_FEATURE_NORM_EPS).reshape(-1)
                    obs_nn[valid, channel] = normalized
        else:
    ...
    def _run_training_phase(phase: str, optimizer: torch.optim.Optimizer, epoch: int):
        """One KalmanNet-V2-style random fixed-size trajectory mini-batch update."""
        ...
        # Official KalmanNet samples exactly N_B sequences without replacement
        # for each optimizer step. No 8+1/remainder optimizer update is used.
        batch_ids = np.asarray(random.sample(range(trajectory_count), k=TRAJECTORY_BATCH_SIZE), dtype=int)
        ...
        for trajectory_id_raw in batch_ids:
            ...
            metrics = _rollout_actual_closed_loop_trajectory(start_sample, stop_sample, phase=phase, backward_scale=1.0 / actual_batch_size, trajectory_id=trajectory_id, optimizer_step_index=0, initialization_mode='kalmannet_v2_truth')
        ...
        if total_count <= 0 or not trajectory_eq30_values:
            raise RuntimeError('KalmanNet-V2 Data01 random mini-batch produced no learned updates')
        ...
        if not _active_gradient_finite(phase):
            raise FloatingPointError(f'non-finite {phase} gradient before optimizer step')
        batch_data_loss = float(np.mean(trajectory_eq30_values))
        batch_objective = batch_data_loss + float(regularization.detach().cpu())
        if not math.isfinite(batch_objective):
            raise FloatingPointError(f'non-finite {phase} trajectory-batch objective before optimizer step')
        ...
        out = {'eq30': float(total_state_sum / total_count), 'position_rmse_m': math.sqrt(total_position_sum / total_count), 'trajectory_mean_eq30': batch_data_loss, 'objective': float(batch_objective), 'learned_updates': int(total_count), 'skipped_no_measurements': int(total_skipped), 'optimizer_steps': 1, 'trajectory_count': int(actual_batch_size), 'available_trajectory_count': int(trajectory_count), 'sequence_batch_size': int(actual_batch_size), 'sampled_trajectory_ids': [int(v) for v in batch_ids.tolist()], 'gradient_l2_norm': float(gradient_l2_norm), 'gain_head_parameter_l2_before': float(gain_head_l2_before), 'gain_head_parameter_l2_after': float(gain_head_l2_after), 'epoch': int(epoch)}
        out.update(maxima)
        return out
    ...
    def _evaluate_actual_closed_loop_training_objective():
        ...
        return metrics
    def _safe_recursive_training_monitor(label: str):
        """Non-blocking recursive monitor used for checkpoint selection."""
        try:
            return _evaluate_actual_closed_loop_training_objective()
        except (FloatingPointError, RuntimeError, OverflowError) as exc:
            print(f"WARNING: {label} recursive monitor diverged: {type(exc).__name__}: {exc}")
            return {
                'eq30': float('inf'),
                'position_rmse_m': float('inf'),
                'objective': float('inf'),
                'state_sum': float('inf'),
                'position_sum': float('inf'),
                'learned_updates': 0,
                'skipped_no_measurements': 0,
                'max_prior_position_error_m': float('inf'),
                'max_innovation_abs_m': float('inf'),
                'max_gain_fro_norm': float('inf'),
                'max_position_correction_norm_m': float('inf'),
                'max_velocity_correction_norm_mps': float('inf'),
                'max_attitude_correction_norm_rad': float('inf'),
            }
    ...
    best_selection_eq30 = float(pretraining_zero_gain_metrics['eq30'])
    best_selection_rmse_m = float(pretraining_zero_gain_metrics['position_rmse_m'])
    best_selection_stage = 'pretraining_zero_gain_Data01_recursive_monitor'
    ...
    for epoch in range(1, KNET_V2_WARMUP_EPOCHS + 1):
        epochs_ran += 1
        filter_phase = _run_training_phase('filter', filter_optimizer, epoch)

        # Validate immediately after the theta optimizer step. This mirrors the
        # original KalmanNet practice of checking performance after a parameter
        # update and tells us whether theta or psi creates instability.
        theta_recursive_monitor = _safe_recursive_training_monitor('V2 theta post-step')
        if float(theta_recursive_monitor['eq30']) < best_selection_eq30:
            best_model_state = _snapshot_model_state()
            best_selection_eq30 = float(theta_recursive_monitor['eq30'])
            best_selection_rmse_m = float(theta_recursive_monitor['position_rmse_m'])
            best_selection_stage = 'kalmannet_v2_theta_Data01_recursive_monitor'
            best_selection_epoch = int(epoch)

        representation_phase = _run_training_phase('representation', representation_optimizer, epoch)
        ...
        psi_recursive_monitor = _safe_recursive_training_monitor('V2 psi post-step')
        if float(psi_recursive_monitor['eq30']) < best_selection_eq30:
            best_model_state = _snapshot_model_state()
            best_selection_eq30 = float(psi_recursive_monitor['eq30'])
            best_selection_rmse_m = float(psi_recursive_monitor['position_rmse_m'])
            best_selection_stage = 'kalmannet_v2_psi_Data01_recursive_monitor'
            best_selection_epoch = int(epoch)
        recursive_monitor = psi_recursive_monitor
        training_history.append({'stage': 'kalmannet_v2_warmup_yan_masked_cla_latent_alternating', 'epoch': epoch, 'global_training_epoch': epochs_ran, 'sequence_length': int(training_sample_count), 'kalmannet_v2_short_trajectory_length': int(KNET_V2_TRAJECTORY_LENGTH), 'kalmannet_v2_initialization': 'ground_truth_navigation_state', 'kalmannet_v2_trained_sample_count': int(v2_trained_sample_count), 'kalmannet_v2_initialization_sample_count': int(v2_initialization_sample_count), 'kalmannet_v2_unused_tail_sample_count': int(v2_unused_sample_count), 'trajectory_count_available': int(trajectory_count), 'trajectory_batch_size': int(TRAJECTORY_BATCH_SIZE), 'filter_phase_sampled_trajectory_ids': filter_phase['sampled_trajectory_ids'], 'filter_phase_total_objective': filter_phase['objective'], 'filter_phase_eq30': filter_phase['eq30'], 'filter_phase_trajectory_mean_eq30': filter_phase['trajectory_mean_eq30'], 'filter_phase_position_rmse_m': filter_phase['position_rmse_m'], 'filter_phase_learned_updates': filter_phase['learned_updates'], 'filter_phase_optimizer_steps': filter_phase['optimizer_steps'], 'filter_phase_gradient_l2_norm': filter_phase['gradient_l2_norm'], 'filter_phase_gain_head_l2_before': filter_phase['gain_head_parameter_l2_before'], 'filter_phase_gain_head_l2_after': filter_phase['gain_head_parameter_l2_after'], 'filter_phase_max_innovation_abs_m': filter_phase['max_innovation_abs_m'], 'filter_phase_max_gain_fro_norm': filter_phase['max_gain_fro_norm'], 'filter_phase_max_position_correction_norm_m': filter_phase['max_position_correction_norm_m'], 'representation_phase_sampled_trajectory_ids': representation_phase['sampled_trajectory_ids'], 'representation_phase_total_objective': representation_phase['objective'], 'representation_phase_eq30': representation_phase['eq30'], 'representation_phase_trajectory_mean_eq30': representation_phase['trajectory_mean_eq30'], 'representation_phase_position_rmse_m': representation_phase['position_rmse_m'], 'representation_phase_learned_updates': representation_phase['learned_updates'], 'representation_phase_optimizer_steps': representation_phase['optimizer_steps'], 'representation_phase_gradient_l2_norm': representation_phase['gradient_l2_norm'], 'representation_phase_gain_head_l2_before': representation_phase['gain_head_parameter_l2_before'], 'representation_phase_gain_head_l2_after': representation_phase['gain_head_parameter_l2_after'], 'representation_phase_max_innovation_abs_m': representation_phase['max_innovation_abs_m'], 'representation_phase_max_gain_fro_norm': representation_phase['max_gain_fro_norm'], 'representation_phase_max_position_correction_norm_m': representation_phase['max_position_correction_norm_m'], 'theta_post_step_Data01_recursive_eq30': float(theta_recursive_monitor['eq30']), 'theta_post_step_Data01_recursive_position_rmse_m': float(theta_recursive_monitor['position_rmse_m']), 'theta_post_step_Data01_recursive_max_gain_fro_norm': float(theta_recursive_monitor['max_gain_fro_norm']), 'theta_post_step_Data01_recursive_max_position_correction_norm_m': float(theta_recursive_monitor['max_position_correction_norm_m']), 'Data01_recursive_monitor_eq30': float(recursive_monitor['eq30']), 'Data01_recursive_monitor_position_rmse_m': float(recursive_monitor['position_rmse_m']), 'best_Data01_recursive_monitor_eq30_so_far': float(best_selection_eq30), 'best_Data01_recursive_monitor_position_rmse_m_so_far': float(best_selection_rmse_m), 'filter_block_learning_rate': current_filter_lr, 'gain_head_learning_rate': float(GAIN_HEAD_LEARNING_RATE), 'representation_block_learning_rate': current_representation_lr})
        print(f"V2 epoch {epoch:03d}/{KNET_V2_WARMUP_EPOCHS}: theta_Eq30={filter_phase['trajectory_mean_eq30']:.6g}, theta_RMSE={filter_phase['position_rmse_m']:.3f}m, theta_grad={filter_phase['gradient_l2_norm']:.3g}, theta_Kparam={filter_phase['gain_head_parameter_l2_before']:.3g}->{filter_phase['gain_head_parameter_l2_after']:.3g}, theta_train_maxK={filter_phase['max_gain_fro_norm']:.3g}, theta_post_recursive_RMSE={theta_recursive_monitor['position_rmse_m']:.3f}m, theta_post_maxK={theta_recursive_monitor['max_gain_fro_norm']:.3g}; psi_Eq30={representation_phase['trajectory_mean_eq30']:.6g}, psi_RMSE={representation_phase['position_rmse_m']:.3f}m, psi_grad={representation_phase['gradient_l2_norm']:.3g}; psi_post_recursive_RMSE={recursive_monitor['position_rmse_m']:.3f}m, best_recursive_RMSE={best_selection_rmse_m:.3f}m")
    ...
    for epoch in range(1, KNET_V1_FINE_TUNE_EPOCHS + 1):
        ...
        current_filter_lr = float(filter_optimizer.param_groups[0]['lr'])
        current_representation_lr = float(representation_optimizer.param_groups[0]['lr'])
        recursive_monitor = _safe_recursive_training_monitor('V1 post-step')
        if float(recursive_monitor['eq30']) < best_selection_eq30:
            best_model_state = _snapshot_model_state()
            best_selection_eq30 = float(recursive_monitor['eq30'])
            best_selection_rmse_m = float(recursive_monitor['position_rmse_m'])
        ...
        print(f"V1 epoch {epoch:03d}/{KNET_V1_FINE_TUNE_EPOCHS}: theta_Eq30={filter_phase['trajectory_mean_eq30']:.6g}, theta_RMSE={filter_phase['position_rmse_m']:.3f}m, theta_grad={filter_phase['gradient_l2_norm']:.3g}, theta_Kparam={filter_phase['gain_head_parameter_l2_before']:.3g}->{filter_phase['gain_head_parameter_l2_after']:.3g}; psi_Eq30={representation_phase['trajectory_mean_eq30']:.6g}, psi_RMSE={representation_phase['position_rmse_m']:.3f}m, psi_grad={representation_phase['gradient_l2_norm']:.3g}; recursive_Eq30={recursive_monitor['eq30']:.6g}, recursive_RMSE={recursive_monitor['position_rmse_m']:.3f}m, best_recursive_RMSE={best_selection_rmse_m:.3f}m")
    if final_training_metrics is None:
        raise RuntimeError('Training completed without producing any epoch metrics')
    # Match the official KalmanNet checkpoint-selection idea without leaking Data02:
    # restore the best frozen Data01 recursive monitor state before final metrics/test.
    _restore_model_state(best_model_state)
    print(f"restored best Data01 recursive-monitor checkpoint: stage={best_selection_stage}, epoch={best_selection_epoch}, Eq30={best_selection_eq30:.6g}, posRMSE={best_selection_rmse_m:.3f} m")
    ...
    def _online_feature_arrays(previous_online, current_sat_ids, innovation_now, current_accel, current_gyro):
        ...
        if KALMANNET_L2_FEATURE_NORMALIZATION_ON:
            def _np_norm_vector(values):
                values = np.asarray(values, dtype=float).reshape(-1)
                return values / max(float(np.linalg.norm(values)), KALMANNET_FEATURE_NORM_EPS)
            fixed_k = np.concatenate((
                _np_norm_vector(fixed_k_raw[0:3]),
                _np_norm_vector(fixed_k_raw[3:6]),
                _np_norm_vector(fixed_k_raw[6:6 + INS_STATE_DIM]),
                _np_norm_vector(fixed_k_raw[6 + INS_STATE_DIM:6 + 2 * INS_STATE_DIM]),
            ))
            obs_k = np.zeros_like(obs_k_raw, dtype=float)
            for channel in range(OBSERVATION_FEATURE_DIM):
                valid = channel_k[:, channel]
                if np.any(valid):
                    values = obs_k_raw[valid, channel]
                    obs_k[valid, channel] = values / max(float(np.linalg.norm(values)), KALMANNET_FEATURE_NORM_EPS)
        else:
