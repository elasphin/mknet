...
Runtime-only optimizations in this version preserve the scientific equations, state
dimensions, units, masks, stochastic seeds/draw ordering, and float precision. They
remove Python/object allocation and redundant deterministic recomputation only.
"""
...
def read_imr(path: str | Path, scaling_mode: str = "cpp_exact") -> IMRData:
    """Read SmartPNT IMR with a vectorized binary decoder.

    The binary layout, endian handling, scaling, units, and returned dtypes used by
    the simulation are unchanged.  NumPy decodes all fixed-size records in native
    code instead of iterating through millions of records in Python.
    """
    path = Path(path)
    ...
    tow = records["tow"].astype(np.float64, copy=True)
    counts = records["counts"]
    # Keep zero-copy raw-count views; the scientific path uses the scaled arrays.
...
def _earth_rotation_transition(dt_s: float) -> Array:
    """Exact cached SciPy matrix exponential for repeated IMU step sizes."""
    matrix = expm(-OMEGA_IE_SKEW * float(dt_s))
...
class GNSSPreprocessor:
    ...
    def _clock_bias(self, sat_id: str, time_gpst_s: float) -> float:
        """Return the same clock source as _state without recomputing SP3 position."""
        if sat_id in self.clock_satellites:
            return float(self.clock.bias(sat_id, time_gpst_s))
        _, _, clock_bias_s, _ = self.orbit.state(sat_id, time_gpst_s)
        if clock_bias_s is None:
            raise KeyError(sat_id)
...
if __name__ == "__main__":
    ...
    # No propagation after the last requested fusion epoch can affect this run.
    # Trimming here reduces RAM in debug/partial runs while keeping the exact same
    # samples used by the event scheduler.
    requested_stop = min(
        int(np.searchsorted(imr_time, fusion_time[-1], side="left")) + 1,
        len(imr_time),
    )
    if requested_stop < len(imr_time):
        imr = IMRData(
            imr.header,
            imr.tow_s[:requested_stop],
            imr.angular_rate_body_radps[:requested_stop],
            imr.acceleration_body_mps2[:requested_stop],
            imr.raw_gyro_counts[:requested_stop],
            imr.raw_accel_counts[:requested_stop],
        )
        imr_time = imr_time[:requested_stop]
    ...
    test_requested_stop = min(
        int(np.searchsorted(test_imr_time, test_fusion_time[-1], side="left")) + 1,
        len(test_imr_time),
    )
    if test_requested_stop < len(test_imr_time):
        test_imr = IMRData(
            test_imr.header,
            test_imr.tow_s[:test_requested_stop],
            test_imr.angular_rate_body_radps[:test_requested_stop],
            test_imr.acceleration_body_mps2[:test_requested_stop],
            test_imr.raw_gyro_counts[:test_requested_stop],
            test_imr.raw_accel_counts[:test_requested_stop],
        )
        test_imr_time = test_imr_time[:test_requested_stop]
