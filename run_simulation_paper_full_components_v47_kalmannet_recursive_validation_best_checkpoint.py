...
if __name__ == "__main__":
    ...
    # Official KalmanNet training uses a distinct cross-validation set after
    # optimizer updates and retains the model with minimum CV state MSE.
    # Yan uses a training dataset and an independent testing dataset but does not
    # publish an internal validation split.  Use a contiguous Data01 tail so
    # Data02 is never consulted for model selection and temporal order is kept.
    # The exact 20% fraction is an explicit overridable implementation completion.
    KNET_VALIDATION_FRACTION = _env_float("MKNET_VALIDATION_FRACTION", 0.20)
    if not (0.0 < KNET_VALIDATION_FRACTION < 0.5):
        raise ValueError("MKNET_VALIDATION_FRACTION must be in (0, 0.5)")
    ...
    data01_sample_count = int(len(feature_time))
    if data01_sample_count <= 2:
        raise ValueError("Data01 produced too few KalmanNet samples")

    validation_sample_count = max(
        1, int(round(data01_sample_count * KNET_VALIDATION_FRACTION))
    )
    training_sample_count = data01_sample_count - validation_sample_count
    validation_start_sample = training_sample_count
    validation_stop_sample = data01_sample_count

    if training_sample_count <= 0 or validation_sample_count <= 0:
        raise ValueError("invalid Data01 train/validation split")

    # Time-series CV holdout: prefix trains, contiguous tail validates.
    # Fit all numerical conditioning only on the training prefix to avoid leakage.
    training_slice = slice(0, training_sample_count)

    if FEATURE_STANDARDIZATION_ON:
        fixed_mean = fixed[training_slice].mean(axis=0)
        fixed_std = fixed[training_slice].std(axis=0)
        ...
        for channel in range(OBSERVATION_FEATURE_DIM):
            values = observations[training_slice, :, channel][
                channel_masks[training_slice, :, channel]
            ]
            if values.size:
    ...
    np.savez(
        ...
        fit_scope=np.asarray("Data01_training_prefix_only"),
        data01_sample_count=np.asarray(data01_sample_count),
        training_sample_count=np.asarray(training_sample_count),
        validation_sample_count=np.asarray(validation_sample_count),
        validation_start_sample=np.asarray(validation_start_sample),
    )
    ...
    print(
        "training setup: "
        f"Data01_total={data01_sample_count}, train={training_sample_count}, "
        f"validation={validation_sample_count}, Nmax={network_nmax} "
    ...
    def _rollout_actual_closed_loop_trajectory(
        ...
        *,
        ...
        if not (0 <= start_sample < stop_sample <= data01_sample_count):
    ...
    checkpoint_selection = {
        ...
        "best_validation_eq30": float("inf"),
        "best_validation_position_rmse_m": float("inf"),
        "best_stage": None,
        "best_phase": None,
        "best_epoch": None,
    ...
    def _evaluate_recursive_validation_objective():
        """Recursive held-out Data01 validation, matching KalmanNet CV semantics."""
        for parameter in model.parameters():
            parameter.requires_grad_(False)
        model.eval()
    ...
    def _validate_and_update_best_checkpoint(
        *,
        stage: str,
        phase: str,
        epoch: int,
        ...
        """Select the minimum recursive validation state MSE checkpoint."""
        metrics = _evaluate_recursive_validation_objective()
        checkpoint_selection["validation_checks"] += 1

        validation_eq30 = float(metrics["eq30"])
        validation_rmse = float(metrics["position_rmse_m"])
        improved = (
            validation_eq30
            < checkpoint_selection["best_validation_eq30"]
        )

        if improved:
            checkpoint_selection["best_validation_eq30"] = validation_eq30
            checkpoint_selection["best_validation_position_rmse_m"] = validation_rmse
            checkpoint_selection["best_stage"] = str(stage)
            checkpoint_selection["best_phase"] = str(phase)
            checkpoint_selection["best_epoch"] = int(epoch)
            checkpoint_selection["best_global_epoch"] = int(global_epoch)
            checkpoint_selection["best_optimizer_step"] = int(optimizer_step)
            checkpoint_selection["best_state_dict"] = {
                key: value.detach().cpu().clone()
                for key, value in model.state_dict().items()
            }

        training_history.append({
            "stage": "recursive_cross_validation",
            "training_stage": str(stage),
            "phase": str(phase),
            "epoch": int(epoch),
            "global_training_epoch": int(global_epoch),
            "optimizer_step": int(optimizer_step),
            "validation_start_sample": int(validation_start_sample),
            "validation_stop_sample": int(validation_stop_sample),
            "validation_sample_count": int(validation_sample_count),
            "validation_eq30": validation_eq30,
            "validation_position_rmse_m": validation_rmse,
            "best_checkpoint_updated": bool(improved),
        })

        print(
            f"  CV after {stage}/{phase} step {optimizer_step}: "
            f"Eq30={validation_eq30:.6g}, "
            f"posRMSE={validation_rmse:.3f}m"
    ...
    def _run_training_phase(
        ...
        for batch_start in range(0, trajectory_count, TRAJECTORY_BATCH_SIZE):
            ...
            _validate_and_update_best_checkpoint(
                stage="V2_warmup",
                phase=phase,
                epoch=epoch,
            ...
            # Validation runs in eval/frozen mode; restore the active alternating
            # parameter block before processing the next short-trajectory batch.
            _set_alternating_phase(phase)
    ...
    for epoch in range(1, KNET_V2_WARMUP_EPOCHS + 1):
        ...
        filter_phase = _run_training_phase(
            "filter", filter_optimizer, epoch, epochs_ran, trajectory_order
        )
        representation_phase = _run_training_phase(
            "representation", representation_optimizer, epoch, epochs_ran, trajectory_order
        )
    ...
    for epoch in range(1, KNET_V1_FINE_TUNE_EPOCHS + 1):
        ...
        filter_phase = _run_v1_training_phase(
            "filter", filter_optimizer, epoch, epochs_ran
        )
        representation_phase = _run_v1_training_phase(
            "representation", representation_optimizer, epoch, epochs_ran
        )
        final_training_metrics = representation_phase
    ...
    if checkpoint_selection["best_state_dict"] is None:
        raise RuntimeError("recursive cross-validation produced no checkpoint")

    # As in official KalmanNet testing, use the minimum-CV-loss checkpoint rather
    # than the final optimizer state. Data02 remains completely untouched here.
    model.load_state_dict(checkpoint_selection["best_state_dict"])
    model.eval()

    print(
        "selected recursive-CV checkpoint: "
        f"stage={checkpoint_selection['best_stage']}, "
        f"phase={checkpoint_selection['best_phase']}, "
        f"epoch={checkpoint_selection['best_epoch']}, "
        f"global_epoch={checkpoint_selection['best_global_epoch']}, "
        f"optimizer_step={checkpoint_selection['best_optimizer_step']}, "
        f"Eq30={checkpoint_selection['best_validation_eq30']:.6g}, "
        f"posRMSE={checkpoint_selection['best_validation_position_rmse_m']:.3f}m"
    )
    ...
    # Official KalmanNet selects the minimum cross-validation state MSE and
    # loads that checkpoint for testing. Data02 is never used for selection.
    model.eval()
    best_epoch = int(checkpoint_selection["best_global_epoch"])
    best_val_state_loss = float(checkpoint_selection["best_validation_eq30"])
    best_val_position_rmse_m = float(
        checkpoint_selection["best_validation_position_rmse_m"]
    )
    selected_training_stage = (
        f"{checkpoint_selection['best_stage']}/"
        f"{checkpoint_selection['best_phase']}/"
        "minimum_recursive_Data01_validation_Eq30"
    )
    ...
    torch.save(
        {
            ...
            "data01_total_sample_count": int(data01_sample_count),
            "training_sample_count": int(training_sample_count),
            "validation_sample_count": int(validation_sample_count),
            "validation_start_sample": int(validation_start_sample),
            "validation_fraction": float(KNET_VALIDATION_FRACTION),
            "validation_split": (
                "contiguous_Data01_tail_project_completion_"
                "Yan_does_not_publish_internal_validation_split"
            ),
            "validation_selection_metric": "recursive_state_MSE_Yan_Eq30",
            "validation_checks": int(checkpoint_selection["validation_checks"]),
            "best_checkpoint_stage": checkpoint_selection["best_stage"],
            "best_checkpoint_phase": checkpoint_selection["best_phase"],
            "best_checkpoint_optimizer_step": int(
                checkpoint_selection["best_optimizer_step"]
            ),
    ...
    summary = {
        ...
        "unavoidable_unpublished_completions": [
            ...
            "KalmanNet_official_pipeline_recursive_CV_after_optimizer_updates_and_minimum_CV_MSE_checkpoint_selection",
            "Data01_contiguous_tail_validation_split_is_project_completion_because_Yan_does_not_publish_internal_validation_partition",
            "normalizer_fit_on_training_prefix_only_to_prevent_validation_leakage",
        ...
        "training": {
            ...
            "data01_total_samples": int(data01_sample_count),
