...
if __name__ == '__main__':
    ...
    for event in training_timeline:
        event_type, event_start, event_end, event_index = event
        ...
        data01_sequence_nmax = max(data01_sequence_nmax, len(epoch[1]) + len(leo_measurements))
        ...
        if not measurements:
            continue
        prior_nav = nav.copy()
        measurement_innovation, measurement_H, measurement_R, measurement_sat_ids, _ = build_measurement_model(nav, measurements, lever_arm_b_m)
        error_state_pred = np.zeros(INS_STATE_DIM)
        correction, P, _ = kalman_measurement_update(P, measurement_innovation, measurement_H, measurement_R)
        ...
        history_rows.append({'time': t, 'sat_ids': measurement_sat_ids, 'innovation': measurement_innovation.copy(), 'residual': posterior_residual.copy(), 'x_pred': error_state_pred.copy(), 'x_post': error_state_post.copy(), 'accel': last_accel.copy(), 'gyro': last_gyro.copy(), 'target_state_9': target_state_9.copy(), 'posterior_nav': nav.copy(), 'fusion_index': int(fusion_index), 'fixed_measurements': fixed_measurements, 'fixed_y_m': np.asarray([m.pseudorange_m for m in fixed_measurements], dtype=float), 'leo_measurements': tuple(leo_measurements), 'preceding_interval_segments': tuple(interval_segments_since_previous_usable_fusion)})
        interval_segments_since_previous_usable_fusion = []
    # ------------------------------------------------------------------
    ...
    v2_split_blocks = []
    for block_start in range(0, v2_source_length, v2_stride):
        block_stop = min(block_start + v2_stride, v2_source_length)
        v2_split_blocks.append((int(block_start), int(block_stop - block_start)))
    v2_split_blocks.pop()  # exact repository Short_Traj_Split behavior
    v2_chunk_starts = [
        block_start for block_start, block_length in v2_split_blocks
        if block_length == v2_stride
    ]
    ...
    for ti in range(0, N_steps):
        ...
        MSE_trainbatch_linear_LOSS = train_batch['loss']
        ...
        with torch.no_grad():
            ...
            MSE_cvbatch_linear_LOSS = validation_eval['loss']
        ...
        train_position_rmse_m = float(train_batch['position_rmse_m'].detach().cpu())
        validation_position_rmse_m = float(validation_eval['position_rmse_m'].detach().cpu())
    ...
    selected_training_stage = 'best_chronological_validation_checkpoint_after_KalmanNet_TSP_Pipeline_EKF_structural_port_V2'
        ...
        try:
            for event in build_exact_fusion_timeline(imr_time, fusion_time, through_last_fusion=True):
                ...
                if diag_previous is None:
                    ...
                    continue
                diag_innovation, diag_H, diag_R, current_sat_ids, _ = build_measurement_model(diag_nav, diag_measurements, lever_arm_b_m)
                fixed_k, obs_k, current_mask, channel_k, innovation_k, fixed_k_raw, obs_k_raw = _online_feature_arrays(diag_previous, current_sat_ids, diag_innovation, diag_feature_accel, diag_feature_gyro)
