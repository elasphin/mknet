...
- Eqs. (10)-(21) feature definitions and Data01 X/M/Y base trajectory, masked CNN/LSTM/attention
...
This revision keeps the non-invasive divergence diagnostics and changes only the
training realization so that it follows the recursive-training principles used by
KalmanNet and the alternating optimization used by Latent-KalmanNet, without
changing Yan et al.'s Masked-CLA architecture, feature definitions, or Eq. (30)/(32)
state criterion:
...
- Across navigation fusion epochs, training uses KalmanNet-style truncated
  backpropagation through time (BPTT): the Data01 trajectory is divided into
  contiguous short trajectories, and the learned state correction at one fusion
  epoch alters the prior, innovation/residual features, and target at subsequent
  fusion epochs inside the same short trajectory.  The configured V2 chunk length
  is now used exactly; it is never shortened automatically to accommodate a small
  capped Data01 run.  If the available Data01 samples cannot form at least two
  short trajectories, the run stops with a clear configuration error instead of
  silently changing the temporal training horizon.  No test-data statistic is used.
- The synthetic first-order noisy-prior augmentation from v20 is removed. No
  invented perturbation distribution is used.
- The differentiable external recurrence is implemented in the published 15-state
  error-state coordinates using the same stored linearized state transition and
  measurement Jacobians already produced by the GNSS/LEO/INS model.  The lagged
  Yan state-innovation feature is the preceding learned posterior-minus-prior
  correction, while the lagged state-residual feature is the stored Data01
  posterior-to-posterior residual plus the difference of the learned posterior
  deviations at those two epochs.  This preserves the temporal meaning of Yan
  Eqs. (12)-(13) inside each truncated trajectory.
- eta_k is not given a fabricated direct label. Its output affects the following
  prior through the existing Eq. (8)+INS sensitivity, so gradients reach eta_k only
  through later Eq. (30) state-estimation losses, as expected in end-to-end
  recursive training.
- Latent-KalmanNet's alternating-minimization principle is retained: the filter
  output heads and the representation block are optimized in alternating phases
  using the same end-to-end state-estimation objective.
- A Data01-only affine input standardization remains a numerical-conditioning
  completion; Data02 is never used for fitting it or for training.
...
- every online fusion row and every recursive-Data01 diagnostic row now carries
  the same branch-specific diagnostic schema, including the INS-only branch reached
  when FDE removes all usable measurements;
- capped short runs print an explicit warning when many training epochs repeatedly
  reuse a very small Data01 subset. The warning is informational only: no requested
  epoch count or paper-supported learning rate is changed automatically;
- Ref. [35] HPL/VPL evaluation uses an algebraically identical overflow-safe
  horizontal-eigenvalue computation. If covariance/position inputs are non-finite,
  protection levels are reported as +inf instead of crashing the navigation run.
...
class MaskedCLA(nn.Module):
    ...
    training, consecutive fusion epochs are unfolded with truncated BPTT following
    the recursive KalmanNet training principle, while the representation/filter
    parameter blocks are optimized alternately following Latent-KalmanNet. No
    synthetic noisy-prior augmentation is added. Exact pooling hyperparameters,
    FC-head tensorization, eta_k timing and behavior when N_test>Nmax_train remain
    explicitly documented completions.
    """
...
if __name__ == "__main__":
    ...
    # KalmanNet [Revach et al., 2022] explicitly trains the recursive estimator
    # with BPTT and proposes truncated BPTT (V2) by splitting long trajectories
    # into shorter trajectories; its text gives T=100 as the concrete example.
    # Yan et al. do not publish a trajectory length, so 100 is used as the
    # reference-paper default and remains overridable for runtime studies.
    TBPTT_CHUNK_LENGTH_REQUESTED = _env_int("MKNET_TBPTT_CHUNK_LENGTH", 100)
    # Yan/Latent-KalmanNet do not publish the batch size for this navigation
    # experiment. Here BATCH_SIZE means the number of short trajectories whose
    # objectives are averaged before one optimizer step.
    ...
    print(
        ...
        "KalmanNet V2-style truncated BPTT across Data01 fusion epochs; "
        f"chunk={TBPTT_CHUNK_LENGTH_REQUESTED}; requested chunk-batch={BATCH_SIZE}; "
        "Masked-CLA LSTM remains intra-epoch and resets for each X_bar[k]",
    )
    print(
        "Training augmentation:",
        "OFF -- no synthetic noisy-prior perturbation; recursive priors are produced by the learned filter inside each BPTT chunk",
    )
    ...
    # retaining these exact pieces lets truncated BPTT pass later state loss
    # gradients through the same Eq. (8)+INS effect without a direct eta label.
    interval_segments_since_previous_usable_fusion = []
    # Cumulative 15-state error transition from the previous usable fusion
    # posterior to the current prior. This is the same linearized Eq. (6) model
    # already used for covariance propagation and is stored for differentiable
    # truncated-BPTT training across fusion epochs.
    transition_since_previous_usable_fusion = np.eye(INS_STATE_DIM, dtype=float)
    ...
    for event in training_timeline:
        if isinstance(event, PropagationSegment):
            ...
            transition_since_previous_usable_fusion = (
                Phi @ transition_since_previous_usable_fusion
            )
            P = Phi @ P @ Phi.T + Qd
            P = 0.5 * (P + P.T)
            continue
        ...
        history_rows.append({
            ...
            "transition_from_previous_posterior": (
                transition_since_previous_usable_fusion.copy()
            ),
        })

        # A new eta_k would become available at this usable fusion epoch.
        interval_segments_since_previous_usable_fusion = []
        transition_since_previous_usable_fusion = np.eye(
            INS_STATE_DIM, dtype=float
        )
    ...
    channel_masks = []
    innovation_padded = []
    measurement_jacobian_padded = []
    previous_residual_jacobian_padded = []
    target_states_9 = []
    classical_corrections_15 = []
    transition_to_next_prior_15 = []
    ...
    for k in range(1, len(history_rows)):
        ...
        H_k = np.zeros((nmax, INS_STATE_DIM), dtype=float)
        H_k[:n] = current["H"]
        ...
        previous_H_by_sat = {
            sat_id: previous["H"][row_index]
            for row_index, sat_id in enumerate(previous["sat_ids"])
        }
        residual_k = np.zeros(nmax)
        residual_mask = np.zeros(nmax, dtype=bool)
        previous_residual_H_k = np.zeros((nmax, INS_STATE_DIM), dtype=float)
        for slot, sat_id in enumerate(current["sat_ids"]):
            if sat_id in previous_residual_by_sat:
                ...
                previous_residual_H_k[slot] = previous_H_by_sat[sat_id]

        # For KalmanNet-style recursive training, each short trajectory starts
        # from the published/offline Data01 context at its first fusion epoch.
        # Inside the chunk, the lagged Yan state features are regenerated from
        # learned prior/posterior history with their original temporal meaning.
        transition_to_next = (
            history_rows[k + 1]["transition_from_previous_posterior"].copy()
            if k + 1 < len(history_rows)
            else np.eye(INS_STATE_DIM, dtype=float)
        )
        ...
        measurement_jacobian_padded.append(H_k)
        previous_residual_jacobian_padded.append(previous_residual_H_k)
        target_states_9.append(current["target_state_9"])
        classical_corrections_15.append(current["x_post"].copy())
        transition_to_next_prior_15.append(transition_to_next)
    ...
    measurement_jacobian_padded = np.stack(measurement_jacobian_padded)
    previous_residual_jacobian_padded = np.stack(
        previous_residual_jacobian_padded
    )
    target_states_9 = np.stack(target_states_9)
    classical_corrections_15 = np.stack(classical_corrections_15)
    transition_to_next_prior_15 = np.stack(transition_to_next_prior_15)

    # Yan Fig. 8 exposes eta_k and Fig. 2 feeds it to the Eq. (8) IMU-error
    # compensation path, but no direct eta label is published. For recursive
    # BPTT, only the local sensitivity of the *next prior* to eta_k is required.
    # Compute that sensitivity around the same classical Data01 posterior
    # trajectory used as the nominal linearization for Phi/H. This introduces no
    # separate eta target and keeps eta inside the end-to-end state-loss graph.
    ...
    for s in range(len(feature_time) - 1):
        ...
        nominal_start_nav = current["posterior_nav"]
        nominal_end_nav = _replay_imu_interval_with_eta(
            nominal_start_nav,
            interval_segments,
            imr,
        ...
        eta_forward_state_sensitivities_9[s] = (
            eta_interval_state_sensitivity_9(
                ...
                interval_segments,
                imr,
                reference_end_nav=nominal_end_nav,
            )
        )
    ...
    if measurement_jacobian_padded.shape != (
        expected_t, nmax, INS_STATE_DIM
    ):
        raise ValueError(
            "measurement_jacobian_padded must have shape [T,Nmax,15]"
        )
    if previous_residual_jacobian_padded.shape != (
        expected_t, nmax, INS_STATE_DIM
    ):
        raise ValueError(
            "previous_residual_jacobian_padded must have shape [T,Nmax,15]"
        )
    if classical_corrections_15.shape != (expected_t, INS_STATE_DIM):
        raise ValueError("classical_corrections_15 must have shape [T,15]")
    if transition_to_next_prior_15.shape != (
        expected_t, INS_STATE_DIM, INS_STATE_DIM
    ):
        raise ValueError(
            "transition_to_next_prior_15 must have shape [T,15,15]"
        )
    ...
    if (
        ...
        or not np.all(np.isfinite(measurement_jacobian_padded))
        or not np.all(np.isfinite(previous_residual_jacobian_padded))
        or not np.all(np.isfinite(classical_corrections_15))
        or not np.all(np.isfinite(transition_to_next_prior_15))
    ...
    # Revach et al. KalmanNet training variant V2: divide a long trajectory into
    # multiple contiguous short trajectories and shuffle those trajectories during
    # optimization. Their T=3000 -> T=100 example is illustrative, not a universal
    # navigation hyperparameter.  For this reproduction, however, once a V2 chunk
    # length is configured it is part of the experiment definition and must not be
    # silently shortened merely because a diagnostic fusion cap leaves too little
    # Data01.  A short diagnostic run can still request a smaller chunk explicitly
    # through MKNET_TBPTT_CHUNK_LENGTH.
    if training_sample_count < 2:
        raise ValueError("KalmanNet V2 requires at least two training samples")

    TBPTT_CHUNK_LENGTH_EFFECTIVE = TBPTT_CHUNK_LENGTH_REQUESTED
    training_chunks = [
        (
            start,
            min(start + TBPTT_CHUNK_LENGTH_EFFECTIVE, training_sample_count),
        )
        for start in range(
            0, training_sample_count, TBPTT_CHUNK_LENGTH_EFFECTIVE
        )
    ]
    if len(training_chunks) < 2:
        cap_hint = (
            " Unset MKNET_MAX_FUSION_EPOCHS (set it to 'none') for the scientific "
            "run."
            if MAX_FUSION_EPOCHS is not None
            else " Increase Data01 coverage or explicitly choose a shorter "
            "MKNET_TBPTT_CHUNK_LENGTH for a diagnostic-only run."
        )
        raise RuntimeError(
            "KalmanNet V2 requires at least two short trajectories at the configured "
            f"chunk length {TBPTT_CHUNK_LENGTH_REQUESTED}, but only "
            f"{training_sample_count} training samples are available." + cap_hint
        )

    # KalmanNet Eq. (14) uses M trajectories from N with M < N.  Preserve the
    # requested batch whenever possible; if Data01 yields fewer chunks, reduce M,
    # not the temporal horizon of each trajectory.
    TBPTT_CHUNK_BATCH_SIZE_EFFECTIVE = min(
        BATCH_SIZE,
        len(training_chunks) - 1,
    )
    if not (1 <= TBPTT_CHUNK_BATCH_SIZE_EFFECTIVE < len(training_chunks)):
        raise RuntimeError("KalmanNet V2 requires trajectory mini-batch M < N")

    print(f"Data01 training samples={training_sample_count}, Nmax={nmax}")
    print(
        "KalmanNet V2 truncated-BPTT chunks:",
        len(training_chunks),
        f"(configured length={TBPTT_CHUNK_LENGTH_REQUESTED}, "
        f"effective length={TBPTT_CHUNK_LENGTH_EFFECTIVE}, "
        f"trajectory batch M={TBPTT_CHUNK_BATCH_SIZE_EFFECTIVE} < "
        f"N={len(training_chunks)})",
    )
    if MAX_FUSION_EPOCHS is not None:
        print(
    ...
    # =========================================================================
    # 8. MASKED CLA + YAN Eq. (30)/(32) + KALMANNET TRUNCATED BPTT
    # =========================================================================
    # Source hierarchy used here:
    #   (1) Yan et al.: architecture, X/M/Y definitions, state MSE, L2 criterion,
    #       Adam LR, and alternating optimization requirement.
    #   (2) KalmanNet (Revach et al.): end-to-end recursive filtering training and
    #       truncated BPTT over short trajectories (V2).
    #   (3) Latent-KalmanNet: alternating optimization of a filter block theta and
    #       a representation block psi using the same end-to-end state objective.
    #
    # The Masked-CLA LSTM itself remains intra-epoch exactly as in v17-v20. The
    # temporal recurrence trained by BPTT is the *external filter recurrence*:
    # learned correction -> next prior -> next innovation/residual/features.
    ...
    print(
        "Kalman gain output:",
        "full 15-state Eq.(7) rows; no fabricated direct b_a/b_g labels",
    )
    ...
    # Keep the entire compact Data01 training trajectory on the selected device.
    # Dynamic recurrent features are rebuilt inside every BPTT chunk; these base
    # tensors are only the physically measured/offline quantities around which
    # the 15-state error recurrence is unfolded.
    fixed_raw_t = torch.tensor(fixed, dtype=torch.float64, device=DEVICE)
    observations_raw_t = torch.tensor(
    ...
    H_current_t = torch.tensor(
        measurement_jacobian_padded, dtype=torch.float64, device=DEVICE
    )
    H_previous_residual_t = torch.tensor(
        previous_residual_jacobian_padded,
        dtype=torch.float64,
        device=DEVICE,
    )
    ...
    classical_correction_t = torch.tensor(
        classical_corrections_15, dtype=torch.float64, device=DEVICE
    )
    Phi_to_next_t = torch.tensor(
        transition_to_next_prior_15, dtype=torch.float64, device=DEVICE
    )
    eta_sensitivity_to_next_t = torch.tensor(
        eta_forward_state_sensitivities_9,
        dtype=torch.float64,
        device=DEVICE,
    )
    fixed_mean_t = torch.tensor(
        fixed_mean, dtype=torch.float64, device=DEVICE
    )
    fixed_std_t = torch.tensor(
    ...
    def _set_alternating_phase(phase: str) -> None:
        """Latent-KalmanNet Algorithm-2-style alternating minimization."""
        if phase not in {"filter", "representation"}:
            raise ValueError("phase must be 'filter' or 'representation'")

        # Alternating minimization freezes parameters, not the forward-mode
        # behavior of the other block. Keep the whole network in training mode
        # so the published LSTM dropout acts identically in both phases.
    ...
    def _normalized_network_input(
        fixed_raw_now: torch.Tensor,
        residual_raw_now: torch.Tensor,
        innovation_raw_now: torch.Tensor,
        channel_mask_now: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        fixed_normalized_now = (
            fixed_raw_now - fixed_mean_t
        ) / fixed_std_t
        obs_raw_now = torch.stack(
            [residual_raw_now, innovation_raw_now], dim=1
        )
        obs_normalized_now = (
            obs_raw_now - obs_mean_t.view(1, OBSERVATION_FEATURE_DIM)
        ) / obs_std_t.view(1, OBSERVATION_FEATURE_DIM)
        obs_normalized_now = torch.where(
            channel_mask_now,
            obs_normalized_now,
            torch.zeros_like(obs_normalized_now),
        )
        return (
            fixed_normalized_now.to(dtype=torch.float32),
            obs_normalized_now.to(dtype=torch.float32),
        )

    def _rollout_training_chunk(start: int, stop: int):
        """Unfold one short Data01 trajectory with differentiable filter recurrence.

        The short trajectory is initialized from the stored offline Data01 context,
        as in KalmanNet's truncated-BPTT treatment of long trajectories. From the
        second fusion epoch onward, the current prior, innovation, previous residual,
        and state-history features are consequences of the network's own preceding
        correction. No synthetic prior/noise sample is injected.
        """
        if not (0 <= start < stop <= training_sample_count):
            raise ValueError("invalid BPTT chunk bounds")

        # Deviation of the learned current prior from the classical Data01 prior.
        # At a truncated-trajectory boundary the stored Data01 prior/context is the
        # known initial condition for that short trajectory.
        prior_deviation = torch.zeros(
            INS_STATE_DIM, dtype=torch.float64, device=DEVICE
        )
        previous_posterior_deviation = torch.zeros(
            INS_STATE_DIM, dtype=torch.float64, device=DEVICE
        )
        previous_previous_posterior_deviation = torch.zeros(
            INS_STATE_DIM, dtype=torch.float64, device=DEVICE
        )

        # At a V2 chunk boundary the stored Data01 history supplies the initial
        # lagged context.  The state-innovation block of the base feature is
        # exactly the preceding posterior-minus-prior correction used by Yan
        # Eq. (12) under this causal timing completion.
        previous_correction = fixed_raw_t[
            start,
            6 + INS_STATE_DIM : 6 + 2 * INS_STATE_DIM,
        ].clone()

        squared_state_errors = []
        squared_position_errors = []

        for sample_index in range(start, stop):
            # Yan Eq. (14) IMU adjacent difference is measurement-derived and does
            # not depend on the filter posterior.  The lagged state features do:
            #   Eq. (12): previous posterior - previous prior = previous correction.
            #   Eq. (13): previous posterior - posterior before it.
            # For Eq. (13), start from the stored Data01 posterior-to-posterior
            # residual and add the corresponding learned posterior deviations.
            delta_imu = fixed_raw_t[sample_index, :6]
            previous_state_innovation = previous_correction
            base_previous_state_residual = fixed_raw_t[
                sample_index,
                6 : 6 + INS_STATE_DIM,
            ]
            previous_state_residual = (
                base_previous_state_residual
                + previous_posterior_deviation
                - previous_previous_posterior_deviation
            )
            fixed_raw_now = torch.cat(
                [
                    delta_imu,
                    previous_state_residual,
                    previous_state_innovation,
            ...
            # Current innovation around the learned prior:
            # nu_network ~= nu_reference - H_k * delta_prior.
            innovation_now = innovation_base_t[sample_index] - torch.mv(
                H_current_t[sample_index], prior_deviation
            )

            # Yan Eq. (11) is the previous completed posterior residual, aligned
            # to the satellites present at the current fusion epoch. Its value is
            # regenerated from the previous learned posterior deviation.
            residual_now = observations_raw_t[sample_index, :, 0] - torch.mv(
                H_previous_residual_t[sample_index],
                previous_posterior_deviation,
            )

            fixed_nn, obs_nn = _normalized_network_input(
                fixed_raw_now,
                residual_now,
                innovation_now,
                channel_masks_t[sample_index],
            )
            output = model(
                fixed_nn.unsqueeze(0),
                obs_nn.unsqueeze(0),
            ...
            correction = fig8_state_update(
                output,
                innovation_now.to(dtype=torch.float32).unsqueeze(0),
            )[0].to(dtype=torch.float64)
            if (
                not torch.all(torch.isfinite(innovation_now))
                or not torch.all(torch.isfinite(correction))
                or not torch.all(torch.isfinite(prior_deviation))
            ):
                raise FloatingPointError(
                    "non-finite KalmanNet V2 recurrence inside chunk "
                    f"[{start},{stop}) at sample {sample_index}"
                )

            # The truth correction changes with the learned prior. This is the
            # end-to-end state estimate used by Yan Eq. (30), not a KG label.
            target_now = (
                target_base_t[sample_index]
                - prior_deviation[:SUPERVISED_STATE_DIM]
            )
            posterior_state_error_9 = (
                target_now - correction[:SUPERVISED_STATE_DIM]
            )
            squared_state_errors.append(
                torch.sum(posterior_state_error_9**2)
            )
            squared_position_errors.append(
                torch.sum(posterior_state_error_9[:3] ** 2)
            )

            # Deviation of the learned posterior from the classical posterior.
            # This makes the next prior depend on the learned update, providing
            # the external recurrence required by KalmanNet BPTT.
            posterior_deviation = (
                prior_deviation
                + correction
                - classical_correction_t[sample_index]
            )

            # eta_k is trained only through its effect on the following prior.
            # The fixed sensitivity is obtained from the same Eq. (8)+INS model;
            # no direct eta label or eta-specific loss is introduced.
            if sample_index + 1 < stop:
                eta_effect_9 = torch.mv(
                    eta_sensitivity_to_next_t[sample_index],
                    output.imu_error[0].to(dtype=torch.float64),
                )
                eta_effect_15 = torch.zeros(
                    INS_STATE_DIM, dtype=torch.float64, device=DEVICE
                )
                eta_effect_15[:SUPERVISED_STATE_DIM] = eta_effect_9
                prior_deviation = (
                    torch.mv(
                        Phi_to_next_t[sample_index], posterior_deviation
                    )
                    + eta_effect_15
                )
            ...
            previous_posterior_deviation = posterior_deviation
            previous_correction = correction

        state_sum = torch.stack(squared_state_errors).sum()
        position_sum = torch.stack(squared_position_errors).sum()
        count = stop - start
        return state_sum, position_sum, count
    ...
    def _run_training_phase(
        ...
        """One KalmanNet-V2 truncated-BPTT pass for one alternating block."""
        _set_alternating_phase(phase)

        # KalmanNet V2 shuffles the short trajectories used for training.
        phase_offset = 0 if phase == "filter" else 1_000_003
        rng = np.random.default_rng(SEED + 1009 * int(epoch) + phase_offset)
        chunk_order = rng.permutation(len(training_chunks)).tolist()
        ...
        total_count = 0
        total_objective_weighted = 0.0

        for order_position, chunk_index in enumerate(chunk_order):
            start, stop = training_chunks[int(chunk_index)]
            state_sum, position_sum, count = _rollout_training_chunk(
                start, stop
            )
            if not torch.isfinite(state_sum) or not torch.isfinite(position_sum):
                raise FloatingPointError(
                    f"non-finite {phase} truncated-BPTT loss at epoch {epoch}, "
                    f"chunk {chunk_index}"
                )

            batch_state_sums.append(state_sum)
            batch_counts.append(int(count))
            total_state_sum += float(state_sum.detach().cpu())
            total_position_sum += float(position_sum.detach().cpu())
            total_count += int(count)

            batch_end = (
                len(batch_state_sums) >= TBPTT_CHUNK_BATCH_SIZE_EFFECTIVE
                or order_position == len(chunk_order) - 1
            )
            if batch_end:
                batch_count = int(sum(batch_counts))
                batch_state_loss = (
                    torch.stack(batch_state_sums).sum() / float(batch_count)
                )
                objective = batch_state_loss + _eq32_regularization()
                if not torch.isfinite(objective):
                    raise FloatingPointError(
                        f"non-finite {phase} Eq.32 objective at epoch {epoch}"
                    )
                objective.backward()
                optimizer.step()
                optimizer.zero_grad(set_to_none=True)

                total_objective_weighted += (
                    float(objective.detach().cpu()) * batch_count
                )
                batch_state_sums.clear()
                batch_counts.clear()

        if total_count <= 0:
            raise RuntimeError("empty truncated-BPTT training phase")
        return {
            "objective": total_objective_weighted / total_count,
            "eq30": total_state_sum / total_count,
            "position_rmse_m": math.sqrt(total_position_sum / total_count),
        }

    def _evaluate_recursive_training_objective():
        for parameter in model.parameters():
            parameter.requires_grad_(True)
        model.eval()
        ...
        with torch.inference_mode():
            for start, stop in training_chunks:
                state_sum, position_sum, count = _rollout_training_chunk(
                    start, stop
                )
                total_state_sum += float(state_sum.cpu())
                total_position_sum += float(position_sum.cpu())
                total_count += int(count)
        eq30 = total_state_sum / total_count
        return {
            "eq30": eq30,
            "position_rmse_m": math.sqrt(total_position_sum / total_count),
            "objective": eq30 + float(_eq32_regularization().detach().cpu()),
        }
    ...
    def _evaluate_teacher_forced_eq30():
        ...
        with torch.inference_mode():
            for sample_index in range(training_sample_count):
                output = model(
                    torch.tensor(
                        fixed_normalized[sample_index][None],
                        dtype=torch.float32,
                        device=DEVICE,
                    ),
                    torch.tensor(
                        observations_normalized[sample_index][None],
                        dtype=torch.float32,
                        device=DEVICE,
                    ),
                    satellite_masks_t[sample_index].unsqueeze(0),
    ...
    print(
        "\n=== YAN EQ.30/EQ.32 + KALMANNET V2 TRUNCATED BPTT + "
        "LATENT-KALMANNET ALTERNATING OPTIMIZATION (DATA01 ONLY) ==="
    )
    ...
    for epoch in range(1, TRAINING_EPOCHS + 1):
        ...
        # Latent-KalmanNet Algorithm 2: optimize theta, then psi. Both phases
        # use the same recursive state-estimation objective; no KG/eta labels.
        ...
        final_training_metrics = _evaluate_recursive_training_objective()
        ...
        training_history.append({
            "stage": "yan_kalmannet_v2_fixed_chunk_tbptt_latent_alternating",
            "epoch": epoch,
            ...
            "train_recursive_total_objective": final_training_metrics[
                "objective"
            ],
            "train_recursive_eq30": final_training_metrics["eq30"],
            "train_recursive_position_rmse_m": final_training_metrics[
                "position_rmse_m"
            ],
            "tbptt_chunk_length_requested": int(TBPTT_CHUNK_LENGTH_REQUESTED),
            "tbptt_chunk_length_effective": int(TBPTT_CHUNK_LENGTH_EFFECTIVE),
            "tbptt_chunk_count": int(len(training_chunks)),
        ...
        print(
            ...
            f"recursive_Eq30={final_training_metrics['eq30']:.6g}, "
            f"recursive_posRMSE={final_training_metrics['position_rmse_m']:.3f} m, "
    ...
    selected_training_stage = (
        "yan_eq30_kalmannet_v2_tbptt_latent_alternating_final_epoch"
    )
    ...
    # =========================================================================
    # 8B. TRAINING SELECTION / CHECKPOINT
    # =========================================================================
    # The only training stage is the Data01-only truncated-BPTT stage above.
    ...
    torch.save(
        {
            ...
            "checkpoint_schema": "full_15_state_gain_tbptt_v2",
            "nmax": nmax,
            ...
            "bias_gain_training": (
                "no_direct_bias_truth_labels; bias_gain_rows_receive only "
                "temporal end_to_end gradient through 15_state recurrence"
            ),
            "fixed_mean": fixed_mean,
            "fixed_std": fixed_std,
            "obs_mean": obs_mean,
            ...
            "eta_training": (
                "no_direct_eta_label; eta_affects_following_prior_through_"
                "Eq8_INS_sensitivity_and_later_Eq30_losses"
            ),
            ...
            "sequence_training": (
                "Yan_Data01_Eqs10_21_features_plus_KalmanNet_V2_truncated_BPTT_"
                "external_filter_recurrence;MaskedCLA_hc_local_to_each_fusion_epoch"
            ),
            "tbptt_chunk_length_requested": int(TBPTT_CHUNK_LENGTH_REQUESTED),
            "tbptt_chunk_length_effective": int(TBPTT_CHUNK_LENGTH_EFFECTIVE),
            "tbptt_chunk_batch_size_requested": int(BATCH_SIZE),
            "tbptt_chunk_batch_size_effective": int(TBPTT_CHUNK_BATCH_SIZE_EFFECTIVE),
            "tbptt_chunk_count": int(len(training_chunks)),
            ...
            "external_filter_state": (
                "recursive_across_fusion_epochs_within_each_tbptt_chunk"
            ),
    ...
    # =========================================================================
    # DIAGNOSTIC ONLY. The Data01 truncated-BPTT training has finished
            ...
            for event in build_exact_fusion_timeline(
                ...
                if FDE_DIA_ON and not diag_measurements:
                    ...
                    diag_rows.append({
                        ...
                        "gain_bias_rows_fro_norm": float("nan"),
                        "correction_position_norm_m": 0.0,
                        "correction_accel_bias_norm_mps2": 0.0,
                        "correction_gyro_bias_norm_radps": 0.0,
        ...
        if FDE_DIA_ON and not measurements:
            if previous_online is not None:
                ...
                online_rows.append({
                    ...
                    "gain_accel_bias_rows_fro_norm": float("nan"),
                    "gain_gyro_bias_rows_fro_norm": float("nan"),
                    "correction_position_norm_m": 0.0,
    ...
    summary = {
        ...
        "paper_components": {
            ...
            "training_Eq30_to_Eq32": "Yan_state_MSE_plus_KalmanNet_V2_truncated_BPTT_plus_Latent_KalmanNet_alternating",
            "Fig8_eta": {
                "enabled": True,
                "feedback_enabled_in_this_run": bool(ETA_FEEDBACK_ON),
                "dimension": 6,
                "interpretation": "[epsilon_g(3),epsilon_a(3)]",
                "training": "no_direct_eta_label; recursive_BPTT_gradient_through_following_Eq8_INS_prior",
            },
            "FDE_Eq33_to_Eq34": {
        ...
        "unavoidable_unpublished_completions": [
            ...
            "KalmanNet_V2_configured_chunk_length_uses_Revach_explicit_T100_example_Yan_does_not_publish_length_no_auto_shrink",
            "differentiable_training_recurrence_uses_same_stored_15_state_Phi_H_linearization",
            "eta_timing_Yan_unpublished_gradient_flows_through_following_Eq8_INS_sensitivity",
            "short_trajectory_batch_size_Yan_batching_unpublished",
        ...
        "training": {
            ...
            "alternating_partition": {
                ...
                "source": "Latent_KalmanNet_Algorithm2_principle",
                "paper_status": "Yan_requires_alternating_but_exact_partition_unpublished",
            },
            "sequence_training": {
                "scope": "Data01_only",
                "method": "KalmanNet_V2_truncated_BPTT",
                "chunk_length_requested": int(TBPTT_CHUNK_LENGTH_REQUESTED),
                "chunk_length_effective": int(TBPTT_CHUNK_LENGTH_EFFECTIVE),
                "chunk_length_auto_shrink": False,
                "chunk_count": int(len(training_chunks)),
                "chunk_batch_size_requested": int(BATCH_SIZE),
                "chunk_batch_size_effective": int(TBPTT_CHUNK_BATCH_SIZE_EFFECTIVE),
                "short_trajectories_shuffled_each_phase": True,
                "external_filter_recurrence": (
                    "learned_correction_to_next_prior_innovation_residual_and_state_features"
                ),
                "lagged_state_feature_timing": (
                    "Eq12_previous_posterior_minus_prior;"
                    "Eq13_base_posterior_residual_plus_learned_posterior_deviation_difference"
                ),
            ...
            "bias_gain_training": (
                "no_direct_bias_labels; indirect_temporal_gradient_through_15_state_Phi"
            ),
            "eta_training": (
                "no_direct_eta_loss; gradient_through_following_prior_via_Eq8_INS_sensitivity"
            ),
            "final_recursive_Eq30": float(final_training_metrics["eq30"]),
            "final_recursive_total_objective": float(
                final_training_metrics["objective"]
            ),
            "final_recursive_position_rmse_m": float(
                final_training_metrics["position_rmse_m"]
            ),
            "final_clean_teacher_forced_Eq30": float(
                final_teacher_forced_metrics["eq30"]
