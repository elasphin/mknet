...
def select_measurements_for_network_nmax(
    measurements,
    nmax: int,
) -> tuple[PseudorangeMeasurement, ...]:
    """Apply the fixed Data01-trained observation capacity without dropping LEO first.

    Yan Eq. (16) defines a fixed Nmax but does not publish a rule for a test epoch
    with Nk > Nmax.  This project completion preserves all available LEO
    pseudoranges first, then fills the remaining capacity with GNSS pseudoranges
    in their existing preprocessing order.  No elevation/CN0/PDOP ranking is
    introduced.

    Output ordering remains [selected GNSS..., selected LEO...], matching the
    existing Data01 measurement ordering and the slot-based Masked-CLA head.
    """
    if nmax <= 0:
        raise ValueError("network Nmax must be positive")

    usable = retain_clock_observable_measurements(measurements)
    if len(usable) <= nmax:
        return usable

    leo = tuple(m for m in usable if m.constellation == "L")
    gnss = tuple(m for m in usable if m.constellation != "L")

    if len(leo) >= nmax:
        # This branch is defensive; with the present datasets LEO count is far
        # below Nmax.  If it ever occurs, capacity is devoted to LEO because the
        # declared selection policy gives LEO priority.
        selected = leo[:nmax]
    else:
        gnss_capacity = nmax - len(leo)
        selected = gnss[:gnss_capacity] + leo

    # Truncation at a constellation boundary could leave exactly one GPS/BDS
    # code, which cannot survive the epoch-wise receiver-clock projection.
    # Remove such a singleton rather than inventing a new satellite ranking rule.
    selected = retain_clock_observable_measurements(selected)

    if len(selected) > nmax:
        raise RuntimeError("Nmax selection returned too many measurements")
    return selected
...
if __name__ == "__main__":
    ...
    # Yan Eq. (16) pads all sequences to the maximum number of observations in
    # the dataset.  Define the neural capacity strictly from the Data01 training
    # observations.  Data02 must not determine any network dimension.
    ...
    # Train-defined fixed network capacity.  The conservative Data01 raw-capacity
    # scan and the Data02 scan are diagnostic only; neither can enlarge the KG
    # head beyond slots that are actually observed during Data01 training.
    ...
    print(
        ...
        f"(source=Data01_training_observed, Data01_raw_capacity={data01_sequence_nmax}, "
        f"Data02_raw_capacity={data02_sequence_nmax}), "
    ...
    def _rollout_actual_closed_loop_trajectory(
        ...
        *,
        ...
        if initialization_mode == "kalmannet_v2_truth":
            ...
            boundary_measurements = select_measurements_for_network_nmax(
        ...
        with grad_context:
            for sample_index in range(start_sample, stop_sample):
                ...
                measurements = select_measurements_for_network_nmax(
    ...
    torch.save(
        {
            ...
            "nmax_source": "Data01_training_observed_only",
            "data01_raw_capacity_diagnostic": int(data01_sequence_nmax),
            "data02_raw_capacity_diagnostic": int(data02_sequence_nmax),
            "test_overflow_selection": (
                "preserve_all_LEO_then_fill_with_GNSS_in_existing_order;"
                "post_selection_clock_observability_cleanup"
            ),
    ...
    if RUN_RECURSIVE_DATA01_DIAGNOSTIC:
        ...
        try:
            for event in build_exact_fusion_timeline(
                ...
                diag_measurements = select_measurements_for_network_nmax(
    ...
    for event in test_timeline:
        ...
        raw_measurements = retain_clock_observable_measurements(
            tuple(gnss_measurements) + tuple(leo_measurements)
        )
        measurements = select_measurements_for_network_nmax(
            raw_measurements,
        ...
        # Training normalization and Nmax are fixed from Data01 only.  If Data02
        # has more valid observations, the deterministic pre-FDE policy above keeps
        # all LEO measurements first and fills the remaining slots with GNSS in
        # existing order.  No Data02 statistic refits normalization or the network.
        n = len(measurements)
    ...
    if RUN_CLASSICAL_TEST_BASELINE:
        ...
        for baseline_event in build_exact_fusion_timeline(
            ...
            baseline_measurements = select_measurements_for_network_nmax(
    ...
    summary = {
        ...
        "unavoidable_unpublished_completions": [
            ...
            "Nmax_defined_only_from_Data01_training_observations_Data02_does_not_set_architecture",
            "Nk_gt_Nmax_policy_preserves_LEO_then_fills_GNSS_in_existing_order_Yan_does_not_publish_overflow_selection_rule",
        ...
        "training": {
            ...
            "Data01_sequence_Nmax": int(data01_sequence_nmax),
            "Data02_sequence_Nmax": int(data02_sequence_nmax),
            "Data02_overflow_measurement_selection": (
                "all_LEO_preserved_first_then_GNSS_existing_order_to_Nmax"
            ),
