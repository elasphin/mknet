...
if __name__ == '__main__':
    ...
    # Save the standard initialization that will actually be trained.
    standard_initial_model_state = _snapshot_model_state()

    # Evaluate a true zero-K navigation baseline without forcing training to
    # start from zero output weights.
    with torch.no_grad():
        model.gain_head.weight.zero_()
        model.gain_head.bias.zero_()
    zero_gain_fallback_state = _snapshot_model_state()
    ...
    # Restore ordinary nn.Linear initialization before any optimizer step.
    _restore_model_state(standard_initial_model_state)
    pretraining_standard_init_metrics = _evaluate_actual_closed_loop_training_objective()
    print(f"pre-training standard-init Data01 monitor: Eq30={pretraining_standard_init_metrics['eq30']:.6g}, posRMSE={pretraining_standard_init_metrics['position_rmse_m']:.3f} m, maxK={pretraining_standard_init_metrics['max_gain_fro_norm']:.3g}, maxInnovation={pretraining_standard_init_metrics['max_innovation_abs_m']:.3g} m, maxPosCorr={pretraining_standard_init_metrics['max_position_correction_norm_m']:.3g} m")

    # Preserve the zero-K solution only as a safety fallback. An initialized or
    # trained nonzero network must beat it on the continuous Data01 monitor.
    best_model_state = zero_gain_fallback_state
    ...
    if float(pretraining_standard_init_metrics['eq30']) < best_selection_eq30:
        best_model_state = _snapshot_model_state()
        best_selection_eq30 = float(pretraining_standard_init_metrics['eq30'])
        best_selection_rmse_m = float(pretraining_standard_init_metrics['position_rmse_m'])
        best_selection_stage = 'pretraining_standard_init_Data01_recursive_monitor'
    ...
    training_history.append({'stage': 'pretraining_standard_init_Data01_recursive_monitor', 'eq30': float(pretraining_standard_init_metrics['eq30']), 'position_rmse_m': float(pretraining_standard_init_metrics['position_rmse_m']), 'max_innovation_abs_m': float(pretraining_standard_init_metrics['max_innovation_abs_m']), 'max_gain_fro_norm': float(pretraining_standard_init_metrics['max_gain_fro_norm']), 'max_position_correction_norm_m': float(pretraining_standard_init_metrics['max_position_correction_norm_m']), 'learned_updates': int(pretraining_standard_init_metrics['learned_updates'])})
    ...
    for epoch in range(1, KNET_V2_WARMUP_EPOCHS + 1):
        ...
        theta_recursive_monitor = _evaluate_actual_closed_loop_training_objective()
        ...
        psi_recursive_monitor = _evaluate_actual_closed_loop_training_objective()
